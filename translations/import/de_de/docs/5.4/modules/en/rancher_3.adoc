...
Nach erfolgreicher {product-name} Bereitstellung sehen Sie eine Zusammenfassung der Bereitstellungen, Daemon-Sets und Cron-Jobs für {product-name}. Sie können auch die in dem Menü 'Service Discovery' auf der linken Seite bereitgestellten Dienste sehen.
...
Wenn diese Single Sign-On (SSO) Zugriffsart zum ersten Mal verwendet wird, wird ein entsprechender Benutzer im {product-name} Cluster für die Rancher-Benutzerauthentifizierung erstellt. Der gleiche Benutzername des angemeldeten Rancher-Benutzers wird in {product-name} erstellt, mit einer Rolle als entweder Admin oder fedAdmin und dem Identitätsanbieter als Rancher.
...
. Wählen Sie einen der Controller aus, mit dem der Manager verbunden werden soll. Ändern Sie die Katalogdatei des Managers docker-compose-manager.yml, setzen Sie CTRL_SERVER_IP auf die IP des Controllers und implementieren Sie dann den Manager-Katalog.
