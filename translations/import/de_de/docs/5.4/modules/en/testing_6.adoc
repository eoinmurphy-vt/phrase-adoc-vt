...
Nachdem Sie die {product-name} Komponenten bereitgestellt haben, können Sie diese mit den bereitgestellten Beispiel-Testanwendungen bewerten. Diese befinden sich im Repository `+'nvbeta+` auf Docker Hub.
...
Um eine Multi-Tier-Anwendung mit Nginx, Nodejs und Redis bereitzustellen, verwenden Sie die untenstehenden Beispiele (in der angegebenen Reihenfolge). Diese müssen möglicherweise für die Bereitstellung auf OpenShift, Rancher und anderen Kubernetes-basierten Tools bearbeitet werden.
