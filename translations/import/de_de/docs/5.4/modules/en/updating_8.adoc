...
Die Version der CVE-Datenbank kann in der Konsole im Tab 'Vulnerabilities' eingesehen werden. Sie können auch das Updater-Container-Image inspizieren. Die neueste Versionsnummer der Datenbank kann auch https://raw.githubusercontent.com/neuvector/manifests/main/versions/scanner[hier] aufgeführt gefunden werden.
...
Upgrade zuerst auf eine 5.1.x-Version wie 5.1.3, dann siehe den Abschnitt xref:kubernetes.adoc[Kubernetes-Bereitstellungsabschnitt] für das Upgrade auf 5.2.x+ für wichtige Änderungen an Dienstkonten und Bindungen.

Für Helm-Benutzer, aktualisieren Sie auf {product-name} Helm-Chart 2.0.0 oder später (vor {product-name} 5.2.0). Wenn Sie einen Operator oder Helm-Installationen auf OpenShift aktualisieren, siehe die Anmerkung unten.
