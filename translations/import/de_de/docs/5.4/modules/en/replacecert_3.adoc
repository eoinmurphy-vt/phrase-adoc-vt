...
Die Schritte zur Generierung des Geheimnisses, das von der Webkonsole von {product-name} verwendet wird und aus dem Schlüssel und dem Zertifikat unter Verwendung einer der PKCS-Methoden stammt, werden im Folgenden erläutert.  Der wichtige Hinweis hier ist, dass die Verwendung des Wildcards für die DNS als Teil des alternativen Betreffs-Namensparameters während der Erstellung des Schlüssels und des Zertifikats es ermöglicht, den Namen Ihrer Wahl der IP-Adresse der Management-Konsole zuzuordnen, ohne auf ein bestimmtes CN beschränkt zu sein.
...
Um End-to-End-TLS zu unterstützen, unterstützen einige Ingresses/Anwendungs-Gateways nur Backend-Server, die vertrauenswürdig sind. {product-name} hat in Version 3.2.2 die Unterstützung für verkettete Zertifikate hinzugefügt.  Microsofts Anwendungs-Gateway ist ein Beispiel für ein Anwendungs-Gateway, das ein verkettetes Zertifikat benötigt, wenn eine nicht gut bekannte CA verwendet wird.
