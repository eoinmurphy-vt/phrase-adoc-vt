...
Der Registry-Bildscan wird vom Scanner durchgeführt und vom Controller verwaltet, und das Bild wird vom Scanner abgerufen und im Speicher erweitert. +
...
* GKE Autopilot.
* AWS ECS wird nicht mehr unterstützt. (HINWEIS: Es wurden keine Funktionen aktiv entfernt, um {product-name} auf ECS-Bereitstellungen zu betreiben. Allerdings wird das Testen auf ECS nicht mehr von SUSE durchgeführt. Während das Schützen von ECS-Workloads mit {product-name} wahrscheinlich wie erwartet funktioniert, werden Probleme nicht untersucht.)
* Docker auf Mac
* Docker auf Windows
...
Die folgende Tabelle zeigt die durchschnittliche Latenz von 2-10%, die mit dem Redis-Benchmark-Tool gemessen wurde. Der Redis-Benchmark verwendet ziemlich kleine Pakete, daher wird die Latenz bei größeren Paketen voraussichtlich niedriger sein.
