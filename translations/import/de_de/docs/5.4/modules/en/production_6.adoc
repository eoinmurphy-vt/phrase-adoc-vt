...
Das im Cluster definierte PersistentVolume ist erforderlich für die Unterstützung von persistenten Volumes. Die Anforderung für {product-name} ist, dass die accessModes ReadWriteMany (RWX) sein müssen. Nicht alle Speichertypen unterstützen den RWX-Zugriffsmodus. Zum Beispiel müssen Sie auf GKE möglicherweise ein RWX-persistentes Volume mit NFS-Speicher erstellen.
