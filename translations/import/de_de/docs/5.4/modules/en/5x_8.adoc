...
* **NVSHAS-9176**: Zeige einen Skriptfehler auf der **Security Risks > Vulnerabilities** Seite an, wenn ein Benutzer mit der Berechtigung für den Namespace `+rt_scan:w+` angemeldet ist.
...
* **NVSHAS-9403**: Filemonitor-UI: Benutzern erlauben, vordefinierte Dateiüberwachungsregeln zu löschen.
...
* **NVSHAS-7518**: Ändern interner Zertifikate für {product-name} Komponenten.
* **NVSHAS-9287**: Aktivieren der internen Zertifikatsrotation.
* **NVSHAS-8562**: Hinzufügen einer Benachrichtigung über das Ablaufdatum interner Zertifikate.
...
* **NVSHAS-9084**: Der PDF-Bericht der Vermögenswerte zeigt NaN an, wenn die Bildliste leer ist.
...
* **NVSHAS-9301**: NV, das auf Rancher Prime bereitgestellt wird, kann nicht erkennen, dass es sich um die Rancher-Version handelt.
...
* Die `+LDAP Authentication+` hat getrennte Felder für `+baseDN+` und `+groupDN+` Konfiguration.
...
* Fügen Sie Unterstützung für Kriterien der Zulassungssteuerung wie Ressourcenlimits hinzu. Ein neues Kriterium wird für Ressourcenlimits hinzugefügt, und zusätzliche Kriterien werden über die Einstellungen für benutzerdefinierte Kriterien unterstützt.
