...
. Wählen Sie einen der Controller aus, mit dem der Manager verbunden werden soll. Ändern Sie die Katalogdatei des Managers docker-compose-manager.yml, setzen Sie CTRL_SERVER_IP auf die IP des Controllers und setzen Sie dann den Katalog des Managers ein.
