...
* *NVSHAS-9787*: Entfernen Sie unnötige Manager-Protokolle.
* *NVSHAS-9788*: Algorithmus zur Generierung von Zertifikaten verfeinern.
* *NVSHAS-9789*: Entfernen Sie unnötige Manager-Protokolle bei der Konfiguration des Remote-Registrierungsdienstes.
...
* **NVSHAS-9821**: Der Prozessname wird nicht von der Befehlszeile für die Prozess Profit Rule Alert übereinstimmt.
...
* **NVSHAS-9783**: Nachdem die Netzwerkpolitik hinzugefügt wurde, um ICMP-Pakete zu verweigern, sieht der Benutzer keinen Alarm in "Sicherheitsereignisse".
...
* **NVSHAS-9756**: Der Enforcer hat die maximale CPU erreicht und hat mehrere Speicher-Druckwarnungen.
...
* **NVSHAS-9670**: Manager: Problem mit doppelten Anführungszeichen in der Antwort und unbenanntem Java-Bibliotheksproblem im sbt-Ausführen.
...
* **NVSHAS-9440**: Unterstützung separater Netzwerkmodi und Prozess- und Dateimodus in CRD.
...
* **NVSHAS-9517**: Die Zulassungskontrolle ist inkonsistent und liefert falsche Ergebnisse.
...
* **NVSHAS-9576**: Passwortfeld für Registrierungsdaten leeren, wenn der Benutzer den Controller-Modus mit Jenkins zum Scannen verwendet.
...
* **NVSHAS-9534**: Fehler in den Zulassungskontrollen anzeigen.
* **NVSHAS-9600**: Controller-Debugging kann nicht deaktiviert werden.
* **NVSHAS-9631**: Einige Durchsetzungsfehler reduzieren.
* **NVSHAS-9645**: Verarbeitung von bereits vorhandenen CRDs schlägt fehl.
* **NVSHAS-9592**: Kein neuer Scan trotz neuer DB-Version.
* **NVSHAS-9212**: Warnmeldung in GET(/v1/eula) anzeigen, wenn die Rolle (Binding) neuvector-binding-secret falsch ist.
* **NVSHAS-9367**: Fehlermeldungen verbessern, wenn die Registrierung nicht verbunden werden kann.
* **NVSHAS-9475**: Hintergrundgitterdruck deckt nicht vollständig ab, wenn das Menü minimiert ist.
...
* In der Version 2.8.3 des Charts haben wir eine zuvor falsch zugewiesene Ressource von crds zu core verschoben. Wenn Sie sowohl crds- als auch core-Charts verwenden, können während des Upgrades Probleme auftreten, wenn Sie zuerst core bereitstellen. Um dies zu beheben, aktualisieren Sie zuerst die crds und dann die core-Charts.
...
* **NVSHAS-7664**: Reduzierung der ISP-Datengebühren während des Registrierungs-Scannens.
...
* (Scanner) Akzeptieren Sie das OCI-Imageformat beim Wechsel zu docker api 1.24.
...
* Externe Ziel-URLs (FQDN) im Dashboard (Egress), PDF- und CSV-Berichten sowie im Netzwerkaktivitätsbildschirm und in der Liste der Sicherheitsereignisse (Verstöße) anzeigen
...
* Aktivieren Sie das ICMP-Lernen (Entdeckungsmodus) und das Blockieren (Schutzmodus) über die neue Controller-Umgebungsvariable CTRL_EN_ICMP_POLICY = 1
...
HTTP Strict Transport Security max-age=15724800, 2) CSP-Header hinzufügen (z. B. eine &apos;`+default-src+`&apos; Direktive festlegen), 3) Offenlegung des Servernamens entfernen
...
* In Assets -> Container -> Containerdetails anzeigen, die in Registrierungen im Vergleich zur Laufzeit gescannt wurden
* Link zur Gruppe in Sicherheitsrisiken -> Schwachstellen -> Auswirkungen Popup hinzufügen, um den Gruppenmodus einfach zu bearbeiten
* Unterstützung von Deep Linking in URLs zur Seite für Bild- und/oder Containeranfälligkeiten
* Option zum Zurücksetzen des Passworts für den Administrator hinzufügen, um das Benutzerpasswort in den Konsoleneinstellungen -> Benutzer zurückzusetzen
...
* Menü "Support abrufen" mit Links zu Slack, Dokumentation und anderen Ressourcen hinzufügen
* Nachrichtenfeld für /v1/log/activity-Protokolle ausfüllen
...
* Entfernen Sie die Einstellung für den Betrieb des Controllers im privilegierten Modus im Helm-Diagramm und die Anforderung, dass Controller/Durchsetzer "/host/cgroup" einbinden müssen.
...
* Unterstützung für NVD API 2.0 für Scan-CVE-Datenbank.
...
* Bieten Sie eine Umgehung für einen Teil der CRD-Gruppen an, die nach dem Löschen von Namespaces nicht erfolgreich beschnitten werden können.
...
* Bericht über den erweiterten Filter für Schwachstellen zeigt Bilder aus allen anderen Namespaces an
...
* Unterstützung für AWS Marketplace PAYG-Abrechnung für {product-name} monatliche Support-Abonnements. Benutzer können sich für {product-name} durch den SUSE-Support anmelden, der monatlich auf ihr AWS-Konto basierend auf der durchschnittlichen Knotenanzahl des Vormonats abgerechnet wird. Details https://open-docs.neuvector.com/deploying/awsmarketplace[hier].
...
* Aktivieren Sie, dass jede Zulassungsregel ihren eigenen Modus von Überwachen oder Schützen hat. Eine Ablehnungsaktion im Überwachungsmodus wird alarmieren, und eine Ablehnungsaktion im Schutzmodus wird blockieren. Erlaubte Aktionen sind nicht betroffen.
* Fügen Sie einen neuen Regex-Operator in Richtlinie > Zulassungssteuerung > Regel hinzufügen für Benutzer und Benutzergruppen hinzu, um Regex zu unterstützen. Unterstützen Sie Operatoren "entspricht IRGENDEINEM Regex in" und "entspricht KEINEM Regex in".
...
* SUSE Linux (SLES) 15.4 CVE wird im Scanner nicht erkannt. Mit diesem Fix wird, wenn die Schwere in dem Feed angegeben ist, die Schwachstelle zur Datenbank hinzugefügt, auch wenn der NVD-Eintrag fehlt. Es ist möglich, dass der Bericht Schwachstellen ohne CVE-Werte enthält.
...
* Verkehr innerhalb des gleichen Containers alarmieren/blockieren
...
* Unterscheidung zwischen Feature-Diagramm und Partner-Diagrammen in der Rancher-UI deutlicher.+ Verbesserung der Ingress-Annotation für Nginx im Rancher-Helm-Diagramm. Hinzufügen / Aktualisieren
