...
Die benutzerdefinierte Ressource NvGroupDefinition stellt die Definition einer Gruppe (`+criteria/comment+`) dar. Die sicherheitsrelevanten Einstellungen bleiben weiterhin in den benutzerdefinierten Ressourcen NvSecurityRule/NvClusterSecurityRule. Alle benutzerdefinierten Ressourcen NvGroupDefinition befinden sich im `+neuvector+` Namensraum.

==== Schemaattribut: `+name_referral+`

Beginnend mit v5.4.3 verwendet NeuVector das Attribut `+name_referral+` (boolean) als Einstellung im Gruppenselektor (`+target.selector+`, `+ingress.items[].selector+`, `+egress.items[].selector+`) im CRD-Schema von NvSecurityRule/NvClusterSecurityRule. Sie können diese Einstellung in der Benutzeroberfläche aktivieren, indem Sie "Name Referral verwenden" im Dialogfeld Gruppenexport aktivieren. 
...
Die Bedeutung des Ressourcentyps NvSecurityRule mit einem Geltungsbereich des Namensraums liegt in der Durchsetzung des konfigurierten Bereichs der Zielgruppe, der mit dem konfigurierten Namensraum in der CRD-Sicherheitsrichtlinie von NeuVector übereinstimmen muss. Dies bietet eine Durchsetzung, um unerwünschte Richtlinien zur Erstellung von übergreifenden Namensräumen zu verhindern, die eine Zielgruppenrichtlinie betreffen.
...
Eine weitere Methode zur Erstellung eines CRD-Manifests erfolgt über die **Richtlinie > Zulassungskontrolle**-Ansicht, indem Sie auf die **Weitere Operationen**-Dropdown-Liste klicken und **Exportieren** auswählen. Unten finden Sie ein Beispiel für ein NvAdmissionControlSecurityRule CRD-Manifest:
...
* Ist der Datenverkehr, der zum Ziel hinführt.

=== Ausgang

* Ist der Datenverkehr, der vom Ziel wegführt.
...
** App: Liste von Apps
** Verhalten: block_access / monitor_change  #Dies blockiert den Zugriff auf den definierten Filter unten.  Wenn monitor_change gewählt wird, wird ein Sicherheitsereignis aus der {product-name} Webkonsole Benachrichtigungen > Sicherheitsereignisse Seite generiert.
...
[NOTE]
====
Beachten Sie, dass dieses Clusterrole 2 Ressourcen konfiguriert hat, nvsecurityrules und nvclustersecurityrules. Beispiel (nvsecnvclustroles.yaml):
...
Wenn jedoch ein Versuch unternommen wird, die Test-YAML-Datei (nvclustersecurity.yaml) zu importieren, wird dies abgelehnt, da die importierte CRD-YAML-Datei mit der Ressource NvClusterSecurityRules definiert ist, die einen Cluster-Scope hat, aber user1 mit einem Namespace-Scope verbunden wurde.  Namespace-Scope hat ein niedrigeres Privileg als Cluster-Scope.  Daher wird Kubernetes RBAC eine solche Anfrage ablehnen.
...
Beachten Sie, dass für den Dienst nv.alpine.default (definiert als nv.xxx.yyy, wobei xxx der Dienstname wie alpine und yyy der Namespace wie default ist) der Modus der Richtlinie definiert werden kann, auf den er eingestellt ist. Hier ist es als Schutzmodus definiert (alle abnormalen Aktivitäten blockieren).
...
Wenn Sie den nv.alpine.default-Richtlinienmodus auf Überwachung ändern, wird {product-name} einfach protokolliert, wenn scp/ssh aufgerufen wird oder es ssh-Verbindungen von extern oder http zu extern gibt.
