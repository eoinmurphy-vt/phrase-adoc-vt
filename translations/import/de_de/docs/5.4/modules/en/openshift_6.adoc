...
[NOTE]
====
Wenn das --node-selector Argument beim Erstellen eines Projekts verwendet wird, wird dies die Platzierung von Pods einschränken, z. B. für den {product-name} Durchsetzer auf bestimmte Knoten.
====
...
Um zu verhindern, dass der Durchsetzer auf einem Controller-Knoten bereitgestellt wird, wenn es sich um einen dedizierten Verwaltungs-Knoten (ohne zu überwachende Anwendungscontainer) handelt, fügen Sie eine nodeAffinity zum YAML-Bereich des Durchsetzers hinzu. Zum Beispiel:
