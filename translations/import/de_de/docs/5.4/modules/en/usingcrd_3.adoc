...
Die Bedeutung des Ressourcentyps NvSecurityRule mit einem Geltungsbereich des Namensraums liegt in der Durchsetzung des konfigurierten Bereichs der Zielgruppe, der mit dem konfigurierten Namensraum in der Sicherheitsrichtlinie von NeuVector übereinstimmen muss. Dies bietet eine Durchsetzung, um unerwünschte Richtlinien zur Erstellung von übergreifenden Namensräumen zu verhindern, die eine Zielgruppenrichtlinienregel betreffen.

Für die benutzerdefinierte Ressourcendefinition NvClusterSecurityRule hat dies einen Geltungsbereich auf Cluster-Ebene und erzwingt daher keine Namensraumgrenze für ein definiertes Ziel. Der Benutzerkontext, der zum Importieren der CRD-YAML-Datei verwendet wird, muss jedoch über die erforderlichen Berechtigungen verfügen, um auf denselben Namensraum zuzugreifen oder sich darin zu befinden, wie der in der CRD-YAML-Datei konfigurierte, oder der Import wird abgelehnt.
...
Eine weitere Methode zur Erstellung eines CRD-Manifests ist die Ansicht **Richtlinie > Zulassungskontrolle**, indem Sie die Dropdown-Liste **Weitere Operationen** öffnen und **Exportieren** auswählen. Unten finden Sie ein Beispiel für ein NvAdmissionControlSecurityRule CRD-Manifest:
...
Das Ziel für diese NvClusterSecurityRule sind alle Container. Es hat eine Eingangsrichtlinie, die keine externen Verbindungen (außerhalb Ihres Clusters) zu Ihren Containern zulässt. Es verweigert auch allen Containern die Verwendung des ssh-Prozesses.  Dieses definierte globale Verhalten gilt für alle Container.
...
Insgesamt wird, da nv.alpine.default im Schutzmodus ist, Containern die Ausführung von ssh und scp verweigert, und es werden auch ssh-Verbindungen von extern oder http zu extern verweigert.
...
== Aktualisierung von CRD-Regeln und Hinzufügen zu bestehenden Gruppen
