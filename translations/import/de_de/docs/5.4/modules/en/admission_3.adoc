...
* PSP Best Practice. Entsprechende Regeln für PSP (Hinweis: PSP wurde vollständig aus Kubernetes 1.25+ entfernt, jedoch kann dieses {product-name} Äquivalent weiterhin in 1.25+ verwendet werden). Beinhaltet Ausführen als privilegiert, Ausführen als Root, Teilen der PID-Namensräume des Hosts, Teilen der IPC-Namensräume des Hosts, Teilen des Netzwerks des Hosts, Erlauben von Privilegieneskalation. 
...
* Überwachen: Es gibt eine Alarmnachricht im Ereignisprotokoll, wenn eine Entscheidung abgelehnt wird. In diesem Fall darf der Cluster-APIServer erfolgreich eine Ressource erstellen. Hinweis: Selbst wenn die Regelaktion Ablehnen ist, wird im Überwachungsmodus nur ein Alarm ausgelöst.
...
== Konfigurieren von Sigstore/Cosign-Überprüfern für die Anforderung der Bildsignierung

Bitte siehe xref:sigstore.adoc[diesen Abschnitt] für die Konfiguration der Überprüfer.
...
Für OpenShift stellen Sie sicher, dass Sie die master-config.yaml bearbeitet haben, um die MutatingAdmissionWebhook-Konfiguration hinzuzufügen und die Master-API-Server neu zu starten.
