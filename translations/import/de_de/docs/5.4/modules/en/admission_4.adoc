...
== Zulassungssteuerungsereignisse/Bemerkungen
...
{product-name} unterstützt viele Kriterien zur Erstellung einer Zulassungssteuerungsregel. Diese umfassen CVE-Hochzählung, CVE-Namen, Bildetiketten, imageScanned, Namespace, Benutzer, runAsRoot usw. Es gibt zwei mögliche Quellen für die Kriterienbewertung: Bildscans und Deployment-Yaml-Dateiscans. Wenn ein Kriterium einen Bildscan erfordert, werden die Scanergebnisse aus der Registry-Überprüfung verwendet. Wenn das Bild nicht gescannt wurde, wird die Zulassungssteuerungsregel nicht angewendet. Wenn ein Kriterium das Scannen des Deployment-Yaml erfordert, wird es aus dem Kubernetes-Deployment bewertet. Einige Kriterien verwenden die Ergebnisse entweder aus einem Bildscan ODER einem Deployment-Yaml-Scan.
...
* Verstößt gegen die PSA-Richtlinie. Stimmt zu, wenn die Bereitstellung entweder eine eingeschränkte oder grundlegende PSA https://kubernetes.io/docs/concepts/security/pod-security-standards/[Pod-Sicherheitsstandard] verletzt (entspricht PSA-Definitionen in Kubernetes 1.25+). 
...
Auf der **Risikoberichte** Seite wird, wenn Geheimnisse erkannt werden, das Alarmformat mit allgemeinen Ausgabeninformationen angezeigt, die als "${variable}=${value}" dargestellt werden. Als Beispiel im Bild unten kann dies mit der Variablen "env1=AIDAJQ..." gesehen werden.
...
Wenn Fehler auftreten und Sie Zugriff auf den Master-Knoten haben, können Sie das kube-apiserver-Protokoll überprüfen, um nach Zulassungs-Webhooks-Ereignissen zu suchen. Beispiele:
