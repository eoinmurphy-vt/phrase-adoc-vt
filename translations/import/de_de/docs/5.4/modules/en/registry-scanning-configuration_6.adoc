...
[NOTE]
====
Der Registry-Image-Scan wird vom Scanner durchgeführt, und das Image wird in den Arbeitsspeicher geladen und entpackt. Wenn mit entpackten Image-Größen von mehr als 500 MB gerechnet wird, ziehen Sie in Betracht, den Speicher des Scanners auf 1,5 GB oder mehr zu erhöhen, um Kapazität und Spielraum für den Scanner bereitzustellen.
====

Um die Leistung und Skalierbarkeit des Registry-Scans zu erhöhen, können mehrere Scanner-Pods auf verschiedenen Knoten bereitgestellt werden, um die Scan-Aufgaben auf mehrere Scanner zu verteilen. Siehe den Abschnitt xref:scanners.adoc[Mehrere parallele Scanner] für Details.
...
[NOTE]
====
. Um alle Bild-Tags zu scannen, fügen Sie den Filter als &#42; oder &#42;:&#42; hinzu. Dies funktioniert für alle Registrierungsarten, außer für die öffentliche Docker-Registry.
. Das Repository sollte der vollständige Name sein, wenn die Organisation für die öffentliche Docker-Registry null ist, oder fügen Sie die Bibliothek vor dem Repository hinzu, wie oben angegeben.
...
** Beachten Sie, dass {product-name} Bilder in einem Repository scannt, die neu/geändert seit dem vorherigen Scan sind.
* Erneuter Scan nach CVE-DB-Update
** Aktivieren Sie diese Option, um alle Bilder nach dem Update der Schwachdatenbank erneut zu scannen.
...
** Repository kann Wildcards mit einem Startstring haben
...
==== Sehen Sie sich das Scanergebnis an
...
Anzeigen der gescannten Bilder für das ausgewählte Repository.

image:6-imagescan.png[gescannt]

Beispiel, das das Schicht-Scan-Ergebnis eines Images zeigt, das die Schwachstellen jeder Schicht, die Schichtgröße und die auf jeder Schicht ausgeführten Befehle anzeigt. Darüber hinaus gibt es eine Compliance-Registerkarte, die die Ergebnisse der Compliance-Tests für das Image anzeigt.
...
* Geben Sie der Registry einen eindeutigen Namen
* Geben Sie die Registry-URL ein https://registry.connect.redhat.com/
...
* Geben Sie der Registry einen eindeutigen Namen
* Geben Sie die Registry-URL ein (erhalten Sie sie aus der Ausgabe des Befehls "oc get is" im OpenShift-Netzwerk, wenn sie von der Standard-URL abweicht)
...
*** oc erstelle sa nvqa
*** oc hole sa
** Weisen Sie dem Cluster-Administrator-Rollendienstkonto die Berechtigung zu, um das gesamte Registry zu lesen
...
* Geben Sie einen Benutzernamen und ein Passwort an, falls dies vom Register gefordert wird
* Fügen Sie das Repository im folgenden Format als Filter hinzu
** Organisation/Repository:Tag
...
==== Fügen Sie das JFrog Artifactory-Register hinzu (Docker-Zugriffsmethode -- Subdomain)

JFrog-Verwaltungsseite admin->HTTP-Einstellung zeigt die Docker-Zugriffsmethode -- Subdomain
...
==== Fügen Sie das JFrog Artifactory-Register hinzu (Docker-Zugriffsmethode -- Port)
...
Zeigt den Azure-Portal-Benutzernamen und das Passwort für den Zugriff auf die Container-Registry
...
* Geben Sie der Registry einen eindeutigen Namen
* Geben Sie die Registry-URL ein https://us.icr.io/
