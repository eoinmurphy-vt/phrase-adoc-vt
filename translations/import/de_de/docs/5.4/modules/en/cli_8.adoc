...
Um die Paketaufnahme auf einem Pod zu starten, müssen Sie die containerID kennen, die Sie in das Feld ID_OR_NAME eingeben müssen. Sie können dies mit `+show container -c <container_name>+` tun. Starten Sie dann den Sniffer mit `+request sniffer start <container_id>+`. Zum Beispiel,
