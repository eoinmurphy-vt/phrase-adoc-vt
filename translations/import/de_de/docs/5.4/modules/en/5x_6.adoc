...
[NOTE]
====
Die Standardtypen des Manager- und Registrierungsadapterdienstes wurden auf ClusterIP geändert.  Benutzer können die Einstellung weiterhin überschreiben, indem sie die `+manager.svc.type+` und `+cve.adapter.svc.type+` überschreiben, wenn NodePort bevorzugt wird.
====
...
* In der Version 2.8.3 des Charts haben wir eine zuvor falsch zugewiesene Ressource von crds nach core verschoben. Wenn Sie sowohl crds- als auch core-Charts verwenden, können während des Upgrades Probleme auftreten, wenn Sie zuerst core bereitstellen. Um dies zu beheben, aktualisieren Sie zuerst die crds und dann die core-Charts.
...
* **NVSHAS-4858**: Containergruppen im Controller nicht erweitern, um die Leistung der Richtlinienbereitstellung zu verbessern und CPU- sowie Speichernutzung zu reduzieren.
...
* **NVSHAS-9189**: Der Scan bleibt nach dem Herunterfahren und Neustarten des Controllers im Zeitplan hängen.
...
* Fügen Sie einen 'Manuell'-Status in den Compliance-Berichten für CIS-Benchmarks hinzu, die manuell von Benutzern ausgeführt werden müssen (nicht von {product-name} ausgeführt).
...
* Unterstützung für AWS Marketplace PAYG-Abrechnung für {product-name} monatliche Support-Abonnements. Benutzer können sich für {product-name} durch SUSE-Support anmelden, der monatlich auf ihr AWS-Konto basierend auf der durchschnittlichen Knotenanzahl des Vormonats abgerechnet wird. Details https://open-docs.neuvector.com/deploying/awsmarketplace[hier].
...
* Aktivieren Sie, dass jede Zulassungsregel ihren eigenen Modus Monitor oder Schutz hat. Eine Ablehnungsaktion im Überwachungsmodus wird alarmieren, und eine Ablehnungsaktion im Schutzmodus wird blockieren. Erlaubte Aktionen sind nicht betroffen.
...
* Fügen Sie Unterstützung für Zulassungssteuerungskriterien wie Ressourcenlimits hinzu. Ein neues Kriterium wird für Ressourcenlimits hinzugefügt, und zusätzliche Kriterien werden über die benutzerdefinierten Kriterieneinstellungen unterstützt.
* Unterstützen Sie das Aufrufen des {product-name} Scanners von Harbor-Registrierungen über die https://github.com/goharbor/pluggable-scanner-spec[pluggable scanner] Schnittstelle. Dies erfordert die Konfiguration der Verbindung zum Controller (exponierte API). Der Harbor-Adapter ruft den Controller-Endpunkt auf, um einen Scan auszulösen, der automatisch beim Pushen scannen kann. Befragungsdienste können für regelmäßige Scans verwendet werden. Scanergebnisse von den primären Controllern der Föderation werden an entfernte Cluster propagiert.  HINWEIS: Es gibt ein Problem mit dem HTTPS-basierten Adapter-Endpunktfehler: Bitte ignorieren Sie den Testverbindungsfehler, es funktioniert, auch wenn ein Fehler angezeigt wird (Zertifikatsvalidierung überspringen).
...
* SSL-Verbindungen blockieren, auch wenn eine Netzwerkregel den Verkehr unter bestimmten Anfangsbedingungen erlaubt.
...
Beigefügtes Beispiel zur Aktivierung von PSP aus dem Rancher-Chart und die unten angegebenen Befehle zum Patchen des Managers und des Scanners. Die Benutzer-ID im Patch-Befehl kann jede Zahl sein.
...
Für Helm-Benutzer, aktualisieren Sie auf {product-name} Helm-Chart 2.0.0 oder später. Wenn Sie einen Operator oder Helm-Installationen auf OpenShift aktualisieren, siehe die Anmerkung unten.
