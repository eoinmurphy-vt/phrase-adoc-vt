...
+
____
Wählen Sie Container nach ihren Diensten aus. Wenn ein Container mit Docker Compose bereitgestellt wird, hat sein Dienst-Tag-Wert den Wert "projekt_name:dienst_name"; wenn ein Container im Dienstmodus von Docker Swarm bereitgestellt wird, hat sein Dienst-Tag-Wert den Namen des Swarm-Dienstes.
____
...
Teilweise Übereinstimmungen werden für Bild-, Knoten-, Container-, Dienst- und Labelkriterien unterstützt. Zum Beispiel wählt image@redis Container aus, deren Bildname den Substring 'redis' enthält; image{caret}redis wählt Container aus, deren Bildname mit 'redis' beginnt.
