...
Um zu verhindern, dass der Vollstrecker auf einem Controller-Knoten bereitgestellt wird, wenn es sich um einen dedizierten Management-Knoten handelt (ohne Anwendungscontainer, die überwacht werden sollen), fügen Sie eine nodeAffinity zum YAML-Bereich des Vollstreckers hinzu. Zum Beispiel:
