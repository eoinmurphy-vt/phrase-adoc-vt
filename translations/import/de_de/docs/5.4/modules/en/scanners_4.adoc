...
Scanergebnisse von allen Scannern werden im Menü Assets -> Registries angezeigt. Zusätzliche Funktionen zur Überwachung von Scannern werden in zukünftigen Versionen hinzugefügt.
...
Große Bilder benötigen mehr Zeit zum Herunterladen und müssen auch entpackt werden, um sie zu scannen, was mehr Speicher verbraucht. Stellen Sie sicher, dass jedem Scanner genügend Speicher zugewiesen ist, um mehr als das größte erwartete Bild (mindestens 10 % mehr) zu verarbeiten.
