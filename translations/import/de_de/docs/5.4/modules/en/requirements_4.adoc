...
* Für die Konfigurationssicherung/HA ist ein RWX PVC von 1Gi oder mehr erforderlich. Siehe xref:production.adoc#_backups_and_persistent_data[Backups und persistente Daten Abschnitt] für weitere Details.
...
[NOTE]
====
PKS ist feldgetestet und erfordert die Aktivierung von privilegierten Containern im Plan/Tile und die Änderung des yaml hostPath wie folgt für Allinone, Controller, Enforcer:
...
Für zusätzliche Ratschläge zur Leistung und Dimensionierung siehe den xref:production.adoc#_best_practices_tips_qa_for_deploying_and_managing_suse_security[Onboarding/Beste Praktiken Abschnitt].
...
Dieser Durchsatz würde voraussichtlich steigen, wenn eine dedizierte CPU dem Enforcer zugewiesen wird, oder sich die CPU-Geschwindigkeit ändert und/oder zusätzlicher Speicher zugewiesen wird. Erneut wird die Skalierung von der Art des Netzwerk-/Anwendungsverkehrs der Workloads abhängen.
