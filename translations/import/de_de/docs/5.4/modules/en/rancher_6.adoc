...
Wenn diese Single Sign-On (SSO) Zugangsart zum ersten Mal verwendet wird, wird ein entsprechender Benutzer im {product-name} Cluster für die Rancher-Benutzerauthentifizierung erstellt. Der gleiche Benutzername des angemeldeten Rancher-Benutzers wird in {product-name} erstellt, mit einer Rolle entweder als Admin oder fedAdmin und dem Identitätsanbieter als Rancher.
