...
== Trennen Sie {product-name} Komponenten mit RedHat OpenShift
...
----
--
. Push {product-name} Bilder in das OpenShift Docker-Registry.
+
--
[NOTE]
====
...
In OpenShift 4.6+ verwenden Sie Folgendes, um zu überprüfen:
...
[IMPORTANT]
====
Das Standard-{product-name} 5.2+ Deployment verwendet am wenigsten privilegierte Dienstkonten anstelle des Standardkontos. Siehe unten, wenn Sie von einer Version vor 5.2 auf 5.2+ aktualisieren.
====
+
--
[CAUTION]
...
[NOTE]
====
Der Typ NodePort wird für die fed-master- und fed-worker-Dienste anstelle von LoadBalancer verwendet. Möglicherweise müssen Sie Ihre Bereitstellung anpassen.
====
...
Alle Taint-Infos müssen übereinstimmen, um Vollstrecker auf Knoten zu planen. Um die Taint-Infos auf einem Knoten (z.B. Master) zu überprüfen:
...
Um zu verhindern, dass der Vollstrecker auf einem Controller-Knoten bereitgestellt wird, wenn es sich um einen dedizierten Management-Knoten handelt (ohne Anwendungscontainer, die überwacht werden sollen), fügen Sie eine nodeAffinity zum Enforcer yaml Abschnitt hinzu. Zum Beispiel:
