...
Die Bedeutung des Ressourcentyps NvSecurityRule mit einem Geltungsbereich auf Namensraumebene liegt in der Durchsetzung des konfigurierten Bereichs der Zielgruppe, der mit dem konfigurierten Namensraum in der Sicherheitsrichtlinie von NeuVector übereinstimmen muss. Dies bietet eine Durchsetzung, um unerwünschte Richtlinien zur Erstellung von übergreifenden Namensräumen zu verhindern, die eine Zielgruppenrichtlinie betreffen.
...
Wenn jedoch ein Versuch unternommen wird, die Test-YAML-Datei (nvclustersecurity.yaml) zu importieren, wird dies abgelehnt, da die Import-CRD-YAML-Datei mit der Ressource NvClusterSecurityRules definiert ist, die einen Cluster-Scope hat, aber user1 mit einem Namespace-Scope gebunden wurde.  Namespace-Scope hat ein niedrigeres Privileg als Cluster-Scope.  Daher wird Kubernetes RBAC eine solche Anfrage ablehnen.
...
Als nächstes können wir den Benutzerkontext auf user2 mit einem breiteren Berechtigungsumfang, dem Cluster-Level-Scope, wechseln.  Dieser user2 hat eine Clusterrolebinding, die nicht an einen Namespace gebunden ist, sondern einen Cluster-Level-Scope hat und mit der Ressource NvClusterSecurityRules assoziiert ist.
