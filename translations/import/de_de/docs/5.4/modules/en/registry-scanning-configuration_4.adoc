...
Um Registries und Repositories für den Scan zu konfigurieren, gehe zum Menü Assets -> Registries in der {product-name} Konsole. Füge Registries hinzu oder bearbeite sie, um sie zu scannen. Verwenden Sie den Filter, um Repositories oder Teilmengen von Bildern zu definieren, die gescannt werden sollen. Wenn Ihr Registry den Zugriff über einen Proxy erfordert, kann dies in den Einstellungen -> Konfiguration konfiguriert werden.
...
. Erstellen Sie ein virtuelles Repository und fügen Sie alle lokalen Repositories hinzu, um alle Tags in einem JFrog-Repository mit der Subdomain-Docker-Zugriffsmethode zu scannen.
...
=== Optionen zum Scannen des Registrierungsbereichs
...
** Wenn Auto-Scan aktiviert ist, wird der Bildscan geplant, sobald ein Bild in das Registry hochgeladen wird.
* Periodischer Scan:
...
** Da viele Admission-Control-Prüfungen auf den Ergebnissen des Bildscans basieren, hilft die Aktivierung des periodischen Scans sicherzustellen, dass Admission Control die aktuellen Informationen zu den Bildern hat.
** Beachten Sie, dass {product-name} Bilder in einem Registry scannt, die neu/geändert sind im Vergleich zum vorherigen Scan.
...
=== Proxy-Server für Registry konfigurieren

Bitte gehen Sie zu Einstellungen -> Konfiguration, um die Proxy-Einstellungen für das Scannen des Registrys zu konfigurieren.
...
* Geben Sie Benutzernamen und Passwort an, falls vom Registry erforderlich
...
Zeige die gescannten Bilder für das ausgewählte Repository an.

image:6-imagescan.png[gescannt]

Beispiel, das das Schicht-Scan-Ergebnis eines Images zeigt, das die Schwachstellen jeder Schicht, die Schichtgröße und die auf jeder Schicht ausgeführten Befehle anzeigt. Darüber hinaus gibt es einen Compliance-Tab, der die Compliance-Test-Ergebnisse für das Image anzeigt.
...
* Geben Sie die folgenden Informationen für die Registry an. Verweisen Sie auf den obigen Amazon-Link, um die folgenden Informationen zu erhalten.
** Registry-ID
** Region
...
** Repository kann Wildcards mit einem Anfangsstring haben
** Beispiel neuvector/all&#42;:2&#42;
** Die Organisation kann leer sein, wenn ein solches Image in der Registry verfügbar ist.
...
Hinzufügen des JFrog Artifactory-Registrys (Docker-Zugriffsmethode -- Repository-Pfad)
...
* Geben Sie einen Benutzernamen und ein Passwort ein, falls dies vom Register gefordert wird
* Fügen Sie das Repository im folgenden Format als Filter hinzu
** Organisation/Repository:Tag
...
** Das Wildcard muss dem vollständigen Namen, Pfad oder Anfangsstring vorangestellt sein
...
* Fügen Sie das Repository im folgenden Format als Filter hinzu
** Organisation/Repository:Tag
** Repository oder Tag kann Wildcards am Ende haben, wie abc/_, abc/n_
** Um alle Tags für ein Repository zu scannen, verwenden Sie zum Beispiel alpine:*
** Das Wildcard muss dem vollständigen Namen, Pfad oder Anfangsstring vorangestellt sein
