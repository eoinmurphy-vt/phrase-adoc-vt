...
Voici un exemple du fichier docker-compose pour déployer le conteneur all-in-one sur le premier nœud. Parce que le conteneur all-in-one a un module enforcer à l'intérieur, les conteneurs d'application sur le même nœud peuvent être sécurisés. Les déploiements greenfield et brownfield sont pris en charge.
...
Le port 18300 et 18301 sont des ports par défaut pour la communication entre les clusters. Ils doivent être identiques pour tous les contrôleurs et enforcers dans le cluster. Veuillez vous référer à la section _"Détails de Docker-compose"_ pour savoir comment changer les ports par défaut.
