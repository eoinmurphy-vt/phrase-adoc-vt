...
*Fichiers python d'exemple* Contient config.py, client.py et multipart.py. Télécharger des fichiers d'exemple : xref:attachment$ImportExport.zip[ImportExport]. Veuillez mettre les trois fichiers dans un seul dossier pour exécuter les commandes ci-dessus. Vous devrez peut-être installer certains modules Python afin d'exécuter le script.
...
N'oubliez pas d'arrêter la session de renifleur après un certain temps pour qu'elle ne fonctionne pas indéfiniment. Le nombre de fichiers à faire tourner a une valeur maximale de 50.
...
Puis vérifiez à nouveau le contrat de licence utilisateur final (EULA).
