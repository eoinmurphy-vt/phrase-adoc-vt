...
+
____
(Optionnel) Définissez la valeur sur 1 pour activer la collecte de données de profil mémoire afin d'aider à enquêter sur les problèmes de pression mémoire.
____
...
+
____
(Optionnel pour tout-en-un) Adresse IP du serveur REST du contrôleur. La valeur par défaut est `+127.0.0.1+`. Pour le conteneur tout-en-un, laissez-le par défaut. Si le Gestionnaire fonctionne séparément, le Gestionnaire doit spécifier cette IP pour se connecter au contrôleur.
____

* CTRL_SERVER_PORT
+
____
(Optionnel pour tout-en-un) Port du serveur REST du contrôleur. La valeur par défaut est `+10443+`. Pour le conteneur tout-en-un, laissez-le par défaut. Si le Manager fonctionne séparément, le Manager doit spécifier cette variable pour se connecter au contrôleur.
____

* MANAGER_SERVER_PORT
+
____
(Optionnel) Port de l'interface utilisateur du Manager. La valeur par défaut est `+8443+`. À moins que le Manager ne fonctionne en mode hôte, laissez-le et utilisez l'option de port docker pour mapper le port sur l'hôte.
____
