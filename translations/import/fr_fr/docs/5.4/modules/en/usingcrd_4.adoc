...
Sélectionnez ensuite les groupes que vous souhaitez exporter, comme les trois dans l'espace de noms de démon ci-dessus. Inspectez le yaml CRD enregistré ci-dessous pour voir comment les règles de réseau, de processus et de fichiers {product-name} sont exprimées.
...
La configuration du mode de politique et la définition de groupe sont prises en charge dans le fichier yaml de configuration CRD. Avec le mode politique configuré dans le fichier de configuration yaml, l'importation de ce fichier définira le groupe cible sur cette valeur pour l'importation CRD.
...
* Deuxièmement, cela permet le déploiement préventif des règles avant que les applications ne soient en ligne, ce qui fournit un flux de travail de déploiement de règles de sécurité proactif et efficace.
