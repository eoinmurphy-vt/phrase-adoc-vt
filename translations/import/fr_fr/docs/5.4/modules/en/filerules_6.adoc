...
[IMPORTANT]
====
{product-name} alertes, et ne bloque pas les modifications des fichiers/répertoires prédéfinis ou dans des conteneurs système tels que ceux de Kubernetes. Le blocage n'est qu'une option pour les fichiers/répertoires personnalisés configurés par l'utilisateur pour des conteneurs non-système. Ceci est pour que les mises à jour régulières du dossier système ou des configurations sensibles ne soient pas bloquées involontairement, entraînant un comportement erratique du système.
====
...
En mode Découverte, {product-name} peut apprendre et mettre sur liste blanche les applications UNIQUEMENT pour les répertoires ou fichiers spécifiés. Pour activer l'apprentissage, une règle personnalisée doit être créée et l'Action doit être définie sur Bloquer, comme décrit ci-dessous.
...
Affichage que l'application python a appris à accéder au fichier sous le répertoire /var/opt lorsque le mode service du pod de nœud était en Découverte. Cela ne se produit que lorsque la règle est définie sur Bloquer et que le service est en mode Découverte.

image:example4.png[RèglesFichiers]

Affichage des règles d'accès aux fichiers prédéfinies pour le service node-pod.demo-nvqa. Cela peut être consulté pour ce service en cliquant sur l'icône d'information &quot;`+show predefined filters+`&quot; dans le coin droit de l'onglet de règle d'accès au fichier.
