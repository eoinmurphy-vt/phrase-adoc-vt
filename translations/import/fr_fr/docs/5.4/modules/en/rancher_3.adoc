...
* Type de service du conteneur : changez en LoadBalancer si disponible sur les déploiements de cloud public.
* Si l'accès est uniquement souhaité via Rancher, toute valeur autorisée fonctionnera ici. Voir la note importante ci-dessous concernant le changement du mot de passe administrateur par défaut dans . Voir la note importante ci-dessous concernant le changement du mot de passe administrateur par défaut dans {product-name}.
...
Pour l'exécuteur, afin d'empêcher son fonctionnement sur un nœud de contrôleur (si désiré) :
