...
. Modifier le CLUSTER_JOIN_ADDR. Trouvez l'adresse IP du nœud, le nom du nœud (si vous utilisez un serveur de noms), ou la variable de nœud (si vous utilisez des outils d'orchestration) pour l'allinone (contrôleur) à utiliser pour le &quot;`+node IP+`&quot; dans les fichiers d'exemple pour l'allinone et l'enforcer.
