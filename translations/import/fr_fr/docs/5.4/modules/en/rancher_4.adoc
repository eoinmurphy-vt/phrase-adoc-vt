...
Pour contrôler sur quels nœuds le gestionnaire et le contrôleur sont déployés, étiquetez chaque nœud. Choisissez les nœuds où les contrôleurs doivent être déployés. Étiquetez-les avec "nvcontroller=true". (Avec le fichier d'exemple actuel, pas plus d'un contrôleur ne peut fonctionner sur le même nœud.).
