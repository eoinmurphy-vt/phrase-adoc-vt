...
{product-name} peut être configuré pour analyser les vulnérabilités déclenchées dans le pipeline de construction Gitlab. Il y a un https://gitlab.com/neuvector/gitlab-plugin[plug-in Gitlab ici] qui peut être configuré et utilisé. Veuillez suivre les instructions sur le site gitlab pour utiliser le plug-in.

L'analyse peut également utiliser l'API REST {product-name} en configurant le script fourni ci-dessous pour accéder au contrôleur.

De plus, assurez-vous qu'il y a un conteneur de scanner {product-name} déployé et configuré pour se connecter à l'Allinone ou au contrôleur. Dans la version 4.0 et ultérieure, le conteneur neuvector/scanner doit être déployé séparément de l'allinone ou du contrôleur.

=== Analyse pendant la construction Gitlab en utilisant l'API REST

Utilisez le script suivant, configuré pour vos identifiants de connexion {product-name}, pour déclencher les analyses de vulnérabilités.
