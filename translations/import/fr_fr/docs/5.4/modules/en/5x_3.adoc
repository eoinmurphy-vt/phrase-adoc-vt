...
* **NVSHAS-9485**: Message incorrect pour 'Mode de Politique de Sécurité Réseau' dans l'UI.
* **NVSHAS-9480**: L'UI NV déployée sur le cluster en aval de Rancher renvoie HTTP/403 après la déconnexion de Rancher.
...
* **NVSHAS-8699** : Impossible de distinguer l'utilisateur si l'utilisateur Rancher AD est le même.
* **NVSHAS-9062** : Affichage des utilisateurs SSO Rancher sur l'interface utilisateur NV ayant le même nom d'utilisateur (Conversion sur le contrôleur).
...
* Autoriser les utilisateurs à définir &apos;`+accepted+`&apos; vulnérabilités lors de l'utilisation des actions Github afin qu'elles n'affectent pas les flux de travail.
...
* Autoriser la vérification de la signature d'image de paire de clés sans racine sans dépendance à Internet ou à sigstore.
...
* Incident inattendu Process.Profile.Violation dans le groupe NV.Protect sur lequel commande sur coreos.
...
* Corriger l'incident inattendu process.profile.violation dans le groupe NV.Protect.
