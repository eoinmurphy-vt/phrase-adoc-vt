...
Il DOIT y avoir une connectivité réseau entre les contrôleurs de chaque cluster sur les ports requis. Le contrôleur est exposé à l'extérieur de son cluster par un service principal ou distant, comme on peut le voir dans l'exemple de fichier yaml de déploiement {product-name}.
