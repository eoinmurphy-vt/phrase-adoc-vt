...
* (Icône de Service Mesh) Double-cliquez pour développer un pod dans un service mesh tel qu'Istio/Linkerd2 pour montrer le conteneur sidecar et les conteneurs de charge de travail dans le pod.
...
Importer/Exporter le fichier de politique de sécurité. Vous pouvez configurer SSO pour SAML et LDAP/AD ici également. Voir la section Intégration d'entreprise pour les détails de configuration. *_Important!_* Soyez prudent lors de l'importation du fichier de configuration. L'importation écrasera les paramètres existants. Si vous importez un fichier &apos;`+policy only+`&apos;, les Groupes et Règles de la Politique seront écrasés. Si vous importez un fichier avec des paramètres &apos;`+all+`&apos;, alors la Politique, les Utilisateurs et les Configurations seront écrasés. Notez que le mot de passe original &apos;`+admin+`&apos; de l'utilisateur de votre contrôleur actuel sera également écrasé avec le mot de passe de l'administrateur original dans le fichier importé.
