...
* Appliquer des stratégies de sécurité globales (voir des exemples pour cela en bas).
...
La ressource personnalisée NvGroupDefinition représente la définition d'un groupe (`+criteria/comment+`). Les paramètres liés à la sécurité restent dans les ressources personnalisées NvSecurityRule/NvClusterSecurityRule. Toutes les ressources personnalisées NvGroupDefinition sont sous l'espace de noms `+neuvector+`.
...
Allez dans le menu des groupes de stratégie -> et cliquez sur Exporter la stratégie de groupe dans le coin supérieur droit.
...
* Des noms de groupe tels que nv.node-pod.demo sont référencés mais non définis dans le CRD, ils sont donc censés exister déjà lors du déploiement. Voir ci-dessous pour définir des groupes.
...
[IMPORTANT]
====
Le mode de politique cible importé ne peut pas être modifié depuis la {product-name} console (Groupes de politique ->). Par exemple, une fois le mode réglé sur Surveillance, il ne peut être changé que par modification du CRD, pas par la console.
====

[NOTE]
====
Le comportement d'importation du CRD ignore le PolicyMode de tout groupe 'lié', laissant le mode de politique inchangé si le groupe lié existe déjà. Si le groupe lié n'existe pas, il sera automatiquement créé et réglé sur le mode de nouveaux services par défaut dans Paramètres -> Configuration.
====
...
Un utilisateur (user1) appartient à l'espace de noms (ns1), tandis que l'autre utilisateur (user2) appartient à l'espace de noms (ns2).  User1 sera Rolebind à ce Clusterrole créé (nvsecnvclustrole), tandis que User2 est Clusterrolebind à ce même Clusterrole (nvsecnvclustrole).

Le point clé ici est d'illustrer qu'en utilisant le Rolebinding, cela aura une portée au niveau de l'espace de noms, tandis qu'en utilisant le Clusterrolebinding, cela aura une portée au niveau du cluster.  User1 sera Rolebind (portée au niveau de l'espace de noms), et User2 sera Clusterrolebind (portée au niveau du cluster).  Cela est particulièrement important lors de l'application de RBAC en fonction du niveau de portée qui limite l'accès des utilisateurs créés.
...
+
--
Exemple de fichier nvsecurity.yaml `+NvSecurityRules+` :
...
Exemple de fichier nvclustersecurity.yaml `+NvClusterSecurityRules+` :
...
Par conséquent, utiliser user2 pour importer l'un ou l'autre fichier yaml (nvsecurity.yaml ou nvclustersecurity.yaml) sera autorisé, puisque le Clusterrolebinding de cet utilisateur n'est pas restreint à la ressource NvSecurityRules (portée d'espace de noms) ou NvClusterSecurityRules (portée de cluster).
...
* Les groupes exportés sans les critères définis comme domain=xx (Espace de noms) sont exportés avec un type de ressource NvClusterSecurityRule et un espace de noms par défaut.  Exemples de groupes exportés sans un espace de noms sont externes, container, etc.
...
Dans l'ensemble, comme nv.alpine.defult est en mode protégé, il refusera l'exécution des conteneurs pour ssh et scp, et refusera également les connexions ssh externes ou http vers l'extérieur.

Si vous changez le mode de politique nv.alpine.defult en mode surveillance, alors {product-name} se contentera de le consigner lorsque scp/ssh est invoqué, ou qu'il y a des connexions ssh externes ou http vers l'extérieur.
