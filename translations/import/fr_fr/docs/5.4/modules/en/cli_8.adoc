...
== Utilisation de l'interface de ligne de commande {product-name}
...
[NOTE]
====
Les fichiers de l'analyseur sont stockés dans le répertoire /var/neuvector/pcap dans le conteneur Enforcer. Assurez-vous de mapper le volume à votre répertoire de machine invitée ou répertoire de système local pour pouvoir accéder aux fichiers. Par exemple, dans le fichier docker-compose, ajoutez &apos;`+- /var/neuvector:/var/neuvector+`&apos; dans les volumes.
====
