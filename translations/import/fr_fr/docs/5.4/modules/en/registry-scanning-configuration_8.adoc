...
La numérisation est prise en charge pour les images sur des registres docker publics et privés basés sur Native Docker, Amazon ECR, Redhat/Openshift, jFrog, Microsoft ACR, Sonatype Nexus, Harbor, Google Cloud et d'autres registres.  Le rapport de numérisation pour l'image comprend l'état de vulnérabilité de divers packages et binaires dans l'image. Le résumé bref du rapport de numérisation peut être envoyé via webhook en utilisant la configuration des règles de réponse dans la Politique -> Règles de réponse, ou par Syslog en configurant un serveur syslog dans les Paramètres -> Configuration. Les résultats peuvent également être consultés dans les journaux d'événements.
...
* Vous devrez générer un mot de passe crypté dans votre serveur/quota Quay, et utiliser ces identifiants ici.  Ensuite, passez les filtres comme décrit ci-dessus.
