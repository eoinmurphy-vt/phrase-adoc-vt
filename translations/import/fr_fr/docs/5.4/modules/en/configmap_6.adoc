...
Les ConfigMaps et la sauvegarde de xref:production.adoc#_backups_and_persistent_data[persistent storage] ne sont lus que lorsqu'un nouveau {product-name} cluster est déployé, ou lorsque le cluster échoue et est redémarré. Ils ne sont pas utilisés lors des mises à niveau progressives.
