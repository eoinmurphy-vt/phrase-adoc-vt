...
== Déployer {product-name} Conteneurs en utilisant Docker Native ou UCP/Swarm
...
La variable d'environnement la plus importante est le *CLUSTER_JOIN_ADDR*. C'est l'adresse IP à laquelle d'autres agents se connectent. Normalement, elle doit être définie sur l'adresse IP du nœud où le conteneur all-in-one est en cours d'exécution.
...
=== Ajouter un conteneur d'application en utilisant docker-compose (mode privilégié)
...
=== Déployer le conteneur Scanner {product-name}
...
Exemple de commande docker run pour déployer le scanner sur le même hôte que le contrôleur
...
=== Déployer allinone (SANS mode privilégié) avec docker run
...
=== Déployer l'enforcer (SANS mode privilégié) avec docker run
