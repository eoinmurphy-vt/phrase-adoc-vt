...
Les grandes images prendront plus de temps à récupérer ainsi que nécessiteront d'être étendues pour les scanner, consommant plus de mémoire. Assurez-vous que chaque scanner dispose de suffisamment de mémoire allouée pour gérer plus que la plus grande image attendue (10 % de plus au minimum).

Plusieurs pods de scanner peuvent être déployés sur le même hôte/nœud, mais des considérations doivent être prises pour s'assurer que l'hôte dispose de suffisamment de mémoire, de CPU et de bande passante réseau pour maximiser les performances du scanner.
