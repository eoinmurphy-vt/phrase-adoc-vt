...
. Créez un nouveau service pour déployer cette nouvelle tâche (suivez les mêmes étapes pour déployer sur ECS). Après achèvement, la nouvelle version de l'Allinone/contrôleur devrait être opérationnelle. Depuis la console de gestion {product-name}, tous les journaux et politiques devraient toujours être là. En option, vous pouvez arrêter le 2ème conteneur Allinone/Controller maintenant, car un Allinone/Controller devrait maintenant être démarré sur le nœud d'origine.
