...
Stellen Sie sicher, dass die docker-compose.yml oder eine andere geeignete Datei bei Bedarf mit der gewünschten Bildversion bearbeitet wird, dann:
...
[NOTE]
====
Wir empfehlen, dass alle \{product-name} Komponenten gleichzeitig auf die neueste Version aktualisiert werden. Rückwärtskompatibilität wird für mindestens eine kleinere Version unterstützt. Obwohl die meisten älteren Versionen rückwärtskompatibel sein werden, kann es Ausnahmen geben, die unerwartetes Verhalten verursachen.
====

== Testen Sie Rolling Updates
...
=== Testen Sie das Beispiel für Kubernetes Rolling Update
...
== Testen Sie die Aktualisierung der Schwachstellen-CVE-Datenbank
...
Die Version der CVE-Datenbank kann in der Konsole im Tab "Schwachstellen" angezeigt werden. Sie können auch das Updater-Containerbild inspizieren. Die neueste Versionsnummer der Datenbank kann auch https://raw.githubusercontent.com/neuvector/manifests/main/versions/scanner[hier] aufgeführt gefunden werden.
...
== Testen Sie das Upgrade von \{product-name} 4.x auf 5.1.x
...
Für Helm-Benutzer, aktualisieren Sie auf das \{product-name} Helm-Chart 2.0.0 oder später (vor \{product-name} 5.2.0). Wenn Sie einen Operator oder eine Helm-Installation auf OpenShift aktualisieren, siehe die Anmerkung unten.
...
=== Testnotiz zu SCC und Upgrade über Operator/Helm
