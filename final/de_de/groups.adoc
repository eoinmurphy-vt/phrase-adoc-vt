...
Der Bildschirm "Gruppen" zeigt auch ein 'Bewertbar'-Symbol in der oberen rechten Ecke an, und eine gelernte Gruppe kann ausgewählt und das Bewertbar-Checkbox aktiviert oder deaktiviert werden. Dies steuert, welche Container verwendet werden, um den Sicherheitsrisikowert im Dashboard zu berechnen. Siehe xref:improve-score.adoc#_improving_the_security_risk_score[Verbessern Sie den Sicherheitsrisikowert] für weitere Details.
...
=== Test_ZweiAutomatische Löschung ungenutzter Gruppen
...
=== Test_ZweiHost-Schutz - die 'Knoten'-Gruppe
...
[NOTE]
====
Netzwerkverbindungsverstöße gegen die in den Netzwerkregeln für Knoten angezeigten Regeln werden niemals blockiert, selbst im Schutzmodus. Nur Verstoßverarbeitungen werden im Schutzmodus auf Knoten blockiert.
====

=== Test_TwoCustom Groups
...
+
____
Wählen Sie Container nach ihren Bildnamen aus. Beispiele: bild=wordpress, bild@redis
____
...
+
____
Wählen Sie Container nach ihren Labels aus. Beispiele: com.docker.compose.project=wordpress, standort@us-west
____

* *Adressen*
+
____
Erstellen Sie eine Gruppe nach DNS-Namen oder IP-Adressbereichen. Beispiele: adresse=www.google.com, adresse=10.1.0.1, adresse=10.1.0.0/24, adresse=10.1.0.1-10.1.0.25. Der DNS-Name kann jeder auflösbare Name sein. Adresskriterien akzeptieren nicht den != Test_Twooperator. Siehe unten für spezielle virtuelle Host-'vh'-Adressgruppen.
____
...
Teilweise Übereinstimmung wird für Bild-, Knoten-, Container-, Dienst- und Labelkriterien unterstützt. Zum Beispiel wählt image@redis Container aus, deren Bildname den Teilstring 'redis' enthält; image{caret}redis wählt Container aus, deren Bildname mit 'redis' beginnt.
...
==== Test_ZweiVirtueller Host ('vh') Basierte Netzwerkpolitik
...
== Test_ZweiBenutzerdefinierte Gruppen Beispiele

=== Test_ZweiAllgemeine Kriterien
...
* Um alle Container NICHT im Namespace 'apache1' und 'apache2' auszuwählen (nach jedem Eintrag die Eingabetaste drücken)
+
____
namespace!=apache1 namespace!=apache2
____
...
=== Test_ZweiIP-Adressen-Kriterien
