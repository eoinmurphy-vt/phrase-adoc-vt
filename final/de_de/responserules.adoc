= Test_ZweiAntwortregeln
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /05.policy/08.responserules/08.responserules.md
:page-opendocs-slug:  /richtlinie/antwortregeln

== Test_TwoPolicy: Antwortregeln
...
[IMPORTANT]
====
Alle Antwortregeln werden ausgewertet, um zu bestimmen, ob sie den Bedingungen/Kriterien entsprechen. Wenn es mehrere Regelübereinstimmungen gibt, werden die jeweiligen Aktionen ausgeführt. Dies unterscheidet sich vom Verhalten der Netzwerkregeln, die von oben nach unten ausgewertet werden und nur die erste übereinstimmende Regel ausgeführt wird.
====

Zusätzliche Ereignisse und Aktionen werden von \{product-name} in zukünftigen Versionen weiterhin hinzugefügt.

== Test_TwoDetaillierte Konfiguration für Antwortregeln
...
Aktionen aus mehreren Regeln werden angewendet, wenn ein Ereignis mehreren Regeln entspricht. Jede Regel kann mehrere Aktionen und mehrere Übereinstimmungskriterien haben. Alle definierten Aktionen werden auf Container angewendet, wenn Ereignisse den Kriterien der Antwortregel entsprechen. Falls es eine Übereinstimmung für Host (nicht Container) Ereignisse gibt, werden derzeit die Aktionen Webhook und Protokollunterdrückung unterstützt.

Es gibt 6 Standard-Antwortregeln, die mit \{product-name} enthalten sind und auf den Status '`deaktiviert,`' gesetzt sind, eine für jede Kategorie. Benutzer können entweder eine Standardregel ändern, um ihren Anforderungen zu entsprechen, oder neue erstellen. Stellen Sie sicher, dass alle Regeln aktiviert sind, die angewendet werden sollen.

=== Test_TwoResponse Rule Parameters Matrix

image:resp1.png[matrix]

=== Test_ZweiVerwendung mehrerer Kriterien in einer einzigen Regel

Die Übereinstimmungslogik für mehrere Kriterien in einer Antwortregel ist:

* Für verschiedene Kriterienarten (z.B. name:Network.Violation, name:Process.Profile.Violation) innerhalb einer einzigen Regel, wende 'und' an

=== Test_TwoActions

* Quarantäne -- Container ist quarantäne. Beachte, dass Quarantäne bedeutet, dass der gesamte Netzwerkverkehr blockiert ist.  Der Container bleibt bestehen und läuft weiter - nur ohne Netzwerkverbindungen.  Kubernetes wird keinen Container starten, um einen quarantänisierten Container zu ersetzen, da der API-Server weiterhin in der Lage ist, den Container zu erreichen.
...
=== Test_ZweiErstellen einer Antwortregel für Sicherheitsereignisprotokolle

* Klicken Sie auf "oben einfügen", um die Regel oben einzufügen
* Wählen Sie einen Dienstgruppennamen, wenn die Regel auf eine bestimmte Dienstgruppe angewendet werden muss
...
=== Test_TwoSample-Regel, um Container zu quarantänisieren und einen Webhook zu senden, wenn das Paket im nv.alpinepython.default-Container aktualisiert wird.

image:resp3.png[rulesample]

=== Test_TwoIcons zur Verwaltung von Regeln - bearbeiten, löschen, deaktivieren und neue Regel unten einfügen

image:resp4.png[verwalten]

=== Test_TwoErstellen einer Antwortregel für Ereignisprotokolle
...
* Wählen Sie die anzuwendenden Aktionen - Quarantäne, Webhook oder Protokoll unterdrücken
* Aktivieren Sie den Status
* Die Protokollebene kann als anderes Übereinstimmungskriterium verwendet werden

=== Test_TwoBeispielereignisse, die für eine Antwortregel ausgewählt werden können

image:resp5.png[ereignisse]

=== Test_TwoBeispielkriterien für Ereignisse zur Zulassungskontrolle

image:admission.png[ereignisse]

=== Test_TwoErstellen einer Antwortregel für die Kategorie cve-Bericht (Protokollebene und Berichtsname als Übereinstimmungskriterium)
...
=== Test_TwoBeispiel-CVE-Berichtstypen, die für die Antwortregel der Kategorie CVE-Bericht ausgewählt werden können

image:resp7.png[cvereport]

=== Test_TwoContainer quarantänisieren und Webhook senden, wenn die Ergebnisse des Schwachstellenscans mehr als 5 hochgradige CVE-Schwachstellen für diesen Container enthalten

image:resp8.png[cvequarantine]

=== Test_TwoSenden Sie einen Webhook, wenn der Container eine Schwachstelle mit dem Namen cve-2018-12 enthält

image:resp8-a.png[webhook]

=== Test_TwoErstellen einer Antwortregel für CIS-Benchmarks (Protokollebene und Benchmark-Nummer als Überechnungskriterien)
...
* Fügen Sie die Protokollebene als Überechnungskriterium oder Benchmark-Nummer hinzu, z.B. "`5.12`" Stellen Sie sicher, dass das Root-Dateisystem des Containers als schreibgeschützt gemountet ist
...
=== Test_TwoEntfernen der Quarantäne eines Containers durch Löschen der Antwortregel

* Sie möchten möglicherweise einen Container aus der Quarantäne entfernen, wenn er von einer Antwortregel quarantiniert wurde
...
* Wählen Sie die Option Quarantäne aufheben, um den Container nach dem Löschen der Regel aus der Quarantäne zu entfernen

=== Test_TwoAnzeigen der Regel-ID, die für die Quarantäne des Containers verantwortlich ist (in Benachrichtigungen -> Ereignisse)

image:resp9.png[unquarantine]

=== Test_TwoPopup der Option Quarantäne aufheben, wenn die entsprechende Antwortregel gelöscht wird

Aktivieren Sie das Kontrollkästchen, um alle Container aus der Quarantäne zu entfernen, die von dieser Regel quarantiniert wurden

image:resp10.png[option]

=== Test_TwoVollständige Liste der kategorisierten Kriterien, die für Antwortregeln konfiguriert werden können
...
==== Test_ZweiVorfälle (Sicherheitsereignis)
...
==== Test_ZweiBedrohungen (Sicherheitsereignis)
...
==== Test_ZweiVerstöße (Sicherheitsereignis)
...
==== Test_ZweiDynamischErzeugteKriterien
