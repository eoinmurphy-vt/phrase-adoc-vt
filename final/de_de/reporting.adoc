= Testberichterstattung & Benachrichtigungen
:revdate: 2025-05-02
...
Berichte können aus mehreren Menüs in der \{product-name} Konsole angezeigt und heruntergeladen werden. Das Dashboard zeigt eine Sicherheitsübersicht an, die als PDF heruntergeladen werden kann. Der PDF-Download kann bei Bedarf nach einem Namensraum gefiltert werden.
...
Alle Ereignisse werden im Speicher für die Anzeige im Dashboard und in den Benachrichtigungsbildschirmen gespeichert. Es wird erwartet, dass Ereignisse über SYSLOG, Webhook oder andere Mittel gesendet werden, um von einem SIEM-System gespeichert und verwaltet zu werden. Derzeit gibt es eine 4K-Grenze für jeden Ereignistyp unten:
...
Deshalb werden, sobald das Limit erreicht ist, nur die aktuellsten 4K Ereignisse dieses Typs angezeigt. Dies betrifft die Benachrichtigungsliste sowie die Anzeigen im Dashboard.

== Test SIEM und SYSLOG

Sie können den SYSLOG-Server und die Webhook-Benachrichtigungen im \{product-name} Konsolenmenü unter Einstellungen -> Konfiguration konfigurieren. Wählen Sie das Protokollierungsniveau, TCP oder UDP, und das Format, wenn JSON gewünscht ist. CVE-Daten können einzeln für jede CVE gesendet werden und/oder geschichtete Scanergebnisse enthalten. Sie können auch wählen, Ereignisse an das Pod-Protokoll des Controllers anstelle von oder zusätzlich zu Syslog zu senden. Beachten Sie, dass Ereignisse nur an das Pod-Protokoll des Hauptcontrollers gesendet werden.
...
=== Testbeispiel SYSLOG-Ausgabe

==== Test Netzwerkverstoß
...
=== Integration mit Splunk testen

Sie können mit Splunk integrieren, um Container-Sicherheitsereignisse zu erfassen und in Splunk zu berichten.

== Benachrichtigungen und Protokolle testen

Im \{product-name} Console im Menü Benachrichtigungen finden Sie Benachrichtigungen für Sicherheitsereignisse, Risiko (Scannen & Compliance) Ereignisse und allgemeine Systemereignisse.

Benachrichtigungen können als CSV oder PDF aus den Benachrichtigungsmenüs heruntergeladen werden. Darüber hinaus können Paketaufzeichnungen für Netzwerkangriffe heruntergeladen werden, und die Ergebnisse von Schwachstellen können aus dem Menü Benachrichtigungen -> Risikoberichte für jedes Scannergebnis heruntergeladen werden.

Sie können die Protokolle auch über die CLI oder die REST-API anzeigen.

=== Sicherheitsereignisse testen
...
==== Test Neue Regeln für Sicherheitsereignisse hinzufügen

Sie können einfach Regeln (Sicherheitsrichtlinie) hinzufügen, um das erkannte Ereignis zuzulassen oder abzulehnen, indem Sie die Schaltfläche Regel überprüfen und eine neue Regel bereitstellen.

image:security_events_review.png[ReviewRule]

Dies ist nützlich, wenn falsche Positiven auftreten oder ein Netzwerk-/Prozessverhalten entdeckt werden sollte, aber während des Entdeckungsmodus nicht aufgetreten ist.

==== Test Erweiterte Filter
...
* Datei. Verstoß gegen den Datei-Zugriff. Entweder wurde auf eine überwachte sensible Datei/ein Verzeichnis zugegriffen (siehe Liste der Standardüberwachung) oder eine benutzerdefinierte Dateiüberwachungsregel wurde ausgelöst. Siehe die xref:./filerules.adoc[Dateizugriffsregeln] Seite für weitere Informationen.
...
Dieser Abschnitt enthält Ereignisse für Schwachstellenscans (Bild, Registrierung, Laufzeit, Container, Host, Plattform), Compliance-Scans (CIS-Benchmarks, benutzerdefinierte Skripte) und Ereignisse zur Zulassungskontrolle (erlaubt, verweigert).

== Testen Sie andere Integrationen
