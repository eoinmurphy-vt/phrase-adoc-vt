...
=== Test_DreiBasismodus Prozessschutz
...
=== Test_DreiVerhaltensbasiertes Lernen Prozessschutz
...
Prozess-Baseline-Informationen werden gelernt und aufgezeichnet, wenn die Dienstgruppe im Entdeckungsmodus (Lernmodus) ist. Wenn im Überwachungs- oder Schutzmodus ein Prozess, der zuvor nicht gesehen wurde, neu gestartet wird oder ein alter Prozess von einem anderen Benutzer als zuvor gestartet wird, wird das Ereignis als verdächtiger Prozess im Überwachungsmodus erkannt und alarmiert oder im Schutzmodus alarmiert und blockiert. Benutzer können das gelernte Profil manuell ändern, um Prozesse zuzulassen oder abzulehnen (Whitelist oder Blacklist), falls erforderlich.
...
[IMPORTANT]
====
Kubernetes-Liveness-Probes sind automatisch erlaubt und werden auch im Monitor-/Schutzmodus zu den gelernten Prozessregeln hinzugefügt.
====

=== Test_DreiProzessregeln für Knoten

Die spezielle reservierte Gruppe 'Knoten' kann konfiguriert werden, um Prozessprofilregeln auf jedem Knoten (Host) im Cluster durchzusetzen. Wählen Sie die Gruppe 'Knoten' aus und überprüfen Sie die Prozessregeln, bearbeiten Sie sie bei Bedarf. Wechseln Sie dann den Schutzmodus auf Monitor oder Schutz. Die Liste der 'lokalen' (gelernten) Prozessregeln ist eine Kombination aller Prozesse von allen Knoten im Cluster im Entdeckungsmodus.

=== Test_DreiProzessregeln für benutzerdefinierte Gruppen
...
=== Test_ThreeProcess Rules Precedence
...
Prozessregeln mit einer Verweigerungsaktion und einem Platzhalter im Namen haben Vorrang vor Erlauben-Aktionen mit Platzhaltern im Namen.

=== Test_DreiEntdeckungsmodus
...
=== Test_DreiMonitor/Schutzmodus (neuer Container im Überwachungs- oder Schutzmodus gestartet)
...
=== Test_DreiErstellen von Prozessprofilregeln
...
Beispiel:  Um den Ping-Prozess aus einem beliebigen Verzeichnis auszuführen
...
=== Test_Drei Eingebaute Verdächtige Prozess Erkennung
