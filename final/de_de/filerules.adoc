...
=== Test_ZweiZero-Drift-Dateischutz
...
=== Test_ZweiBasis-Dateischutzmaßnahmen
...
=== Test_ZweiVerhaltensbasiertes Lernen erlaubte Anwendungen im Entdeckungsmodus
...
=== Test_ZweiErstellen benutzerdefinierter Datei-/Verzeichnisüberwachungsregeln
...
* Dateiänderungen überwachen. Benachrichtigungen (Alerts) für Änderungen generieren
...
=== Test_TwoDateizugriffsregel Reihenfolge Priorität
...
Datei-Zugriffsregeln haben die folgende Priorität, wenn der Dateiname mit vordefinierten Zugriffsregeln der automatisch gelernten Gruppe und den Regelvererbungen mehrerer Gruppen in Konflikt steht.
...
Anzeigen der Datei-Zugriffsregel zum Schutz der Datei /etc/hostname des node-pod-Dienstes und Erlauben der vi-Anwendung, die Datei zu ändern.
...
Anzeigen der Zugriffsregel, die die Datei /etc/passwd schützt, die eine der Dateien ist, die durch die vordefinierte Zugriffsregel abgedeckt ist, um die Datei-Zugriffsaktion für Änderungen sowie zum Lesen zu ändern. Diese benutzerdefinierte Regel ändert die Standardaktion der vordefinierten Datei-Zugriffsregel. Die Anwendung Nano kann 'lesen und ändern' Zugriff auf diese Dateien haben. Muss auch die Nano-Anwendung (Prozess) als 'erlauben' Regel im Prozessprofil für diesen Dienst hinzufügen, um die Nano-Anwendung innerhalb des Dienstes auszuführen (falls sie dort nicht bereits auf die Whitelist gesetzt wurde), andernfalls wird der Prozess von \{product-name} blockiert.
...
== Test_ZweiGeteilte Modus Datei-Schutzmaßnahmen
