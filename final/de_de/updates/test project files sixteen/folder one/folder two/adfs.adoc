= Testprojekt sechzehn SAML (ADFS)
:revdate: 2025-05-08
...
* Sehen Sie sich Endpunkte von AD FS-Verwaltung > Dienst an und verwenden Sie die "`SAML 2.0/WS-Federation`" Endpunkt-URL.
...
=== Test_ZweiMapping Gruppen zu Rollen und Namespaces

Bitte sehen Sie den Abschnitt xref:users.adoc#_mapping_groups_to_roles_and_namespaces[Benutzer und Rollen] für Informationen, wie Gruppen auf vordefinierte und benutzerdefinierte Rollen sowie Namespaces in {product-name} abgebildet werden können.
...
[TIP]
====
Sie können die Authentifizierungszeiten neu laden, indem Sie die Konfiguration in der {product-name} UI wie folgt deaktivieren und wieder aktivieren:
...
Das ausgewählte Zertifikat muss gültig und korrekt generiert sein, einschließlich seiner `CA Root` und `Intermediate Certificates`. Sie können sie mit Ihrer vertrauenswürdigen Zertifizierungsstelle, Windows oder einem Automatisierungstool wie https://letsencrypt.org/[LetsEncrypt] erstellen.
