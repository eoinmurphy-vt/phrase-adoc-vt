= Testprojekt sechzehn 5.x Versionshinweise
:revdate: 2025-08-26
:page-revdate: {revdate}
:page-opendocs-origin: /14.releasenotes/01.5x/01.5x.md
:page-opendocs-slug:  /releasenotes/5x

== Test_ZweiVersionshinweise für 5.x

[NOTE]
====
Um E-Mail-Benachrichtigungen über neue Releases zu erhalten, abonnieren Sie bitte diese SUSE-Mailingliste: https://lists.suse.com/mailman/listinfo/neuvector-updates
====

=== Test_Two5.4.6 August 2025

==== Test_ZweiNeue Funktionen

* *NVSHAS-6733*: Exportieren Sie Antwortregeln als CRD.
* *NVSHAS-9899*: NeuVector Prozessprofilwarnungen für Java-Dienste enthalten sensible Daten.
...
* *NVSHAS-9934*: Verdächtige Fehlfunktion der Zero-Drift-Funktionalität.
* *NVSHAS-9940*: NV-Scan JFrog Subdomain-Modusproblem.
...
* *NVSHAS-9946*: Anzeigeproblem mit der Admission Control-Warnung für den Anmeldetyp.
...
* *NVSHAS-9969*: Fatales Fehler: gleichzeitige Kartenänderungen führen zu einem Neustart der Enforcer-Komponente.
...
* *NVSHAS-9998*: Kann Gruppe nicht von NeuVector-Föderationsmaster exportieren.
...
=== Test_Two5.4.4 Mai 2025

==== Test_ZweiNeue Funktionen

* **NVSHAS-9915**: Zeige die Scanergebnisse des Harbor-Scanner-Moduls in der NeuVector-Benutzeroberfläche an.
...
* **NVSHAS-8160**: [Controller] Passen Sie einige Elemente für die Berechnung des Sicherheitsrisikowertes an.
* **NVSHAS-4673**: Vorschlag, eine Nachricht vor dem Exportieren von Gruppen hinzuzufügen.

==== Test_TwoFehlerbehebungen

* **NVSHAS-9931**: Fügen Sie eine Warnung hinzu, wenn inkonsistente Versionen von NeuVector-Produkten in einem Multi-Cluster vorhanden sind.
...
* **NVSHAS-9912**: Aktualisieren Sie Helm über einige K8s RBAC-Berechtigungen, die vom Controller-Dienstkonto benötigt werden.
...
* **NVSHAS-9176**: Zeige einen Skriptfehler auf der **Security Risks > Vulnerabilities** Seite an, wenn ein Benutzer mit der Berechtigung des Namensraums `rt_scan:w` angemeldet ist.

=== Test_Two5.4.3 March 2025
...
* **NVSHAS-9426**: Füge hostPath für Scanner zum Helm-Diagramm hinzu.
...
* **NVSHAS-9821**: Der Prozessname stimmt nicht mit der Befehlszeile für die Prozessgewinnregelwarnung überein.
* **NVSHAS-9817**: Die Erstellung der NvClusterSecurityRule CRD zeigt eine erfolgreiche Erstellung, wurde jedoch aufgrund doppelter Prozesseinträge nicht tatsächlich erstellt.
...
* **NVSHAS-9784**: NeuVector gibt während des Scannens des Jfrog-Image-Repositorys 404 zurück.
...
* **NVSHAS-9227**: Der Registry-Scan bleibt mitten im Scanprozess hängen und wechselt in den "Leerlauf"-Zustand.
...
=== Test_Two5.4.2 Januar 2025

==== Test_ZweiNeue Funktionen

* **NVSHAS-9726**: Der Monitor übergibt jetzt die Proxy-URL.
* **NVSHAS-9719**: Gibt die Pensionierung von integrierten Zertifikaten bekannt.
* **NVSHAS-9715**: Unterstützung von Helm-Chart-Werten zum Festlegen von Nodeport auf Controller und Manager.
...
* **NVSHAS-9590**: Möglichkeit, auszuwählen, welcher Schwachstellenscore für alle Assets verwendet werden soll.
...
* **NVSHAS-9754**: [UI] Verhindern, dass Rancher-bezogene SSO-Benutzer die Authentifizierung von OpenShift oder Ranchers RBAC deaktivieren.
* **NVSHAS-9751**: [Runtime Protection] Überwachungsmodus + Zero Drift erzeugt keine Warnungen, wenn ein Kindprozess ausgeführt wird.
...
* **NVSHAS-9670**: Manager: Problem mit doppelten Anführungszeichen in der einfachen Textantwort und Problem mit der nicht benannten Java-Bibliothek im sbt-Lauf.
...
==== Test_TwoUpgrade von vorherigen Versionen auf 5.4.2:
...
Für aktuelle Benutzer mit Versionen vor 5.4.2 bleibt das Zertifikat bis zum 31. März 2025 im `neuvector/scanner:latest` verfügbar. Nach diesem Datum wird es entfernt. Benutzer sollten planen, dasselbe interne Zertifikat dem Controller, dem Durchsetzer, dem Scanner und dem Registrierungsadapter bereitzustellen, um den Scanner weiterhin zu verwenden.

Bitte beachten Sie, dass der eigenständige Scanner von diesen Änderungen nicht betroffen sein wird.

=== Test_Two5.4.1 November 2024

==== Test_ZweiNeue Funktionen

* **NVSHAS-8583**: Festlegen von granularen Richtlinienmodi für Regelsets, separater Netzwerkrichtlinienmodus und Profilmodus auf Gruppenebene.
...
* **NVSHAS-9467**: Benutzerdefinierte Gruppen, die durch das Pod-Label definiert sind, propagieren ihre Profildaten nicht auf die untergeordneten Container.
...
* **NVSHAS-8824**: Benutzer kann eigene Gruppen nicht löschen und kann keine namespace-spezifischen Gruppen erstellen.
* **NVSHAS-9605**: Exportgruppe mit ungültigen Richtlinienmodus- und Prozessprofilmoduswerten wird fälschlicherweise erlaubt.
* **NVSHAS-9608**: Scanner meldet keinen Fehler, wenn der Controller einen Fehler für große Scanergebnisse ~23MB meldet.
* **NVSHAS-9534**: Fehler in den Zugriffssteuerungen anzeigen.
...
* **NVSHAS-9212**: Zeige Warnmeldung in GET(/v1/eula) an, wenn die Rolle (Binding) des neuvector-binding-secret falsch ist.
...
* **NVSHAS-9480**: NV UI, das im Rancher-Downstream-Cluster bereitgestellt wird, gibt nach der Abmeldung von Rancher HTTP/403 zurück.
...
* **NVSHAS-9561**: Die Gesamtbewertung der Sicherheit des Dashboards sollte mit der tatsächlichen Bewertung übereinstimmen.
* **NVSHAS-9572**: [Schwachstellen] Gefilterte Daten wurden unabhängig von der Benutzeraktualisierung oder dem erneuten Anmelden auf der Seite beibehalten.
...
==== Test_ZweiBekannte Probleme

* In der Chart-Version 2.8.3 haben wir eine zuvor falsch zugewiesene Ressource von crds nach core verschoben. Wenn Sie sowohl crds- als auch core-Charts verwenden, können während des Upgrades Probleme auftreten, wenn Sie zuerst core bereitstellen. Um dies zu beheben, aktualisieren Sie zuerst die crds und dann die core-Charts.

=== Test_Two5.4 September 2024
...
* **NVSHAS-9012**: Anzeige von Rancher SSO-Benutzern in der NV-Benutzeroberfläche mit demselben Benutzernamen.
...
* **NVSHAS-8148**: Link von Container-Image zu Registry-Image-Scan-Ergebnissen.
...
* **NVSHAS-9218**: Unterstützung für Bundes- und CRD-Gruppen für DDoS-Überwachung.
* **NVSHAS-8461**: Unterstützung von CIS-Benchmarks für verwaltete k8s-Dienste in der Cloud.
* **NVSHAS-7664**: Reduzierung der ISP-Datengebühren während des Registry-Scannens.
* **NVSHAS-8868**: Scanner-Cache-Statistiken offenlegen.
* **NVSHAS-8676**: Verbesserung von NV Protect für Benchmark-Skripte.
* **NVSHAS-9255**: Anpassung der Zulassungssteuerungssuchregistrierungen für Bildnamen ohne FQDN.
* **NVSHAS-9144**: ID für das Schwachstellenprofil hinzugefügt, um die Identifizierung zu erleichtern.
* **NVSHAS-7687**: Unterstützung für die Konfiguration des Protokollniveaus (debug/error/info/warn) für Durchsetzer und Controller über die CLI.
...
* **NVSHAS-8799**: Erstellen Sie ein Compliance-Framework zum Importieren von Compliance-Vorlagen.
...
* **NVSHAS-9085**: Der PDF-Bericht der Assets-Ansicht zeigt 0% Verwundbarkeit, selbst bei vorhandenen Verwundbarkeiten.
* **NVSHAS-9084**: Der PDF-Bericht der Assets-Ansicht zeigt NaN, wenn die Bildliste leer ist.
...
* **NVSHAS-9025**: Neuvector-Verwundbarkeitsakzeptanzbereich für Container.
...
* **NVSHAS-9266**: Registrierungs-Scan: Der Scanbericht-Button nach Schicht sollte ausgeblendet oder deaktiviert sein, wenn keine Schwachstelle vorhanden ist.
...
* **NVSHAS-8947**: NV-Konfiguration kann nicht importiert werden, wenn über Rancher SSO authentifiziert.
...
* **NVSHAS-9362**: Der Sicherheitsrisiko-Filter für Schwachstellen gibt 0 Ergebnisse zurück.
* **NVSHAS-8699**: Es ist nicht möglich, den Benutzer zu unterscheiden, wenn der Rancher AD-Benutzer derselbe ist.
* **NVSHAS-9062**: Rancher SSO-Benutzer auf der NV UI anzeigen, die denselben Benutzernamen haben (Konvertierung auf dem Controller).
...
* **NVSHAS-9091**: Es ist nicht möglich, alle Module unter ol:9.1, photon:5.0, rhel:9.1 und amzn:2023 im Repo, in der Registry und im Standalone-Scan zu melden.
...
* **NVSHAS-9080**: Fed Reader-Benutzer kann auf einige REST-APIs nicht zugreifen.
* **NVSHAS-9092**: Namespaced-Benutzer sollte keine globalen Assets sehen.
* **NVSHAS-9116**: Der Worker-Cluster kann verlassen werden, wenn die Verbindung unterbrochen wird.
* **NVSHAS-8980**: Host- und Tunnel-Schnittstelle auf dem Knoten erfolgreich in oc 4.15 abrufen.
* **NVSHAS-9188**: Setze mgmt-br-Schnittstelle als Hostschnittstelle für den Ernteknoten.
* **NVSHAS-4858**: Containergruppe im Controller nicht erweitern, um die Leistung der Richtlinienbereitstellung zu verbessern und CPU- sowie Speicherauslastung zu reduzieren.
...
* **NVSHAS-9189**: Der Scan wird nach dem Herunterfahren und Neustarten des Controllers im Scheduling stecken bleiben.
...
* **NVSHAS-9292**: Beheben Sie die Anzeige von 0 Schwachstellen bei Ingress Egress.
* **NVSHAS-9270**: Unterstützen Sie k3s für die CIS-Benchmark-Pipeline.
...
* **NVSHAS-9443**: Upgrade/Installation über ArgoCD schlägt fehl, da es das Objekt leases.coordination.k8s.io nicht erstellen kann.
* **Workaround**: Erstellen Sie die angegebenen Lease-Objekte, bevor Sie auf 5.4.0 mit ARGO CD aktualisieren. Ändern Sie den Namespace, wenn er anders als neuvector ist.
+
--
[,bash]
----
...
=== Test_Zwei5.3.4 Juli 2024

==== Test_ZweiFehlerbehebungen

* Die `host` und `tunnel` Schnittstelle wurde erfolgreich mit OpenShift CLI v4.15 abgerufen.
...
* Die `LDAP Authentication` hat separate Felder für die Konfiguration von `baseDN` und `groupDN`.
* Die `Egress and Ingress chart` hat eine neue Spalte für Schwachstellen, die die Anzahl der `High` und `Medium` Schwachstellen für jeden Dienst enthält.

==== Test_ZweiFehlerbehebungen

* Fehler behoben, der mit `regex` auftrat, wenn ein Komma (`+,+`) in einem Mehrfacheintrag `Admission Control user criteria` verwendet wurde.
* Fehler behoben, bei dem der CVE-Scan von `jar` Paketen nicht alle von derselben CVE betroffenen Pakete anzeigte. Jetzt werden alle Vorkommen gemeldet.
* Behebt folgende CVEs:
...
* Die Versionierung des Prometheus-Exportcontainers wurde überprüft und von der `controller` Versionierung disssoziiert.
...
* Erlauben Sie Benutzern, '`akzeptierte`' Schwachstellen bei der Verwendung von Github-Aktionen zu definieren, damit sie die Workflows nicht beeinträchtigen.
* Fügen Sie Schweregrad-, Punktzahl- und Feed-Bewertungsfilter zur Ansicht Assets > Registry > Bildschwachstellen hinzu.
...
* Erlauben Sie die signaturüberprüfung von rootlosen Schlüsselpaare-Bildern ohne Internet- oder Sigstore-Abhängigkeit.
...
* (Scanner) OCI-Imageformat akzeptieren, wenn auf Docker-API 1.24 umgeschaltet wird.
* (Scanner) Registry-Scan sollte keine Nicht-Image-Artefakte scannen / keinen Fehler protokollieren.
* (Scanner) Unterstützung für PHP Composer-Scan hinzufügen.

==== Test_Two{product-name} UI Extension v. 1.0 für Rancher März 2024

* Nach der Installation von {product-name}, xref:rancher.adoc#_suse_security_ui_extension_for_rancher[Aktivierung/Installation] wird die {product-name} UI-Erweiterung von Rancher ein Dashboard für den Cluster anzeigen, einschließlich Links zu SSO zum vollständigen {product-name} Cluster. HINWEIS: Die Erweiterung kann als Drittanbieter angezeigt werden, was in einer zukünftigen Version behoben wird. Außerdem können Rancher 2.7.x-Benutzer nach der Installation zwei {product-name} UI-Ext-Symbole in der Liste sehen (Fehler). Ein Symbol wird Deinstallieren sagen (was bedeutet, dass es installiert ist), und das andere sollte Installieren sagen. Dies kann so belassen werden, d.h. nicht erneut installieren, wenn die Erweiterung bereits installiert ist.

=== Test_Two5.2.4-s2 Februar 2024
...
* Im Entdeckungsmodus automatisch Egress zu externen FQDN-Adressgruppen lernen. Eine neue externe FQDN-Benutzergruppe wird erstellt, es sei denn, die externe Verbindung entspricht einer bestehenden Regel.
* Aktivieren Sie das ICMP-Lernen (Entdeckungsmodus) und die Blockierung (Schutzmodus) über die neue Controller-Umgebungsvariable CTRL_EN_ICMP_POLICY = Test_Two1
...
* Verbessern Sie die Funktion zur Überprüfung der Zulassungskontrolle, um die Ergebnisse aller Regeln einzuschließen. Listen Sie das Ergebnis jeder Regel auf und fügen Sie einen weiteren Eintrag für die endgültige Aktion hinzu, die auftreten würde, wenn sie in einer Live-Zulassungskontrolle bewertet wird.
* Anwendung deaktivierter Zulassungskontrollregeln über CRD oder yaml (kubectl)
* Exportieren / Importieren von Schwachstellenprofilen über Konsole, CRD oder REST-API. Das Importieren ersetzt das vorhandene Profil. Das Löschen der CRD führt zu einem leeren Profil.
* Exportieren / Importieren von Compliance-Profilvorlagen über Konsole, CRD oder REST-API. Das Importieren ersetzt die vorhandene Vorlage.
* Fügen Sie einen 'Manuell'-Status in den Compliance-Berichten für CIS-Benchmarks hinzu, die manuell von Benutzern ausgeführt werden müssen (nicht ausgeführt von {product-name}).
* Verbessern Sie das Laden/ die Leistung der Benutzeroberfläche der Schwachstellenseite
* Vereinheitlichen Sie die Anmeldung der Browsersitzung. Damit teilen sich alle Registerkarten im Browser dieselbe Anmeldesitzung, das Öffnen einer neuen Registerkarte aus einer bestehenden Sitzung erfordert keine Anmeldeinformationen, und wenn eine Registerkarte sich abmeldet, werden alle Registerkarten abgemeldet.
...
HTTP Strict Transport Security max-age=15724800, 2) Fügen Sie den CSP-Header hinzu (z. B. setzen Sie eine '`default-src`'-Richtlinie), 3) Entfernen Sie die Offenlegung des Servernamens
* Unterstützen Sie neuere Versionen der CIS-Benchmarks. Kubernetes (1.8.0), Kubernetes V1.24 (1.0.0), Kubernetes V1.23 (1.0.1), RedHat OpenShift Container Platform (1.4.0)
* Zeigen Sie in Assets -> Container -> Containerdetails an, die in Registrierungen im Vergleich zur Laufzeit gescannt wurden
...
* Unterstützen Sie Deep Linking in URLs zur Bild- und/oder Container-Schwachstellenseite
...
* Fügen Sie das Menü "Get Support" mit Links zu Slack, Dokumentation und anderen Ressourcen hinzu
* Füllen Sie das Nachrichtenfeld zu /v1/log/activity logs
...
* Das Löschen verwundbarer Dateien (z. B. jar) entfernt sie nicht aus der Schwachstellenliste
* Ungültiges Syslog-Zertifikat mit dem Signaturalgorithmus SHA256withECDSA
* {product-name} zeigt Sicherheitsereignisse an, die durch eine Netzwerkregel erlaubt sein sollten
* Unverwalteter Knoten mit "Zombie"-Enforcer, der läuft
* Erweiterter Filter zeigt Felder für Behebung und Auswirkungen leer an
* Behebe die Zeichenfolgenverarbeitung, um unerwartete Neustarts des Enforcers zu verhindern
* Unerwartete Verstöße in Bezug auf integrierte Gruppen
* Support-Bundle Enforcer Debug RPC-Aufruf für Daten gibt Fehler zurück
* Gruppe stimmt in Sicherheitsereignissen nicht überein
...
* Entfernen Sie die Einstellung für den Betrieb des Controllers im privilegierten Modus im Helm-Diagramm und die Anforderung, dass der Controller/Enforcer "/host/cgroup" einbindet.
...
==== Test_TwoHervorgehobene Änderungen, die möglicherweise Änderungen für manuelle Bereitstellungen erfordern (alle Änderungen sind bereits im neuesten Helm-Diagramm für 5.3.x enthalten)
...
* Die Entfernung der Anforderung, den Controller im privilegierten Modus auszuführen, beseitigt die Notwendigkeit, den Laufzeitsocket zu montieren und /host/cgroup/ zu montieren.
...
=== Test_Two5.2.4-s1 Januar 2024

==== Test_TwoSecurity Patch Release

* Behebt CVE-2023-6129 in openssl und CVE-2023-46219, CVE-2023-46218 in curl.

=== Test_Two5.2.4 November 2023

==== Test_ZweiFehlerbehebungen

* Änderungen an der Azure AKS ValidatingWebhookConfiguration und Fehlerprotokollierung.

=== Test_Two5.2.3 November 2023

==== Test_TwoEnhancements

* Unterstützung für die NVD API 2.0 im Scanner hinzufügen.
...
=== Test_Two5.2.2-s1 Oktober 2023

==== Test_TwoSecurity Update

* Pakete aktualisieren, um CVEs zu beheben, einschließlich Hoch CVE-2023-38545 und CVE-2023-43804.

=== Test_Two5.2.2 Oktober 2023

==== Test_TwoSecurity Advisory for CVE-2023-32188

* Beheben Sie CVE-2023-32188 "`JWT-Token-Kompromittierung kann böswillige Aktionen einschließlich Remote Code Execution (RCE) ermöglichen.`" durch die automatische Generierung des Zertifikats, das zum Signieren des JWT-Tokens bei der Bereitstellung und Aktualisierung verwendet wird, und die automatische Generierung des Manager/RESTful API-Zertifikats während der Helm-basierten Bereitstellungen.
 ...
 ** Die automatische Generierung des Manager-, REST-API- und Registrierungsadapterzertifikats erfordert die Verwendung einer Helm-basierten Installation mit {product-name} Helm-Version 2.6.3 oder höher.
...
* Fügen Sie zusätzliche Scandaten zu den von SYSLOG gesendeten CVE-Ergebnissen für geschichtete Scans hinzu
* Unterstützung für die NVD-API 2.0 für die CVE-Datenbank scannen
...
* Aktivieren/Deaktivieren der TLS 1.0- und TLS 1.1-Erkennung/Alarmierung mit Umgebungsvariablen für Enforcer THRT_SSL_TLS_1DOT0, THRT_SSL_TLS_1DOT1. Standardmäßig deaktiviert.
* Fügen Sie die Umgebungsvariable AUTO_PROFILE_COLLECT für Controller und Enforcer hinzu, um bei der Erfassung des Speicherverbrauchs bei der Untersuchung von Speicherbelastungsereignissen zu helfen. Setzen Sie den Wert = Test_Two1, um zu aktivieren.
* Konfigurationsbewertungen gegen die Zulassungssteuerung sollten alle Verstöße mit einem Scan anzeigen.
* Fügen Sie weitere Optionen für die CVE-Berichtskriterien in den Reaktionsregeln hinzu. Beispiel 1 - "cve-high-with-fix:X" bedeutet: Wenn die Anzahl der (hochgefährlichen Schwachstellen, die behoben wurden) >= Test_TwoX, lösen Sie die Reaktionsregel aus. Beispiel 2 - "cve-high-with-fix:X/Y" bedeutet: Wenn die Anzahl der (hochgefährlichen Schwachstellen, die vor Y Tagen gemeldet wurden und behoben wurden) >= Test_TwoX, lösen Sie die Reaktionsregel aus.

==== Test_ZweiFehlerbehebungen

* Der Export der Gruppenrichtlinie gibt keine tatsächlichen YAML-Inhalte zurück
...
=== Test_Two5.2.1 August 2023

==== Test_TwoEnhancements

* Berichten Sie über die Ergebnisse der geschichteten Scans und zusätzliche CVE-Daten in SYSLOG-Nachrichten. Dies wird über ein Kontrollkästchen in den Einstellungen -> Konfiguration -> SYSLOG aktiviert.
* Exportieren Sie NIST 800-53-Zuordnungen (zu Docker CIS-Benchmarks) im exportierten CSV-Compliance-Bericht.
* Unterstützung der Proxy-Einstellungen bei der Überprüfung von Bildsignaturen.
* Fügen Sie das Ergebnis des Bildsignatur-Scans im heruntergeladenen CVE-Bericht hinzu.
* Unterstützung von Pod-Annotationen für Zulassungskontrollrichtlinien, verfügbar über die benutzerdefinierten Kriterien.
* Fügen Sie das Feld "Zuletzt geändert" hinzu, um die Berichterstattung über Schwachstellen zu filtern, sowie den erweiterten Filter in der Ansicht Schwachstellen.

==== Test_TwoFehlerbehebungen

* Erstellen Sie keinen Standardadministrator mit dem Standardpasswort in der initialen {product-name} Bereitstellung für das AWS-Abrechnungsangebot (CSP-Adapter), wodurch der Benutzer gezwungen wird, ein Geheimnis zu verwenden, um den Benutzernamen und das Passwort des Administrators zu erstellen.
...
* Wenn das Helm-CRD-Diagramm zuerst installiert wird, bevor das {product-name} Core-Diagramm installiert wird, fehlen die Dienstkonten.
* Das Ergebnis der Compliance-Prüfung des Bildscans I.4.1 ist falsch.
* Der Bericht über den erweiterten Schwachstellenfilter zeigt Bilder aus allen anderen Namespaces an.

=== Test_Two5.2.0 Juli 2023

==== Test_TwoEnhancements

* Unterstützung von Tokens für den {product-name} API-Zugriff. Siehe Einstellungen -> Benutzer, API-Schlüssel..., um einen neuen API-Schlüssel zu erstellen. Schlüssel können auf Standard- oder benutzerdefinierte Rollen gesetzt werden.
* Unterstützung der AWS Marketplace PAYG-Abrechnung für {product-name} monatliche Support-Abonnements. Benutzer können sich für {product-name} durch den SUSE-Support anmelden, der monatlich auf ihr AWS-Konto basierend auf der durchschnittlichen Nutzung der Knotenanzahl des vorherigen Monats abgerechnet wird. Details https://open-docs.neuvector.com/deploying/awsmarketplace[hier].
* Unterstützung der Bildsignierung für Zugangssteuerungen. Benutzer können {product-name} verlangen, dass Bilder von bestimmten Parteien signiert sind, bevor sie in die Produktionsumgebung bereitgestellt werden, durch eine Integration mit Sigstore/Cosign. Siehe Assets -> Sigstore-Überprüfer zum Erstellen neuer Signatur-Assets. Regeln können dann mit den Kriterien Bildsignierung und/oder Bild-Sigstore-Überprüfer erstellt werden.
* Aktivieren Sie, dass jede Zugangssteuerungsregel ihren eigenen Modus Monitor oder Protect hat. Eine Ablehnungsaktion im Überwachungsmodus wird alarmieren, und eine Ablehnungsaktion im Schutzmodus wird blockieren. Erlaubte Aktionen sind nicht betroffen.
* Fügen Sie einen neuen Regex-Operator in Richtlinie > Zugangssteuerung > Regel hinzufügen für Benutzer und Benutzergruppen hinzu, um Regex zu unterstützen. Unterstützung von Operatoren "entspricht JEDER Regex in" und "entspricht KEINER Regex in".
* Fügen Sie Unterstützung für Kriterien der Zugangssteuerung wie Ressourcenlimits hinzu. Ein neues Kriterium wird für Ressourcenlimits hinzugefügt, und zusätzliche Kriterien werden über die benutzerdefinierten Kriterieneinstellungen unterstützt.
* Unterstützung für das Aufrufen des {product-name} Scanners von Harbor-Registrierungen über die https://github.com/goharbor/pluggable-scanner-spec[pluggable scanner] Schnittstelle. Dies erfordert die Konfiguration der Verbindung zum Controller (exponierte API). Der Harbor-Adapter ruft den Controller-Endpunkt auf, um einen Scan auszulösen, der automatisch beim Pushen scannen kann. Befragungsdienste können für regelmäßige Scans verwendet werden. Scanergebnisse von Federation Primary-Controllern WERDEN an entfernte Cluster propagiert.  HINWEIS: Es gibt ein Problem mit dem HTTPS-basierten Adapter-Endpunktfehler: Bitte ignorieren Sie den Testverbindungsfehler, es funktioniert, auch wenn ein Fehler angezeigt wird (Zertifikatsvalidierung überspringen).
* Durchsuchbarer SaaS-Dienst für CVE-Abfragen. Durchsuchen Sie die neueste {product-name} CVE-Datenbank, um zu sehen, ob ein bestimmter CVE in der Datenbank vorhanden ist. Dieser Dienst steht für {product-name} Prime (kostenpflichtiges Support-Abonnement) Kunden zur Verfügung. Kontaktieren Sie den Support über Ihr SCC-Portal für den Zugriff.
...
* Verwenden Sie weniger privilegierte Dienstkonten, wie für jede {product-name} Komponente erforderlich. Eine Variable "`leastPrivilege`" wird eingeführt. Der Standardwert ist falsch. HINWEIS: Die Verwendung des aktuellen Helm-Diagramms mit dieser Variablen in einer Version vor 5.2.0 funktioniert nicht richtig.
* Binden Sie an ein nicht standardmäßiges Dienstkonto, um die CIS 1.5 5.1.5 Empfehlung zu erfüllen.
* Ermöglichen Sie dem Administrator, die standardmäßige Sitzungstimeout für Benutzer in den Einstellungen -> Benutzer, API-Schlüssel und Rollen zu konfigurieren.
...
* Fügen Sie den CSV-Export für Registry / Details hinzu, um CVEs für alle Bilder im konfigurierten Registry in Assets -> Registries für eine ausgewählte Registry zu exportieren.
...
* Reduzieren Sie sich wiederholende Protokolle in den Protokollen des Durchsetzungs-/Steuerungssystems.
...
* Blockierung von SSL-Verbindungen, auch wenn eine Netzwerkregel den Datenverkehr unter bestimmten Anfangsbedingungen erlaubt.
...
* Fügen Sie die Einstellung "leastPrivilege" im Helm-Diagramm hinzu. Fügen Sie die Helm-Option für New_Service_Profile_Baseline hinzu. Eine neue Helm-Diagrammversion (Kern) wird für 5.2 veröffentlicht.
* Aktivieren Sie die Integrationsparameter für AWS Marketplace (Abrechnungsadapter) im Helm-Diagramm.
...
* Fügen Sie eine Überprüfung der imagePullPolicy in die Kriterien der Zulassungssteuerungsregeln ein.
* Zeigen Sie eine Warnmeldung an, wenn das CRD-Schema veraltet ist.

==== Test_ZweiFehlerbehebungen

* Der Bildschirm für Netzwerkaktivität wird nicht gerendert oder wird falsch gerendert.
* Die automatische Entfernung leerer Gruppen dauert 2 Stunden zum Löschen anstelle von 1 Stunde gemäß Zeitplan.
...
==== Test_TwoScanner Beispielausgabe
...
=== Test_Two5.1.2 March 2023

==== Test_TwoEnhancements

* Unterstützen Sie die adressbasierte Gruppen- und Richtlinienübereinstimmung für Netzwerkschutz. Dies ermöglicht einen Anwendungsfall, bei dem zwei verschiedene FQDN-Adressen auf dieselbe IP-Adresse aufgelöst werden, aber unterschiedliche Regeln für jeden FQDN durchgesetzt werden sollten. Eine neue benutzerdefinierte Gruppe mit '`address=vh:xxx.yyy`' kann mit dem '`vh:`' Indikator erstellt werden, um diesen Schutz zu aktivieren. Eine Netzwerkregel kann dann die benutzerdefinierte Gruppe als '`From`' Quelle basierend auf dem virtuellen Hostnamen (anstatt der aufgelösten IP-Adresse) verwenden, um unterschiedliche Regeln für virtuelle Hosts durchzusetzen.
* Liste der Compliance-Container, um beendete Container auszuschließen.
* Verbessern Sie die DLP-Regeln, um einfache Wildcards im Muster zu unterstützen.
...
* Aktivierung der automatischen Scanfunktion und Löschung von nicht verwendeten Gruppen im configMap hinzufügen.
* IP-Adresse für externe Quelle/Ziel in csv/pdf für implizite Verstoßverletzungen einfügen.
* Verschiedene Leistungs- und Skalierungsoptimierungen für CPU- und Speicherverbrauch von Controller und Durchsetzer.

==== Test_ZweiFehlerbehebungen

* Anwendungs-Verlangsamung auf GKE Container Optimized OS (COS)-Knoten im Schutzmodus beheben.
* SUSE Linux (SLES) 15.4 CVE wird im Scanner nicht erkannt. Mit diesem Fix wird, wenn der Schweregrad im Feed angegeben ist, die Schwachstelle zur Datenbank hinzugefügt, auch wenn der NVD-Eintrag fehlt. Es ist möglich, dass der Bericht Schwachstellen ohne CVE-Werte enthält.

==== Test_TwoOther

* Verbessern Sie die Optionen für Admission Control CRD in helm https://github.com/neuvector/neuvector-helm/pull/237..
* Neue Durchsetzer-Umgebungsvariablen zum Helm-Chart hinzufügen.

=== Test_Two5.1.1 Februar 2023.

==== Test_TwoEnhancements

* Fügen Sie "`package`" als Information zum syslog-Ereignis für eine erkannte Schwachstelle hinzu.
* Fügen Sie die Durchsetzer-Umgebungsvariable ENF_NETPOLICY_PULL_INTERVAL hinzu - Wert in Sekunden (empfohlener Wert 60), um den Netzwerkverkehr und den resultierenden Ressourcenverbrauch durch den Durchsetzer aufgrund von Richtlinienaktualisierungen/-neuberechnungen zu reduzieren. (Hinweis: Dies war bis August 2023 eine nicht dokumentierte Ergänzung).

[,yaml]
----
           - name: ENF_NETPOLICY_PULL_INTERVAL
             value: "60"   <== Test_Tworegulate the pulling gap by 60 seconds
----

==== Test_ZweiFehlerbehebungen

* Fehler beim Löschen leerer Gruppen "Objekt nicht gefunden".
* Verkehr innerhalb des gleichen Containers alarmierend/blockierend
* Unerwartete implizite Verstöße für Istio Egress-Verkehr mit vorhandener Erlaubensregel
* Beim Upgrade von {product-name} 4.x-Version verursacht falsche Pod-Gruppenmitgliedschaft unerwartete Richtlinienverletzungen
...
* Das Blockieren des Logins nach fehlgeschlagenen Anmeldeversuchen wird jetzt zur Standardoption. Der Standardwert beträgt 5 Versuche und ist in den Einstellungen -> Benutzer & Rollen-> Passwortprofil konfigurierbar.
* Fügen Sie eine neue Umgebungsvariable für die Leistungsoptimierung ENF_NO_SYSTEM_PROFILES hinzu, Wert: "1". Wenn aktiviert, werden die Prozess- und Dateimonitoren deaktiviert. Keine Lernprozesse, keine Profilmodi, keine Prozess-/Dateivorfälle (Paket) und kein Dateiaktivitätsmonitor werden durchgeführt. Dies wird die CPU-/Speicherressourcennutzung und Dateioperationen reduzieren.
 ...
 ** *_Verzögerte Strategie:_*
  ...
  *** Wenn der Hauptcontroller kontinuierlich sieht, dass die "Aufgabenanzahl" 0 für > 180 Sekunden ist, wird ein Scanner-Pod heruntergefahren, wenn minScannerPods noch nicht erreicht ist.
...
=== Test_Two5.0.6-s1 March, 2023

==== Test_ZweiFehlerbehebungen

* Aktualisieren Sie Alpine-Pakete, um CVEs in curl zu beheben, einschließlich CVE-2023-23914, CVE-2023-23915 und CVE-2023-23916.

=== Test_Two5.0.6 Februar 2023
...
* tcpdump kann nicht erfolgreich gestartet werden, wenn der Datenverkehr in einem Container gesnifft wird
...
* Unerwarteter Prozess.Profile.Violation Vorfall in der NV.Protect-Gruppe, auf dem Befehl auf coreos.

=== Test_Two5.0.4 October, 2022

==== Test_TwoSicherheitsupdates

* Alpine aktualisieren, um die kritische CVE-2022-40674 in der Manager-Expat-Bibliothek sowie andere kleinere CVEs zu entfernen.
...
=== Test_Two5.0.3 September, 2022

==== Test_TwoEnhancements

* Die EULA nach erfolgreichem Neustart vom persistenten Volume nicht anzeigen.
* Verwenden Sie den Bildfilter in den Einstellungen des Schwachstellenprofils, um die Container-Scan-Ergebnisse zu überspringen.
* Scanner in GitHub-Aktionen unterstützen bei https://github.com/neuvector/neuvector-image-scan-action.
...
* Der Durchsetzer wird aufgrund des Fehlers = zu viele offene Dateien (rke2/cilium) ständig neu gestartet.
* Support-Protokoll kann nicht erfolgreich heruntergeladen werden.

=== Test_Two5.0.1 June 2022

==== Test_TwoEnhancements

* Support-Sicherheitsüberprüfung von openSUSE Leap OS (im Scanner-Image).
* Scanner: Implementieren Sie Wischattribute während der Rekonstruktion des Image-Repos.
* Überprüfen Sie {product-name} Bereitstellung und Unterstützung für Hosts mit aktivem SELinux. Siehe unten für Details zu vorübergehenden Patches, bis das Helm-Diagramm aktualisiert wird.
* Unterscheiden Sie zwischen Feature-Diagramm und Partner-Diagrammen in der Rancher-Benutzeroberfläche deutlicher. + Verbesserung der Ingress-Annotation für nginx im Rancher-Helm-Diagramm. Fügen Sie hinzu / aktualisieren Sie
...
* AKS-Cluster könnte unerwarteten Schlüssel im Admission-Control-WebHook hinzufügen.
* Der Durchsetzer wird in einem k8s 1.24-Cluster mit 1.64 containerd-Laufzeit nicht betriebsbereit. Getrennt startet der Durchsetzer manchmal nicht.
...
* Wenn SSO den Standard-Admin von Rancher in {product-name} im Master-Cluster verwendet, ist die {product-name} Anmelderolle admin, nicht fedAdmin.
...
* OKD 3.11 - Clusterrollenfehler wird angezeigt, auch wenn sie existiert.

==== Test_TwoCVE Remediations
...
==== Test_TwoDetails zur SELinux-Unterstützung

{product-name} benötigt keine zusätzlichen Einstellungen für SELinux-aktivierte Cluster, um bereitgestellt und zu funktionieren. Getestet die Bereitstellung von {product-name} auf einem SELinux-aktivierten RKE2 gehärteten Cluster, der auf RHEL 8.5 basiert. {product-name} wurde erfolgreich bereitgestellt, wenn PSP aktiviert ist und das Patch-Manager- und Scanner-Deployment. Die nächste Chart-Version sollte das untenstehende Problem beheben.

Beigefügtes Beispiel zum Aktivieren von PSP aus dem Rancher-Chart und die unten angegebenen Befehle für das Patch-Manager- und Scanner-Deployment. Die Benutzer-ID im Patch-Befehl kann jede Zahl sein.
...
Beispiel zum Aktivieren von PSP:
...
=== Test_Two5.0.0 Allgemeine Verfügbarkeit (GA) Veröffentlichung Mai 2022

==== Test_TwoEnhancements

* Automatisierte Förderung von Gruppenmodi. Fördert den Schutzmodus einer Gruppe basierend auf der verstrichenen Zeit und Kriterien. Gilt nicht für CRD erstellte Gruppen. Dieses Feature ermöglicht es einer neuen Anwendung, für einen bestimmten Zeitraum in Discover zu laufen, das Verhalten zu lernen und {product-name} Erlauben-Listenregeln für Netzwerk und Prozess zu erstellen, um dann automatisch in den Monitor- und dann in den Schutzmodus zu wechseln. Kriterium von Discover zu Monitor: Verstrichene Zeit zum Lernen aller Netzwerk- und Prozessaktivitäten von mindestens einem aktiven Pod in der Gruppe. Kriterium von Monitor zu Protect: Es gibt keine Sicherheitsereignisse (Netzwerk, Prozess usw.) für den festgelegten Zeitraum der Gruppe.
* Unterstützung für Rancher 2.6.5 Apps und Marketplace-Diagramm. Wird in den Namespace cattle-neuvector-system bereitgestellt und ermöglicht SSO von Rancher zu {product-name}. Hinweis: Frühere Bereitstellungen von Rancher (z. B. Partnerkatalog-Diagramme, Version 1.9.x und früher) müssen vollständig entfernt werden, um auf das neue Diagramm zu aktualisieren.
* Unterstützung für das Scannen von SUSE Linux (SLE, SLES) und Microsoft Mariner.
* Null-Drift-Prozess- und Dateischutz. Dies ist der neue Standardmodus für Prozess- und Dateischutz. Null-Drift erlaubt automatisch nur Prozesse, die vom übergeordneten Prozess stammen, der im ursprünglichen Container-Image enthalten ist, und erlaubt keine Dateiaktualisierungen oder die Installation neuer Dateien. Im Discover- oder Monitor-Modus wird Null-Drift bei verdächtigen Prozess- oder Dateiaktivitäten alarmieren. Im Schutzmodus wird es solche Aktivitäten blockieren. Null-Drift erfordert nicht, dass Prozesse gelernt oder zu einer Erlauben-Liste hinzugefügt werden. Das Deaktivieren von Null-Drift für eine Gruppe führt dazu, dass die für die Gruppe aufgeführten Prozess- und Dateiregeln wirksam werden.
* Split-Policy-Modus-Schutz für Netzwerk, Prozess/Datei. Es gibt jetzt eine globale Einstellung in den Einstellungen -> Konfiguration, um den Netzwerk-Schutzmodus zur Durchsetzung von Netzwerkregeln separat festzulegen. Wenn dies aktiviert ist (Standard ist deaktiviert), werden alle Netzwerkregeln im ausgewählten Schutzmodus (Entdecken, Überwachen, Schützen) ausgeführt, während die Regeln für Prozesse/Dateien im Schutzmodus für diese Gruppe bleiben, wie im Bildschirm Richtlinien -> Gruppen angezeigt. Auf diese Weise können Netzwerkregeln auf Schützen (blockieren) gesetzt werden, während die Richtlinien für Prozesse/Dateien auf Überwachen gesetzt werden können, oder umgekehrt.
...
* CRD für WAF, DLP und Zulassungssteuerungen. HINWEIS: erforderliche zusätzliche Clusterrollenbindungen/Berechtigungen. Siehe Kubernetes- und OpenShift-Bereitstellungsabschnitte. CRD-Import/Export und Versionierung für Zulassungssteuerungen, unterstützt durch CRD.
...
* Unterstützung für die Föderation von Clustern (Multi-Cluster-Manager) über einen Proxy. Konfigurieren Sie den Proxy in den Einstellungen -> Konfiguration und aktivieren Sie den Proxy, wenn Sie Föderationsverbindungen konfigurieren.
* Überwachen Sie die erforderlichen RBAC-Clusterrollen/Bindungen und alarmieren Sie in Ereignissen und der Benutzeroberfläche, wenn welche fehlen.
* Unterstützen Sie Kriterien für Ressourcenbeschränkungen in Zulassungssteuerungsregeln.
* Unterstützen Sie das Microsoft Teams-Format für Webhooks.
* Unterstützung für AD/LDAP-Nested-Gruppen unter zugeordneten Rollengruppen.
* Unterstützung für Clusterrolebindings oder Rolebindings mit Gruppeninformationen in IDP für Openshift.
* Erlaube Netzwerkregeln und Zulassungssteuerungsregeln, zu einer föderierten Regel befördert zu werden.

==== Test_ZweiFehlerbehebungen

* Behebe das Problem, dass das Backup der Worker-Föderationsrolle in nicht-föderierte Cluster wiederhergestellt werden sollte.
* Verbessere die Ladezeiten der Seite für eine große Anzahl von CVEs in Sicherheitsrisiken -> Verwundbarkeiten.
* Erlaube dem Benutzer, den Modus zu wechseln, wenn er alle Gruppen im Menü Richtlinie -> Gruppen auswählt. Warnen, wenn die Knoten-Gruppe ebenfalls ausgewählt ist.
* Falte Compliance-Prüfposten mit demselben Namen zusammen und mache sie erweiterbar.
* Verbessere die Sicherheit der gRPC-Kommunikation.
* Behoben: konnte keine korrekten privilegierten Informationen zur Arbeitslast in der rke2-Installation abrufen.
* Behebe das Problem mit der Unterstützung von openSUSE Leap 15.3 (k8s/crio).

==== Test_ZweiAndere Updates
...
* Die Unterstützung für die Bereitstellung auf ECS wird nicht mehr bereitgestellt. Das All-in-One sollte weiterhin auf ECS bereitgestellt werden können, jedoch wird die Dokumentation der Schritte und Einstellungen nicht mehr unterstützt.

=== Test_ZweiAktualisierung von {product-name} 4.x auf 5.x (vor 5.2.x)
...
Für Helm-Benutzer, aktualisieren Sie auf {product-name} Helm-Chart 2.0.0 oder höher. Wenn Sie einen Operator oder eine Helm-Installation auf OpenShift aktualisieren, siehe die Anmerkung unten.
...
. Wenden Sie das neue Aktualisierungsverb für die neuvector-binding-customresourcedefinition-Clusterrolle an
...
. Erstellen Sie eine neue Admission-, DLP- und WAF-Clusterrolle und Clusterrollenbindung
...
. Aktualisieren Sie die Bildnamen und -pfade zum Abrufen von {product-name} Bildern aus dem Docker-Hub (docker.io), z.B.
...
Optional, entfernen Sie alle Verweise auf die {product-name} Lizenz und das Registry-Geheimnis in Helm-Charts, Deployment-YAML, ConfigMap, Skripten usw., da diese nicht mehr erforderlich sind, um die Bilder abzurufen oder {product-name} zu verwenden.

*Hinweis zu SCC und Upgrade über Operator/Helm*

Privileged SCC wird dem Dienstkonto hinzugefügt, das in der Deployment-YAML von Operator-Version 1.3.4 und höher in neuen Bereitstellungen angegeben ist. Im Falle eines Upgrades des {product-name} Operators von einer vorherigen Version auf 1.3.4 oder Helm auf 2.0.0, bitte Privileged SCC vor dem Upgrade löschen.
...
==== Test_TwoBeta 1 Version veröffentlicht April 2022

* Funktionalität vollständig, einschließlich automatisierter Förderung von Gruppenmodi. Fördert den Schutzmodus einer Gruppe basierend auf der verstrichenen Zeit und Kriterien. Gilt nicht für CRD erstellte Gruppen. Dieses Feature ermöglicht es einer neuen Anwendung, für einen bestimmten Zeitraum in Discover zu laufen, das Verhalten zu lernen und {product-name} Erlauben-Listenregeln für Netzwerk und Prozess zu erstellen, um dann automatisch in den Monitor- und dann in den Schutzmodus zu wechseln. Kriterium von Discover zu Monitor: Verstrichene Zeit zum Lernen aller Netzwerk- und Prozessaktivitäten von mindestens einem aktiven Pod in der Gruppe. Kriterium von Monitor zu Protect: Es gibt keine Sicherheitsereignisse (Netzwerk, Prozess usw.) für den festgelegten Zeitraum der Gruppe.
* Unterstützung für Rancher 2.6.5 Apps und Marketplace-Diagramm. Wird in den Namespace cattle-neuvector-system bereitgestellt und ermöglicht SSO von Rancher zu {product-name}. Hinweis: Frühere Bereitstellungen von Rancher (z. B. Partnerkatalog-Diagramme, Version 1.9.x und früher) müssen vollständig entfernt werden, um auf das neue Diagramm zu aktualisieren.
* Tags für Enforcer, Manager, Controller: 5.0.0-b1 (z.B. neuvector/controller:5.0.0-b1)

==== Test_TwoPreview.3 Version veröffentlicht im März 2022

[CAUTION]
.wichtig
====

Um frühere Vorschau-Bereitstellungen für neue CRD WAF-, DLP- und Admission-Control-Funktionen zu aktualisieren, aktualisieren Sie bitte die CRD-YAML und fügen Sie neue rbac/role-Bindungen hinzu:
...
* CRD für WAF, DLP und Zulassungssteuerungen. HINWEIS: erforderliche zusätzliche Clusterrollenbindungen/Berechtigungen. Siehe Kubernetes- und OpenShift-Bereitstellungsabschnitte. CRD-Import/Export und Versionierung für Zulassungssteuerungen, unterstützt durch CRD.
* Rancher SSO-Integration zum Starten der {product-name} Konsole über Rancher Manager. Dieses Feature ist nur verfügbar, wenn die {product-name} Container über Rancher bereitgestellt werden. HINWEIS: Erfordert aktualisierte Rancher-Version (Datum/Version TBD).
* Unterstützt die Bereitstellung auf RKE2.
...
==== Test_TwoPreview.2 Version veröffentlicht im Februar 2022

* Kleine Datei- und Lizenzänderungen im Quellcode, keine neuen Funktionen hinzugefügt.

==== Test_TwoSupport für Bereitstellung auf AWS ECS Abgelehnt

Die Unterstützung für die Bereitstellung auf ECS wird nicht mehr bereitgestellt. Das All-in-One sollte weiterhin auf ECS bereitgestellt werden können, jedoch wird die Dokumentation der Schritte und Einstellungen nicht mehr unterstützt.

=== Test_Two5.0 'Tech Preview' Januar 2022

==== Test_TwoEnhancements

* Erste Veröffentlichung einer nicht unterstützten, 'Tech-Preview'-Version von {product-name} 5.0 Open-Source-Version.
...
* Entfernt Serverless-Scanning-Funktionen.
...
* Helm-Chart v1.8.9 ist für 5.0.0-Bereitstellungen veröffentlicht. Wenn Sie dies mit der Vorschau-Version von 5.0.0 verwenden, sollten die folgenden Änderungen an values.yml vorgenommen werden:
 ** Aktualisieren Sie das Registry auf docker.io
 ** Aktualisieren Sie die Bildnamen/Tags auf die Vorschau-Version auf Docker Hub
