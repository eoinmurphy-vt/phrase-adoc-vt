...
==== Test_TwoBehobene Fehler

* *NVSHAS-10062*: Manager zeigt keinen FEHLER an, wenn das Erstellen des Admin-Passworts beim 1. Login fehlschlägt.
...
* https://github.com/neuvector/neuvector/security/advisories/GHSA-w54x-xfxg-4gxq[Prozesse mit sensiblen Argumenten führen zu Datenlecks]
...
* *NVSHAS-9998*: Kann die Gruppe nicht vom NeuVector-föderierten Master exportieren.
...
* **NVSHAS-9873**: UI Erweiterter Filter Suchproblem in NeuVector 5.4.2.
...
* **NVSHAS-9811**: [Manager] Zugriff auf die GUI nicht möglich, wenn ein benutzerdefiniertes Zertifikat (CA) mit Root-CA und Zwischen-CA im Geheimnis verwendet wird.
...
* **NVSHAS-9590**: Möglichkeit, welche Schwachstelleneinstufung für alle Assets auszuwählen.
* **NVSHAS-7555**: Option "Auto Refresh" unter Sicherheitsereignisse einfügen.
...
* **NVSHAS-9670**: Manager: Problem mit doppelten Anführungszeichen in der einfachen Textantwort und unbenanntem Java-Bibliotheksproblem beim SBT-Run.
...
* **NVSHAS-9255**: Anpassung der Suchregistrierungen für Admission Control für Bildnamen ohne FQDN.
...
* **NVSHAS-9188**: Setzen Sie die mgmt-br-Schnittstelle als Hostschnittstelle für den Ernteknoten.
...
* Die `host` und `tunnel` Schnittstelle wurden erfolgreich mit OpenShift CLI v4.15 abgerufen.
...
* Ein Problem wurde behoben, bei dem der OpenID-Issuer-URL-RegEx fehlschlug.
...
* Beheben der Zeichenfolgenverarbeitung, um unerwartete Enforcer-Neustarts zu verhindern
...
* Anzeigen von Sicherheitsereignissen für erlaubte Netzwerkregeln

==== Test_TwoOther

* Automatische Erkennung der Laufzeit-Container-Engine (Socket) zum Helm-Diagramm hinzufügen
* Entfernen der Einstellung für den Betrieb des Controllers im privilegierten Modus im Helm-Diagramm und der Anforderung, dass Controller/Enforcer "/host/cgroup" einhängt.
...
* Behebung von CVE-2023-32188 "`JWT-Token-Kompromittierung kann böswillige Aktionen einschließlich Remote Code Execution (RCE) ermöglichen.`" durch automatische Generierung des Zertifikats, das zum Signieren des JWT-Tokens bei Bereitstellung und Upgrade verwendet wird, und automatische Generierung des Manager/RESTful API-Zertifikats während der Helm-basierten Bereitstellungen.
...
* Zusätzliche Kontrollen für benutzerdefinierte Compliance-Skripte hinzufügen. Standardmäßig dürfen benutzerdefinierte Skripte jetzt nicht hinzugefügt werden, es sei denn, die Umgebungsvariable CUSTOM_CHECK_CONTROL wird dem Controller und dem Enforcer hinzugefügt. Die Werte sind "deaktivieren" (Standard, nicht erlaubt), "streng" (nur Admin-Rolle) oder "locker" (Admin-, Compliance- und Runtime-Policy-Rollen).
* LDAP-Injection verhindern - das Benutzernamenfeld wird escaped.
...
* Unterstützung der Proxy-Einstellung bei der Bildsignaturüberprüfung.
...
* Erstellen Sie keinen Standardadministrator mit dem Standardpasswort in der initialen {product-name} Bereitstellung für das AWS-Abrechnungsangebot (CSP-Adapter), wodurch der Benutzer gezwungen wird, ein Geheimnis zu verwenden, um den Administratorkontonamen und das Passwort zu erstellen.
...
* Beim Installieren des Helm-CRD-Diagramms zuerst, bevor das {product-name} Core-Diagramm installiert wird, fehlen Dienstkonten.
...
* Unterstützung der Bildsignierung für Zugangssteuerungen. Benutzer können {product-name} verlangen, dass Bilder von bestimmten Parteien signiert werden, bevor sie in die Produktionsumgebung bereitgestellt werden, durch eine Integration mit Sigstore/Cosign. Siehe Assets -> Sigstore-Überprüfer zum Erstellen neuer Signatur-Assets. Regeln können dann mit den Kriterien Bildsignierung und/oder Bild-Sigstore-Überprüfer erstellt werden.
...
* Fügen Sie einen neuen Regex-Operator in Richtlinie > Zugangssteuerung > Regel hinzufügen für Benutzer und Benutzergruppen hinzu, um Regex zu unterstützen. Unterstützung der Operatoren "entspricht JEDER Regex in" und "entspricht KEINER Regex in".
...
* Sicherheitsereigniswarnung, auch mit erlaubten Netzwerkregeln aufgrund eines Problems mit der Richtlinienaktualisierung in der Synchronisierung.
...
* Quellschweregrad/Klassifizierung (z. B. Red Hat, Ubuntu...) zusammen mit dem NVD-Schweregrad in der Konsole anzeigen.
* SSO/RBAC-Deaktivierung für Rancher und OpenShift nicht zulassen, wenn der Benutzer über SSO authentifiziert ist.
* Aktivierung der automatischen Überprüfung und Löschung von nicht verwendeten Gruppen im configMap hinzufügen.
...
* Zentralisierte, Multi-Cluster-Scanning (CVE)-Datenbank. Der primäre (Master-)Cluster kann ein als föderiertes Repository bezeichnetes Registry/Repo scannen. Die Scanergebnisse aus diesen Registrierungen werden mit allen verwalteten (remote) Clustern synchronisiert. Dies ermöglicht die Anzeige der Scanergebnisse in der Konsole des verwalteten Clusters sowie die Verwendung der Ergebnisse in den Zulassungssteuerungsregeln des verwalteten Clusters. Registrierungen müssen nur einmal gescannt werden, anstatt von jedem Cluster, was die CPU-/Speicher- und Netzwerkbandbreitennutzung reduziert.
 ...
 ** Fügen Sie Kriterien hinzu, um hochriskante RBAC-Einstellungen für Dienstkonten beim Bereitstellen von Pods zu überprüfen. Diese umfassen Kriterien wie 'jede Aktion von Arbeitslastressourcen', 'jede Aktion auf RBAC', 'Arbeitslastressourcen erstellen', 'Geheimnisse auflisten' und 'in einen Container ausführen'.
...
* Das Blockieren des Logins nach fehlgeschlagenen Anmeldeversuchen wird jetzt zur Standardoption. Der Standardwert beträgt 5 Versuche und kann in den Einstellungen -> Benutzer & Rollen-> Passwortprofil konfiguriert werden.
 ...
 ** *_Sofortige Strategie:_*
  ...
  *** Wenn der Hauptcontroller kontinuierlich sieht, dass die "Aufgabenanzahl" 0 für > 180 Sekunden beträgt, wird ein Scanner-Pod heruntergefahren, wenn minScannerPods noch nicht erreicht ist.
* Benutzerdefinierte Gruppen können jetzt Namespace-Labels verwenden, einschließlich der Namespace-Labels von Rancher. Allgemein können jetzt Pod- und Namespace-Labels zu benutzerdefinierten Gruppen hinzugefügt werden.
...
* Anonyme Telemetriedaten (Anzahl der Knoten, Gruppen, Regeln) werden jetzt bei der Bereitstellung an einen Rancher-Cloud-Dienst gemeldet, um dem Projektteam zu helfen, das Nutzungsverhalten zu verstehen. Dies kann in der UI oder mit configMap (No_Telemetry_Report) oder REST-API deaktiviert werden (Opt-out).
* (Nachtrag Januar 2023). Unterstützung für ServiceEntry-basierte Netzwerkrichtlinien mit Istio. Die Durchsetzung von Egress-Netzwerkrichtlinien wurde in Version 5.1.0 für Pods zu ServiceEntry-Zielen, die mit Istio deklariert sind, hinzugefügt. Typischerweise definiert ein ServiceEntry, wie ein externes Dienst, das über einen DNS-Namen angesprochen wird, in eine Ziel-IP aufgelöst wird. Vor v5.1 konnte {product-name} keine Regeln für Verbindungen zu einem ServiceEntry erkennen und durchsetzen, sodass alle Verbindungen als extern klassifiziert wurden. Mit 5.1 können Regeln für spezifische ServiceEntry-Ziele durchgesetzt werden. WICHTIG: Wenn Sie auf v5.1 mit einer Istio-basierten Bereitstellung aktualisieren, müssen neue Regeln erstellt werden, um diese Verbindungen zuzulassen und Verletzungswarnungen zu vermeiden. Nach dem Upgrade werden implizite Verstöße für neu sichtbaren Verkehr gemeldet, wenn keine Erlaubungsregeln existieren. Neue Verkehrsregeln können im Entdeckungsmodus gelernt und automatisch erstellt werden. Um diesen Verkehr zuzulassen, können Sie die Gruppe in den Entdeckungsmodus versetzen oder eine benutzerdefinierte Gruppe mit Adressen (oder DNS-Namen) und einer neuen Netzwerkregel zu diesem Ziel erstellen, um den Verkehr zuzulassen. HINWEIS: Es gibt einen Fehler in 5.1.0 im Ziel, das von den Verweigerungsverstößen gemeldet wird, die nicht das korrekte Ziel darstellen.  Der Fehler berichtet, dass sowohl server_name als auch client_name gleich sind.  Dieses Problem wird in einem kommenden Patch-Release behoben.
...
* Hoher Speicherverbrauch im dp-Prozess im Enforcer, wenn viele erlernte Richtlinienregeln mit nicht verwaltetem Arbeitsaufwand (Speicherleck) vorhanden sind
...
* Unterscheiden Sie zwischen Feature-Diagramm und Partner-Diagrammen in der Rancher-Benutzeroberfläche deutlicher. + Verbesserung der Ingress-Annotation für nginx im Rancher-Helm-Diagramm. Fügen Sie hinzu / aktualisieren
...
* Der Enforcer wird in einem k8s 1.24-Cluster mit 1.64 containerd-Laufzeit nicht betriebsbereit. Getrennt startet der Enforcer manchmal nicht.
...
* Implizierte Verletzung von Host-IP, die nicht mit dem Knoten verbunden ist.
...
{product-name} benötigt keine zusätzlichen Einstellungen für SELinux-aktivierte Cluster, um bereitgestellt und zu funktionieren. Das Bereitstellen von {product-name} auf einem RHEL 8.5 basierten, SELinux-aktivierten, RKE2-härteten Cluster wurde getestet. {product-name} wurde erfolgreich bereitgestellt, wenn PSP aktiviert ist und das Patch-Manager- und Scanner-Bereitstellung. Die nächste Chart-Version sollte das untenstehende Problem beheben.

Beigefügtes Beispiel zum Aktivieren von PSP aus dem Rancher-Chart und die unten angegebenen Befehle zur Bereitstellung von Patch-Manager und Scanner. Die Benutzer-ID im Patch-Befehl kann jede Zahl sein.
...
* WAF-Regelerkennung, verbesserte DLP-Regeln (Header, URL, vollständiges Paket). Verwendet für eingehende Verbindungen zu Webanwendungs-Pods sowie ausgehende Verbindungen zu API-Diensten zur Durchsetzung der API-Sicherheit.
* CRD für WAF, DLP und Zulassungssteuerungen. HINWEIS: Zusätzliche Clusterrollenbindungen/Berechtigungen erforderlich. Siehe Kubernetes- und OpenShift-Bereitstellungsabschnitte. CRD-Import/Export und Versionierung für Zulassungssteuerungen, unterstützt durch CRD.
* Rancher SSO-Integration zum Starten der {product-name} Konsole über Rancher Manager. Dieses Feature ist nur verfügbar, wenn die {product-name} Container über Rancher bereitgestellt werden. Diese Bereitstellung zieht aus dem gespiegelten Rancher-Repository (z.B. rancher/mirrored-neuvector-controller:5.0.0) und wird im Namespace cattle-neuvector-system bereitgestellt. HINWEIS: Erfordert aktualisierte Rancher-Version 2.6.5 Mai 2022 oder später, und nur Admin- und Clusterbesitzerrollen werden derzeit unterstützt.
* Unterstützt die Bereitstellung auf RKE2.
* Unterstützung für die Föderation von Clustern (Multi-Cluster-Manager) über einen Proxy. Proxy in den Einstellungen -> Konfiguration konfigurieren und Proxy aktivieren, wenn Sie Föderationsverbindungen konfigurieren.
...
* Unterstützung für AD/LDAP-Verschachtelung von Gruppen unter zugeordneten Rollengruppen.
* Unterstützung für Clusterrollenbindungen oder Rollenbindungen mit Gruppeninformationen im IDP für Openshift.
...
* Funktionalität vollständig, einschließlich automatisierter Förderung von Gruppenmodi. Fördert den Schutzmodus einer Gruppe basierend auf verstrichener Zeit und Kriterien. Gilt nicht für CRD erstellte Gruppen. Dieses Feature ermöglicht es einer neuen Anwendung, für einen bestimmten Zeitraum in Discover zu laufen, das Verhalten zu lernen und {product-name} Erlauben-Listenregeln für Netzwerk und Prozess zu erstellen, bevor sie automatisch in den Überwachungsmodus und dann in den Schutzmodus wechselt. Kriterium von Discover zu Monitor: Verstrichene Zeit zum Lernen aller Netzwerk- und Prozessaktivitäten von mindestens einem aktiven Pod in der Gruppe. Kriterium von Monitor zu Protect: Es gibt keine Sicherheitsereignisse (Netzwerk, Prozess usw.) für den festgelegten Zeitraum der Gruppe.
* Unterstützung für Rancher 2.6.5 Apps und Marketplace-Diagramm. Wird in den Namespace cattle-neuvector-system bereitgestellt und ermöglicht SSO von Rancher zu {product-name}. Hinweis: Frühere Bereitstellungen von Rancher (z. B. Partnerkatalog-Diagramme, Version 1.9.x und früher) müssen vollständig entfernt werden, um auf das neue Diagramm zu aktualisieren.
...
=== Test_Two5.0 'Technische Vorschau' Januar 2022

==== Test_TwoEnhancements

* Erste Veröffentlichung einer nicht unterstützten, 'technischen Vorschau'-Version von {product-name} 5.0 Open-Source-Version.
* Fügen Sie Unterstützung für OWASP Top-10, WAF-ähnliche Regeln zur Erkennung von Netzwerkangriffen in Headern oder im Body hinzu. Beinhaltet Unterstützung für CRD-Definitionen von Signaturen und deren Anwendung auf die entsprechenden Gruppen.
