...
. Überprüfen Sie "`Unterstützung für das SAML 2.0 WebSSO-Protokoll aktivieren`" und geben Sie die SAML-Redirect-URI von {product-name} Einstellungen>SAML-Einstellungsseite in das Feld "`Relying party SAML 2.0 SSO-Dienst-URL`" ein.  Wählen Sie "`Weiter`" aus, um fortzufahren.
+
image:adfs6.png[adfsSetup]

. Geben Sie dieselbe SAML-Redirect-URI in das Feld "`Identifikator der vertrauenden Partei`" ein und klicken Sie auf "`Hinzufügen`"; wählen Sie dann "`Weiter`" aus, um fortzufahren.
+
image:adfs7.png[adfsSetup]

. Passen Sie die Zugriffskontrolle an; wählen Sie dann "`Weiter`" aus, um fortzufahren.
+
...
. Identifizieren Sie die URL für die Anbieter-Single-Sign-On
