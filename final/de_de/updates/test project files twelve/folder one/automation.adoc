...
Das neueste Update finden Sie https://raw.githubusercontent.com/neuvector/neuvector/main/controller/api/apis.yaml[hier]. Auch im {product-name} GitHub-Quellcode https://github.com/neuvector/neuvector/blob/main/controller/api/apis.yaml[repo].  Die apis.yaml vom Haupt-Repository kann nicht veröffentlichte Funktionen enthalten.  Es wird empfohlen, den entsprechenden veröffentlichten Quellcode herunterzuladen und die apis.yaml aus dem controller/api-Ordner zu extrahieren.
...
{product-name} unterstützt auch Antwortregeln, um häufige Antworten auf Sicherheitsereignisse oder erkannte Schwachstellen zu automatisieren. Bitte sehen Sie den Abschnitt Sicherheitsrichtlinie -> Antwortregeln für weitere Details ein.
...
Stellen Sie sicher, dass Sie die IP-Adresse des Controllers im Skript eingeben und den Containerbildnamen in den gewünschten Namen ändern. Aktualisieren Sie auch die Felder für Benutzername/Passwort.
...
[NOTE]
====
Bei einer Bereitstellung mit mehreren Controllern müssen die Anfragen an eine einzelne Controller-IP gesendet werden, damit mehrere Anfragen zum Status von lang laufenden Bildscans an den Controller gesendet werden, der den Scan durchführt.
====
...
=== Test_ZweiErstellen von Richtlinienregeln automatisch

Um eine neue Regel im {product-name} Richtliniencontroller zu erstellen, müssen die Gruppen für die FELDER FROM und TO zuerst existieren. Das folgende Beispiel erstellt eine neue Gruppe basierend auf dem Containerlabel nv-service-type=data und eine weitere Gruppe für das Label nv-service-type=website. Eine Regel wird dann erstellt, um den Verkehr vom Wordpress-Container zum MySQL-Container nur unter Verwendung des MySQL-Protokolls zuzulassen.
...
=== Test_TwoExport/Import-Konfigurationsdatei
...
=== Test_TwoEinstellung oder Ändern des Benutzerpassworts

Verwenden Sie die REST-API-Aufrufe für die Benutzerverwaltung.
...
=== Test_TwoStarten der Paketaufzeichnung in einem Container
...
=== Test_TwoÜberprüfen und Akzeptieren der EULA (neue Bereitstellungen)

Holen Sie sich das Authentifizierungstoken wie oben beschrieben. Ersetzen Sie auch die IP-Adresse des Controllers durch Ihre, wie es angemessen ist.
...
=== Test_TwoRegistry-Scanning konfigurieren
...
=== Test_TwoAktivieren Sie die Paketaufzeichnung für alle Pods in einem Namespace
...
=== Test_TwoAktivieren Sie die Deaktivierung der Containerquarantäne
...
=== Test_TwoAktivieren Sie den Debug-Modus für {product-name} Support
...
=== Test_TwoBerichten Sie, ob eine Schwachstelle in den Basisschichten des Images vorhanden ist

Um CVEs im Basisimage zu identifizieren, wenn Sie die REST-API zum Scannen von Images verwenden, muss das Basisimage im API-Aufruf identifiziert werden, wie im folgenden Beispiel.
...
Wenn das zu scannende Image ein Remote-Image ist, mit "Registry" angegeben, muss das Basisimage ebenfalls ein Remote-Image sein, und der Name muss mit http oder https beginnen.  Wenn das zu scannende Image ein lokales Image ist, muss das Basisimage ebenfalls ein lokales Image sein.
...
=== Test_ZweiErhalten Sie die CVE-Datenbankversion und das Datum
...
=== Test_ZweiVerwalten Sie die Föderation für Master- und Remote-(Arbeits-)Cluster

Im Allgemeinen kann die Auflistung der Föderationsmitglieder einen GET an den folgenden Endpunkt verwenden (siehe Beispiele für spezifische Syntax):
`+https://neuvector-svc-controller.neuvector:10443/v1/fed/member+`

Für Informationen zur Automatisierung der Föderation über ConfigMap siehe die Dokumentation xref:configmap.adoc#_federated_configmap_examples_fedinitcfg_yaml[Föderierte ConfigMap-Beispiele].

Ausgewählte API zur Verwaltung der Föderation:
