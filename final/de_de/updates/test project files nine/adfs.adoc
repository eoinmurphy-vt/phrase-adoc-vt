...
== Test_TwoEinrichtung von ADFS und {product-name} Integration

Dieser Abschnitt beschreibt die Einrichtungsschritte zuerst in ADFS und dann in der {product-name} Konsole.

=== Test_TwoADFS Setup

. Klicken Sie mit der rechten Maustaste auf "`Relying Party Trusts`" in der AD FS-Verwaltung und wählen Sie "`Relying Party Trust hinzufügen...`".
+
...
. Wählen Sie "`Geben Sie die Daten über die vertrauende Partei manuell ein`" und wählen Sie "`Weiter`".
+
...
. Überprüfen Sie "`Unterstützung für das SAML 2.0 WebSSO-Protokoll aktivieren`" und geben Sie die SAML-Redirect-URI von {product-name} Einstellungen>SAML-Einstellungsseite in das Feld "`Relying party SAML 2.0 SSO-Dienst-URL`" ein.  Wählen Sie "`Weiter`", um fortzufahren.
+
image:adfs6.png[adfsSetup]

. Geben Sie dieselbe SAML-Redirect-URI in das Feld "`Identifikator der vertrauenden Partei`" ein und klicken Sie auf "`Hinzufügen`"; wählen Sie dann "`Weiter`", um fortzufahren.
+
...
. Wählen Sie Richtlinien zur Anspruchsvergabe bearbeiten...
+
...
* Sehen Sie sich Endpunkte aus AD FS-Verwaltung > Dienst an und verwenden Sie die "`SAML 2.0/WS-Federation`" Endpunkt-URL.
...
* Speichern Sie es als Base-64-codierte x.509 (.CER) Datei
...
. Standardrolle
* Empfohlen, "`Keine`" zu sein, es sei denn, Sie möchten, dass jeder authentifizierte Benutzer eine Standardrolle hat.
...
+
Attributnamen sind groß- und kleinschreibungsempfindlich. Stellen Sie sicher, dass jeder hier konfigurierte SAML-Attributname genau mit der Anwendungs-Konfiguration übereinstimmt. SAML muss auf die richtige URL zur Authentifizierung verweisen.
+
Alle Felder in `+{product-name} UI -> Settings -> SAML Settings+` sind groß- und kleinschreibungsempfindlich.
+
Die Protokolle des {product-name} Controllers enthalten die relevanten Informationen zur Authentifizierung mit dem ADFS-Server und Fehler, die helfen, die Ursache zu identifizieren. Wir empfehlen, die fehlgeschlagene Anmeldebedingung zu reproduzieren und die Protokolle zu überprüfen.
