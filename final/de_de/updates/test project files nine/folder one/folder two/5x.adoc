...
* *NVSHAS-9947*: [UI-ext] Compliance-Diagramm fehlt "Manueller" Status in Rancher NeuVector.
...
* *NVSHAS-9986*: Prozess- und Dateizugriffsregeln können in der Ansicht der föderierten Richtliniengruppe nicht bearbeitet oder entfernt werden.
...
* **NVSHAS-9884**: [Knoten-Scan][Container-Scan] Der Scan schlägt fehl.
* **NVSHAS-9873**: UI-Problem mit dem erweiterten Filter in NeuVector 5.4.2.
...
* **NVSHAS-9828**: [Durchsetzer: Prozess] Der NeuVector-Durchsetzer kann kurzlebige Pods (weniger als 5 Sekunden) nicht erkennen.
...
* **NVSHAS-9176**: Zeige einen Skriptfehler auf der **Sicherheitsrisiken > Schwachstellen** Seite an, wenn ein Benutzer mit der Berechtigung für den Namespace `rt_scan:w` angemeldet ist.
...
* **NVSHAS-9849**: Durchsetzer registrieren sich nicht bei Controllern.
...
* **NVSHAS-9670**: Manager: Problem mit doppelten Anführungszeichen in der einfachen Textantwort und unbenanntem Java-Bibliotheksproblem beim SBT-Ausführen.
...
* **NVSHAS-9664**: Richtlinientyp: Das Löschen des benutzerdefinierten Skripts erzeugt einen "setRowData" TypeError.
...
* **NVSHAS-9576**: Lösche das Passwortfeld für Registrierungsdaten, wenn der Benutzer den Controller-Modus mit Jenkins zum Scannen verwendet.
...
* **NVSHAS-9485**: Falsche Nachricht für 'Netzwerksicherheitsrichtlinienmodus' in der UI.
...
* **NVSHAS-7687**: Unterstützung der Konfiguration des Protokollniveaus (debug/error/info/warn) für den Durchsetzer und Controller über die CLI.
...
* **NVSHAS-9085**: Der PDF-Bericht der Asset-Ansicht zeigt 0% Verwundbarkeit, selbst bei vorhandenen Verwundbarkeiten.
...
* **NVSHAS-9285**: UI: Der Dropdown-Listenbutton überlappt sich mit anderen Elementen.
...
* **NVSHAS-9091**: Es ist nicht möglich, alle Module unter ol:9.1, photon:5.0, rhel:9.1 und amzn:2023 in Repo, Registry und Standalone-Scan zu melden.
...
* **NVSHAS-9116**: Der Worker-Cluster kann verlassen, wenn die Verbindung unterbrochen wird.
* **NVSHAS-8980**: Host- und Tunnel-Schnittstelle auf Knoten erfolgreich in oc 4.15 abrufen.
...
* (Scanner) Unterstützung für PHP Composer-Scans hinzugefügt.
...
* Beheben Sie die Zeichenfolgenverarbeitung, um unerwartete Enforcer-Neustarts zu verhindern
...
* Die Entfernung der Anforderung, den Controller im privilegierten Modus auszuführen, beseitigt die Notwendigkeit, den Laufzeitsocket und /host/cgroup/ einzubinden.
...
* Fügen Sie das Feld "Zuletzt geändert" hinzu, um den Bericht über Schwachstellen zu filtern, sowie den erweiterten Filter in der Ansicht Schwachstellen.
...
* Der erweiterte Bericht über Schwachstellenfilter zeigt Bilder aus allen anderen Namespaces an.
...
* Unterstützen Sie die adressgruppenbasierte virtuelle Host- und Richtlinienübereinstimmung für Netzwerkschutzmaßnahmen. Dies ermöglicht einen Anwendungsfall, bei dem zwei verschiedene FQDN-Adressen auf dieselbe IP-Adresse aufgelöst werden, aber unterschiedliche Regeln für jede FQDN durchgesetzt werden sollten. Eine neue benutzerdefinierte Gruppe mit '`address=vh:xxx.yyy`' kann mit dem '`vh:`' Indikator erstellt werden, um diesen Schutz zu aktivieren. Eine Netzwerkregel kann dann die benutzerdefinierte Gruppe als '`From`' Quelle basierend auf dem virtuellen Hostnamen (anstatt der aufgelösten IP-Adresse) verwenden, um unterschiedliche Regeln für virtuelle Hosts durchzusetzen.
 ...
 ** *_Sofortige Strategie:_*
  ...
  *** Wenn der Hauptcontroller kontinuierlich sieht, dass die "Aufgabenanzahl" 0 für > 180 Sekunden beträgt, wird ein Scanner-Pod heruntergefahren, wenn minScannerPods noch nicht erreicht ist.
* Benutzerdefinierte Gruppen können jetzt Namespace-Labels verwenden, einschließlich der Namespace-Labels von Rancher. Im Allgemeinen können jetzt Pod- und Namespace-Labels zu benutzerdefinierten Gruppen hinzugefügt werden.
...
* Enforcer erleben einen Go-Routine-Panik, was zu einem dp-Kill führt.  WebUI zeigt den Enforcer nicht als online an.
...
* Scanner: Implementierung von Wischattributen während der Rekonstruktion des Image-Repos.
...
{product-name} benötigt keine zusätzlichen Einstellungen für SELinux-aktivierte Cluster, um bereitgestellt und zu funktionieren. Getestet die Bereitstellung von {product-name} auf einem SELinux-aktivierten RKE2 gehärteten Cluster basierend auf RHEL 8.5. {product-name} wurde erfolgreich bereitgestellt, wenn PSP aktiviert ist und das Patch-Manager- und Scanner-Deployment. Die nächste Chart-Version sollte das untenstehende Problem beheben.

Beigefügtes Beispiel zum Aktivieren von PSP aus dem Rancher-Chart und unten die Befehle für das Patch-Manager- und Scanner-Deployment. Die Benutzer-ID im Patch-Befehl kann jede Zahl sein.
...
* CRD für WAF, DLP und Zulassungssteuerungen. HINWEIS: Zusätzliche Clusterrollenbindungen/Berechtigungen erforderlich. Siehe Kubernetes- und OpenShift-Bereitstellungsabschnitte. CRD-Import/Export und Versionierung für Zulassungssteuerungen, die durch CRD unterstützt werden.
...
* Serverloses Scannen-Funktion/ Menü entfernt.
...
. Erstellen Sie eine neue Admission-, DLP- und WAF-Clusterrolle sowie eine Clusterrolebinding
