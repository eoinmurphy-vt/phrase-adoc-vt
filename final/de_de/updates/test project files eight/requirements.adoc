...
Widmen Sie 1 oder mehr CPUs für die Hochvolumen-(10k+) Bildscans. +
...
Beispiel - größte Bildgröße = Test_Eight1,3GB, der Speicher des Scanner-Containers sollte 1,8 GB betragen.
...
* Für Konfigurationssicherung/HA, ein RWX PVC von 1Gi oder mehr. Siehe xref:production.adoc#_backups_and_persistent_data[Sicherungen und persistente Daten Abschnitt] für weitere Details.
* Empfohlener Browser: Chrome für bessere Leistung

== Test_Eight Unterstützte Plattformen
...
* Offiziell unterstützte Kubernetes- und Docker-konforme Containerverwaltungssysteme. Die folgenden Plattformen werden mit jeder Veröffentlichung von \{product-name} getestet: Kubernetes 1.19-1.32, SUSE Rancher (RKE, RKE2, K3s usw.), RedHat OpenShift 4.6-4.16 (3.x bis 4.12 vor \{product-name} 5.2.x unterstützt), Google GKE, Amazon EKS, Microsoft Azure AKS, IBM IKS, nativer Docker, Docker Swarm. Die folgenden Kubernetes- und Docker-kompatiblen Plattformen werden unterstützt und wurden verifiziert, um mit \{product-name} zu arbeiten: VMware Photon und Tanzu, SUSE CaaS, Oracle OKE, Mirantis Kubernetes Engine, Nutanix Kubernetes Engine, Docker UCP/DataCenter, Docker Cloud.
...
* Minikube wird für eine einfache erste Bewertung unterstützt, jedoch nicht für einen vollständigen Proof of Concept. Siehe unten für erforderliche Änderungen, damit die All-in-One-YAML auf Minikube ausgeführt werden kann.
...
== Test_EightNichtUnterstützt

* GKE Autopilot.
* AWS ECS wird nicht mehr unterstützt. (HINWEIS: Es wurden keine Funktionen aktiv entfernt, um \{product-name} auf ECS-Bereitstellungen zu betreiben. Allerdings wird das Testen auf ECS von SUSE nicht mehr durchgeführt. Während das Schützen von ECS-Arbeitslasten mit \{product-name} voraussichtlich wie erwartet funktioniert, werden Probleme nicht untersucht.)
* Docker auf Mac
* Docker auf Windows
...
== Test_EightPerformance und Skalierung
...
Für die Leistungsoptimierung des Controllers und Scanners für das Scannen von Registern siehe die oben genannten Systemanforderungen.
...
Dieser Durchsatz würde voraussichtlich steigen, wenn eine dedizierte CPU dem Enforcer zugewiesen wird, oder sich die CPU-Geschwindigkeit ändert und/oder zusätzlicher Speicher zugewiesen wird. Die Skalierung hängt erneut von der Art des Netzwerk-/Anwendungsverkehrs der Arbeitslasten ab.
...
Die folgende Tabelle zeigt die durchschnittliche Latenz von 2-10%, die mit dem Redis-Benchmark-Tool bewertet wurde. Der Redis-Benchmark verwendet ziemlich kleine Pakete, sodass die Latenz mit größeren Paketen voraussichtlich niedriger sein wird.
...
=== Test_EightHinzufügen von Skalierungsbeschränkungen für große Arbeitslastumgebungen
