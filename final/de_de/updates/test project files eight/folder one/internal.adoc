= Test_EightInterne Zertifikate ersetzen
:revdate: 2025-01-27
...
== Test_EightInterne Kommunikation und Zertifikate

\{product-name} enthält standardmäßig selbstsignierte Zertifikate für die Verschlüsselung für den Manager (Konsolen-/UI-Zugriff), Controller (REST API, intern), Enforcer (intern) und Scanner (intern) Kommunikation.
...
+
Wenn Ihre Zertifikatdateien vor der \{product-name}-Version 5.3 erstellt wurden, müssen Sie das Zertifikat mit mindestens einem Subject Alternative Name (SAN) aktualisieren. Wenn Sie die Dateien `ca.key` und `ca.crt` noch zugänglich haben, führen Sie die Befehle wie folgt aus:
+
[source,bash]
----
kubectl delete secret internal-cert -n neuvector 
...
+
Wenn Ihre Zertifikatdateien vor der \{product-name}-Version 5.3 erstellt wurden, müssen Sie das Zertifikat mit mindestens einem Subject Alternative Name (SAN) aktualisieren. Wenn Sie die Dateien `ca.key` und `ca.crt` nicht mehr haben, befolgen Sie die folgenden Schritte: 
+
...
=== Test_EightAktualisieren/Deployen mit Helm
