...
. Wählen Sie "`Close`", um abzuschließen.
...
* Endpunkte aus AD FS-Verwaltung > Dienst anzeigen und verwenden Sie die "`SAML 2.0/WS-Federation`" Endpunkt-URL.
...
* Wählen Sie die Registerkarte Details und klicken Sie "`Copy to File`"
...
. Standardrolle
* Empfohlen, "`None`" zu sein, es sei denn, Sie möchten einem beliebigen authentifizierten Benutzer eine Standardrolle erlauben.
...
Bitte überprüfen Sie und bestätigen Sie, dass sowohl ADFS als auch {product-name} Hosts synchronisiert sind und die möglichen Verzögerungen 10 Sekunden nicht überschreiten. Sie können Linux- und Windows-Befehle verwenden, um Daten, Zeiten und die Aktivität des NTP-Servers zu überprüfen.
