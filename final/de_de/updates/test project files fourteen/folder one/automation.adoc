= Testprojekt vierzehn REST API und Automatisierung
:revdate: 2025-05-08
...
Der API-Aufruf zur Quarantäne erfolgt über PATCH an /v1/workload/:id mit dem folgenden Body. Die Arbeitslast-ID ist die Container-/Pod-ID.
...
=== Test_TwoBerichten Sie, ob eine Schwachstelle in den Basisschichtbildern vorhanden ist

Um CVEs in dem Basisbild zu identifizieren, wenn Sie die REST-API zum Scannen von Bildern verwenden, muss das Basisbild im API-Aufruf identifiziert werden, wie im folgenden Beispiel.
