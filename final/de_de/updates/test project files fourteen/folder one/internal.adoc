= Testprojekt vierzehn Ersetzen interner Zertifikate
:revdate: 2025-01-27
...
{product-name} enthält standardmäßige selbstsignierte Zertifikate zur Verschlüsselung für den Manager (Konsole/UI-Zugriff), Controller (REST-API, intern), Enforcer (intern) und Scanner (intern) Kommunikationen.
...
[WARNING]
====
Es wird empfohlen, das Ersetzen von Zertifikaten nur während der ersten Bereitstellung von {product-name} durchzuführen. Das Ersetzen in einem laufenden Cluster (auch mit einem Rolling Upgrade) kann zu einem instabilen Zustand führen, in dem {product-name} Pods nicht mehr miteinander kommunizieren können, aufgrund eines Zertifikatsmismatch, und DATENVERLUST kann auftreten.
====

=== Test_ZweiErsetzen von Zertifikaten, die in internen Kommunikationen von {product-name} verwendet werden
...
+
* Löschen Sie die alten `ca.crt`, `tls.key`, `tls.crt`, Kubernetes-Geheimnisse und generieren Sie neue:
+
[source,bash]
----
...
----
+
* Mergen Sie die alten und neuen `ca.crt` Dateien 
+
[source,bash]
----
...
+
Wenn Sie das interne Zertifikat zuvor nicht ersetzt haben und auf ein neues Set von Zertifikaten migrieren möchten, befolgen Sie die folgenden Schritte:
+
