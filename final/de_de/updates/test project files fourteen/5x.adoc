= Testprojekt vierzehn 5.x Versionshinweise
:revdate: 2025-08-26
...
* *NVSHAS-9990*: Übernehmen Sie den neuen Hash-Algorithmus für Benutzerpasswörter.
...
* *NVSHAS-10003*: Die Seite neu laden funktioniert nicht auf der eigenständigen NV-Seite, während die Rancher UI geöffnet ist.
...
* **NVSHAS-9915**: Zeige die Scanergebnisse des Harbor-Scanner-Moduls in der NeuVector UI an.
...
* **NVSHAS-9715**: Helm Chart-Wertunterstützung für die Einstellung von Nodeport auf Controller und Manager.
...
* **NVSHAS-9754**: [UI] Verhindern Sie, dass Rancher-bezogene SSO-Benutzer die Authentifizierung von OpenShift oder Ranchers RBAC deaktivieren.
...
* **NVSHAS-8583**: Einstellen granularer Richtlinienmodi für Regelsets, separater Netzwerkrichtlinienmodus und Profilmodus auf Gruppenebene.
...
* **NVSHAS-9367**: Verbessere Fehlermeldungen, wenn die Verbindung zum Registry fehlschlägt.
...
* **NVSHAS-9570**: [Schwachstellen] Ändern Sie die Legendenbeschreibung für verschiedene Status von Assets.
...
* **NVSHAS-7664**: Reduzierung der ISP-Datengebühren während des Registrierungs-Scans.
* **NVSHAS-8868**: Scanner-Cache-Statistiken offenlegen.
* **NVSHAS-8676**: Verbesserung von NV Protect für Benchmark-Skripte.
* **NVSHAS-9255**: Anpassung der Suchregistrierungen für die Zulassungskontrolle für Bildnamen ohne FQDN.
...
* **NVSHAS-9302**: Kann keinen APIKEY mit der Rolle FedReader und FedAdmin erstellen.
...
* **NVSHAS-9362**: Der Sicherheitsrisiko-Filter gibt 0 Ergebnisse zurück.
...
* **NVSHAS-9292**: Beheben Sie die Ingress Egress-Exposition, die 0 Schwachstellen anzeigt.
...
* Die `LDAP Authentication` hat getrennte Felder für die Konfiguration von `baseDN` und `groupDN`.
...
* Die Versionierung des Prometheus-Exporters wurde überprüft und von der `controller` Versionierung disssoziiert.
...
* Erlauben Sie bei der Konfiguration eines Registrys, ob der definierte Proxy für die Scans von Registry-Bildern verwendet werden soll.
...
* Erlauben Sie die Überprüfung der Bildsignatur von rootlosen Schlüsselpaaren ohne Internet- oder Sigstore-Abhängigkeit.
...
* Erweiterter Filter zeigt die Felder Remediation und Impact leer an
* Behebe die Zeichenfolgenbehandlung, um unerwartete Neustarts des Enforcers zu verhindern
...
* Füge automatische Erkennung der Laufzeit-Container-Engine (Socket) zum Helm-Chart hinzu
* Entferne die Einstellung für den Betrieb des Controllers im privilegierten Modus im Helm-Chart und die Anforderung, dass der Controller/Enforcer "/host/cgroup" einbindet.
...
==== Test_TwoHervorgehobene Änderungen, die möglicherweise Änderungen für manuelle Bereitstellungen erfordern (alle Änderungen sind bereits im neuesten Helm-Chart für 5.3.x enthalten)

* Die automatische Erkennung der Containerlaufzeit (Socket) entfernt die Notwendigkeit, die Containerlaufzeit und den Socket-Pfad anzugeben.
 ...
 ** Zertifikat für die JWT-Signierung wird automatisch vom Controller mit einer Gültigkeit von 90 Tagen erstellt und automatisch rotiert.
 ** Die automatische Generierung von Manager-, REST-API- und Registrierungsadapterzertifikaten erfordert die Verwendung einer Helm-basierten Installation mit {product-name} Helm-Version 2.6.3 oder höher.
...
* Zusätzliche Kontrollen für benutzerdefinierte Compliance-Skripte hinzufügen. Standardmäßig dürfen benutzerdefinierte Skripte jetzt nicht hinzugefügt werden, es sei denn, die Umgebungsvariable CUSTOM_CHECK_CONTROL wird dem Controller und dem Durchsetzer hinzugefügt. Werte sind "deaktivieren" (Standard, nicht erlaubt), "streng" (nur Admin-Rolle) oder "locker" (Admin-, Compliance- und Runtime-Policy-Rollen).
...
* Unterstützung der Proxy-Einstellungen bei der Überprüfung der Bildsignatur.
...
* Erstellen Sie keinen Standardadministrator mit dem Standardpasswort in der ersten {product-name} Bereitstellung für das AWS-Abrechnungsangebot (CSP-Adapter), wodurch der Benutzer gezwungen wird, ein Geheimnis zu verwenden, um den Benutzernamen und das Passwort des Administrators zu erstellen.
...
* Unterstützung der AWS Marketplace PAYG-Abrechnung für {product-name} monatliche Support-Abonnements. Benutzer können sich über den SUSE-Support für {product-name} anmelden, die monatlich auf ihr AWS-Konto basierend auf der durchschnittlichen Nutzung der Knotenanzahl des vorherigen Monats abgerechnet werden. Details https://open-docs.neuvector.com/deploying/awsmarketplace[hier].
* Unterstützung der Bildsignierung für Zugangssteuerungen. Benutzer können {product-name} verlangen, dass Bilder von bestimmten Parteien signiert sind, bevor sie in die Produktionsumgebung bereitgestellt werden, durch eine Integration mit Sigstore/Cosign. Siehe Assets -> Sigstore-Überprüfer zur Erstellung neuer Signatur-Assets. Regeln können dann mit den Kriterien Bildsignierung und/oder Bild-Sigstore-Überprüfer erstellt werden.
...
* Wiederholte Protokolle in den Protokollen des Durchsetzungs-/Steuerungssystems reduzieren.
...
* Sicherheitsereignisse warnen, selbst mit erlaubten Netzwerkregeln aufgrund eines Problems mit der Richtlinienaktualisierung in der Synchronisierung.
* Netzwerkaktivitäten, die den Datenverkehr der benutzerdefinierten Gruppe fälschlicherweise extern zuordnen.
...
* Aktivieren Sie die Integrationsparameter für den AWS Marketplace (Abrechnungsadapter) im Helm-Diagramm.
...
* Usability-Verbesserung: Verwenden Sie den Scanner, um ein einzelnes Bild zu scannen und das Ergebnis auszugeben (siehe Beispiel unten).
...
* Aktivierung der automatischen Scans und Löschung von nicht verwendeten Gruppen im configMap hinzufügen.
...
* Neue Umgebungsvariablen für den Durchsetzer zum Helm-Chart hinzufügen.
...
* Fügen Sie die Umgebungsvariable ENF_NETPOLICY_PULL_INTERVAL des Durchsetzers hinzu - Wert in Sekunden (empfohlener Wert 60), um den Netzwerkverkehr und den resultierenden Ressourcenverbrauch des Durchsetzers aufgrund von Richtlinienaktualisierungen/-neuberechnungen zu reduzieren. (Hinweis: Dies war bis August 2023 eine nicht dokumentierte Ergänzung).
...
* Verkehr innerhalb des gleichen Containers alarmiert/blockiert
...
* OIDC-Authentifizierung mit ADFS fehlgeschlagen, als zusätzliche Kodierungszeichen in der Anfrage erscheinen
...
* Enforcer, die einen Go-Routine-Panik erleben, was zu einem dp-Kill führt.  WebUI zeigt den Enforcer nicht als online an.
...
* Deutlicher zwischen Feature-Diagramm und Partner-Diagrammen in der Rancher-UI unterscheiden. + Verbesserung der Ingress-Annotation für Nginx im Rancher-Helm-Diagramm. Hinzufügen / Aktualisieren
...
* Der Durchsetzer wird in einem k8s 1.24-Cluster mit 1.64 Containerd-Laufzeit nicht betriebsbereit. Getrennt, der Durchsetzer startet manchmal nicht.
...
* CRD für WAF, DLP und Zulassungssteuerungen. HINWEIS: Zusätzliche Clusterrollenbindungen/Berechtigungen erforderlich. Siehe Kubernetes- und OpenShift-Bereitstellungsabschnitte. CRD-Import/Export und Versionierung für Zulassungssteuerungen werden durch CRD unterstützt.
...
* Unterstützung von AD/LDAP-Nested-Gruppen unter zugeordneten Rollengruppen.
* Unterstützung von Clusterrolebindings oder Rolebindings mit Gruppeninformationen im IDP für Openshift.
...
Für Helm-Benutzer, aktualisieren Sie auf {product-name} Helm-Chart 2.0.0 oder höher. Wenn Sie einen Operator oder eine Helm-Installation auf OpenShift aktualisieren, siehe die folgende Anmerkung.
...
* CRD für WAF, DLP und Zulassungssteuerungen. HINWEIS: Zusätzliche Clusterrollenbindungen/Berechtigungen erforderlich. Siehe Kubernetes- und OpenShift-Bereitstellungsabschnitte. CRD-Import/Export und Versionierung für Zulassungssteuerungen werden durch CRD unterstützt.
* Rancher SSO-Integration zum Starten der {product-name} Konsole über Rancher Manager. Dieses Feature ist nur verfügbar, wenn die {product-name} Container über Rancher bereitgestellt werden. HINWEIS: Erfordert aktualisierte Rancher-Version (Datum/Version noch festzulegen).
* Unterstützt die Bereitstellung auf RKE2.
...
* Helm-Chart v1.8.9 wurde für 5.0.0-Bereitstellungen veröffentlicht. Wenn Sie dies mit der Vorschau-Version von 5.0.0 verwenden, sollten die folgenden Änderungen an values.yml vorgenommen werden:
