= Test_SixParallel & Standalone Scanner
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /06.scanning/06.scanners/06.scanners.md
:page-opendocs-slug:  /scanning/scanner

== Test_SixErhöhung der Scanner-Skalierbarkeit mit mehreren Scannern
...
=== Test_SixAutomatische Skalierung von Scanner-Pods
...
=== Test_SixOperations und Debugging

Jeder Scanner-Pod wird die zu scannenden Registrierungen abfragen, um die vollständige Liste der verfügbaren Bilder und anderer Daten abzurufen. Jeder Scanner wird dann ein Bild zugewiesen, das aus dem Repository abgerufen und gescannt werden soll.
...
== Test_SixStandalone Scanner für lokale Scans
...
* SCANNER_REGISTRY= Test_SixURL des Registrys (optional anstelle des lokalen Scans)
* SCANNER_REPOSITORY= Test_SixRepository zum Scannen
* SCANNER_TAG= Test_SixVersions-Tag
* SCANNER_REGISTRY_USERNAME= Test_SixBenutzer (optional anstelle des lokalen Scans)
* SCANNER_REGISTRY_PASSWORD= Test_SixPasswort (optional anstelle des lokalen Scans)
* SCANNER_SCAN_LAYERS= Test_Sixwahr oder falsch (um geschichtete Scanergebnisse zurückzugeben)
...
=== Test_SixHost Scannen im Standalone-Modus
...
=== Test_SixManual Bereitstellung mehrerer Scanner auf Kubernetes
