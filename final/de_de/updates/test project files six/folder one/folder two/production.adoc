...
=== Test_SixBeste Praktiken, Tipps, Fragen und Antworten für die Bereitstellung und Verwaltung von \{product-name}

Laden Sie dieses xref:attachment$NV_Onboarding_5.0.pdf[Dokument zu den besten Praktiken für die Bereitstellung] herunter und überprüfen Sie es auf Tipps zu Themen wie Leistung und Dimensionierung, bewährte Praktiken und häufig gestellte Fragen zu Bereitstellungen.

== Test_SixBereitstellung mit Helm oder Operatoren
...
== Test_SixBereitstellung mit ConfigMap
...
== Test_SixBereitstellung der Controller
...
== Test_SixBackups und Persistente Daten
...
=== Test_SixPersistentes Volume Beispiel
...
Fügen Sie auch die folgende Umgebungsvariable in den Controller oder die All-in-One-Beispiel-YAML-Dateien für die Unterstützung des persistenten Speichers hinzu. Dies wird den Controller dazu bringen, die Sicherungskonfiguration beim Start zu lesen.
...
=== Test_SixConfigMaps und Persistenter Speicher
...
== Test_SixAktualisierung der CVE-Sicherheitsdatenbank in der Produktion
...
== Test_SixZugriff auf die Konsole
...
== Test_SixHanding Host-Updates oder Auto-Scaling-Knoten mit einem Pod-Störungshaushalt

Wartungs- oder Skalierungsaktivitäten können die Controller auf Knoten beeinträchtigen. Öffentliche Cloud-Anbieter unterstützen die Möglichkeit, Knoten automatisch zu skalieren, was Pods einschließlich der \{product-name} Controller dynamisch ausstoßen kann. Um Störungen der Controller zu verhindern, kann ein \{product-name} Pod-Störungshaushalt erstellt werden.
...
== Test_SixDeploy ohne privilegierten Modus
...
== Test_SixMulti-Site, Multi-Cluster-Architektur
