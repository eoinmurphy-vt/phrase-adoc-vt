= Test_FünfInterne Zertifikate ersetzen
:revdate: 2025-01-27
...
== Test_FünfInterne Kommunikation und Zertifikate

\{product-name} enthält standardmäßig selbstsignierte Zertifikate zur Verschlüsselung für die Verwaltung (Konsole/UI-Zugriff), Controller (REST-API, intern), Enforcer (intern) und Scanner (intern) Kommunikation.
...
----
[req]
distinguished_name = Test_Fivereq_distinguished_name
x509_extensions = Test_Fivev3_req
prompt = Test_Fiveno
[req_distinguished_name]
C = Test_FiveUS
ST = Test_FiveCalifornia
L = Test_FiveSan Jose
O = Test_Five{product-name} Inc.
OU = Test_FiveNeuvector
CN = Test_FiveNeuvector
[v3_req]
keyUsage = Test_FivedigitalSignature, keyEncipherment, dataEncipherment
extendedKeyUsage = Test_FiveserverAuth, clientAuth
subjectAltName = Test_Five@alt_names
[alt_names]
DNS.1 = Test_FiveNeuvector
----
...
+
Beispiel-Patch-Befehle für den Controller (Namespace bei Bedarf auf cattle-neuvector-system ändern und für die Verwendung mit Enforcer, Scanner anpassen)
+
[source,bash]
----
NAMESPACE=neuvector
...
+
Wenn Sie das interne Zertifikat nicht ersetzt haben und auf ein neues Set von Zertifikaten migrieren möchten, folgen Sie den folgenden Schritten:
+
...
+
Wenn Sie das automatisch generierte interne Zertifikat nicht verwenden und dies nicht tun können, folgen Sie den folgenden Schritten:
+
...
+
* Danach folgen Sie den Schritten im `Regenerate certificate files and add SANs` Tab und migrieren zu Ihrem eigenen Zertifikat.

========

=== Test_FiveUpdating/Deploying mit Helm
