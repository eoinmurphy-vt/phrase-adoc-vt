= Test_FiveDeploying \{product-name}
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /02.deploying/01.production/01.production.md
:page-opendocs-slug: /deploying/production

== Test_FivePlanning Deployments

Die \{product-name} Container in einem Standard-Deployment umfassen den Controller, Manager, Enforcer, Scanner und Updater. Die Platzierung, wo diese Container (auf welchen Knoten) bereitgestellt werden, muss berücksichtigt werden, und geeignete Labels, Taints oder Toleranzen müssen erstellt werden, um sie zu steuern.
...
Der Controller verwaltet den Cluster von Enforcern und kann auf demselben Knoten wie ein Enforcer oder auf einem separaten Management-Knoten bereitgestellt werden. Der Manager sollte auf dem Knoten bereitgestellt werden, auf dem der Controller läuft, und wird den Konsolenzugriff auf den Controller bereitstellen. Andere erforderliche \{product-name} Container wie der Manager, Scanner und Updater werden im Best Practices-Leitfaden, auf den unten verwiesen wird, detaillierter beschrieben.

Wenn Sie dies noch nicht getan haben, ziehen Sie die Images vom \{product-name} Docker Hub.

Die Images befinden sich im \{product-name} Docker Hub-Registry. Verwenden Sie das entsprechende Versions-Tag für den Manager, Controller, Enforcer und lassen Sie die Version für Scanner und Updater als 'latest'. Zum Beispiel:
...
=== Test_FünfBeste Praktiken, Tipps, Fragen und Antworten zum Bereitstellen und Verwalten von \{product-name}

Laden Sie dieses xref:attachment$NV_Onboarding_5.0.pdf[Dokument zu den besten Praktiken für die Bereitstellung] herunter und überprüfen Sie es auf Tipps zu Themen wie Leistung und Größenanpassung, bewährte Verfahren und häufig gestellte Fragen zu Bereitstellungen.

== Test_FünfBereitstellung mit Helm oder Operatoren
...
== Test_FünfBereitstellung mit ConfigMap
...
== Test_FünfBereitstellung der Controller
...
== Test_FiveBackups und persistente Daten

Stellen Sie sicher, dass Sie die Konfigurationsdatei regelmäßig aus der Konsole exportieren und als Backup speichern.

Wenn Sie mehrere Controller in einer HA-Konfiguration ausführen, werden alle Daten zwischen den Controllern synchronisiert, solange mindestens ein Controller immer aktiv ist.
...
=== Test_FivePersistentes Volume Beispiel
...
Fügen Sie auch die folgende Umgebungsvariable in den Controller- oder All-in-one-Beispiel-YAML-Dateien für die Unterstützung von persistentem Speicher hinzu. Dies wird den Controller dazu bringen, die Backup-Konfiguration beim Start zu lesen.
...
=== Test_FiveConfigMaps und Persistenter Speicher
...
== Test_FiveAktualisierung der CVE-Sicherheitsdatenbank in der Produktion

Bitte sehen Sie sich jeden Beispielabschnitt für Anweisungen an, wie die CVE-Datenbank aktualisiert werden kann.

Die Version der CVE-Datenbank kann in der Konsole im Tab "Vulnerabilities" angezeigt werden. Sie können auch das Updater-Container-Image inspizieren.
...
== Test_FiveZugriff auf die Konsole

Standardmäßig wird die Konsole als Dienst auf Port 8443 oder als NodePort mit einem zufälligen Port auf jedem Host bereitgestellt. Bitte sehen Sie sich den ersten Abschnitt Grundlagen -> xref:rest-api.adoc[Connect to Manager] für Optionen zum Deaktivieren von HTTPS oder zum Zugriff auf die Konsole durch eine Unternehmensfirewall an, die den Port 8443 für den Konsolenzugriff nicht zulässt.

== Test_FiveVerwaltung von Host-Updates oder automatischem Skalieren von Knoten mit einem Pod-Störungshaushalt

Wartungs- oder Skalierungsaktivitäten können die Controller auf Knoten beeinflussen. Öffentliche Cloud-Anbieter unterstützen die Möglichkeit, Knoten automatisch zu skalieren, was Pods, einschließlich der \{product-name} Controller, dynamisch entfernen kann. Um Unterbrechungen der Controller zu verhindern, kann ein \{product-name} Pod-Störungshaushalt erstellt werden.

Erstellen Sie beispielsweise die Datei unten nv_pdb.yaml, um sicherzustellen, dass jederzeit mindestens 2 Controller ausgeführt werden.
...
== Test_FiveDeploy ohne privilegierten Modus
...
== Test_FiveMulti-Site, Multi-Cluster-Architektur

Für Unternehmen mit mehreren Standorten, an denen ein separater \{product-name} Cluster für jeden Standort bereitgestellt werden kann, wird die folgende Referenzarchitektur vorgeschlagen. Jeder Cluster hat seine eigenen Controller und wird separat verwaltet.
