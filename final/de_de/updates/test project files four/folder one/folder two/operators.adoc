= Test_FiveDeploy Verwendung von Operatoren
:revdate: 2025-06-02
...
Operatoren nehmen menschliches operatives Wissen und kodieren es in Software, die leichter mit Verbrauchern geteilt werden kann. Operatoren sind Softwareteile, die die operationale Komplexität des Betriebs eines anderen Softwareteils erleichtern. Technisch gesehen sind Operatoren eine Methode zum Verpacken, Bereitstellen und Verwalten einer Kubernetes-Anwendung.

== Test_Five\{product-name} Operatoren
...
[IMPORTANT]
====
\{product-name} Zertifizierte Operator-Versionen sind an \{product-name} Produktversionen gebunden, und jede neue Version muss einen Zertifizierungsprozess mit Red Hat durchlaufen, bevor sie veröffentlicht wird. Die zertifizierte Operator-Version für 5.3.x ist an die Helm-Version 2.7.2 und die \{product-name} App-Version 5.3.2 gebunden. Die zertifizierte Operator-Version 1.3.9 ist an die \{product-name} Version 5.2.0 gebunden. Die zertifizierte Operator-Version 1.3.7 ist an die \{product-name} Version 5.1.0 gebunden. Die Version 1.3.4 des Operators ist an \{product-name} 5.0.0 gebunden. Wenn Sie die Versionstags der bereitgestellten \{product-name}-Container ändern möchten, verwenden Sie bitte die Community-Version.
====

.Bereitstellung mit zertifiziertem Operator
[%collapsible]
======
*Bereitstellung mit dem Red Hat zertifizierten Operator aus dem Operator Hub*
...
* Die \{product-name} Manager-Benutzeroberfläche wird typischerweise über eine OpenShift-Passthrough-Route auf einer Domain bereitgestellt. Zum Beispiel auf IBM Cloud neuvector-route-webui-neuvector.(cluster_name)-(random_hash)-0000.(region).containers.appdomain.cloud. Es kann auch als der Dienst neuvector-service-webui über eine Node-Port-Adresse oder eine öffentliche IP bereitgestellt werden.
...
** Navigieren Sie in der OpenShift-Konsole zur OperatorHub 
** Suchen Sie nach dem \{product-name} Operator und wählen Sie die Liste ohne Community- oder Marktplatzabzeichen aus 
...
. Bereiten Sie die YAML-Konfigurationswerte für die \{product-name} Installation wie im folgenden Beispiel-Screenshot gezeigt vor. Die im OpenShift Console präsentierte YAML bietet alle verfügbaren Konfigurationsoptionen und deren Standardwerte. 
+
...
** Überprüfen Sie, dass die Ressourcen den Status Erstellt oder Ausgeführt haben
...
* Die \{product-name}-Manager-Benutzeroberfläche wird typischerweise über eine OpenShift-Passthrough-Route auf einer Domain bereitgestellt. Zum Beispiel auf IBM Cloud neuvector-route-webui-neuvector.(cluster_name)-(random_hash)-0000.(region).containers.appdomain.cloud. Es kann auch als der Dienst neuvector-service-webui über eine Node-Port-Adresse oder eine öffentliche IP bereitgestellt werden. 
* OpenShift-Version 4.6+ 
* Es wird empfohlen, die Installationskonfiguration von \{product-name} zu überprüfen und zu ändern, indem Sie die YAML-Werte vor der Erstellung der \{product-name}-Instanz anpassen. Beispiele sind imagePullSecrets-Name, Tag-Version, Ingress-/Konsole-Zugriff, Multi-Cluster-Föderation, persistente Volumen-PVC usw. Bitte beachten Sie die Helm-Anweisungen unter https://github.com/neuvector/neuvector-helm für die Werte, die während der Installation geändert werden können.
