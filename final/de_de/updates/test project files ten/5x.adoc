...
* **NVSHAS-9833**: Konfigurationswiederherstellung schlägt in Rancher-Bereitstellungen fehl.
...
* **NVSHAS-9480**: NV UI, das im Rancher-Downstream-Cluster bereitgestellt wird, gibt nach der Abmeldung von Rancher einen HTTP/403-Fehler aus.
* **NVSHAS-9547**: Die Sortierung ist auf der Tabelle der Sicherheitsrisiken --> Schwachstellen defekt.
* **NVSHAS-9570**: [Schwachstellen] Ändern Sie die Legendenbeschreibung für verschiedene Status auf Assets.
...
* **NVSHAS-8148**: Link vom Container-Image zu den Scan-Ergebnissen des Registrierungs-Images.
...
* **NVSHAS-7687**: Unterstützung für die Konfiguration des Protokollniveaus (debug/error/info/warn) für den Durchsetzer und Controller über die CLI.
...
* **NVSHAS-8845**: APIKEY mit Rolle FedReader und FedAdmin erstellen.
...
* **NVSHAS-9078**: Unterstützung für die Signierung von Bildern für OCI-Bilder.
...
* **NVSHAS-4858**: Containergruppe im Controller nicht erweitern, um die Leistung der Richtlinienbereitstellung zu verbessern und CPU- sowie Speichernutzung zu reduzieren.
...
* Die Versionierung des Prometheus-Exportcontainers wurde überprüft und von der `controller` Versionierung dissociiert.
...
* Beheben der Zeichenfolgenverarbeitung, um unerwartete Neustarts des Enforcers zu verhindern
...
* Behebung von CVE-2023-32188 "`JWT-Token-Kompromittierung kann böswillige Aktionen einschließlich Remote Code Execution (RCE) ermöglichen.`" durch automatische Generierung des Zertifikats, das zur Signierung des JWT-Tokens bei Bereitstellung und Upgrade verwendet wird, und automatische Generierung des Manager/RESTful API-Zertifikats während Helm-basierter Bereitstellungen.
...
* Fügen Sie weitere Optionen für die CVE-Berichtskriterien in den Reaktionsregeln hinzu. Beispiel 1 - "cve-high-with-fix:X" bedeutet: Wenn die Anzahl der (hochgradigen Schwachstellen, die behoben wurden) >= Test_TwoX, lösen Sie die Reaktionsregel aus. Beispiel 2 - "cve-high-with-fix:X/Y" bedeutet: Wenn die Anzahl der (hochgradigen Schwachstellen, die vor Y Tagen gemeldet wurden und behoben wurden) >= Test_TwoX, lösen Sie die Reaktionsregel aus.
...
* Unterstützung der Proxy-Einstellung bei der Überprüfung von Bildsignaturen.
...
* Fügen Sie das Feld "Zuletzt geändert" hinzu, um die Berichterstattung über Schwachstellen zu filtern, sowie den erweiterten Filter in der Schwachstellenansicht.
...
* Die Seite mit mehreren Clustern wird bei hohen Anfragen nicht richtig gerendert.
...
* Liste der Compliance-Container zur Ausschluss von beendeten Containern.
...
* Die Sperrung des Zugriffs nach fehlgeschlagenen Anmeldeversuchen wird nun zur Standardoption. Der Standardwert beträgt 5 Versuche und kann in den Einstellungen -> Benutzer & Rollen-> Passwortprofil konfiguriert werden.
...
* Unterstützung für die Sicherheitsüberprüfung von openSUSE Leap OS (im Scanner-Bild).
...
{product-name} benötigt keine zusätzlichen Einstellungen für SELinux-aktivierte Cluster, um bereitgestellt und zu funktionieren. Getestet die Bereitstellung von {product-name} auf einem SELinux-aktivierten RKE2 gehärteten Cluster basierend auf RHEL 8.5. {product-name} wurde erfolgreich bereitgestellt, wenn PSP aktiviert ist und das Patch-Manager- und Scanner-Bereitstellung. Die nächste Chart-Version sollte das untenstehende Problem beheben.
...
* Unterstützt Kriterien für Ressourcenbeschränkungen in Zulassungssteuerungsregeln.
* Unterstützt Microsoft Teams-Format für Webhooks.
* Unterstützung von AD/LDAP-Verschachtelungen in der zugeordneten Rollengruppe.
* Unterstützung von Clusterrollenbindungen oder Rollenbindungen mit Gruppeninformationen im IDP für Openshift.
...
* CRD für WAF, DLP und Zulassungssteuerungen. HINWEIS: Zusätzliche Clusterrollenbindungen/Berechtigungen erforderlich. Siehe Kubernetes- und OpenShift-Bereitstellungsabschnitte. CRD-Import/Export und Versionierung für Zulassungssteuerungen werden durch CRD unterstützt.
* Rancher SSO-Integration zum Starten der {product-name} Konsole über Rancher Manager. Dieses Feature ist nur verfügbar, wenn die {product-name} Container über Rancher bereitgestellt werden. HINWEIS: Benötigt aktualisierte Rancher-Version (Datum/Version noch festzulegen).
* Unterstützt die Bereitstellung auf RKE2.
...
* Unterstützung für OWASP Top-10, WAF-ähnliche Regeln zur Erkennung von Netzwerkangriffen in Headern oder im Body hinzufügen. Beinhaltet Unterstützung für CRD-Definitionen von Signaturen und deren Anwendung auf die entsprechenden Gruppen.
...
* Helm-Chart v1.8.9 ist für 5.0.0-Bereitstellungen veröffentlicht. Wenn dies mit der Vorschau-Version von 5.0.0 verwendet wird, sollten die folgenden Änderungen an values.yml vorgenommen werden:
 ** Aktualisieren Sie das Registry auf docker.io
 ** Aktualisieren Sie die Bildnamen/Tags auf die Vorschau-Version im Docker-Hub
