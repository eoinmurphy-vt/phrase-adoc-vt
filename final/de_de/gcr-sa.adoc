...
Es ist eine bewährte Praxis, nicht von benutzerspezifischen Konten für Integrationen abhängig zu sein.  GCP unterstützt die Verwendung eines Dienstkontos zum Zugriff auf GCR.  Hier sind die Schritte, um ein Dienstkonto für GCR zu aktivieren und es zu verwenden, um Repository-Scans von \{product-name} auszulösen.
...
. Filter - Seien Sie sich bewusst, dass Sie wahrscheinlich alle Filter hier durch den tatsächlichen Namen des Repos ersetzen müssen. Nochmals, das ist in der GCR-Oberfläche.
...
[upperalpha]
. Siehe das "`gcr.io`" unter Hostname? Das gehört in Punkt #2, Registry in der \{product-name} Schnittstelle. (Vergessen Sie nicht den https:// Teil)
...
Wie Sie bereits festgestellt haben, ist JSON hier der Schlüssel. Die Auswahl von "`ERSTELLEN`" führt zu einer Datei, die Sie in Ihrem Browser herunterladen können. Der Inhalt dieser Datei sollte in das Feld 3, JSON-Schlüssel in \{product-name} eingefügt werden; siehe Abbildung 1.

Bevor Sie zu aufgeregt werden, gibt es noch eine Sache zu beachten. Damit der Scanner in \{product-name} die API verwenden kann, um Ihre Bilder zu scannen und zu schützen, muss die API in Ihrem GCP-Konto aktiviert sein. Sie können es entweder über die Befehlszeile aktivieren über
...
Oder Sie können die GCP-GUI verwenden. Gehen Sie zu "`API-Bibliothek`" und suchen Sie nach "`Artifact Registry API`" und stellen Sie sicher, dass es für Ihr Projekt aktiviert ist. Siehe Abbildung 7.

image:gcr8.png[GCR]

Sie sollten bereit sein! Siehe Abbildung 8 unten für ein richtig konfiguriertes Repository mit den Daten aus unserem Beispiel:
