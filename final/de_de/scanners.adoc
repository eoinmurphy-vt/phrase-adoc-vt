= Test_Drei parallele und eigenständige Scanner
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /06.scanning/06.scanners/06.scanners.md
:page-opendocs-slug:  /scanning/scanners

== Test_Drei Erhöhung der Scanner-Skalierbarkeit mit mehreren Scannern
...
=== Test_Drei Automatische Skalierung von Scanner-Pods
...
=== Test_DreiOperationen und Debugging

Jeder Scanner-Pod wird die zu scannenden Registrierungen abfragen, um die vollständige Liste der verfügbaren Bilder und anderer Daten abzurufen. Jeder Scanner wird dann ein Bild zugewiesen, das er aus dem Repository abrufen und scannen soll.
...
== Test_DreiStandalone Scanner für lokale Scans
...
* SCANNER_REGISTRY= Test_DreiURL des Registrys (optional anstelle des lokalen Scans)
* SCANNER_REPOSITORY= Test_DreiRepository zum Scannen
* SCANNER_TAG= Test_DreiVersions-Tag
* SCANNER_REGISTRY_USERNAME= Test_DreiBenutzer (optional anstelle des lokalen Scans)
* SCANNER_REGISTRY_PASSWORD= Test_DreiPasswort (optional anstelle des lokalen Scans)
* SCANNER_SCAN_LAYERS= Test_DreiWahr oder falsch (um geschichtete Scannergebnisse zurückzugeben)
...
=== Test_ThreeHost Scannen im Standalone-Modus
...
=== Test_ThreeManual Bereitstellung mehrerer Scanner auf Kubernetes
