...
--
. Créez un nouveau service pour la tâche Enforcer. Encore une fois, définissez le placement de la tâche sur {Q}&quot;`Une tâche par hôte`&quot;{/Q} afin qu'un seul Enforcer soit déployé sur chaque hôte. Notez également que la contrainte supplémentaire doit montrer qu'elle empêche le déploiement sur un nœud allinone.
+
--
image:5taskplacement.png[EnforcerPlacement]

Déployez ce service avec le nombre souhaité de nœuds enforcer dans {Q}&quot;`Nombre de tâches`&quot;{/Q}. Très bientôt, tous les enforcers seront opérationnels. Depuis la console {product-name}, vous pourrez voir tous les nœuds détectés avec des enforcers.
--
...
. Si vous avez plusieurs contrôleurs ou Allinones déployés en tant que cluster, ignorez cette étape. S'il n'y a qu'un seul Allinone/contrôleur dans le système, trouvez une nouvelle instance ECS et déployez un 2ème conteneur Allinone/contrôleur dessus (suivez les étapes de déploiement ECS de l'allinone/contrôleur {product-name}). Après le déploiement, dans la console de gestion {product-name}, vous verrez ce nouveau contrôleur opérationnel (sous Ressources > Contrôleurs). C'est nécessaire pour que toutes les données d'état soient répliquées entre les contrôleurs.
. Dans les services ECS, réinitialisez et supprimez l'ancien service Allinone/contrôleur. Tirez les images {product-name} mises à jour manuellement ou déclenchez AWS ECS pour tirer de nouvelles versions des conteneurs Allinone/contrôleur depuis Dockerhub ou votre registre privé.
. Créez une nouvelle révision de la tâche Allinone/Controller, mettez à jour le {Q}&quot;+`CLUSTER_JOIN_ADDR`&quot;{/Q} à l'adresse IP du nœud privé du 2ème Allinone/contrôleur.
