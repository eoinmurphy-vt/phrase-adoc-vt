...
Voici un exemple de la façon de déployer {product-name} en utilisant ECS.
...
====
--
. Créez un nouveau service pour la tâche Allinone. Définissez le &quot;`Modèles de Placement`&quot; sur &quot;`Une Tâche Par Hôte`&quot; afin qu'un seul Allinone/Contrôleur puisse s'exécuter sur n'importe quel hôte. Vous verrez également que la contrainte sera utilisée "+`membreDe(attribut:allinone-node=~true) qui nécessite que le nœud ait cet attribut.
+
--
image:3taskplacement.png[AllinonePlace]
--
. Vous pouvez maintenant déployer le service Allinone. Définissez le &quot;`Number of tasks`+&quot; sur le nombre souhaité d'Allinone/Contrôleur. Maintenant, les {product-name} conteneurs Allinone ou Contrôleur commenceront à s'exécuter sur les nœuds sélectionnés. Après le démarrage de l'Allinone, vous devriez pouvoir vous connecter à la {product-name} console via HTTPS sur le port 8443.
...
Déployez ce service avec le nombre souhaité de nœuds enforcer dans &quot;`Nombre de tâches`&quot;. Très bientôt, tous les enforcers seront opérationnels. Depuis la {product-name} console, vous pourrez voir tous les nœuds détectés avec des enforcers.
--
...
. Si vous utilisez le conteneur Manager séparé au lieu de l'Allinone (qui a déjà le manager dedans), éteignez simplement et supprimez l'ancien conteneur manager. Puis tirez la nouvelle version du manager et déployez-la, en pointant le CLUSTER_JOIN_ADDR vers l'IP du contrôleur.
