...
L'exemple suivant est un exemple de fichier docker-compose pour déployer le conteneur allinone sur le premier nœud. Parce que le conteneur allinone a un module d'application à l'intérieur, les conteneurs d'application sur le même nœud peuvent être sécurisés. Les déploiements greenfield et brownfield sont pris en charge.
...
La variable d'environnement la plus importante est *CLUSTER_JOIN_ADDR*. C'est l'adresse IP à laquelle d'autres agents se connectent. Normalement, elle doit être définie sur l'adresse IP du nœud où le conteneur all-in-one est en cours d'exécution.
