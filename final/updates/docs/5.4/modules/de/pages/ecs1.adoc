...
--
. Erstellen Sie die Allinone-Aufgabenbeschreibung. Erstellen Sie eine neue Aufgabenbeschreibung für den Allinone-Container. Sie können die ECS-Oberfläche verwenden, um sie manuell zu erstellen oder die Beispiel-JSON-Datei (siehe unten für Beispiele) einzufügen. Verweisen Sie auf Abschnitt &quot;`1. Bereitstellung {product-name}`&quot; dieser Dokumente zur Konfiguration des Allinone.
+
--
Geben Sie die Platzierungsbeschränkung ein. Wenn Sie beispielsweise das oben genannte Attribut-Labeling verwendet haben, geben Sie dies in der Beschränkung ein.
...
--
. Jetzt können Sie den Allinone-Dienst bereitstellen. Setzen Sie die &quot;`Number of tasks`+&quot; auf die gewünschte Anzahl an Allinone/Controller. Jetzt werden die {product-name} Allinone- oder Controller-Container auf den ausgewählten Knoten ausgeführt. Nachdem der Allinone gestartet wurde, sollten Sie in der Lage sein, über HTTPS auf den {product-name} Konsolenport 8443 zuzugreifen.
. Erstellen Sie die Enforcer-Aufgabenbeschreibung. Dies ist ähnlich wie die All-in-One-Aufgabe. Manuell über die ECS-Konsole konfigurieren oder das JSON-Beispiel unten verwenden.
+
--
...
== Beispiel für ECS JSON-Aufgabendefinitionen
...
Sie können eine Live-Aktualisierung der {product-name} Container in ECS durchführen, ohne die Dienste zu unterbrechen. Die Dienste von {product-name} können einfach aktualisiert oder aufgerüstet werden, ohne dass laufende Dienste unterbrochen werden. Um dies in Amazon ECS zu tun:

. Wenn Sie mehrere Controller oder Allinones als Cluster bereitgestellt haben, ignorieren Sie diesen Schritt. Wenn sich nur ein einzelner Allinone/Controller im System befindet, finden Sie eine neue ECS-Instanz und stellen Sie einen 2. Allinone/Controller-Container darauf bereit (folgen Sie den {product-name} Allinone/Controller ECS-Bereitstellungsschritten). Nach der Bereitstellung sehen Sie in der {product-name} Verwaltungs-Konsole diesen neuen Controller, der läuft (unter Ressourcen > Controller). Dies ist erforderlich, damit alle zustandsbehafteten Daten zwischen den Controllern repliziert werden.
. Setzen Sie in den ECS-Diensten den alten Allinone/Controller-Dienst zurück und löschen Sie ihn. Ziehen Sie die aktualisierten {product-name} Bilder manuell oder lösen Sie AWS ECS aus, um neue Versionen der Allinone/Controller-Container von Dockerhub oder Ihrem privaten Repository zu ziehen.
...
. Erstellen Sie einen neuen Dienst, um diese neue Aufgabe bereitzustellen (folgen Sie denselben Schritten zur Bereitstellung in ECS). Nach Abschluss sollte die neue Version des Allinone/Controllers laufen. Aus der {product-name} Verwaltungs-Konsole sollten alle Protokolle und Richtlinien weiterhin vorhanden sein. Optional können Sie den 2. Allinone/Controller-Container jetzt herunterfahren, da jetzt ein Allinone/Controller auf dem ursprünglichen Knoten gestartet sein sollte.
. Fahren Sie die Enforcer in den ECS-Diensten herunter und aktualisieren Sie sie. Ziehen Sie manuell oder automatisch die neuen Enforcer-Bilder. Starten oder aktualisieren Sie dann den Enforcer auf allen Knoten neu. Aus der {product-name} Konsole sehen Sie, dass alle Enforcer auf dem neuesten Stand sind.
