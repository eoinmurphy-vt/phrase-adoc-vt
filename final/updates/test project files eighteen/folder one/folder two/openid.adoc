...
Im mittleren Bereich klicken Sie auf {Q}`Create App Integration`{/Q}:

image:okta1.png[erstellen]

Ein neues Fenster wird angezeigt, um die {Q}`Sign-in method`{/Q} auszuwählen:

image:okta2.png[newapp]

Wählen Sie die {Q}`OIDC -- OpenID Connect`{/Q} Option aus.

Ein abgeleitetes Fenster erscheint zur Auswahl von {Q}`Application Type`{/Q}:

image:okta3.png[apptype]

Wählen Sie die {Q}`Native Application`{/Q} Option aus.

Der zentrale Bereich zeigt nun das Formular zur Integration von nativen Apps, in dem Sie die folgenden Werte entsprechend ausfüllen müssen:
...
Gehen Sie zu Ihrer {product-name} Konsole und navigieren Sie zu {Q}`Settings`{/Q} -> {Q}`OpenId Connect Settings`{/Q}.  Oben auf der Seite, neben dem {Q}`OpenID Connect Redirect URI`{/Q} Label, klicken Sie auf {Q}`Copy to Clipboard`{/Q}.
...
Wählen Sie {Q}`Allow everyone in your organization to access`{/Q} aus, um diese Integration für alle in Ihrer Organisation verfügbar zu machen.
...
Gehen Sie im {Q}`Client Credentials`{/Q} Abschnitt auf Bearbeiten und ändern Sie den {Q}`Client Authentication`{/Q} Abschnitt von {Q}`Use PKCE (for public clients)`{/Q} zu {Q}`Use Client Authentication`{/Q} und klicken Sie auf Speichern. Dies wird automatisch ein neues Geheimnis generieren, das wir in den kommenden {product-name} Einrichtungsschritten benötigen:

image:okta7.png[clientauth]

Navigieren Sie zum {Q}`Sign On`{/Q} Tab und bearbeiten Sie den {Q}`OpenID Connect ID Token`{/Q} Abschnitt:
Ändern Sie den Aussteller von {Q}`Dynamic (based on request domain)`{/Q} zu dem festen {Q}`Okta URL`{/Q}:
...
Im {Q}`OpenID Connect ID Token`{/Q} Abschnitt können Sie neue Ansprüche für die Gruppen des Benutzers erstellen und den Anspruch im ID-Token tragen (ein ID-Token ist ein JSON-Web-Token, ein kompaktes, URL-sicheres Mittel zur Darstellung von Ansprüchen, die zwischen zwei Parteien übertragen werden, sodass Identitätsinformationen über den Benutzer direkt im Token kodiert sind und das Token definitiv verifiziert werden kann, um zu beweisen, dass es nicht manipuliert wurde). Wenn ein spezifischer Bereich konfiguriert ist, stellen Sie sicher, dass Sie den Bereich auf der {product-name} OpenID Connect-Einstellungsseite hinzufügen, damit der Anspruch nach der Authentifizierung des Benutzers einbezogen werden kann:

image:okta11.png[scopes]

Standardmäßig sucht {product-name} nach "Gruppen" im Anspruch, um die Gruppenmitgliedschaft des Benutzers zu identifizieren. Wenn ein anderer Anspruchsname verwendet wird, können Sie den Anspruchsnamen auf der Seite der OpenID Connect-Einstellungen von {product-name} anpassen. Um Ansprüche zu konfigurieren, bearbeiten Sie den {Q}`OpenID Connect ID Token`{/Q} Abschnitt wie im nächsten Bild gezeigt:

image:okta12.png[claims]

Navigieren Sie auf Ihrer Anwendungsintegrationsseite zum {Q}`Assignments`{/Q} Tab und stellen Sie sicher, dass die entsprechenden Zuweisungen aufgelistet sind:
...
Nachdem der Benutzer authentifiziert ist, kann die richtige Rolle mit der gruppenbasierten Rollenzuordnungs-Konfiguration abgeleitet werden. Um die gruppenbasierte Rollenzuordnung einzurichten,
