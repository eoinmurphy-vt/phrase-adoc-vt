= Testprojekt achtzehn 5.x Versionshinweise
:revdate: 2025-08-26
...
* *NVSHAS-9934*: Verdächtige Nulldrift-Funktionalitätsstörung.
...
* **NVSHAS-9670**: Manager: Problem mit doppelten Anführungszeichen in der einfachen Textantwort und Problem mit der nicht benannten Java-Bibliothek im sbt-Ausführen.
...
** Scanner-Cache-Statistiken-Exposition.
...
* **NVSHAS-8939**: Bereitstellung einer Option in der NV-Benutzeroberfläche, damit Rancher SSO-Sitzungsbenutzer das aktuelle JWT-Token abmelden können (d.h. ausloggen).
...
* **NVSHAS-9091**: Kann nicht alle Module unter ol:9.1, photon:5.0, rhel:9.1 und amzn:2023 im Repo, Registry und Standalone-Scan melden.
...
* **NVSHAS-9189**: Der Scan wird beim Planen stecken bleiben, nachdem der Controller heruntergefahren und neu gestartet wurde.
...
* Exportieren / Importieren des Schwachstellenprofils über die Konsole, CRD oder REST-API. Das Importieren ersetzt das vorhandene Profil. Das Löschen der CRD führt zu einem leeren Profil.
* Exportieren / Importieren der Compliance-Profilvorlage über die Konsole, CRD oder REST-API. Das Importieren ersetzt die vorhandene Vorlage.
...
* Unterstützen Sie Deep Linking in URLs zur Seite für Bild- und/oder Containeranfälligkeiten
...
* Fügen Sie das Menü "Support anfordern" mit Links zu Slack, Dokumentation und anderen Ressourcen hinzu
...
* Das Löschen anfälliger Dateien (z. B. jar) entfernt sie nicht aus der Liste der Schwachstellen
...
* Behebe die Zeichenfolgenbehandlung, um unerwartete Enforcer-Neustarts zu verhindern
...
* Füge automatische Erkennung der Laufzeit-Container-Engine (Socket) zum Helm-Diagramm hinzu
* Entferne die Einstellung für den Betrieb des Controllers im privilegierten Modus im Helm-Diagramm und die Anforderung, dass der Controller/Enforcer "/host/cgroup" einbindet.
...
==== Test_ZweiHervorgehobene Änderungen, die möglicherweise Änderungen für manuelle Bereitstellungen erfordern (alle Änderungen sind bereits im neuesten Helm-Diagramm für 5.3.x enthalten)
...
* Behebung von CVE-2023-32188 {Q}`JWT token compromise can allow malicious actions including Remote Code Execution (RCE)`{/Q} durch automatische Generierung des für die Signierung des JWT-Tokens verwendeten Zertifikats bei Bereitstellung und Upgrade sowie automatische Generierung des Manager/RESTful API-Zertifikats während der Helm-basierten Bereitstellungen.
 ...
 ** Die Verwendung von benutzerspezifizierten Zertifikaten wird wie zuvor sowohl für Helm- als auch für yaml-basierte Bereitstellungen weiterhin unterstützt.
...
* Unterstützung der AWS Marketplace PAYG-Abrechnung für {product-name} monatliche Support-Abonnements. Benutzer können sich über den SUSE-Support für {product-name} anmelden, die monatlich basierend auf der durchschnittlichen Nutzung der Knotenanzahl des vorherigen Monats auf ihrem AWS-Konto abgerechnet werden. Details https://open-docs.neuvector.com/deploying/awsmarketplace[hier].
* Unterstützung der Bildsignierung für Zugangssteuerungen. Benutzer können {product-name} verlangen, dass Bilder von bestimmten Parteien signiert werden, bevor sie in die Produktionsumgebung bereitgestellt werden, durch eine Integration mit Sigstore/Cosign. Siehe Assets -> Sigstore-Überprüfer zur Erstellung neuer Signatur-Assets. Regeln können dann mit den Kriterien Bildsignierung und/oder Bild-Sigstore-Überprüfer erstellt werden.
...
* Verwenden Sie weniger privilegierte Dienstkonten, wie für jede {product-name} Komponente erforderlich. Eine Variable {Q}`leastPrivilege`{/Q} wird eingeführt. Der Standardwert ist falsch. HINWEIS: Die Verwendung des aktuellen Helm-Diagramms mit dieser Variablen in einer Version vor 5.2.0 funktioniert nicht richtig.
...
* SUSE Linux (SLES) 15.4 CVE stimmt nicht mit dem Scanner überein. Mit diesem Fix wird, wenn der Schweregrad im Feed angegeben ist, die Schwachstelle zur Datenbank hinzugefügt, auch wenn der NVD-Eintrag fehlt. Es ist möglich, dass der Bericht Schwachstellen ohne CVE-Werte enthält.
...
* Fügen Sie {Q}`package`{/Q} als Informationen zum syslog-Ereignis für eine erkannte Schwachstelle hinzu.
...
* Fügen Sie Enforcer-Umgebungsvariablen hinzu, um das Scannen von Geheimnissen zu deaktivieren und CIS-Benchmarks auszuführen
...
* Erlauben Sie, dass Netzwerkregeln und Zulassungssteuerungsregeln zu einer föderierten Regel befördert werden.
...
* Erlauben Sie dem Benutzer, den Modus zu wechseln, wenn er alle Gruppen im Menü Richtlinie -> Gruppen auswählt. Warnen Sie, wenn die Knoten-Gruppe ebenfalls ausgewählt ist.
