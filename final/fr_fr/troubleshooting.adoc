...
== Test de dépannage des déploiements \{product-name}

Les conteneurs \{product-name} sont déployés, gérés et mis à jour en utilisant le même outil d'orchestration utilisé pour les charges de travail des applications. Veuillez vous assurer de consulter la documentation en ligne pour chaque étape nécessaire lors du déploiement. Souvent, les déploiements sont tentés simplement en copiant les fichiers yaml d'exemple et en les déployant sans examiner les étapes préalables, telles que la configuration appropriée des registres, des secrets ou des RBAC/liaisons de rôles.

=== Test de déploiement initial

* Vérifiez que les conteneurs \{product-name} peuvent être récupérés avec une authentification correcte. Vérifiez le secret utilisé et assurez-vous que le cluster peut accéder au registre approprié.
...
* Vérifiez la plateforme et le temps d'exécution du conteneur et apportez les modifications nécessaires (par exemple, PKS, containerd, CRI-O).

=== Test de connexion et de configuration initiale
...
== Tester les journaux de débogage
...
Ces journaux peuvent montrer des problèmes de connectivité de cluster, des actions administratives, des activités de numérisation et d'autres entrées utiles. S'il y a plusieurs contrôleurs en cours d'exécution, il peut être nécessaire d'inspecter chacun d'eux. Ces journaux peuvent être redirigés vers un fichier à envoyer au support \{product-name}.

=== Tester l'activation du mode débogage pour les contrôleurs \{product-name}
...
=== Tester les journaux Kubernetes, OpenShift et autres orchestrations
...
== Tester le journal de support

Le journal de support contient des informations supplémentaires utiles pour le support \{product-name}, y compris la configuration système, les conteneurs, les politiques, les notifications et les détails des conteneurs \{product-name}.

Pour télécharger le journal de support, allez dans Paramètres -> Configuration et sélectionnez Collecter le journal.

== Testez en utilisant l'interface de ligne de commande pour activer le mode débogage
...
Obtenez la liste des contrôleurs. Trouvez le contrôleur avec le Leader = Test True.
...
== Testez en utilisant l'API REST pour activer le mode débogage
...
Désactivez le débogage sur tous les contrôleurs dans un cluster
