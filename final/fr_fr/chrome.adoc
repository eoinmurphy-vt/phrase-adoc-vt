= Test_ThreeTéléchargement du certificat Chrome - MacOS
:revdate: 2024-09-27
...
== Test_ThreeActivation des navigateurs Chrome pour accepter le certificat auto-signé \{product-name} sur MacOS
...
=== Test_ThreeExportation du certificat en utilisant Accès au trousseau de macOS
...
=== Test_TroisLancement de l'application Accès au trousseau macOS
...
=== Test_TroisLancement de l'Accès au trousseau depuis le navigateur Chrome
...
=== Test_TroisImportation de certificat dans le magasin Accès au trousseau macOS
...
=== Test_TroisActivation de la confiance pour le certificat \{product-name}
