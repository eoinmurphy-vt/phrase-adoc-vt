...
== Test_TwoImportant: Le déploiement sur Amazon ECS n'est plus pris en charge
...
== Test_TwoDéployer sur AWS en utilisant ECS
...
. Préparez plusieurs instances Amazon ECS qui ont le moteur Docker et les conteneurs de l'agent ECS intégrés. Choisissez un nœud pour la console de gestion. Ensuite, définissez des règles de groupe de sécurité qui permettent le port TCP entrant 8443 (port de console de gestion par défaut de \{product-name}) pour l'accès par votre navigateur client.
. Définissez un groupe de sécurité qui permet les ports TCP et UDP sur 18300, 18301, 18400, 18401. Ceci est utilisé par les agents de \{product-name} pour communiquer avec les contrôleurs/Allinone. Appliquez ce groupe de sécurité à toutes les instances ECS qui déploieront les agents et contrôleurs/allinone de \{product-name}.
...
Sélectionnez l'instance, puis choisissez "`Voir/Modifier les attributs`" dans le menu déroulant Actions.
...
--
. Créez la définition de tâche Allinone. Créez une nouvelle définition de tâche pour le conteneur Allinone. Vous pouvez utiliser l'interface ECS pour la créer manuellement ou coller le fichier JSON d'exemple (voir ci-dessous pour des exemples). Reportez-vous à la section "`1. Déploiement de \{product-name}`" de ces documents pour configurer l'Allinone.
+
--
Entrez la contrainte de placement. Par exemple, si vous avez utilisé l'étiquetage d'attribut ci-dessus, entrez cela dans la contrainte.
...
--
. Créez un nouveau service pour la tâche Enforcer. Encore une fois, définissez le placement de la tâche sur "`Une tâche par hôte`" afin qu'un seul Enforcer soit déployé sur chaque hôte. Notez également que la contrainte supplémentaire doit montrer qu'elle empêche le déploiement sur un nœud allinone.
+
--
image:5taskplacement.png[EnforcerPlacement]

Déployez ce service avec le nombre souhaité de nœuds enforcer dans "`Nombre de tâches`". Très bientôt, tous les enforcers seront opérationnels. Depuis la console \{product-name}, vous pourrez voir tous les nœuds détectés avec des enforcers.
--

== Test_TwoSample définitions de tâches ECS JSON
...
== Test_TwoLive Mise à jour \{product-name}

Vous pouvez effectuer une mise à jour en direct des conteneurs \{product-name} dans ECS sans interrompre les services. Les services de \{product-name} peuvent être facilement mis à jour ou améliorés sans interrompre les services en cours d'exécution. Pour ce faire dans Amazon ECS :

. Si vous avez plusieurs contrôleurs ou Allinones déployés en tant que cluster, ignorez cette étape. S'il n'y a qu'un seul Allinone/contrôleur dans le système, trouvez une nouvelle instance ECS et déployez un 2ème conteneur Allinone/contrôleur dessus (suivez les étapes de déploiement ECS de \{product-name} allinone/contrôleur). Après le déploiement, dans la console de gestion \{product-name}, vous verrez ce nouveau contrôleur opérationnel (sous Ressources > Contrôleurs). Ceci est nécessaire pour que toutes les données d'état soient répliquées entre les contrôleurs.
. Dans les Services ECS, réinitialisez et supprimez l'ancien service Allinone/contrôleur. Tirez manuellement les images mises à jour de \{product-name} ou déclenchez AWS ECS pour tirer de nouvelles versions des conteneurs Allinone/contrôleur depuis Dockerhub ou votre registre privé.
...
. Créez un nouveau service pour déployer cette nouvelle tâche (suivez les mêmes étapes pour déployer sur ECS). Une fois terminé, la nouvelle version de l'Allinone/contrôleur devrait être opérationnelle. Depuis la console de gestion \{product-name}, tous les journaux et politiques devraient toujours être là. En option, vous pouvez arrêter le 2ème conteneur Allinone/Controller maintenant, car un Allinone/Controller devrait maintenant être démarré sur le nœud d'origine.
. Depuis les Services ECS, arrêtez et mettez à jour les Enforcers. Déclenchez manuellement ou automatiquement le tirage de nouvelles images d'Enforcer. Puis redémarrez ou mettez à jour l'Enforcer sur tous les nœuds. Depuis la console \{product-name}, vous verrez que tous les Enforcers sont à jour.
...
Tous les conteneurs \{product-name} sont maintenant mis à jour en direct. Toutes les politiques, journaux et configurations ne sont pas affectés. La vue graphique en direct sera régénérée automatiquement dès qu'il y aura un nouveau trafic en direct circulant entre les conteneurs.
