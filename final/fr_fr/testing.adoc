= Test_TroisÉvaluation et Test \{product-name}
:revdate: 2025-05-08
:page-revdate: {revdate}
:page-opendocs-origin: /09.testing/01.testing/01.testing.md
:page-opendocs-slug:  /test/test

== Test_ThreeSample Applications

Après avoir déployé les composants \{product-name}, vous pouvez l'évaluer en utilisant les applications de test d'exemple que nous fournissons. Ceci est situé dans le dépôt `'nvbeta`' sur docker hub.
...
== Test_TroisPlan de Test Kubernetes
...
Puis connectez-vous à l'adresse IP publique/port pour l'un des nœuds Kubernetes, par exemple `+http://(public_IP):(NodePort)+`

Après avoir déployé \{product-name}, vous pouvez exécuter du trafic de test à travers les applications de démonstration pour générer les règles de liste blanche, puis déplacer tous les services en mode Surveillance ou Protection pour voir les violations et les attaques.

=== Test_TroisGénération de Violations Réseau sur Kubernetes
...
==== Test_TroisGénérer une Menace/Attaque
...
=== Test_TroisTests de Protection des Processus et des Fichiers
...
=== Test_TroisAnalyse de Registre et Contrôle d'Admission

Un test populaire consiste à configurer l'analyse d'images d'un registre dans Actifs -> Registres. Après la fin de l'analyse, configurez une règle de Contrôle d'Admission dans la Politique. Assurez-vous d'activer les Contrôles d'Admission et de définir une règle pour Refuser lorsqu'il y a de fortes vulnérabilités dans une image. Choisissez ensuite une image ayant de fortes vulnérabilités et essayez de la déployer dans Kubernetes. Le déploiement sera bloqué en mode Protection et vous verrez un événement dans Notifications -> Risques de Sécurité.
...
==== Test_TroisDéployer une Autre Application

L'application de démonstration Kubernetes Guestbook peut également être déployée sur Kubernetes. Il est recommandé de la déployer dans son propre espace de noms afin que vous puissiez voir le filtrage basé sur l'espace de noms dans la console \{product-name}.

== Test_ThreeDocker-native Test Plan

Après avoir déployé les composants \{product-name} et les applications d'exemple, vous pourrez découvrir, surveiller et protéger les conteneurs en cours d'exécution. Le plan de test ci-dessous fournit des suggestions pour générer des violations de comportement d'application autorisé en temps d'exécution et scanner les conteneurs pour des vulnérabilités.
