= Test_TwoRègles de Réseau
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /05.policy/05.networkrules/05.networkrules.md
:page-opendocs-slug:  /stratégie/règlesréseau

== Test_TwoPolicy: Règles de Réseau
...
[IMPORTANT]
====
Si vous modifiez (ajoutez, supprimez, changez) des règles, vos modifications ne seront PAS appliquées tant que vous n'aurez pas cliqué sur le bouton Enregistrer en haut. Si vous quittez cette page sans déployer vos modifications, elles seront perdues.
====

=== Test_TwoAjout de Nouvelles Règles
...
+
____
(Optionnel) Entrez un numéro. Les règles réseau sont initialement ordonnées de la plus basse à la plus haute, mais l'ordre des règles peut être modifié en les faisant glisser et en les déposant dans la liste.
____

* *De*
+
____
Spécifiez le GROUPE d'où la connexion va provenir. Commencez à taper et \{product-name} fera correspondre tous les groupes découverts précédemment, ainsi que tous les nouveaux groupes définis.
____
...
== Test_TwoEgress Control : Autoriser les connexions aux services internes de confiance sur d'autres réseaux

Un cas d'utilisation courant pour personnaliser les règles est de permettre à un service de conteneur de se connecter à un réseau en dehors du réseau géré par le cluster \{product-name}. Dans de nombreux cas, puisque \{product-name} ne reconnaît pas ce réseau, il le classera comme un réseau '`Externe`', même s'il s'agit d'un réseau interne.
...
=== Test_TwoIngress IP Politique Basée sur X-FORWARDED-FOR
...
Pour préserver l'adresse IP source d'origine, l'utilisateur doit ajouter la ligne suivante aux services exposés, dans la section 'spec' du répartiteur de charge ou du contrôleur d'ingress faisant face à l'extérieur. (Ref: https://kubernetes.io/docs/tutorials/services/source-ip/)
...
== Test_Détection des menaces réseau intégrées à deux volets
...
* Attaque par inondation DNS DDOS
* Attaque de type nul DNS
* attaque par tunneling DNS
