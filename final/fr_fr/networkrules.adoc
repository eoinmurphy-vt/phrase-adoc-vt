...
=== Test_TroisAjout de Nouvelles Règles
...
+
____
(Optionnel) Entrez un numéro. Les règles réseau sont initialement classées de la plus basse à la plus haute, mais l'ordre des règles peut être modifié en les faisant glisser et en les déposant dans la liste.
____
...
== Test_TroisContrôle de Sortie : Autoriser les connexions aux services internes de confiance sur d'autres réseaux
...
=== Test_TroisIngress IP Politique Basée sur X-FORWARDED-FOR
...
=== Test_TroisApplication Spéciale pour les Destinations Istio ServiceEntry
...
=== Test_TroisPolitique Réseau Basée sur l'Hôte Virtuel
...
== Test_ThreeSplit Mode Network Protections
