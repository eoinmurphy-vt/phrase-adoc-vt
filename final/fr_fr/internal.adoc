...
[IMPORTANT]
====
Les versions de NeuVector 5.4.2 et ultérieures doivent permettre aux utilisateurs de générer/remplacer des certificats internes avant d'utiliser NeuVector.
Après mars 2025, les versions de NeuVector antérieures à 5.4.2 doivent permettre aux utilisateurs de générer/remplacer des certificats internes avant d'utiliser NeuVector.
====
...
----
+
* Puis modifiez les fichiers de déploiement du Controller, de l'Enforcer et du Scanner, en ajoutant :
+
[source,yaml]
----
      containers:
...
----
+
Ensuite, procédez à déployer \{product-name} comme auparavant. Vous pouvez également accéder aux pods du contrôleur/enforcer/scanner pour confirmer que les fichiers ca.crt, tls.key, tls.crt sont ceux personnalisés et que les communications de \{product-name} fonctionnent avec les nouveaux certificats.
+
Exemples de commandes de patch pour le contrôleur (changez le namespace en cattle-neuvector-system si nécessaire, et modifiez pour l'utiliser sur l'enforcer, le scanner)
+
[source,bash]
----
NAMESPACE=neuvector
...
Mettre à jour le certificat actuel avec des SANs::
+
...
=== Test_TwoUpdating/Déploiement avec Helm
