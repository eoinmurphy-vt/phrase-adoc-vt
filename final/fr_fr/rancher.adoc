...
== Déployer et gérer {product-name} via les extensions Rancher ou Apps & Marketplace

{product-name} peut être déployé facilement soit via les extensions Rancher pour les clients Prime, soit via les Apps et Marketplace de Rancher. Le déploiement par défaut (basé sur Helm) de {product-name} déploiera {product-name} conteneurs dans l'espace de noms cattle-neuvector-system.

[NOTE]
====
Seuls les déploiements de {product-name} via les extensions Rancher ({product-name}) de la version 2.7.0+ de Rancher, ou les Apps & Marketplace de la version 2.6.5+ de Rancher peuvent être gérés directement (authentification unique au console de {product-name}) via Rancher. Si des clusters sont ajoutés à Rancher avec {product-name} déjà déployé, ou où {product-name} a été déployé directement sur le cluster, ces clusters ne seront pas activés pour l'intégration SSO.
====

=== Extension UI {product-name} pour Rancher

Les clients Prime de Rancher peuvent facilement déployer {product-name} et l'extension UI {product-name} pour Rancher. Cela permettra aux utilisateurs Prime de surveiller et de gérer certaines fonctions et événements de {product-name} directement via l'interface utilisateur de Rancher. Pour les utilisateurs de la communauté, veuillez consulter la section Déployer {product-name} ci-dessous pour déployer depuis les Apps et Marketplace de Rancher.
...
--
. Installez le {product-name}-UI-Ext depuis la liste Disponible
+
--
...
Après un {product-name} déploiement réussi, vous verrez un résumé des déploiements, des ensembles de démons et des tâches cron pour {product-name}. Vous pourrez également voir les services déployés dans le menu Découverte des services à gauche.
...
Voici un fichier compose Rancher d'exemple pour un déploiement sans mode privilégié :
...
Pour l'enforcer, pour l'empêcher de fonctionner sur un nœud de contrôleur (si désiré) :
