= Test_TwoScanners parallèles et autonomes
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /06.scanning/06.scanners/06.scanners.md
:page-opendocs-slug:  /scanning/scanners

== Test_TwoAugmenter l'évolutivité des scanners avec plusieurs scanners
...
Les résultats de numérisation de tous les scanners sont affichés dans le menu Actifs -> Registres. Des fonctionnalités de surveillance supplémentaires des scanners seront ajoutées dans les futures versions.

=== Test_TwoMise à l'échelle automatique des pods de scanner
...
=== Test_TwoOperations et Débogage
...
== Test_TwoPerformance Planification

Expérimentez avec des nombres variables de scanners sur des registres avec un grand nombre d'images pour observer le comportement du temps d'achèvement du scan dans votre environnement. 2-5 scanners en tant que paramètre de réplique devraient suffire dans la plupart des cas.
...
== Test_TwoStandalone Scanner pour le scan local
...
* SCANNER_REGISTRY= Test_Twourl du registre (optionnel au lieu du scan local)
* SCANNER_REPOSITORY= Test_Tworepository à scanner
* SCANNER_TAG= Test_Twoversion tag
* SCANNER_REGISTRY_USERNAME= Test_Twouser (optionnel au lieu du scan local)
* SCANNER_REGISTRY_PASSWORD= Test_Twopassword (optionnel au lieu du scan local)
* SCANNER_SCAN_LAYERS= Test_Twotrue ou false (pour retourner les résultats de scan en couches)
...
=== Test_TwoHost Analyse en mode autonome
...
=== Test_TwoDéploiement manuel de plusieurs scanners sur Kubernetes
