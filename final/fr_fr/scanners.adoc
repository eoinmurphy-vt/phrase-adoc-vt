= Tester les scanners parallèles et autonomes
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /06.scanning/06.scanners/06.scanners.md
:page-opendocs-slug:  /scanning/scanners

== Tester l'augmentation de l'évolutivité des scanners avec plusieurs scanners
...
Les résultats des scans de tous les scanners sont affichés dans le menu Actifs -> Registres. Des fonctionnalités de surveillance supplémentaires des scanners seront ajoutées dans de futures versions.
...
=== Opérations de test et débogage
...
== Planification des performances de test

Expérimentez avec des nombres variables de scanners sur des registres avec un grand nombre d'images pour observer le comportement du temps de complétion du scan dans votre environnement. 2-5 scanners comme paramètre de réplique devraient suffire dans la plupart des cas.
...
== Tester le scanner autonome pour le scan local
...
* SCANNER_REGISTRY= URL de test du registre (optionnel au lieu du scan local)
* SCANNER_REPOSITORY= Dépôt de test à scanner
* SCANNER_TAG= Tag de version de test
* SCANNER_REGISTRY_USERNAME= Utilisateur de test (optionnel au lieu du scan local)
* SCANNER_REGISTRY_PASSWORD= Mot de passe de test (optionnel au lieu du scan local)
* SCANNER_SCAN_LAYERS= Test vrai ou faux (pour retourner les résultats de scan en couches)
...
=== Test de l'hôte en mode autonome
...
=== Test de déploiement manuel de plusieurs scanners sur Kubernetes
