...
Une fonctionnalité de Kubernetes permet de s'assurer qu'un nombre minimum de contrôleurs fonctionne à tout moment. Ceci est utile pour le drainage des nœuds ou d'autres activités de maintenance qui pourraient supprimer des pods de contrôleur. Par exemple, créez et appliquez le fichier ci-dessous nv_pdb.yaml pour vous assurer qu'il y a au moins 2 contrôleurs fonctionnant à tout moment.
...
Optionnellement, supprimez toutes les références à la licence et aux secrets de \{product-name} dans les charts Helm, le yaml de déploiement, le configmap, les scripts, etc., car ceux-ci ne sont plus nécessaires pour tirer les images ou commencer à utiliser \{product-name}.
