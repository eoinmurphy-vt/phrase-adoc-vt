...
De plus, assurez-vous qu'il y a un {product-name} conteneur scanner déployé en mode autonome et configuré pour se connecter au Controller (si le Controller est utilisé).
...
Après avoir installé le plugin, trouvez la section '`{product-name} Scanner de vulnérabilités`' sur la page de configuration globale (Jenkins '`Configurer le système`'). Entrez les valeurs pour l'IP du {product-name} Controller, le port, le nom d'utilisateur et le mot de passe. Vous pouvez cliquer sur le bouton '`Tester la connexion`' pour valider les valeurs. Il affichera '`Connexion réussie`' ou un message d'erreur.
...
Cliquez sur le '`Ajouter un registre`' pour entrer les valeurs du registre que vous utiliserez dans votre projet. Si vous ne scannez que des images locales, vous n'avez pas besoin d'ajouter un registre ici.
...
Pour la configuration globale du registre, suivez les instructions ci-dessus pour local, puis ajoutez les détails du registre comme ci-dessous.

image:registry_console.png[registre-global]

==== Scanner autonome

Exécuter le scan Jenkins en mode autonome est un moyen léger de scanner les vulnérabilités des images dans le pipeline. Le scanner est invoqué dynamiquement et aucune installation de configuration de contrôleur n'est requise. C'est particulièrement utile lorsque vous scannez une image avant qu'elle ne soit poussée vers un registre. Il n'y a également aucune limite au nombre de tâches de scan pouvant s'exécuter en même temps.

Pour exécuter un scan de vulnérabilité en mode autonome, le plugin Jenkins doit tirer l'image du scanner vers l'hôte où l'agent s'exécute, donc vous devez entrer {product-name} l'URL du registre du scanner, le dépôt d'images et les informations d'identification si nécessaire, dans {product-name} la page de configuration du plugin.

Le résultat du scan peut également être soumis au contrôleur et utilisé dans la fonction de contrôle d'admission. Dans ce cas, vous avez besoin d'une configuration de contrôleur et de spécifier comment se connecter au contrôleur dans {product-name} la page de configuration du plugin.
...
Après la construction, un rapport {product-name} sera généré. Il affichera les détails du scan et les erreurs s'il y en a.
...
== Exemple de route OpenShift et de jeton de registre
...
Pour utiliser l'authentification basée sur un jeton pour le registre OpenShift, utilisez NONAME comme utilisateur et entrez le jeton dans le mot de passe.
