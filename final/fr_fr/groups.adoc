...
=== Test_TwoSuppression automatique des groupes inutilisés
...
=== Test_TwoProtection des hôtes - le groupe 'Nodes'
...
Pour activer la protection des hôtes avec des règles de profil de processus, sélectionnez le groupe 'nodes' et examinez les processus appris sur le nœud. Personnalisez si nécessaire en ajoutant, supprimant ou modifiant des règles de processus. Puis passez en mode Surveillance ou Protection.
...
+
____
Sélectionnez des conteneurs par leurs services. Si un conteneur est déployé par Docker Compose, sa valeur d'étiquette de service sera "nom_projet:nom_service" ; si un conteneur est déployé par le service en mode Docker swarm, sa valeur d'étiquette de service sera le nom du service swarm.
____
...
+
____
Créez un groupe par nom DNS ou plages d'adresses IP. Exemples : address=www.google.com, address=10.1.0.1, address=10.1.0.0/24, address=10.1.0.1-10.1.0.25. Le nom DNS peut être n'importe quel nom résolvable. Les critères d'adresse n'acceptent pas l'opérateur != Test_Two. Voir ci-dessous pour les groupes d'adresses d'hôte virtuel 'vh' spéciaux.
____
...
=== Test_DoublonCritères d'adresse IP
