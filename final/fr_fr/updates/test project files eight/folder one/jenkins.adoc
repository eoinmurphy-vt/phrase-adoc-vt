...
=== Test_EightInstallation du plug-in Jenkins
...
Déployez le conteneur \{product-name} Controller et Scanner si vous ne l'avez pas déjà fait sur un hôte accessible par le serveur Jenkins. Cela peut être sur le même serveur que Jenkins si désiré. Notez l'adresse IP de l'hôte où le Controller est en cours d'exécution. Remarque : Le port API REST par défaut est 10443. Ce port doit être exposé via l'Allinone ou le Controller à travers un service dans Kubernetes ou une carte de port (par exemple - 10443:10443) dans le fichier Docker run ou compose.

De plus, assurez-vous qu'il y a un conteneur scanner \{product-name} déployé en mode autonome et configuré pour se connecter au Controller (si le Controller est utilisé).
...
=== Test_HuitConfiguration globale dans Jenkins

Après avoir installé le plugin, trouvez la section '`Scanner de vulnérabilité \{product-name}`' dans la page de configuration globale (Jenkins '`Configurer le système`'). Entrez les valeurs pour l'IP du Controller \{product-name}, le port, le nom d'utilisateur et le mot de passe. Vous pouvez cliquer sur le bouton '`Tester la connexion`' pour valider les valeurs. Il affichera '`Succès de la connexion`' ou un message d'erreur.
...
=== Test_HuitConfiguration pour scanner un hôte Docker distant

==== Test_HuitConditions préalables pour un scan local sur un hôte Docker distant
...
=== Test_HuitÉtapes supplémentaires
...
== Test_HuitExemple de route OpenShift et de token de registre
...
== Test_HuitCas d'utilisation spécial pour Jenkins dans le même cluster Kubernetes
