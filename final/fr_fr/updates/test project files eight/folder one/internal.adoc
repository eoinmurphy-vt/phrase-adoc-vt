= Test_HuitRemplacement des certificats internes
:revdate: 2025-01-27
...
[IMPORTANT]
====
Les versions de NeuVector 5.4.2 et ultérieures doivent avoir des utilisateurs générer/remplacer des certificats internes avant d'utiliser NeuVector.
Après mars 2025, les versions de NeuVector antérieures à 5.4.2 doivent avoir des utilisateurs générer/remplacer des certificats internes avant d'utiliser NeuVector.
====

== Test_HuitCommunication interne et certificats

\{nom-du-produit} inclut des certificats auto-signés par défaut pour le chiffrement pour le Manager (accès console/UI), le Contrôleur (API REST, interne), l'Enforcer (interne) et les communications du Scanner (interne).

Ces certificats peuvent être remplacés par les vôtres pour renforcer davantage la communication. Pour remplacer les certificats utilisés par l'accès externe à \{nom-du-produit} (c'est-à-dire, du navigateur au Manager, ou de l'API REST au Contrôleur), veuillez consulter xref:replacecert.adoc[cette section]. Voir ci-dessous pour remplacer les certificats utilisés dans la communication interne entre les conteneurs de \{nom-du-produit}.

[WARNING]
====
Il est recommandé de remplacer les certificats uniquement lors du déploiement initial de \{nom-du-produit}. Le remplacement sur un cluster en cours d'exécution (même avec une mise à niveau progressive) peut entraîner un état instable où les pods de \{nom-du-produit} ne peuvent pas communiquer entre eux en raison d'un décalage dans les certificats, et une PERTE DE DONNÉES peut se produire.
====

=== Test_HuitRemplacement des certificats utilisés dans les communications internes de \{nom-du-produit}
...
----
[req]
distinguished_name = Test_Eightreq_distinguished_name
x509_extensions = Test_Eightv3_req
prompt = Test_Eightno
[req_distinguished_name]
C = Test_EightUS
ST = Test_EightCalifornia
L = Test_EightSan Jose
O = Test_Eight{product-name} Inc.
OU = Test_EightNeuvector
CN = Test_EightNeuvector
[v3_req]
keyUsage = Test_EightdigitalSignature, keyEncipherment, dataEncipherment
extendedKeyUsage = Test_EightserverAuth, clientAuth
subjectAltName = Test_Eight@alt_names
[alt_names]
DNS.1 = Test_EightNeuvector
----
...
----
+
* Puis modifiez les yamls de déploiement du Contrôleur, de l'Enforcer et du Scanner, en ajoutant :
+
[source,yaml]
----
      containers:
...
+
Si vos fichiers de certificat ont été créés avant la version 5.3 de \{product-name}, vous devez mettre à jour le certificat avec au moins un Nom Alternatif de Sujet, ou SAN pour faire court. Si vous avez toujours les fichiers `ca.key` et `ca.crt` accessibles, exécutez les commandes comme suit :
+
[source,bash]
----
kubectl delete secret internal-cert -n neuvector 
...
+
Si vos fichiers de certificat ont été créés avant la version 5.3 de \{product-name}, vous devez mettre à jour le certificat avec au moins un Nom Alternatif de Sujet, ou SAN pour faire court. Si vous n'avez plus les fichiers `ca.key` et `ca.crt`, suivez les étapes ci-dessous : 
+
...
=== Test_HuitMise à jour/Déploiement avec Helm
