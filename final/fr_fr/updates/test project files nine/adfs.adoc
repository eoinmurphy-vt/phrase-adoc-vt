...
== Test_TwoConfiguration de ADFS et de l'intégration {product-name}

Cette section décrit d'abord les étapes de configuration dans ADFS, puis dans la console {product-name}.

=== Test_TwoADFS Setup

. Dans la gestion AD FS, faites un clic droit sur "`Confiance de partie de confiance`" et sélectionnez "`Ajouter une confiance de partie de confiance...`".
+
...
. Cochez "`Activer le support pour le protocole SAML 2.0 WebSSO`" et entrez l'URI de redirection SAML depuis la page {product-name} Paramètres>Page de paramètres SAML dans le champ "`URL de service SSO SAML 2.0 de la partie de confiance`".  Sélectionnez "`Suivant`" pour continuer.
+
