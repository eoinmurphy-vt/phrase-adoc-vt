= Notes de version de Test_Two5.x
:revdate: 2025-08-26
...
==== Nouvelles fonctionnalités de Test_Two
...
Dans la version 5.4.2 de NeuVector, le support pour les déploiements utilisant le certificat interne intégré est interrompu. Le certificat trouvé à `/etc/neuvector/certs/internal` dans les images de conteneur NeuVector 5.4.2 sera supprimé. Pour continuer à utiliser NeuVector, les utilisateurs doivent :
...
* **NVSHAS-9631**: Réduire certaines erreurs d'application.
...
* **NVSHAS-9212**: Afficher le message d'alerte dans GET(/v1/eula) si le rôle neuvector-binding-secret (binding) est incorrect.
...
** Prise en charge des benchmarks CIS et de la signature d'images OCI.
* **Réseau & Surveillance**: 
...
** Prise en charge du mot de passe de démarrage.
...
* **NVSHAS-7687**: Support de la configuration du niveau de journalisation (déboguer/erreur/info/avertir) pour l'exécuteur et le contrôleur depuis l'interface de ligne de commande.
...
* **NVSHAS-9092** : L'utilisateur namespace ne devrait pas voir les ressources globales.
...
* Le `Egress and Ingress chart` a une nouvelle colonne de vulnérabilité qui contient le compte de vulnérabilités `High` et `Medium` pour chaque service.
...
* Ajouter des filtres de gravité, de niveau de score et de notation de flux à la vue des vulnérabilités d'image dans Assets > Registry.
* Autoriser lors de la configuration d'un registre s'il doit utiliser le proxy défini pour les analyses d'image du registre.
...
* Ne pas créer d'administrateur par défaut avec un mot de passe par défaut dans le {product-name} déploiement initial pour l'offre de facturation AWS (adaptateur CSP), nécessitant que l'utilisateur utilise un secret pour créer un nom d'utilisateur et un mot de passe administrateur
...
* La suppression automatique du groupe vide prend 2 heures pour être supprimée au lieu de 1 heure selon le calendrier.
* Le fichier de conformité ne s'affiche pas dans la console UI.
...
* Ajoutez la possibilité de masquer les espaces de noms et les groupes sélectionnés dans la vue d'activité réseau.
...
Exemple ci-joint pour activer psp à partir du graphique Rancher et ci-dessous les commandes pour patcher le déploiement du Manager et du Scanner. L'ID utilisateur dans la commande de patch peut être n'importe quel nombre.
...
* Corriger le problème avec le support d'openSUSE Leap 15.3 (k8s/crio).
...
* Ajoute le support pour OWASP Top-10, règles similaires à WAF pour détecter les attaques réseau dans les en-têtes ou le corps. Inclut le support pour les définitions CRD de signatures et l'application aux groupes appropriés.
