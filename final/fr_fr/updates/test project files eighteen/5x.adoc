...
=== Test_Two5.4.6 Août 2025

==== Test_TwoNouvelles fonctionnalités
...
* *NVSHAS-9975* : [Gestionnaire] TypeError : `this.mousemoveListener` n'est pas une fonction observée dans NV GUI.
...
=== Test_Two5.4.3 Mars 2025

==== Test_TwoNouvelles fonctionnalités

* **NVSHAS-9793**: Autoriser les rôles Fed Global dans LDAP/userinitcfg lors du déploiement de NeuVector via le ConfigMap et Secret.
...
* **NVSHAS-9755**: Demande d'afficher les noms des variables d'environnement avec les valeurs dans les alertes pour les secrets.
...
* **NVSHAS-9811**: [Gestionnaire] Impossible d'accéder à l'interface graphique lors de l'utilisation d'un certificat personnalisé (CA) avec CA racine et CA intermédiaire sur le secret.
...
* **NVSHAS-9662**: Logique de rôle/liaison de rôle incohérente dans le graphique Helm 2.8.2.
...
* **NVSHAS-9754**: [UI] Empêcher Rancher de désactiver l'authentification de l'utilisateur SSO de OpenShift ou du RBAC de Rancher.
...
* **NVSHAS-9670**: Gestionnaire: Problème de guillemets doubles dans la réponse en texte brut et problème de bibliothèque java sans nom dans l'exécution sbt.
* **NVSHAS-9667**: Définir la variable d'environnement `CTRL_PATH_DEBUG` sur erreur dans le déploiement du contrôleur ne fonctionne pas.
...
[NOTE]
====
Les types par défaut du gestionnaire et du service d'adaptateur de registre ont été changés en ClusterIP.  Les utilisateurs peuvent toujours remplacer le paramètre en remplaçant `manager.svc.type` et `cve.adapter.svc.type` si NodePort est préféré.
====
...
==== Test_Two5.4.2 Nouvelle installation :

===== Test_TwoUtilisation de Helm :
...
==== Test_TwoMise à niveau des versions précédentes vers 5.4.2 :
...
Les étapes suivantes ne sont nécessaires que si votre déploiement utilise un fichier `.yaml`. La mise à niveau utilisant Helm n'a pas besoin de ces étapes supplémentaires en raison des certificats internes créés par défaut via les drapeaux suivants : `internal.autoGenerateCert` et `internal.autoRotateCert`.
...
Informations supplémentaires sur xref:internal.adoc[certificats internes].
...
* **NVSHAS-9447**: Les pods du contrôleur/scanner plantent - "Sortie système non prise en charge".
...
==== Test_TwoProblèmes connus

* Dans la version 2.8.3 du graphique, nous avons déplacé une ressource précédemment mal allouée des crds au noyau. Si vous utilisez à la fois les graphiques crds et noyau, vous pourriez rencontrer des problèmes lors de la mise à niveau si vous déployez d'abord le noyau. Pour résoudre cela, mettez à niveau les crds d'abord puis les graphiques noyau.

=== Test_Two5.4 Septembre 2024
...
* **NVSHAS-9255**: Personnaliser les registres de contrôle d'admission pour les noms d'images sans FQDN.
...
* **NVSHAS-9005**: TypeError dans les registres : Impossible de lire les propriétés de non défini (lecture 'total_records').
...
* **NVSHAS-8539** : La reconfiguration des paramètres du proxy perd le mot de passe.
...
* **NVSHAS-9071** : Certains modules ne sont signalés que dans l'analyse du conteneur.
...
* **NVSHAS-9092** : L'utilisateur namespace ne doit pas voir les actifs globaux.
...
* **NVSHAS-8700** : L'utilisateur AD Rancher ne peut pas se connecter à {product-name} parfois.
...
* Permettre aux utilisateurs de définir '`accepté`' les vulnérabilités lors de l'utilisation des actions Github afin qu'elles n'affectent pas les flux de travail.
...
* Activer l'apprentissage ICMP (mode Découverte) et le blocage (mode Protection) via une nouvelle variable d'environnement du contrôleur CTRL_EN_ICMP_POLICY = Test_Two1
...
* Ajouter le support pour la plateforme ARM64. Le téléchargement depuis des plateformes basées sur ARM téléchargera automatiquement les images ARM64 appropriées {product-name}.
...
* Exportation / importation du profil de vulnérabilité via la console, CRD ou l'API REST. L'importation remplacera le profil existant. La suppression du CRD entraînera un profil vide.
* Exportation / importation du modèle de profil de conformité via la console, CRD ou l'API REST. L'importation remplacera le modèle existant.
...
* Afficher dans les Actifs -> Conteneurs -> Détails des conteneurs qui ont été analysés dans les registres par rapport à l'exécution
* Ajouter un lien vers le Groupe dans les Risques de Sécurité -> Vulnérabilités -> Popup d'Impact pour modifier facilement le mode de groupe
...
* Supprimer l'exigence pour le contrôleur/enforcer de monter "/host/cgroup".
...
* Supprimer le paramètre pour exécuter le contrôleur en mode privilégié dans le graphique Helm, et l'exigence pour le contrôleur/enforcer de monter "/host/cgroup".
...
==== Test_TwoModifications mises en évidence qui peuvent nécessiter des modifications pour les déploiements manuels (toutes les modifications sont déjà reflétées dans le dernier graphique Helm pour 5.3.x)
...
=== Test_Two5.2.4-s1 Janvier 2024

==== Test_TwoPublication de correctif de sécurité

* Remédie à CVE-2023-6129 dans openssl, et CVE-2023-46219, CVE-2023-46218 dans curl.

=== Test_Two5.2.4 Novembre 2023

==== Test_TwoCorrections de bogues

* Modifications de Azure AKS ValidatingWebhookConfiguration et journalisation des erreurs.

=== Test_Two5.2.3 Novembre 2023
 ...
 ** La génération automatique du certificat Manager, API REST et adaptateur de registre nécessite d'utiliser une installation basée sur Helm avec {product-name} la version helm 2.6.3 ou ultérieure.
 ...
 ** Le xref:replacecert.adoc[remplacement manuel de certificat] est toujours pris en charge et recommandé pour les versions précédentes ou les déploiements basés sur yaml. Voir le {product-name} avis de sécurité GitHub https://github.com/neuvector/neuvector/security/advisories/GHSA-622h-h2p8-743x[ici] pour une description.
...
* Fournir la date de construction de l'image du conteneur dans les détails des actifs -> Conteneur
...
* Ajouter la variable d'environnement AUTO_PROFILE_COLLECT pour le Contrôleur et l'Enforcer afin d'aider à capturer l'utilisation de la mémoire lors de l'examen des événements de pression mémoire. Définir la valeur = Test_Two1 pour activer.
...
* Ajouter plus d'options pour les critères de rapport CVE dans les règles de réponse. Exemple 1 - "cve-high-with-fix:X" signifie : Lorsque le nombre de (vulnérabilités élevées qui ont été corrigées) >= Test_TwoX, déclencher la règle de réponse. Exemple 2 - "cve-high-with-fix:X/Y" signifie : Lorsque le nombre de (vulnérabilités élevées qui ont été signalées il y a Y jours et qui ont été corrigées) >= Test_TwoX, déclencher la règle de réponse.
...
=== Test_Two5.2.1 août 2023

==== Test_TwoEnhancements

* Rapporter les résultats de scan superposés et les données CVE supplémentaires dans les messages SYSLOG. Ceci est activé par une case à cocher dans les paramètres -> Configuration -> SYSLOG.
...
* Lors de l'installation du graphique helm crd d'abord avant d'installer le graphique {product-name} noyau, les comptes de service manquent
...
* Support pour le scan de vulnérabilités Debian 12 (Bookworm).
...
* La suppression automatique de groupe vide prend 2 heures pour être supprimée au lieu de 1 heure selon le calendrier.
...
* Malgré la définition des règles réseau, les violations sont enregistrées sous les événements de sécurité (faux positifs) lorsque le conteneur s'est arrêté en raison d'une erreur de mémoire (OOM).
...
* Mettre à jour le configmap pour prendre en charge de nouvelles fonctionnalités (plusieurs certificats ADFS, dérive nulle, New_Service_Profile_Baseline, SYSLOG TLS, délai d'expiration de l'utilisateur)
...
* Ajouter une vérification imagePullPolicy dans les critères des règles de contrôle d'admission.
...
==== Sortie d'exemple Test_TwoScanner
...
=== Test_Two5.1.2 Mars 2023

==== Test_TwoEnhancements

* Prise en charge du groupe d'adresses basé sur l'hôte virtuel et de la stratégie de correspondance des protections réseau. Cela permet un cas d'utilisation où deux adresses FQDN différentes sont résolues à la même adresse IP, mais des règles différentes pour chaque FQDN doivent être appliquées. Un nouveau groupe personnalisé avec '`adresse=vh:xxx.yyy`' peut être créé en utilisant l'indicateur '`vh:`' pour activer cette protection. Une règle réseau peut ensuite utiliser le groupe personnalisé comme source '`De`' en fonction du nom d'hôte virtuel (au lieu de l'adresse IP résolue) pour appliquer des règles différentes pour les hôtes virtuels.
...
* Afficher la ressource d'espace de noms du cluster dans la console / UI.
* Afficher la gravité/classification de la source (par exemple, Red Hat, Ubuntu...) avec le score de gravité NVD dans la console.
* Ne pas autoriser la désactivation SSO/RBAC pour Rancher et OpenShift si l'utilisateur est authentifié via SSO.
...
* Trafic au sein du même container alertant/bloquant
* Violations implicites inattendues pour le trafic egress istio avec règle d'autorisation en place
* Lors de la mise à niveau depuis la version {product-name} 4.x, une adhésion incorrecte au groupe de pods provoque une violation de politique inattendue
...
* Mise à jour d'alpine pour remédier à plusieurs CVE, y compris Manager : CVE-2022-37454, CVE-2022-42919, CVE-2022-45061, CVE-2021-46848; Enforcer: CVE-2022-43551, CVE-2022-43552
 ...
 ** *_Stratégie immédiate :_*
  ...
  *** Lorsque le contrôleur principal voit en continu que "le nombre de tâches" est 0 pendant > 180 secondes, il réduit d'un pod de scanner si minScannerPods n'est pas encore atteint.
* Les groupes personnalisés peuvent désormais utiliser des étiquettes de namespace, y compris les étiquettes de namespace de Rancher. En général, les étiquettes de pod et de namespace peuvent désormais être ajoutées aux groupes personnalisés.
* Ajoutez la possibilité de masquer des namespaces sélectionnés, des groupes dans la vue d'activité réseau.
...
* (Addendum janvier 2023). Support pour la politique réseau basée sur ServiceEntry avec Istio. La fonctionnalité d'application de la politique réseau de sortie a été ajoutée dans la version 5.1.0 pour les pods vers les destinations ServiceEntry déclarées avec Istio. Typiquement, un ServiceEntry définit comment un service externe référencé par un nom DNS est résolu en une adresse IP de destination. Avant la v5.1, {product-name} ne pouvait pas détecter et appliquer des règles pour les connexions à un ServiceEntry, donc toutes les connexions étaient classées comme externes. Avec la 5.1, des règles peuvent être appliquées pour des destinations ServiceEntry spécifiques. IMPORTANT : Si vous mettez à niveau vers la v5.1 avec un déploiement basé sur Istio, de nouvelles règles doivent être créées pour permettre ces connexions et éviter les alertes de violation. Après la mise à niveau, les violations implicites seront signalées pour le trafic nouvellement visible si les règles d'autorisation n'existent pas. De nouvelles règles de trafic peuvent être apprises et auto-créées en mode Découverte. Pour permettre ce trafic, vous pouvez mettre le groupe en mode découverte ou créer un groupe personnalisé avec des adresses (ou un nom DNS) et une nouvelle règle réseau vers cette destination pour permettre le trafic. REMARQUE : Il y a un bug dans la version 5.1.0 dans la destination signalée par les violations de refus qui ne représentent pas la destination correcte.  Le bug signale que server_name et client_name sont identiques.  Ce problème sera traité dans une prochaine version de correctif.

==== Test_TwoCorrections de bugs
...
* Incident de violation inattendu Process.Profile.Violation dans le groupe NV.Protect sur lequel commande sur coreos.

=== Test_Two5.0.4 Octobre, 2022

==== Test_TwoMises à jour de sécurité
...
* Corriger l'incident de violation de process.profile inattendu dans le groupe NV.Protect.
...
=== Test_Two5.0.1 Juin 2022

==== Test_TwoEnhancements

* Prise en charge du scan de vulnérabilité de openSUSE Leap OS (dans l'image du scanner).
* Scanner : implémentez des attributs de suppression lors de la reconstruction du dépôt d'images.
...
* Distinguez plus clairement entre le graphique des fonctionnalités et les graphiques des partenaires dans l'interface utilisateur de Rancher. + Améliorez l'annotation d'entrée pour nginx dans le graphique helm de Rancher. Ajoutez / mettez à jour
ingress.kubernetes.io/protocol : https à nginx.ingress.kubernetes.io/backend-protocol : "HTTPS".
* L'opérateur OpenShift actuel prend en charge les routes de passage pour les services API et de fédération. Des paramètres supplémentaires de valeur Helm sont ajoutés pour prendre en charge les types de terminaison de route edge et re-encrypt.
...
* Tout utilisateur avec le rôle d'administrateur (utilisateur local ou SSO) qui promeut un cluster en maître fed devrait être automatiquement promu au rôle fedAdmin.
...
==== Test_TwoDetails sur le support SELinux

{product-name} n'a pas besoin de paramètres supplémentaires pour les clusters avec SELinux activé pour déployer et fonctionner. Testé le déploiement de {product-name} sur un cluster RKE2 durci avec SELinux activé basé sur RHEL 8.5. {product-name} déployé avec succès si PSP est activé et déploiement du gestionnaire de patch et du scanner. La prochaine version du graphique devrait corriger le problème ci-dessous.
...
=== Test_Two5.0.0 Version de disponibilité générale (GA) Mai 2022
...
* Permettre à l'utilisateur de changer de mode lorsqu'il sélectionne tous les groupes dans la Politique -> menu Groupes. Avertir si le groupe des Nœuds est également sélectionné.
...
==== Test_DeuxAutres Mises à jour

* Mettre à jour la version de l'application appVersion du graphique Helm à 5.0.0 et la version du graphique à 2.2.0
...
=== Test_TwoMise à niveau de {product-name} 4.x à 5.x (avant 5.2.x)
...
Pour les utilisateurs de Helm, mettez à jour vers {product-name} le chart Helm 2.0.0 ou ultérieur. Si vous mettez à jour un installateur ou Helm sur OpenShift, voir la note ci-dessous.

. Supprimer l'ancienne définition de rôle de cluster neuvector-binding-customresourcedefinition
...
. Appliquer le nouveau verbe de mise à jour pour le rôle de cluster neuvector-binding-customresourcedefinition
...
. Créer un nouveau rôle de cluster Admission, DLP et WAF et une liaison de rôle de cluster
...
. Mettre à jour les noms et chemins d'image pour tirer {product-name} des images de Docker hub (docker.io), par exemple.
...
En option, supprimer toute référence à la {product-name} licence et au secret de registre dans les charts Helm, yaml de déploiement, configmap, scripts, etc., car ceux-ci ne sont plus nécessaires pour tirer les images ou pour commencer à utiliser {product-name}.

*Note sur SCC et mise à niveau via Operator/Helm*

Le SCC privilégié est ajouté au compte de service spécifié dans le yaml de déploiement par la version de l'Operator 1.3.4 et supérieure dans les nouveaux déploiements. Dans le cas de la mise à niveau de l'Operator {product-name} d'une version précédente à 1.3.4 ou de Helm à 2.0.0, veuillez supprimer le SCC privilégié avant la mise à niveau.
...
==== Test_TwoBeta 1 version publiée en avril 2022
...
==== Test_TwoPreview.3 version publiée en mars 2022

[CAUTION]
.important
====

Pour mettre à jour les déploiements de prévisualisation précédents pour les nouvelles fonctionnalités CRD WAF, DLP et contrôle d'admission, veuillez mettre à jour le yaml CRD et ajouter de nouveaux rbac/liaisons de rôle :
...
* CRD pour WAF, DLP et contrôles d'admission. REMARQUE : des liaisons de rôle de cluster/permissions supplémentaires sont requises. Voir les sections de déploiement Kubernetes et OpenShift. Import/export de CRD et versionnage pour les contrôles d'admission pris en charge via CRD.
* Intégration SSO Rancher pour lancer {product-name} console via Rancher Manager. Cette fonctionnalité n'est disponible que si les conteneurs {product-name} sont déployés via Rancher. REMARQUE : Nécessite une version mise à jour de Rancher (date/version à déterminer).
* Prend en charge le déploiement sur RKE2.
...
==== Test_TwoPreview.2 version publiée en février 2022

* Modifications mineures de fichiers et de licences dans la source, aucune fonctionnalité ajoutée.

==== Test_TwoSupport pour le déploiement sur AWS ECS Obsolète

Le support pour le déploiement sur ECS n'est plus fourni. L'allinone devrait toujours pouvoir être déployé sur ECS, cependant, la documentation des étapes et des paramètres n'est plus supportée.

=== Test_Two5.0 'Tech Preview' janvier 2022

==== Test_TwoEnhancements

* Première version d'une version 'tech-preview' non prise en charge de {product-name} 5.0 version open source.
* Ajouter le support pour OWASP Top-10, règles similaires à WAF pour détecter les attaques réseau dans les en-têtes ou le corps. Comprend le support pour les définitions CRD de signatures et l'application aux groupes appropriés.
...
* Le graphique Helm v1.8.9 est publié pour les déploiements 5.0.0. Si vous utilisez cela avec la version de prévisualisation de 5.0.0, les modifications suivantes doivent être apportées à values.yml :
 ** Mettre à jour le registre vers docker.io
 ** Mettre à jour les noms/étiquettes d'image vers la version de prévisualisation sur Docker hub
