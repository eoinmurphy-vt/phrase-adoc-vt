= Notes de version de Test projet seize 5.x
:revdate: 2025-08-26
...
* *NVSHAS-9998*: Impossible d'exporter le groupe du maître fédéré NeuVector.
...
* **NVSHAS-9817**: La création de la règle de sécurité NvCluster CRD montre une création réussie, mais elle n'a pas été réellement créée en raison d'entrées de règles de processus dupliquées.
...
* **NVSHAS-9754**: [UI] Empêcher Rancher de désactiver l'authentification SSO de l'utilisateur lié à Rancher pour OpenShift ou RBAC de Rancher.
...
Veuillez créer et configurer des certificats internes à partir du scanner pour le contrôleur, l'exécuteur et l'adaptateur de registre afin d'obtenir une mise à jour continue sans perte de données. Il est toujours recommandé de faire une sauvegarde de votre configuration avant la mise à niveau. 
...
* **NVSHAS-8583**: Définir des modes de politique granulaires pour les ensembles de règles, mode de politique réseau séparé et mode de fichier de contrôle par niveau de groupe.
...
* **NVSHAS-9467**: Le groupe personnalisé défini par l'étiquette du pod ne propage pas ses données de fichier de contrôle sur les conteneurs enfants.
...
* **NVSHAS-9571**: [Registres] Filtre pour toutes les images scannées ne fonctionne pas bien.
...
* **NVSHAS-9608**: Le scanner ne signale aucune erreur lorsque le contrôleur signale une erreur pour d'énormes résultats de scan ~23 Mo.
...
* **NVSHAS-9092** : L'utilisateur namespace ne devrait pas voir les actifs globaux.
* **NVSHAS-9116** : Le cluster de travail peut partir si la connexion est interrompue.
...
* **NVSHAS-7687**: Ajouter le paramètre de niveau de journalisation pour l'enforcer.
...
* Correction d'un bug où le scan CVE des paquets `jar` n'affichait pas tous les paquets affectés par un même CVE. Maintenant, toutes les occurrences sont signalées.
* Remédie aux CVEs suivants :
...
* Après la mise à niveau vers v5.3.1 à partir d'une version précédente {product-name}, les ressources personnalisées NvClusterSecurityRule préexistantes peuvent être supprimées par inadvertance. REMARQUE : La version 5.3.1 a été supprimée de docker hub afin de prévenir le problème de mise à niveau.
...
* Le conteneur ne parvient pas à s'ajouter au travail avec succès (occurrences fréquentes). Résultant d'un blocage des messages de canal.
...
* Remédie aux CVEs suivants :
* CVE élevée : CVE-2023-52425 dans expat, CVE-2024-20952 et CVE-2024-20918 dans openjdk11
* CVE moyenne : CVE-2023-52426 dans expat, CVE-2024-20926, CVE-2024-20921, CVE-2024-20945 et CVE-2024-20919 dans openjdk11, CVE-2024-0727 et CVE-2023-6237 dans openssl
...
* Ajouter le support pour la plateforme ARM64. Le tirage depuis des plateformes basées sur ARM tirera automatiquement les images ARM64 appropriées {product-name}.
...
* Exportation / importation du profil de vulnérabilité via la console, CRD ou REST API. L'importation remplacera le profil existant. La suppression du CRD entraînera un profil vide.
* Exportation / importation du modèle de profil de conformité via la console, CRD ou REST API. L'importation remplacera le modèle existant.
...
==== Test_TwoModifications mises en surbrillance qui peuvent nécessiter des modifications pour les déploiements manuels (toutes les modifications sont déjà reflétées dans le dernier graphique Helm pour 5.3.x)
...
* Remédier à CVE-2023-32188 "`la compromission du jeton JWT peut permettre des actions malveillantes, y compris l'exécution de code à distance (RCE)+`" en générant automatiquement le certificat utilisé pour signer le jeton JWT lors du déploiement et de la mise à niveau, et en générant automatiquement le certificat Manager/RESTful API lors des déploiements basés sur Helm.
 ...
 ** La génération automatique du certificat Manager, REST API et adaptateur de registre nécessite d'utiliser une installation basée sur Helm avec {product-name} helm version 2.6.3 ou ultérieure.
 ...
 ** Le xref:replacecert.adoc[remplacement manuel du certificat] est toujours pris en charge et recommandé pour les versions précédentes ou les déploiements basés sur yaml. Voir le {product-name} avis de sécurité GitHub https://github.com/neuvector/neuvector/security/advisories/GHSA-622h-h2p8-743x[ici] pour une description.
...
* Lors de l'installation du graphique crd helm d'abord avant d'installer le graphique {product-name} noyau, les comptes de service manquent
...
* Utilisez des comptes de services moins privilégiés selon les besoins pour chaque composant {product-name}. Une variable "`moindrePrivilège`" est introduite. La valeur par défaut est fausse. REMARQUE : Utiliser le graphique helm actuel avec cette variable sur une version antérieure à 5.2.0 ne fonctionnera pas correctement.
...
* Ajouter "`paquet`" comme information à l'événement syslog pour une vulnérabilité détectée.
...
* Lors de la mise à niveau depuis {product-name} version 4.x, une adhésion incorrecte au groupe de pods provoque une violation de politique inattendue
...
* Ajouter un paramètre d'auto-scaling personnalisé pour les pods de scanner, avec les valeurs Retardé, Immédiat et Désactivé. Important : L'auto-scaling du scanner n'est pas pris en charge lorsque le scanner est déployé avec un opérateur OpenShift, car l'opérateur changera toujours le nombre de pods à sa valeur configurée.
...
* Retirer la licence de l'écran de configuration (plus nécessaire).
...
* Mettre à jour alpine pour supprimer le CVE critique-2022-40674 dans la bibliothèque expat du manager, ainsi que d'autres CVE mineurs.
...
* Ne pas afficher l'EULA après un redémarrage réussi à partir du volume persistant.
...
* Distinguer plus clairement entre le graphique des fonctionnalités et les graphiques des partenaires dans l'interface utilisateur de Rancher. + Améliorer l'annotation d'entrée pour nginx dans le graphique helm de Rancher. Ajouter / mettre à jour
...
* Corriger le problème de sauvegarde du rôle de fédération des travailleurs qui devrait restaurer dans des clusters non fédérés.
...
* Corriger le problème de support de openSUSE Leap 15.3 (k8s/crio).

==== Test_DeuxAutres Mises à jour

* Mettre à jour la version de l'application Helm chart à 5.0.0 et la version du chart à 2.2.0
