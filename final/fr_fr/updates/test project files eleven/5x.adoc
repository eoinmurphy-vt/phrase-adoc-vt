...
* https://github.com/neuvector/neuvector/security/advisories/GHSA-8ff6-pc43-jwv3[Gestion des mots de passe non sécurisée vulnérable aux attaques par arc-en-ciel]
...
* *NVSHAS-9947*: [UI-ext] Le graphique de conformité manque le statut "Manuel" dans Rancher NeuVector.
...
* *NVSHAS-9986*: Les règles d'accès aux processus et aux fichiers ne peuvent pas être modifiées ou supprimées dans la vue du groupe de stratégie fédérée.
...
* **NVSHAS-9729**: Un nombre incorrect de vulnérabilités est observé lorsque plusieurs statefulsets avec le "même nom" dans les namespaces sont déployés dans un projet.
...
* **NVSHAS-9662**: Logique de rôle/RoleBinding incohérente dans le graphique Helm 2.8.2.
...
* **NVSHAS-8583**: Définir des modes de stratégie granulaires pour les ensembles de règles, mode de stratégie réseau séparé et mode de fichier de contrôle par niveau de groupe.
...
* **NVSHAS-9447**: Les pods Controller/Scanner se bloquent - "Sortie système non prise en charge".
...
* **NVSHAS-8824**: L'utilisateur échoue à supprimer ses propres groupes, ne peut pas créer de groupes à portée de namespace.
...
* **NVSHAS-9608**: Le scanner ne signale aucune erreur lorsque le contrôleur signale une erreur pour des résultats de scan énormes ~23 Mo.
...
** Support du mot de passe Bootstrap.
...
* **NVSHAS-9144**: ID ajouté pour le fichier de contrôle de vulnérabilité pour une identification facile.
* **NVSHAS-7687**: Prise en charge de la configuration du niveau de journalisation (déboguer/erreur/info/avertir) pour l'application et le contrôleur depuis l'interface de ligne de commande.
...
* **NVSHAS-9155**: Nom de colonne incorrect et nom de fichier manquant lors de l'analyse de l'image du registre
...
* **NVSHAS-8700** : L'utilisateur Rancher AD ne peut parfois pas se connecter à {product-name}.
...
* Activer l'apprentissage ICMP (mode Découverte) et le blocage (mode Protection) via la nouvelle variable d'environnement du contrôleur CTRL_EN_ICMP_POLICY = Test_Two1
...
* Ajouter le support pour la plateforme ARM64. Le téléchargement depuis des plateformes basées sur ARM téléchargera automatiquement les images ARM64 {product-name} appropriées.
 ...
 ** Le certificat intégré est toujours utilisé pour les déploiements basés sur yaml s'il n'est pas remplacé lors du déploiement ; cependant, il est recommandé de les remplacer (voir la ligne suivante).
...
* Ajouter le paramètre "leastPrivilege" dans le graphique Helm. Ajouter l'option helm pour New_Service_Profile_Baseline. Une nouvelle version de graphique Helm (noyau) est publiée pour 5.2.
...
* Lors de la mise à niveau depuis {product-name} version 4.x, une appartenance incorrecte au groupe de pods provoque une violation de politique inattendue
...
* Le blocage de la connexion après des tentatives de connexion échouées devient désormais le défaut. La valeur par défaut est de 5 tentatives, et configurable dans les paramètres -> Utilisateurs & Rôles-> Profil de mot de passe.
...
* (Addendum janvier 2023). Support pour la stratégie de réseau basée sur ServiceEntry avec Istio. La fonctionnalité d'application de la stratégie de réseau sortant a été ajoutée dans la version 5.1.0 pour les pods vers les destinations ServiceEntry déclarées avec Istio. Typiquement, un ServiceEntry définit comment un service externe référencé par un nom DNS est résolu en une adresse IP de destination. Avant la v5.1, {product-name} ne pouvait pas détecter et appliquer des règles pour les connexions à un ServiceEntry, donc toutes les connexions étaient classées comme externes. Avec la 5.1, des règles peuvent être appliquées pour des destinations spécifiques de ServiceEntry. IMPORTANT : Si vous mettez à niveau vers la v5.1 avec un déploiement basé sur Istio, de nouvelles règles doivent être créées pour permettre ces connexions et éviter les alertes de violation. Après la mise à niveau, des violations implicites seront signalées pour le trafic nouvellement visible si les règles d'autorisation n'existent pas. De nouvelles règles de trafic peuvent être apprises et auto-créées en mode Découverte. Pour permettre ce trafic, vous pouvez mettre le groupe en mode découverte ou créer un groupe personnalisé avec des adresses (ou un nom DNS) et une nouvelle règle de réseau vers cette destination pour permettre le trafic. NOTE : Il y a un bug dans la 5.1.0 dans la destination signalée par les violations de refus qui ne représentent pas la destination correcte.  Le bug signale que server_name et client_name sont identiques.  Ce problème sera résolu dans une prochaine version de correctif.
...
* L'outil d'examen et d'amélioration des risques entraînera un message d'erreur "Échec de la mise à jour de la configuration système : Demande au format incorrect".
...
{product-name} n'a besoin d'aucun paramètre supplémentaire pour les clusters avec SELinux activé pour déployer et fonctionner. Testé le déploiement de {product-name} sur un cluster RKE2 durci basé sur RHEL 8.5 avec SELinux activé. {product-name} déployé avec succès si PSP est activé et déploiement du gestionnaire de patch et du scanner. La prochaine version du graphique devrait corriger le problème ci-dessous.
...
==== Test_D'autres Mises à jour

* Mettre à jour la version de l'application appVersion à 5.0.0 et la version du chart à 2.2.0
...
* CRD pour WAF, DLP et contrôles d'admission. REMARQUE : des liaisons de rôle de cluster/permissions supplémentaires sont requises. Voir les sections de déploiement Kubernetes et OpenShift. Import/export de CRD et versionnage pour les contrôles d'admission pris en charge via CRD.
* Intégration SSO Rancher pour lancer {product-name} console via Rancher Manager. Cette fonctionnalité n'est disponible que si les conteneurs {product-name} sont déployés via Rancher. NOTE : Nécessite une version mise à jour de Rancher (date/version à déterminer).
* Prend en charge le déploiement sur RKE2.
