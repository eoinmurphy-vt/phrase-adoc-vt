= Test_TwoDéployer en utilisant des opérateurs
:revdate: 2025-06-02
...
Les opérateurs prennent les connaissances opérationnelles humaines et les encodent dans un logiciel qui est plus facilement partagé avec les consommateurs. Les opérateurs sont des morceaux de logiciel qui facilitent la complexité opérationnelle de l'exécution d'un autre morceau de logiciel. Plus techniquement, les opérateurs sont une méthode d'emballage, de déploiement et de gestion d'une application Kubernetes.

== Test_Two\{nom-du-produit} Opérateurs

L'opérateur \{nom-du-produit} est basé sur le graphique Helm \{nom-du-produit}. L'opérateur \{nom-du-produit} RedHat OpenShift fonctionne dans la plateforme de conteneurs OpenShift pour déployer et gérer les composants du cluster de sécurité \{nom-du-produit}. L'opérateur \{nom-du-produit} contient toutes les informations nécessaires pour déployer \{nom-du-produit} en utilisant des graphiques Helm. Vous devez simplement installer l'opérateur \{nom-du-produit} depuis le hub d'opérateurs intégré d'OpenShift et créer l'instance \{nom-du-produit}.

Pour déployer les dernières versions de conteneurs \{nom-du-produit}, utilisez soit le link:https://catalog.redhat.com/search?searchType=software&deployed_as=Operator&partnerName={product-name}&p=1[Opérateur Certifié Red Hat] du Hub d'Opérateurs ou le link:https://github.com/redhat-openshift-ecosystem/community-operators-prod/tree/main/operators/neuvector-community-operator[opérateur communautaire]. La documentation pour l'opérateur communautaire peut être trouvée https://github.com/neuvector/neuvector-operator[\{nom-du-produit} opérateur].

[NOTE]
.Note sur SCC et mise à niveau
====

Le SCC privilégié est ajouté au compte de service spécifié dans le yaml de déploiement par la version de l'opérateur 1.3.4 et supérieure dans les nouveaux déploiements. Dans le cas de la mise à niveau de l'opérateur \{nom-du-produit} d'une version précédente à 1.3.4, veuillez supprimer le SCC privilégié avant la mise à niveau.
...
[IMPORTANT]
====
Les versions d'opérateur certifié \{nom-du-produit} sont liées aux versions de produit \{nom-du-produit}, et chaque nouvelle version doit passer par un processus de certification avec Red Hat avant d'être publiée. La version d'opérateur certifié pour 5.3.x est liée à la version helm 2.7.2 et à la version de l'application \{nom-du-produit} 5.3.2. La version d'opérateur certifié 1.3.9 est liée à la version \{nom-du-produit} 5.2.0. La version d'opérateur certifié 1.3.7 est liée à la version \{nom-du-produit} 5.1.0. La version 1.3.4 de l'opérateur est liée à \{product-name} 5.0.0. Si vous souhaitez pouvoir changer les balises de version des conteneurs \{product-name} déployés, veuillez utiliser la version Community.
====
...
[IMPORTANT]
====
Les versions de l'opérateur \{product-name} sont liées aux versions du produit \{product-name}, et chaque nouvelle version de produit doit passer par un processus de certification avec Red Hat avant d'être publiée.
====

*Notes techniques*

* Les images de conteneur \{product-name} sont tirées de registry.connect.redhat.com en utilisant le secret de tirage d'image du marché RedHat.
* L'interface utilisateur du gestionnaire \{product-name} est généralement exposée via un itinéraire de passage OpenShift sur un domaine. Par exemple, sur IBM Cloud neuvector-route-webui-neuvector.(nom_du_cluster)-(hash_aleatoire)-0000.(region).containers.appdomain.cloud. Elle peut également être exposée en tant que service neuvector-service-webui via une adresse de port de nœud ou une IP publique.
...
* Acceptez le contrat de licence utilisateur final de \{product-name}. 
...
Mettez à niveau la version de \{product-name} en mettant à jour la version de l'Opérateur associée à la version souhaitée de \{product-name}.
======
...
* Les images de conteneur \{product-name} sont tirées de Docker Hub depuis le compte \{product-name}. 
* L'interface utilisateur du gestionnaire \{product-name} est généralement exposée via un itinéraire de passage OpenShift sur un domaine. Par exemple, sur IBM Cloud neuvector-route-webui-neuvector.(nom_du_cluster)-(hash_aleatoire)-0000.(region).containers.appdomain.cloud. Elle peut également être exposée en tant que service neuvector-service-webui via une adresse de port de nœud ou une IP publique. 
* OpenShift version 4.6+ 
* Il est recommandé de revoir et de modifier la configuration d'installation de \{product-name} en modifiant les valeurs yaml avant de créer l'instance \{product-name}. Les exemples incluent le nom imagePullSecrets, la version de balise, l'accès ingress/console, la fédération multi-cluster, le PVC de volume persistant, etc. Veuillez vous référer aux instructions Helm à https://github.com/neuvector/neuvector-helm pour les valeurs qui peuvent être modifiées lors de l'installation.
...
----
--
. Préparez les valeurs de configuration YAML pour l'installation de \{product-name} en commençant par le snippet YAML suivant. Assurez-vous de spécifier la version souhaitée de \{product-name} dans la valeur 'tag'. Vérifiez la référence des valeurs dans le chart Helm de \{product-name} pour obtenir les options de configuration disponibles. Il existe d'autres valeurs Helm possibles qui peuvent être configurées dans le YAML, telles que la configuration du cluster pour permettre la gestion multi-cluster en exposant les services Master (Master fédéré) ou distant (Travailleur fédéré).
+
--
[,yaml]
----
...
** Connectez-vous avec le nom d'utilisateur initial admin et le mot de passe admin. 
** Acceptez l'accord de licence utilisateur final de \{product-name}. 
** Changez le mot de passe de l'utilisateur admin. 
** Optionnellement, vous pouvez également créer des utilisateurs supplémentaires dans le menu Paramètres -> Utilisateurs et rôles.

Maintenant, vous êtes prêt à naviguer dans la console \{product-name} pour commencer l'analyse des vulnérabilités, observer les pods d'application en cours d'exécution et appliquer des protections de sécurité aux conteneurs.

*Mettre à niveau \{product-name}*

. Depuis Opérateurs > Opérateurs installés > Opérateur \{product-name} 
+
...
* Examinez et vérifiez les valeurs du chart Helm de \{product-name}
* Assurez-vous que le chemin du registre et la balise de version sont correctement définis (opérateur communautaire ; certifié utilisera les valeurs par défaut)
