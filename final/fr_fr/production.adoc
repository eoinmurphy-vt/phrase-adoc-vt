= Tester le déploiement de \{product-name}
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /02.deploying/01.production/01.production.md
:page-opendocs-slug: /déploiement/production

== Tester la planification des déploiements

Les conteneurs \{product-name} dans un déploiement par défaut incluent le contrôleur, le gestionnaire, l'exécuteur, le scanner et le metteur à jour. L'emplacement où ces conteneurs (sur quels nœuds) sont déployés doit être pris en compte, et des étiquettes, des taints ou des tolérances appropriés doivent être créés pour les contrôler.
...
Le contrôleur gère le cluster d'exécuteurs et peut être déployé sur le même nœud qu'un exécuteur ou sur un nœud de gestion séparé. Le gestionnaire doit être déployé sur le nœud où le contrôleur s'exécute et fournira un accès console au contrôleur. D'autres conteneurs \{product-name} requis, tels que le gestionnaire, le scanner et le metteur à jour, sont décrits plus en détail dans le guide des meilleures pratiques référencé ci-dessous.

Si vous ne l'avez pas encore fait, tirez les images du Docker Hub de \{product-name}.

Les images sont sur le registre Docker Hub de \{product-name}. Utilisez le tag de version approprié pour le gestionnaire, le contrôleur, l'exécuteur, et laissez la version comme 'latest' pour le scanner et le metteur à jour. Par exemple :
...
Si vous déployez avec le chart Helm actuel de \{product-name} (v1.8.9+), les modifications suivantes doivent être apportées à values.yml :

* Mettre à jour le registre à docker.io
* Mettre à jour les noms/étiquettes d'image à la version actuelle sur Docker Hub, comme indiqué ci-dessus
* Laissez les imagePullSecrets vides

=== Tester les meilleures pratiques, conseils, questions-réponses pour le déploiement et la gestion de \{product-name}
...
== Test de déploiement utilisant Helm ou des opérateurs
...
== Test de déploiement utilisant ConfigMap

Le déploiement automatisé sur Kubernetes est pris en charge en utilisant un ConfigMap. Veuillez consulter la section xref:configmap.adoc[Déploiement utilisant ConfigMap] pour plus de détails.

== Test du déploiement des contrôleurs

Nous recommandons que plusieurs contrôleurs soient exécutés pour une configuration de haute disponibilité (HA). Les contrôleurs utilisent le protocole RAFT basé sur le consensus pour élire un leader et si le leader tombe, pour élire un autre leader. En raison de cela, le nombre de contrôleurs actifs doit être impair, par exemple 3, 5, 7, etc.

== Test de la haute disponibilité des contrôleurs
...
== Test des sauvegardes et des données persistantes
...
Une fois le PersistentVolume créé, un PersistentVolumeClaim doit être créé comme ci-dessous pour le contrôleur. Actuellement, le volume persistant est utilisé uniquement pour les fichiers de sauvegarde de configuration de \{product-name} dans le contrôleur (politiques, règles, données utilisateur, intégrations, etc.) et les résultats de scan de registre.
...
Après la création du Persistent Volume Claim, modifiez le fichier yaml d'exemple de \{product-name} comme indiqué ci-dessous (ancienne section commentée) :
...
=== Tester les ConfigMaps et le stockage persistant
...
== Tester la mise à jour de la base de données des vulnérabilités CVE en production
...
== Tester le déploiement sans mode privilégié
...
== Tester l'architecture multi-sites et multi-clusters
