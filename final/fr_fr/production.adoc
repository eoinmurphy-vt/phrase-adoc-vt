...
Le contrôleur gère le cluster d'exécuteurs et peut être déployé sur le même nœud qu'un exécuteur ou sur un nœud de gestion séparé. Le gestionnaire doit être déployé sur le nœud où le contrôleur fonctionne et fournira un accès console au contrôleur. D'autres conteneurs \{product-name} requis tels que le gestionnaire, le scanner et le metteur à jour sont décrits plus en détail dans le guide des Meilleures Pratiques référencé ci-dessous.
...
=== Test_TwoMeilleures Pratiques, Conseils, Q&R pour Déployer et Gérer \{product-name}
...
== Test_TwoDéploiement utilisant Helm ou des opérateurs
...
== Test_TwoDéploiement utilisant ConfigMap
...
== Test_TwoDéploiement des contrôleurs

Nous recommandons que plusieurs contrôleurs soient exécutés pour une configuration de haute disponibilité (HA). Les contrôleurs utilisent le protocole RAFT basé sur le consensus pour élire un leader et si le leader tombe, élire un autre leader. En raison de cela, le nombre de contrôleurs actifs doit être un nombre impair, par exemple 3, 5, 7, etc.
...
== Test_TwoSauvegardes et données persistantes
...
=== Test_DeuxExemple de Volume Persistant
...
=== Test_DeuxConfigMaps et Stockage Persistant

Les ConfigMaps et la sauvegarde de stockage persistant ne sont lus que lorsqu'un nouveau cluster \{product-name} est déployé, ou lorsque le cluster échoue et redémarre. Ils ne sont pas utilisés lors des mises à niveau progressives.
...
== Test_TwoMise à jour de la base de données des vulnérabilités CVE en production
...
== Test_TwoGestion des mises à jour d'hôte ou des nœuds d'auto-scaling avec un budget de perturbation de pod.
...
== Test_TwoDeploy sans mode privilégié
...
== Test_TwoMulti-site, architecture multi-cluster

Pour les entreprises ayant plusieurs emplacements et où un cluster \{product-name} séparé peut être déployé pour chaque emplacement, l'architecture de référence proposée est la suivante. Chaque cluster a son propre ensemble de contrôleurs et est géré séparément.
