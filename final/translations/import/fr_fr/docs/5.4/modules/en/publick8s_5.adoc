...
La plupart des services K8s activent automatiquement/tiennent compte de toute communication inter-pod / inter-cluster entre les nœuds, ce qui permet également aux conteneurs {product-name} (enforceurs, contrôleurs, gestionnaire) de communiquer au sein du cluster.
...
Le support GKE Auto Pilot est disponible avec NeuVector v5.4.3 et ultérieur. Veuillez suivre les étapes ci-dessous pour déployer NeuVector sur le cluster Auto Pilot.
