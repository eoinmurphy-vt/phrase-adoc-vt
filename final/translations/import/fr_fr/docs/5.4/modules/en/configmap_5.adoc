...
Le paramètre 'always_reload: true' peut être ajouté dans n'importe quel yaml de ConfigMap pour forcer le rechargement de ce yaml chaque fois que le contrôleur démarre (version 4.3.2+). Sinon, le ConfigMap ne sera chargé qu'au démarrage initial ou après un redémarrage complet du cluster (voir la section sur le stockage persistant ci-dessous).

=== Exemple complet de {product-name} ConfigMap (initcfg.yaml)
...
Notez que le secret est référencé dans les fichiers yaml de déploiement standard de Kubernetes et OpenShift xref:kubernetes.adoc#_deploy_suse_security[fichiers yaml de déploiement] sous Volumes.

== ConfigMaps et stockage persistant

Les ConfigMaps et la sauvegarde de xref:production.adoc#_backups_and_persistent_data[stockage persistant] ne sont lus que lorsqu'un nouveau {product-name} cluster est déployé, ou lorsque le cluster échoue et redémarre. Ils ne sont pas utilisés lors des mises à niveau progressives.
