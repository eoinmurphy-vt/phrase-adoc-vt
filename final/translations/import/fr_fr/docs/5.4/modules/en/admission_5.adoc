...
== Contrôle des images / déploiements de conteneurs
...
Avant que {product-name} n'effectue la correspondance avec les règles de contrôle d'admission, {product-name} récupère les informations sur l'image (par exemple, 10.1.127.3:5000/neuvector/toolbox/iperf:latest) à partir de l'apiserver du cluster
(Veuillez vous référer à la section Demande de l'apiserver ci-dessous). L'image est composée par le serveur de registre (`https://10.1.127.3:5000`), le dépôt (neuvector/toolbox/iperf) et le tag (latest).

{product-name} utilise ces informations pour faire correspondre les résultats dans {product-name} Assets -> page de scan du registre et collecte les informations correspondantes telles que le nom cve, le nombre cve élevé ou moyen, etc. Les violations de conformité des images sont considérées comme toute image contenant des secrets ou des violations setuid/setgid.
...
* Variables d'environnement avec des secrets. Si le yaml de déploiement ou le résultat du scan d'image contient (ou ne contient pas) des variables d'environnement avec des secrets. Voir les critères pour les secrets correspondants ci-dessous. 
...
* Violations de conformité d'image. Correspond à si l'image (registre) les résultats de l'analyse entraînent des violations de conformité. Voir xref:compliance.adoc#_managing_compliance_and_cis_benchmarks[conformité] pour des détails sur les vérifications de conformité. 
* Image sans informations sur le système d'exploitation. Correspond à si l'image (registre) les résultats de l'analyse entraînent l'incapacité de récupérer des informations sur le système d'exploitation. 
* Registre d'images. Correspond sur des noms de registre d'images spécifiques. Utilisé pour restreindre généralement les déploiements à partir de certains registres ou exiger des déploiements uniquement à partir de certains registres approuvés. Souvent utilisé avec d'autres critères tels que les espaces de noms. 
...
* Modules. Exige ou exclut certains modules (packages, bibliothèques) d'être présents dans l'image à la suite du scan d'image (registre). 
...
* Partager le réseau de l'hôte. Autoriser ou interdire les déploiements de partager le réseau de l'hôte. 
* {blank}
...
. Règles d'autorisation CRD appliquées (si elles existent)
. Règles de refus CRD appliquées (si elles existent)
...
Le journal ci-dessus indique que le kube-apiserver du cluster est incapable d'envoyer la demande au webhook {product-name} avec succès car il échoue à résoudre le nom neuvector-svc-admission-webhook.neuvector.svc.
...
----
--
. Après le test, si la valeur de l'étiquette tag-neuvector-svc-admission-webhook ne change pas, cela signifie que le service du contrôleur n'a pas réussi à mettre à jour le service neuvector-svc-admission-webhook. Vérifiez si neuvector-binding-app clusterrole/clusterrolebinding sont configurés correctement.
