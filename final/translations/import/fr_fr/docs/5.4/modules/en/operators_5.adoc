...
[%collapsible]
======
*Déployer en utilisant l'Opérateur Communautaire {product-name} depuis le Hub d'Opérateurs*
...
----
--
. Installez l'Opérateur Communautaire {product-name} depuis le Hub d'Opérateurs 
** Dans l'interface utilisateur de la console OpenShift, naviguez vers OperatorHub 
...
** Configurer la stratégie d'approbation 
** Confirmer l'installation
. Téléchargez le manifeste secret Kubernetes qui contient les identifiants pour accéder au registre de conteneurs {product-name}. Enregistrez le fichier manifeste YAML sous ./neuvector-secret-registry.yaml.
. Appliquez le manifeste secret Kubernetes contenant les identifiants du registre.
+
--
[,shell]
----
