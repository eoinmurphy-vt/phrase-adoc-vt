...
:page-opendocs-slug:  /politique/règlesdereponse
...
=== Utiliser plusieurs critères dans une seule règle
...
=== Créer une règle de réponse pour les journaux d'événements de sécurité
...
=== Événements d'exemple qui peuvent être choisis pour une règle de réponse
...
* Cliquez sur "insérer en haut" pour insérer la règle en haut
* Choisissez un nom de groupe de services si la règle doit être appliquée à un groupe de services particulier
* Choisissez la catégorie CVE-Report
* Ajoutez le niveau de journal comme critère de correspondance ou type de cve-report
...
=== Types de rapports CVE d'exemple qui peuvent être choisis pour la règle de réponse de la catégorie CVE-Report

image:resp7.png[rapportcve]
...
* Sélectionnez les actions à appliquer : Quarantaine, Webhook et suppression de journal (la quarantaine n'est pas applicable au benchmark Host Docker et Kubernetes)
* Activer le statut

image:resp8-b.png[cis]

=== Déquarantaine un conteneur en supprimant la règle de réponse
...
=== Affichage de l'identifiant de la règle responsable de la mise en quarantaine du conteneur (dans Notifications -> Événements)

image:resp9.png[unquarantine]

=== Popup d'option de déquarantaine lorsque la règle de réponse appropriée est supprimée
