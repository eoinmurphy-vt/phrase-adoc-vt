...
Überprüfen Sie nach Ausführen des Befehls `kubectl apply -f <YAML file>`, ob `AllowlistSynchronizer` bereit ist.
...
Wenn Sie die YAML-Bereitstellung verwenden, fügen Sie bitte die `podLabels` und Ressourcengrenzen in den YAML-Konfigurationen `enforcer` und `scanner` entsprechend hinzu.

Um mehr über `allowlistSynchronizer` zu erfahren, sehen Sie sich bitte die https://cloud.google.com/kubernetes-engine/docs/how-to/run-autopilot-partner-workloads[GKE-Dokumentation] an.
