...
Experimentieren Sie mit unterschiedlichen Zahlen von Scannern auf Registrierungen mit einer großen Anzahl von Bildern, um das Verhalten der Scanabschlusszeit in Ihrer Umgebung zu beobachten. 2-5 Scanner als die Replikateinstellung sollte in den meisten Fällen ausreichend sein.
...
* CLUSTER_JOIN_ADDR (optional), CLUSTER_JOIN_PORT (optional) - um Ergebnisse an den Controller zu senden, die in Zulassungssteuerungsregeln verwendet werden (Kubernetes-bereitgestellter Controller).
