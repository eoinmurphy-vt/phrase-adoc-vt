...
+
____
(Optional) Geben Sie eine Nummer ein. Netzwerkregeln sind zunächst von niedrigster bis höchster Reihenfolge angeordnet, aber die Reihenfolge der Regeln kann durch Ziehen und Ablegen in der Liste geändert werden.
____
...
. Erstellen Sie eine Gruppe. In den Richtlinien -> Gruppen klicken Sie, um eine neue Gruppe hinzuzufügen. Benennen Sie die Gruppe (z.B. intern) und geben Sie dann die Kriterien für die Gruppe an. Geben Sie beispielsweise den DNS-Namen, die IP-Adresse oder den Adressbereich der internen Dienste an. Speichern Sie die neue Gruppe.
. Erstellen Sie eine Regel. In den Richtlinien -> Regeln klicken Sie, um eine neue Regel hinzuzufügen. Wählen Sie die Gruppe aus, die den Container repräsentiert, von dem die Verbindungen ausgehen, und dann die Zielgruppe (z.B. intern). Sie können die Regel weiter verfeinern mit spezifischen Protokollen oder Ports oder sie leer lassen. Stellen Sie sicher, dass der Selektor auf Erlauben (grün) eingestellt ist.
...
=== Spezielle Durchsetzung für Istio ServiceEntry-Ziele
...
* TCP-geteilte Handshake-Angriff
* TCP Kleine MSS-Angriff
