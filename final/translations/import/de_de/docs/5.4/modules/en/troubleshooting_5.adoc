...
* Verzeichnisintegration. {product-name} unterstützt spezifische Konfigurationen für LDAP/AD und andere Integrationen für Gruppen und Rollen. Kontaktieren Sie {product-name} für zusätzliche Schritte zur Fehlerbehebung und ein Tool zur AD-Fehlerbehebung.
...
Um die Protokolle eines {product-name} Containers, z. B. eines Controller-Pods, anzuzeigen
...
Führen Sie die Aktivität in {product-name} aus, die Sie debuggen möchten. Überprüfen Sie dann die Controller-Protokolle (in einem separaten Terminalfenster).
