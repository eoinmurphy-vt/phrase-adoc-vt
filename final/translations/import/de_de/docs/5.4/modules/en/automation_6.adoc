...
=== Triggern Sie die Schwachstellenscans von einem Skript aus

{product-name} kann automatisch ausgelöst werden, um ein Bild auf Schwachstellen zu scannen. Dies kann durch Konfigurieren eines zu überwachenden Registrierungs-/Repositorys, unter Verwendung des {product-name} Jenkins-Plugins oder über die REST-API erfolgen. Bitte sehen Sie sich den Abschnitt über Scannen & Compliance für weitere Details an.
...
=== Exportieren/Importieren der Konfigurationsdatei
...
Deaktivieren Sie den Debug-Modus für alle Controller in einem Cluster
