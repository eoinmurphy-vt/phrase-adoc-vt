...
--
. Créez un nouveau service pour la tâche de l'Enforcer. Encore une fois, définissez le placement de la tâche sur "\`One Task Per Host\`" afin qu'un seul Enforcer soit déployé sur chaque hôte. Notez également que la contrainte supplémentaire doit montrer qu'elle empêche le déploiement sur un nœud allinone.
+
--
