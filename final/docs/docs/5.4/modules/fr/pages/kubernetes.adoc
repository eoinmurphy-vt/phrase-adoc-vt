...
[NOTE]
====
PKS est testé sur le terrain et nécessite d'activer les conteneurs privilégiés pour le plan/tile, et de modifier le hostPath yaml comme suit pour Allinone, Controller, Enforcer :
...
Pour contrôler sur quels nœuds le Manager et le Controller sont déployés, étiquetez chaque nœud. Remplacez nodename par le nom de nœud approprié ('\`kubectl get nodes\`'). Remarque : Par défaut, Kubernetes ne planifiera pas de pods sur le nœud maître.
...
Pour empêcher l'enforcer d'être déployé sur un nœud de contrôleur, s'il s'agit d'un nœud de gestion dédié (sans conteneurs d'application à surveiller), ajoutez une nodeAffinity à la section yaml de l'Enforcer. Par exemple :
...
[NOTE]
====
PKS is field-tested and requires enabling privileged containers for the plan/tile, and changing the hostPath yaml as follows for Allinone, Enforcer:
