...
Wenn Sie mit dem aktuellen Helm-Diagramm {product-name} (v1.8.9+) bereitstellen, müssen die folgenden Änderungen an values.yml vorgenommen werden:
...
[NOTE]
====
Wenn Sie aus dem Rancher Manager 2.6.5+ {product-name} Diagramm bereitstellen, werden die Bilder automatisch aus dem Mirror-Image-Repository des Rancher-Registrys abgerufen und im Namespace cattle-neuvector-system bereitgestellt.
====
...
Das ist alles! Sie sollten in der Lage sein, sich mit admin:admin, z.B. `https://<public-ip>:8443`, bei der Konsole {product-name} anzumelden.
--

[NOTE]
====
Der im neuvector.yaml-Datei angegebene NodePort-Dienst öffnet einen zufälligen Port auf allen Kubernetes-Knoten für den Port der Webmanagementkonsole {product-name}. Alternativ können Sie einen LoadBalancer oder Ingress verwenden, indem Sie eine öffentliche IP und den Standardport 8443 nutzen. Für NodePort stellen Sie sicher, dass der Zugriff über die Firewalls für diesen Port, falls erforderlich, geöffnet ist. Wenn Sie sehen möchten, welcher Port auf den Host-Knoten geöffnet ist, führen Sie die folgenden Befehle aus:
...
Wenn es zusätzliche Taints wie oben gibt, fügen Sie diese zur Toleranzen-YAML-Sektion des Beispiels hinzu:
...
Für den Controller als Deployment (auch für den Manager zu tun)
