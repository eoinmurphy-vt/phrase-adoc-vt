...
* GKE Autopilot.
* AWS ECS n'est plus supporté. (REMARQUE : Aucune fonctionnalité n'a été activement supprimée pour l'exploitation de {product-name} sur les déploiements ECS. Cependant, les tests sur ECS ne sont plus effectués par SUSE. Bien que la protection des charges de travail ECS avec {product-name} fonctionnera probablement comme prévu, les problèmes ne seront pas examinés.)
* Docker sur Mac
* Docker sur Windows
