...
[IMPORTANT]
====
{product-name} alertes, et ne bloque pas les modifications des fichiers/répertoires prédéfinis ou dans des conteneurs système tels que ceux de Kubernetes. Le blocage n'est une option que pour les fichiers/répertoires personnalisés configurés par l'utilisateur pour des conteneurs non-système. Ceci est pour que les mises à jour régulières du dossier système ou des configurations sensibles ne soient pas bloquées involontairement, entraînant un comportement erratique du système.
====
...
* /bin, /usr/bin, /usr/sbin, /usr/local/bin - conteneur, agent
* Fichiers avec attribut setuid et setgid - conteneur, hôte, agent
* Bibliothèques : libc, pthread, ld-linux.* - conteneur, hôte, agent
* Installation de paquets : dpkg, rpm, apk - conteneur, hôte, agent
* /etc/hosts, /etc/passwd, /etc/resolv.conf - conteneur, hôte, agent
