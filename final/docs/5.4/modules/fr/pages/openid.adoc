...
Localisez l'URI de redirection OpenID Connect en haut de la page des paramètres OpenID Connect {product-name}. Vous devrez copier cet URI dans les *URI de redirection de connexion* pour Okta et les *URL de réponse* pour Microsoft Azure.
...
Par défaut, {product-name} recherche des "groupes" dans la revendication pour identifier l'appartenance du groupe de l'utilisateur. Si un autre nom de revendication est utilisé, vous pouvez personnaliser le nom de la revendication dans la page des paramètres OpenID Connect de {product-name}.
...
Allez dans votre {product-name} console et naviguez vers "`Settings`" -> "`OpenId Connect Settings`".  En haut de la page, à côté de l'étiquette "`OpenID Connect Redirect URI`", cliquez sur "`Copy to Clipboard`".
