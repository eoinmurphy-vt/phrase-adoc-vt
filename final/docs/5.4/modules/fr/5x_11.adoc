...
* **NVSHAS-9572**: [Vulnérabilités] Les données filtrées ont été conservées peu importe le rafraîchissement ou la reconnexion de l'utilisateur sur la page.
...
* **NVSHAS-9301**: NV déployé sur Rancher Prime ne peut pas indiquer son type Rancher.
...
* En mode Découverte, apprendre les egress vers des groupes d'adresses FQDN externes automatiquement. Un nouveau groupe personnalisé FQDN externe sera créé à moins que la connexion externe ne corresponde à une règle existante.
...
* Les évaluations de configuration par rapport au contrôle d'admission doivent montrer toutes les violations en une seule analyse.
* Ajouter plus d'options pour les critères de rapport CVE dans les règles de réponse. Exemple 1 - "cve-high-with-fix:X" signifie : Lorsque le nombre de (vulnérabilités élevées qui ont été corrigées) >= X, déclencher la règle de réponse. Exemple 2 - "cve-high-with-fix:X/Y" signifie : Lorsque le nombre de (vulnérabilités élevées qui ont été signalées il y a Y jours et ont été corrigées) >= X, déclencher la règle de réponse.
...
* Rapporter les résultats d'analyse superposés et les données CVE supplémentaires dans les messages SYSLOG. Cela est activé par une case à cocher dans les paramètres -> Configuration -> SYSLOG.
