...
{product-name} crée automatiquement un groupe appelé 'nœuds' qui représente chaque nœud (hôte) dans le cluster. {product-name} fournit une surveillance de base automatisée des hôtes pour des processus suspects (tels que des analyses de ports, des shells inversés, etc.) et des escalades de privilèges. De plus, {product-name} apprendra le comportement des processus de chaque nœud pendant qu'il est en mode Découverte pour mettre sur liste blanche ces processus, de manière similaire à ce qui est fait avec les processus de conteneurs.  La liste des règles de processus 'local' (appris) est une combinaison de tous les processus de tous les nœuds dans le cluster pendant le mode Découverte.

Les nœuds peuvent ensuite être mis en mode Surveillance ou Protection, où {product-name} alertera si un processus démarre pendant le mode Surveillance, et bloquera ce processus en mode Protection.

image:host_protection.png[nodeGroup]

Pour activer la protection de l'hôte avec des règles de profil de processus, sélectionnez le groupe 'nœuds' et examinez les processus appris sur le nœud. Personnalisez si nécessaire en ajoutant, supprimant ou modifiant des règles de processus. Puis passez en mode Surveillance ou Protection.
...
+
____
Sélectionnez des conteneurs par leurs services. Si un conteneur est déployé par Docker Compose, sa valeur de tag de service sera "project_name:service_name" ; si un conteneur est déployé par le service en mode Docker swarm, sa valeur de tag de service sera le nom du service swarm.
____
...
+
____
Créez un groupe par nom DNS ou plages d'adresses IP. Exemples : address=www.google.com, address=10.1.0.1, address=10.1.0.0/24, address=10.1.0.1-10.1.0.25. Le nom DNS peut être n'importe quel nom résolvable. Les critères d'adresse n'acceptent pas l'opérateur !=. Voir ci-dessous pour les groupes d'adresses d'hôtes virtuels spéciaux 'vh'.
____

Un groupe peut être créé avec des types de critères mixtes, sauf le type 'adresse', qui ne peut pas être utilisé avec d'autres critères. Les critères mixtes imposent une opération '`AND`' entre les critères, par exemple label service_type=data ET image=mysql. Plusieurs entrées pour un ou plusieurs critères sont traitées comme OR, par exemple address=google.com OU address=yahoo.com. Remarque : Pour aider à analyser les connexions d'entrée/sortie, une liste d'IPs d'entrée et de sortie peut être téléchargée depuis le tableau de bord -> section Détails d'entrée/sortie sous forme de rapport d'exposition.

La correspondance partielle est prise en charge pour les critères d'image, de nœud, de conteneur, de service et d'étiquette. Par exemple, image@redis, sélectionne des conteneurs dont le nom d'image contient le sous-chaîne 'redis' ; image{caret}redis, sélectionne des conteneurs dont le nom d'image commence par 'redis'.

Il n'est pas recommandé d'utiliser des critères d'adresse pour correspondre aux IP internes ou aux sous-réseaux, en particulier ceux protégés par des enforceurs, il est plutôt recommandé d'utiliser leurs métadonnées, telles que l'image, le service ou les étiquettes. Les cas d'utilisation typiques pour le groupe d'adresses sont de définir des politiques entre des conteneurs gérés et des sous-réseaux IP externes, par exemple, des services fonctionnant sur Internet ou un autre centre de données. Le groupe d'adresses n'a pas de membres de groupe.

Des jokers '_' peuvent être utilisés dans les critères, par exemple 'address=_.google.com'. Pour une correspondance plus flexible, utilisez le tilde '~' pour indiquer qu'une correspondance regex est souhaitée. Par exemple, pour correspondre aux étiquettes 'policy~public.*-ext1' pour l'étiquette policy.
...
==== Politique de réseau basée sur l'hôte virtuel ('vh')
...
* Pour sélectionner tous les conteneurs dans l'espace de noms 'default' (espace de noms pris en charge depuis v2.2)
+
____
namespace=default
____
...
* dropbox.com et ses sous-domaines (appuyez sur entrer après chaque entrée)
+
____
