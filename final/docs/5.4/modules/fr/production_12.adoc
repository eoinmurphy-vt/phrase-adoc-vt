...
Les conteneurs {product-name} dans un déploiement par défaut incluent le contrôleur, le gestionnaire, l'exécuteur, le scanner et le mise à jour. L'emplacement où ces conteneurs (sur quels nœuds) sont déployés doit être pris en compte, et des étiquettes, des taints ou des tolérances appropriés doivent être créés pour les contrôler.
...
* Mettez à jour le registre à docker.io
* Mettez à jour les noms/étiquettes d'image à la version actuelle sur Docker Hub, comme indiqué ci-dessus
