= Bewertung und Testen {product-name}
:revdate: 2025-05-08
...
Nachdem Sie die {product-name} Komponenten bereitgestellt haben, können Sie sie mit den bereitgestellten Beispiel-Testanwendungen bewerten. Diese befinden sich im Repository `'nvbeta` auf Docker Hub.
...
Um eine Verletzung von einem Nodejs-Pod zu erzeugen, finden Sie einen Pod:
...
==== Eine Bedrohung/Angriff generieren
...
Die Kubernetes Guestbook-Demoanwendung kann ebenfalls auf Kubernetes bereitgestellt werden. Es wird empfohlen, es in seinen eigenen Namespace zu implementieren, damit Sie die namespacebasierte Filterung in der {product-name} Konsole sehen können.
...
Wenn der obige Link nicht funktioniert, können Sie ihn http://neuvector.com/sample-applications-test-plan/[von unserer Website herunterladen] mit dem Passwort nv1851blvd.
