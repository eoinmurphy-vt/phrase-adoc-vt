...
== Fehlerbehebung {product-name} Bereitstellungen
...
* Stellen Sie sicher, dass die erforderlichen Änderungen an der YAML (z. B. NodePort oder LoadBalancer) oder Helm-Werteinstellungen entsprechend festgelegt sind.
...
* Verzeichnisintegration. {product-name} unterstützt spezifische Konfigurationen für LDAP/AD und andere Integrationen für Gruppen und Rollen. Kontaktieren Sie {product-name} für zusätzliche Schritte zur Fehlerbehebung und ein Tool zur Fehlerbehebung für AD.
...
* Verwenden Sie rollierende Updates für den Controller. Wenn Sie Hosts neu starten, stellen Sie sicher, dass Sie die Controller überwachen, während sie zu anderen Hosts wechseln oder auf den neu gestarteten Hosts neu bereitgestellt werden, um sicherzustellen, dass sie starten, dem Controller-Cluster beitreten und stabilisieren/synchronisieren können. Das gleichzeitige oder zu schnelle Neustarten aller Hosts kann zu unbekannten Zuständen für die Controller führen.
...
* Beim Aktualisieren auf eine neue Version sollten Sie die Online-Dokumentation überprüfen, um Änderungen/Ergänzungen zur erforderlichen YAML zu identifizieren, sowie andere Änderungen wie Rollenbindungen oder neue Dienste (z. B. Zulassungssteuerungs-Webhooks, persistente Volume-Ansprüche usw.).
...
Das Supportprotokoll enthält zusätzliche Informationen, die für {product-name} Support nützlich sind, einschließlich Systemkonfiguration, Container, Richtlinien, Benachrichtigungen und {product-name} Containerdetails.

Um das Supportprotokoll herunterzuladen, gehen Sie zu Einstellungen -> Konfiguration und wählen Sie Protokoll sammeln aus.

== Verwenden Sie die CLI, um den Debug-Modus zu aktivieren.
...
== Verwenden Sie die REST-API, um den Debug-Modus zu aktivieren.
