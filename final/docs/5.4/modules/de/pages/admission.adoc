...
Die wichtigsten Informationen in der ValidatingWebhookConfiguration-Ressource für {product-name} sind Cluster-Ressourcen. Derzeit wird, sobald eine Cluster-Ressource wie Deployment {product-name} registriert ist, die Anfrage vom Orchestrierungsplattform-ApiServer an einen der {product-name} Controller gesendet, um zu bestimmen, ob sie basierend auf den benutzerdefinierten Regeln in der {product-name} Richtlinie -> Zulassungssteuerungsseite erlaubt oder abgelehnt werden soll.
...
== Zulassungssteuerungsereignisse/Bekanntmachungen
...
(Bitte beachten Sie den Abschnitt Anfrage vom APIServer unten). Das Bild setzt sich aus dem Registrierungsserver (`https://10.1.127.3:5000`), dem Repository (neuvector/toolbox/iperf) und dem Tag (latest) zusammen.
...
* Anzahl der hochgradigen CVEs mit Fix. Dies nimmt die Ergebnisse eines Bild- (Registry-) Scans und vergleicht sie mit hochgradigen (CVSS-Werte von 7 oder höher), UND wenn ein Fix für die CVE verfügbar ist. Wählen Sie dies aus, wenn Sie nur planen, die Bereitstellung von hochgradigen CVEs zu blockieren, wenn ein Fix hätte angewendet werden sollen. Ein zusätzlicher Operator kann hinzugefügt werden, um auf CVEs zu beschränken, die eine bestimmte Anzahl von Tagen zuvor gemeldet wurden, um Zeit für die Behebung neuer CVEs zu geben. 
...
* Umgebungsvariablen. Verwenden Sie dies, um bestimmte Umgebungsvariablen im Bereitstellungs-YAML oder im Bildscan zu verlangen oder auszuschließen. 
* Bild. Übereinstimmung mit bestimmten Bildnamen, typischerweise kombiniert mit anderen Kriterien für die Regel. 
* Verstöße gegen die Bildkonformität. Übereinstimmungen, wenn die Bild- (Registry-)Scans zu Konformitätsverstößen führen. Siehe xref:compliance.adoc#_managing_compliance_and_cis_benchmarks[Compliance] für Details zu den Konformitätsprüfungen. 
* Bild ohne OS-Informationen. Übereinstimmungen, wenn die Bild- (Registry-)Scans zu einer Unfähigkeit führen, OS-Informationen abzurufen. 
...
* Bild signiert. Verlangen, dass ein Bild durch die Integration von Sigstore/Cosign signiert wird. Dieses Kriterium überprüft einfach, ob es einen Verifier im Scanergebnis gibt.
...
* Servicekonto gebundene Hochrisiko-Rolle. Kann auf mehrere Kriterien übereinstimmen, die eine hochriskante Servicekonto-Rolle darstellen könnten, einschließlich der Auflistung von Geheimnissen, der Durchführung von Operationen an Workloads, der Modifikation von RBAC-Ressourcen, der Erstellung von Arbeitslastressourcen und der Erlaubnis, in einen Container zu exec. 
...
Auf der **Risikoberichte** Seite wird, wenn Geheimnisse erkannt werden, das Alarmformat mit allgemeinen Ausgabedaten angezeigt, die als "${variable}=${value}" dargestellt werden. Als Beispiel im Bild unten kann dies mit der Variablen "env1=AIDAJQ..." gesehen werden.
...
* Überwachen: Es gibt eine Alarmnachricht im Ereignisprotokoll, wenn eine Entscheidung abgelehnt wird. In diesem Fall darf der Cluster-Api-Server erfolgreich eine Ressource erstellen. Hinweis: Selbst wenn die Regelaktion Ablehnen ist, wird im Überwachungsmodus nur ein Alarm ausgelöst.
...
In jeder der Übereinstimmungsphasen (1~7) spielt die Reihenfolge der Regeln keine Rolle. Solange die Anfrage einem Regelkriterium entspricht, wird die Aktion (erlauben oder verweigern) ausgeführt und die Anfrage wird erlaubt oder verweigert.
...
== Konfigurieren von Sigstore/Cosign-Überprüfern für die Anforderung von Bildsignierungen
