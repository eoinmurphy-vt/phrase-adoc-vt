...
== Scannen auf Schwachstellen während der Gitlab-Bau-Pipeline

{product-name} kann konfiguriert werden, um nach Schwachstellen zu scannen, die in der Gitlab-Bau-Pipeline ausgelöst werden. Es gibt ein https://gitlab.com/neuvector/gitlab-plugin[Gitlab-Plugin hier], das konfiguriert und verwendet werden kann. Bitte folgen Sie den Anweisungen auf der Gitlab-Website zur Verwendung des Plugins.

Der Scan kann auch die {product-name} REST-API verwenden, indem das bereitgestellte Skript unten konfiguriert wird, um auf den Controller zuzugreifen.
...
=== Scannen während des Gitlab-Baus mit der REST-API
