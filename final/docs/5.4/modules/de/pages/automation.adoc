...
Stellen Sie sicher, dass Sie die IP-Adresse des Controllers im Skript eingeben und den Containerbildnamen auf denjenigen ändern, den Sie scannen möchten. Aktualisieren Sie auch die Felder für Benutzername/Passwort.
...
Für Informationen zur Automatisierung der Föderation über ConfigMap sehen Sie sich die xref:configmap.adoc#_federated_configmap_examples_fedinitcfg_yaml[Beispiele für föderierte ConfigMap] Dokumentation an.
