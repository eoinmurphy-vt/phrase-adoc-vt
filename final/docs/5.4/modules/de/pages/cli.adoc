...
== Verwendung der {product-name} Befehlszeile

Die {product-name} Lösung unterstützt eine begrenzte Anzahl von Funktionen über die CLI. Die CLI wird über den Manager unterstützt, der seinerseits eine RestAPI verwendet, um Befehle an den Controller zu senden. Der Controller verwaltet dann die Durchsetzer entsprechend. Ein vollständiger Satz von Operationen wird über die REST API unterstützt, die direkt vom Controller bereitgestellt werden kann. Sie können auf die {product-name} CLI zugreifen, indem Sie den cli-Befehl für den Manager oder Allinone eingeben, zum Beispiel:
...
Wo '`allinone`' der Containername für den Controller ist. Möglicherweise müssen Sie die Container-ID für den Namen verwenden.

Obwohl die CLI über den Manager verfügbar ist, empfehlen wir, die xref:automation.adoc[REST API] direkt im Controller für Abfragen und Automatisierung zu verwenden.

=== CLI-Befehlsbeispiele

Hier sind einige der häufigsten CLI-Befehle:
...
Verwenden Sie denselben Benutzer/Passwort, den Sie für die Konsole verwenden.
...
Um die Paketaufzeichnung auf einem Pod zu starten, müssen Sie die containerID kennen, die Sie in das Feld ID_OR_NAME eingeben müssen. Sie können dies mit `show container -c <container_name>` tun. Starten Sie dann den Sniffer mit `request sniffer start <container_id>`. Zum Beispiel,
...
[IMPORTANT]
====
Wenn die Dauer nicht festgelegt ist, müssen Sie die Sniffer-ID finden, um den Sniffer zu stoppen.  Um dies zu tun, `show sniffer -c <containerID>`.  Folgen Sie mit `request sniffer stop <sniffer_ID>`.
====
