...
Überprüfen Sie nach Ausführung des Befehls `kubectl apply -f <YAML file>`, ob das `AllowlistSynchronizer` bereit ist.
...
Um mehr über das `allowlistSynchronizer` zu erfahren, sehen Sie sich die https://cloud.google.com/kubernetes-engine/docs/how-to/run-autopilot-partner-workloads[GKE-Dokumentation] an.
