...
. Klicken Sie mit der rechten Maustaste auf "`Relying Party Trusts`" in der AD FS-Verwaltung und wählen Sie "`Add Relying Party Trust...`" aus.
+
...
. Überprüfen Sie "`Enable support for the SAML 2.0 WebSSO protocol`" und geben Sie die SAML-Redirect-URI aus den {product-name} Einstellungen > SAML-Einstellungsseite in das Feld "`Relying party SAML 2.0 SSO service URL`" ein.  Wählen Sie "`Next`" aus, um fortzufahren.
+
image:adfs6.png[adfsSetup]

. Geben Sie dieselbe SAML-Redirect-URI in das Feld "`Relying party trust identifier`" ein und klicken Sie auf "`Add`"; wählen Sie dann "`Next`" aus, um fortzufahren.
+
image:adfs7.png[adfsSetup]

. Zugriffssteuerung anpassen; wählen Sie dann "`Next`" aus, um fortzufahren.
+
...
. Wählen Sie "`Close`" aus, um abzuschließen.
. Wählen Sie die Bearbeitung der Anspruchsausstellungsrichtlinie ... aus.
+
image:adfs10-11.png[adfsSetup]

. Wählen Sie "`Add Rule...`" aus und wählen Sie "`Send LDAP Attributes as Claims`"; wählen Sie dann "`Next`" aus.  Benennen Sie die Regel und wählen Sie Active Directory als Attributspeicher aus. Nur der ausgehende Anspruch des Benutzernamens ist erforderlich, wenn die Standardrolle festgelegt ist; andernfalls sind Gruppen für die Rollenzuordnung erforderlich.  E-Mail ist optional.
+
--
...
--
. Wählen Sie "`Add Rule...`" aus und wählen Sie "`Transform an Incoming Claim`"; wählen Sie dann "`Next`" aus.  Benennen Sie die Regel und setzen Sie das Feld wie im Screenshot unten fest.  Das Format der ausgehenden Namens-ID muss Transient Identifier sein.
+
...
. Standardrolle
* Es wird empfohlen, "`None`" zu verwenden, es sei denn, Sie möchten, dass jeder authentifizierte Benutzer eine Standardrolle hat.
...
=== Gruppen zu Rollen und Namensräumen zuordnen

Bitte sehen Sie sich den Abschnitt xref:users.adoc#_mapping_groups_to_roles_and_namespaces[Benutzer und Rollen] an, um zu erfahren, wie Gruppen vordefinierten und benutzerdefinierten Rollen sowie Namensräumen in {product-name} zugeordnet werden.
...
+
Attributnamen sind groß- und kleinschreibungsempfindlich. Stellen Sie sicher, dass jeder hier konfigurierte SAML-Attributname genau mit der Anwendungs-Konfiguration übereinstimmt. SAML muss auf die richtige URL zeigen, um sich zu authentifizieren.
+
