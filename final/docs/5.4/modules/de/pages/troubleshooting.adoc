...
* Zugriffssteuerung. Siehe den Abschnitt zur Fehlerbehebung im Abschnitt Sicherheitsrisiken... -> Zugriffssteuerungen.
...
* Überprüfen Sie bei der Aktualisierung auf eine neue Version die Online-Dokumentation, um Änderungen/Ergänzungen zur erforderlichen YAML zu identifizieren, sowie andere Änderungen wie Rollenbindungen oder neue Dienste (z. B. Zugriffssteuerungs-Webhooks, persistente Volume-Ansprüche usw.).

== Debug-Protokolle

Um die Protokolle eines {product-name} Containers anzuzeigen, beispielsweise eines Controller-Pods
...
Um das Supportprotokoll herunterzuladen, gehen Sie zu Einstellungen -> Konfiguration und wählen Sie Protokoll sammeln.
...
[NOTE]
====
Wenn Sie von außerhalb des Clusters auf die REST-API zugreifen, siehe die Anweisungen im Abschnitt Automatisierung.
====
