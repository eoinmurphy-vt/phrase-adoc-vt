...
Stellen Sie sicher, dass die Volumes richtig gemappt sind. {product-name} benötigt diese, um zu funktionieren (/var/neuvector ist nur auf Controller/All-in-One erforderlich). Zum Beispiel:
...
Stellen Sie sicher, dass die erforderlichen Ports richtig gemappt und auf dem Host geöffnet sind. Der Manager oder All-in-One benötigt 8443 (wenn die Konsole verwendet wird). Der All-in-One und der Controller benötigen 18300, 18301, 18400, 18401 und optional 10443, 11443, 20443, 30443. Der Enforcer benötigt 18301 und 18401.

[NOTE]
====
Wenn Docker nativ (einschließlich SWARM) bereitgestellt wird, stellen Sie sicher, dass keine Host-Firewall den Zugriff auf erforderliche Ports wie firewalld blockiert. Wenn aktiviert, muss die docker0-Schnittstelle als vertrauenswürdige Zone für die All-in-One/Controller-Hosts hinzugefügt werden.
====
