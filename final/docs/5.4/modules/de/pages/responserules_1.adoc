...
[IMPORTANT]
====
Alle Antwortregeln werden ausgewertet, um zu bestimmen, ob sie der Bedingung/Kriterium entsprechen. Wenn es mehrere Regelübereinstimmungen gibt, werden die jeweiligen Aktionen ausgeführt. Dies unterscheidet sich vom Verhalten der Netzwerkregeln, die von oben nach unten ausgewertet werden und nur die erste übereinstimmende Regel ausgeführt wird.
====
...
* Fügen Sie den Namen des Ereignisprotokolls hinzu, der als Überechnungskriterium enthalten sein soll
* Wählen Sie die anzuwendenden Aktionen - Quarantäne, Webhook oder Protokoll unterdrücken
* Status aktivieren
* Die Protokollebene kann als anderes Überechnungskriterium verwendet werden
