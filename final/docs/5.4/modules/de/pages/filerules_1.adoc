...
* /bin, /usr/bin, /usr/sbin, /usr/local/bin - Container, Durchsetzungsmechanismus
* Dateien mit setuid- und setgid-Attribut - Container, Host, Durchsetzungsmechanismus
* Bibliotheken: libc, pthread, ld-linux.* - Container, Host, Durchsetzungsmechanismus
* Paketinstallation: dpkg, rpm, apk - Container, Host, Durchsetzungsmechanismus
* /etc/hosts, /etc/passwd, /etc/resolv.conf - Container, Host, Durchsetzungsmechanismus
...
[NOTE]
====
Wenn die Regel auf Blockieren gesetzt ist und der Dienst im Entdeckungsmodus ist, wird {product-name} die Anwendungen, die auf die Datei zugreifen, erlernen und diese zu den erlaubten Anwendungen für die Regel hinzufügen.
====

[NOTE]
====
Container-Plattformen, die den AUFS-Speicher-Driver verwenden, unterstützen die Verweigerung (Blockieren) der Aktion im Schutzmodus zum Erstellen/Ändern von Dateien aufgrund der Einschränkungen des Treibers nicht. Das Verhalten wird dasselbe wie im Überwachungsmodus sein, mit Alarmierung bei verdächtiger Aktivität.
====
...
Anzeigen der Datei-Zugriffsregel zum Schutz der /etc/hostname-Datei des node-pod-Dienstes und Erlauben der vi-Anwendung, die Datei zu ändern.

image:example1.png[FileRules]

Anzeigen der Datei-Zugriffsregel zum Schutz von Dateien im Verzeichnis /var/opt/ rekursiv für Änderungen sowie zum Lesen. Die erlaubte Anwendung python kann Lese- und Änderungszugriff auf diese Dateien haben.
...
Zeigt, dass die Anwendung Python beim Zugriff auf die Datei im Verzeichnis /var/opt gelernt hat, als der Dienstmodus des Node-Pods im Entdeckungsmodus war. Dies tritt nur auf, wenn die Regel auf Blockieren gesetzt ist und der Dienst im Entdeckungsmodus ist.
...
Zeigt ein Beispiel für ein Sicherheitsereignis in Benachrichtigungen -> Sicherheitsereignisse, das als Datei-Zugriffsverweigerung alarmiert wurde, als die Modifikation der Datei /etc/hostname durch die Anwendung Python aufgrund einer benutzerdefinierten Datei-Zugriffsregel mit Blockierungsaktion verweigert wurde.
