...
=== Test_ThreeSample script to check if container has root account with no password.
...
==== Test_ThreeSample script to check dirty cow file in the container.
...
== Test_ThreeCreating a custom check script
...
== Test_ThreeCreating a response rule for compliance report
