= Test project seventeen REST API and Automation
:revdate: 2025-05-08
...
=== Test_TwoAuthentication for REST API
...
=== Test_TwoTrigger Vulnerability Scanning from a script
...
=== Test_TwoCreate Policy Rules Automatically
...
=== Test_TwoExport/Import Configuration File
...
=== Test_TwoSetting or Changing User Password
...
=== Test_TwoStarting Packet Capture on a Container
...
=== Test_TwoCheck and Accept the EULA (new deployments)
...
=== Test_TwoEnable Packet Capture on All Pods in a Namespace
...
=== Test_TwoEnable Disable Container Quarantine
...
=== Test_TwoEnable Debugging Mode for {product-name} Support
...
=== Test_TwoReport if a vulnerability is in the base image layers
...
=== Test_TwoGet the CVE Database Version and Date
...
=== Test_TwoManage Federation for Master and Remote (Worker) Clusters
