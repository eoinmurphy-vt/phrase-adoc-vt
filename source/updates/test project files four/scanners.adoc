...
=== Test_FiveAuto-scaling of Scanner Pods
...
=== Test_FiveOperations and Debugging
...
== Test_FiveStandalone Scanner for Local Scanning
...
* SCANNER_REGISTRY= Test_Fiveurl of the registry (optional instead of local scan)
* SCANNER_REPOSITORY= Test_Fiverepository to scan
* SCANNER_TAG= Test_Fiveversion tag
* SCANNER_REGISTRY_USERNAME= Test_Fiveuser (optional instead of local scan)
* SCANNER_REGISTRY_PASSWORD= Test_Fivepassword (optional instead of local scan)
* SCANNER_SCAN_LAYERS= Test_Fivetrue or false (to return layered scan results)
...
=== Test_FiveHost Scanning in Standalone Mode
...
=== Test_FiveManual Deployment of Multiple Scanners on Kubernetes
