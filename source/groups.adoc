...
=== Test Auto Deletion of Unused Groups
...
=== Test Host Protection - the 'Nodes' Group
...
+
____
Create a group by DNS name or IP address ranges. Examples: address=www.google.com, address=10.1.0.1, address=10.1.0.0/24, address=10.1.0.1-10.1.0.25. DNS name can be any name resolvable. Address criteria do not accept the != Test operator. See below for special virtual host 'vh' address groups.
____
...
==== Test Virtual Host ('vh') Based Network Policy
...
== Test Custom Group Examples

=== Test General Criteria
