...
=== Test Login and Initial Configuration
...
=== Test Turning on Debug mode for {product-name} Controllers
...
=== Test Kubernetes, OpenShift and Other Orchestration Logs
...
== Test Using the CLI to turn on Debug Mode
...
Get the list of controllers. Find the controller with the Leader = Test True.
...
== Test Using the REST API to turn on Debug Mode
