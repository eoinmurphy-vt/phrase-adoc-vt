...
=== Test Auto-scaling of Scanner Pods
...
== Test Standalone Scanner for Local Scanning
...
* SCANNER_REGISTRY= Test url of the registry (optional instead of local scan)
* SCANNER_REPOSITORY= Test repository to scan
* SCANNER_TAG= Test version tag
* SCANNER_REGISTRY_USERNAME= Test user (optional instead of local scan)
* SCANNER_REGISTRY_PASSWORD= Test password (optional instead of local scan)
* SCANNER_SCAN_LAYERS= Test true or false (to return layered scan results)
...
=== Test Host Scanning in Standalone Mode
...
=== Test Manual Deployment of Multiple Scanners on Kubernetes
