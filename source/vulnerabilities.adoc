...
== Test_TwoManaging Vulnerabilities with {product-name}
...
=== Test_TwoSecurity Risks Menu - Vulnerabilities
...
* Green = Test_Twoprotected by {product-name} with Monitor or Protect mode
* Red = Test_Twounprotected by {product-name}, still in Discover mode

The Impact analysis window (showing affected images, nodes, containers) color scheme is:

* Black = Test_Twounaffected. There are no containers using this image in production
* Purple = Test_Tworunning in Monitor mode in production
* Dark Green = Test_Tworunning in Protect mode in production
* Light Blue = Test_Tworunning in Discover mode in production (unprotected)
...
=== Test_TwoAccepting Vulnerabilities
...
=== Test_TwoManaging Vulnerability Profiles
