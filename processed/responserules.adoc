...
== Test_TwoDetailed Configuration for Response Rules
...
=== Test_TwoUsing Multiple Criteria in a Single Rule
...
=== Test_TwoCreating a response rule for security event logs
...
=== Test_TwoSample rule to quarantine container and send webhook when package is updated in the nv.alpinepython.default container.

image:resp3.png[rulesample]

=== Test_TwoIcons to manage rules - edit, delete, disable and insert new rule below

image:resp4.png[manage]

=== Test_TwoCreating a response rule for event logs
...
=== Test_TwoSample events that can be chosen for a response rule

image:resp5.png[events]

=== Test_TwoSample criteria for Admission control events

image:admission.png[events]

=== Test_TwoCreating a response rule for cve-report category (log level and report name as matching criteria)
...
=== Test_TwoSample CVE report types that can be chosen for CVE-Report category response rule

image:resp7.png[cvereport]

=== Test_TwoQuarantine container and send webhook when vulnerability scan results contain more than 5 high level CVE vulnerabilities for that container

image:resp8.png[cvequarantine]

=== Test_TwoSend a webhook if container contains vulnerability with name cve-2018-12

image:resp8-a.png[webhook]

=== Test_TwoCreating response rule for CIS benchmarks (log level and benchmark number as matching criteria)
...
=== Test_TwoUnquarantine a container by deleting response rule
...
=== Test_TwoViewing the rule id responsible for the container quarantine (in Notifications -> Events)

image:resp9.png[unquarantine]

=== Test_TwoUnquarantine option popup when the appropriate response rule is deleted

Check the box to unquarantine any containers which were quarantined by this rule

image:resp10.png[option]

=== Test_TwoComplete list of categoried criteria that can be configured for Response Rules
...
==== Test_TwoIncidents (Security Event)
...
==== Test_TwoThreats (Security Event)
...
==== Test_TwoViolations (Security Event)
...
==== Test_TwoDynamically Generated Criteria
