...
== Test_SixDetailed Configuration for the Jenkins Plugin
...
=== Test_SixGlobal Configuration in Jenkins
...
=== Test_SixLocal Configuration for scanning a remote Docker Host

==== Test_SixPrerequisites for Local Scan on a Remote Docker Host
...
=== Test_SixSetting Up the Pipeline to Build Large Scale Parallel Scans
...
== Test_SixOpenShift Route and Registry Token Example
...
== Test_SixSpecial Use Case for Jenkins in the Same Kubernetes Cluster
