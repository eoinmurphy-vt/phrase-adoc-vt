...
=== Test_SixAuto-scaling of Scanner Pods
...
=== Test_SixOperations and Debugging
...
== Test_SixStandalone Scanner for Local Scanning
...
* SCANNER_REGISTRY= Test_Sixurl of the registry (optional instead of local scan)
* SCANNER_REPOSITORY= Test_Sixrepository to scan
* SCANNER_TAG= Test_Sixversion tag
* SCANNER_REGISTRY_USERNAME= Test_Sixuser (optional instead of local scan)
* SCANNER_REGISTRY_PASSWORD= Test_Sixpassword (optional instead of local scan)
* SCANNER_SCAN_LAYERS= Test_Sixtrue or false (to return layered scan results)
...
=== Test_SixHost Scanning in Standalone Mode
...
=== Test_SixManual Deployment of Multiple Scanners on Kubernetes
