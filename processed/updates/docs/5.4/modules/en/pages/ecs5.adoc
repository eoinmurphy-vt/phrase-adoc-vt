...
Select the instance, then pick {Q}&quot;`++View/Edit Attributes++`&quot;{/Q} from the Actions drop down menu.

image:1viewattributes.png[Attributes]

Then add a new attribute. For example {Q}&quot;`++allinone-node++`&quot;{/Q} with value {Q}&quot;`++true++`&quot;{/Q}.

image:2addattribute.png[AddAttributes]
--
. Create the Allinone Task Definition. Create a new Task Definition for the Allinone container. You can use the ECS interface to manually create it or paste in the sample JSON file (see below for samples). Refer to section {Q}&quot;`++1. Deploying {product-name}++`&quot;{/Q} of these docs for how to configure the Allinone.
+
--
...
====
--
. Create a new service for the Allinone task. Set the {Q}&quot;`++Placement Templates++`&quot;{/Q} to {Q}&quot;`++One Task Per Host++`&quot;{/Q} so that only one Allinone/Controller can run on any host. You will also see the constraint will be used "`+memberOf(attribute:allinone-node=~true) which requires the node to have that attribute.
+
--
image:3taskplacement.png[AllinonePlace]
--
. Now you can deploy the Allinone service. Set the {Q}&quot;+`+Number of tasks+`+&quot;{/Q} to the desired Allinone/Controller number. Now the {product-name} Allinone or Controller containers will start running on the nodes selected. After the Allinone starts running you should be able to connect to the {product-name} console through HTTPS on port 8443.
...
--
. Create a new service for the Enforcer task. Again, set the Task Placement to {Q}&quot;+`+One Task Per Host+`+&quot;{/Q} so only one Enforcer is deployed on each host. Also note the additional constraint should show that it prevents deployment on an allinone node.
+
--
image:5taskplacement.png[EnforcerPlacement]

Deploy this service with desired number of enforcer nodes in {Q}&quot;+`+Number of tasks+`+&quot;{/Q}. Very shortly all the enforcers will be up and running. From the {product-name} console you will be able to see all nodes being detected with enforcers.
--
...
. Create a new revision of the Allinone/Controller task, update the {Q}&quot;+`+CLUSTER_JOIN_ADDR+`&quot;{/Q} to the 2nd Allinone/controller's private node IP address.
