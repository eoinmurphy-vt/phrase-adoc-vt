= Test_EightReplacing Internal Certificates
:revdate: 2025-01-27
...
== Test_EightInternal Communication and Certificates
...
=== Test_EightReplacing Certificates Used in Internal Communications of {product-name}
...
+
If your certificate files were created before {product-name} version 5.3, you need to upate the certificate with at least one Subject Alternative Name, or SAN for short. If you donÂ´t have the files `+ca.key+` and `+ca.crt+` anymore, follow the steps below: 
+
...
=== Test_EightUpdating/Deploying with Helm
