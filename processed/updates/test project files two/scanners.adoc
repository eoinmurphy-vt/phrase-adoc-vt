...
=== Test_TwoOperations and Debugging
...
== Test_TwoStandalone Scanner for Local Scanning
...
* SCANNER_REGISTRY= Test_Twourl of the registry (optional instead of local scan)
* SCANNER_REPOSITORY= Test_Tworepository to scan
* SCANNER_TAG= Test_Twoversion tag
* SCANNER_REGISTRY_USERNAME= Test_Twouser (optional instead of local scan)
* SCANNER_REGISTRY_PASSWORD= Test_Twopassword (optional instead of local scan)
* SCANNER_SCAN_LAYERS= Test_Twotrue or false (to return layered scan results)
...
=== Test_TwoHost Scanning in Standalone Mode
...
=== Test_TwoManual Deployment of Multiple Scanners on Kubernetes
