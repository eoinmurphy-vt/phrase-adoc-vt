= Test project eighteen OpenID Connect Azure/Okta
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /08.integration/07.openid/07.openid.md
:page-opendocs-slug:  /integration/openid

== Test_TwoIntegrating with OpenID Connect (OIDC) for Azure and Okta
...
In the center pane, click {Q}&quot;`++Create App Integration++`&quot;{/Q}:

image:okta1.png[create]

A new pane will pop up to select the {Q}&quot;`++Sign-in method++`&quot;{/Q}:

image:okta2.png[newapp]

Select {Q}&quot;`++OIDC -- OpenID Connect++`&quot;{/Q} option.

A derived pane will appear, for {Q}&quot;`++Application Type++`&quot;{/Q} selection:

image:okta3.png[apptype]

Select {Q}&quot;`++Native Application++`&quot;{/Q} option.
...
Go to your {product-name} console and navigate to {Q}&quot;`++Settings++`&quot;{/Q} -> {Q}&quot;`++OpenId Connect Settings++`&quot;{/Q}.  At the top of the page, next to {Q}&quot;`++OpenID Connect Redirect URI++`&quot;{/Q} label click {Q}&quot;`++Copy to Clipboard++`&quot;{/Q}.
...
Select {Q}&quot;`++Allow everyone in your organization to access++`&quot;{/Q} to have this integration available for everyone in your org.
...
In {Q}&quot;`++Client Credentials++`&quot;{/Q} section, click edit and modify the {Q}&quot;`++Client Authentication++`&quot;{/Q} section from {Q}&quot;`++Use PKCE (for public clients)++`&quot;{/Q} to {Q}&quot;`++Use Client Authentication++`&quot;{/Q}, and hit save. This will generate a new secret automatically which we will need in upcoming {product-name} setup steps:

image:okta7.png[clientauth]

Navigate to the {Q}&quot;`++Sign On++`&quot;{/Q} tab and edit the {Q}&quot;`++OpenID Connect ID Token++`&quot;{/Q} section:
Change the Issuer from     {Q}&quot;`++Dynamic (based on request domain)++`&quot;{/Q} to the fixed {Q}&quot;`++Okta URL++`&quot;{/Q}:
...
In {Q}&quot;`++OpenID Connect ID Token++`&quot;{/Q} section, you can create new claims for user's Groups and carry the claim in the ID Token (an ID Token is a JSON Web Token, a compact URL-Safe means of representing claims to be transferred between two parties, so identity information about the user is encoded right into the token and the token can be definitively verified to prove that is hasn't been tampered with). If a specific scope is configured, make sure to add the scope to {product-name} OpenID Connect setting page, so that the claim can be included after the user is authenticated:

image:okta11.png[scopes]

By default, {product-name} looks for "groups" in the claim to identify the user's group membership. If other claim name is used, you can customize the claim name in {product-name}'s OpenID Connect Setting page. To configure claims, edit the {Q}&quot;`++OpenID Connect ID Token++`&quot;{/Q} section as shown in the next image:

image:okta12.png[claims]

In your application integration page, navigate to {Q}&quot;`++Assignments++`&quot;{/Q} tab and make sure you have the corresponding assignments listed:

image:okta13.png[assignments]

=== Test_Two{product-name} OpenID Connect Configuration
