...
==== Test_Two5.4.2 New Installation:
...
==== Test_TwoUpgrading from Previous Versions to 5.4.2:
...
==== Test_Two{product-name} UI Extension v. 1.0 for Rancher March 2024
...
=== Test_Two5.2.4-s2 February 2024
...
* Enable ICMP learning (Discover mode) and blocking (Protect mode) through new Controller environment variable CTRL_EN_ICMP_POLICY = Test_Two1
...
==== Test_TwoHighlighted Changes Which May Require Changes for Manual Deployments (all changes are already reflected in latest Helm chart for 5.3.x)
...
=== Test_Two5.2.2 October 2023

==== Test_TwoSecurity Advisory for CVE-2023-32188

* Remediate CVE-2023-32188 {Q}&quot;`++JWT token compromise can allow malicious actions including Remote Code Execution (RCE)++`&quot;{/Q} by auto-generating certificate used for signing JWT token upon deployment and upgrade, and auto-generating Manager/RESTful API certificate during Helm based deployments.
...
* Add environment variable AUTO_PROFILE_COLLECT for Controller and Enforcer to assist in capturing memory usage when investigating memory pressure events. Set value = Test_Two1 to enable.
* Configuration assessments against Admission Control should show all violations with one scan.
* Add more options for CVE report criteria in Response Rules. Example 1 - "cve-high-with-fix:X" means: When # of (high vulnerability that have been fixed) >= Test_TwoX, trigger the response rule. Example 2 - "cve-high-with-fix:X/Y" means: When # of (high vulnerability that were reported Y days ago & have been fixed) >= Test_TwoX, trigger the response rule.
...
* Use less privileged services accounts as required for each {product-name} component. A variable {Q}&quot;`++leastPrivilege++`&quot;{/Q} is introduced. The default is false. NOTE: Using the current helm chart with this variable on a release prior to 5.2.0 will not function properly.
...
=== Test_Two5.1.1 February, 2023

==== Test_TwoEnhancements

* Add {Q}&quot;`++package++`&quot;{/Q} as information to the syslog-event for a detected vulnerability.
...
==== Test_TwoDetails on SELinux Support
...
=== Test_Two5.0.0 General Availability (GA) Release May 2022
...
=== Test_TwoUpgrading from {product-name} 4.x to 5.x (prior to 5.2.x)
...
==== Test_TwoBeta 1 version released April 2022
...
==== Test_TwoPreview.3 version released March 2022
...
==== Test_TwoPreview.2 version released Feb 2022

* Minor file and license changes in source, no features added.

==== Test_TwoSupport for deployment on AWS ECS Deprecated

Support for deployment on ECS is no longer provided. The allinone should still be able to be deployed on ECS, however, the documentation of the steps and settings is no longer supported.

=== Test_Two5.0 'Tech Preview' January 2022
