...
Wählen Sie Local oder einen Registrierungsnamen, der der Spitzname ist, den Sie in der globalen Konfiguration eingegeben haben. Geben Sie den Repository- und Bild-Tag-Namen ein, der gescannt werden soll. Sie können die Standardumgebungsvariablen von Jenkins für das Repository oder den Tag wählen, z.B. $JOB_NAME, $BUILD_TAG, $BUILD_NUMBER. Geben Sie die Werte für die Anzahl der hohen oder mittleren und für jeden Namen der vorhandenen Schwachstellen ein, um den Build fehlschlagen zu lassen.
