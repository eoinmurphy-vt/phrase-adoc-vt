...
== Test_ThreeManaging Vulnerabilities with {product-name}
...
=== Test_ThreeSecurity Risks Menu - Vulnerabilities
...
* Green = Test_Threeprotected by {product-name} with Monitor or Protect mode
* Red = Test_Threeunprotected by {product-name}, still in Discover mode

The Impact analysis window (showing affected images, nodes, containers) color scheme is:

* Black = Test_Threeunaffected. There are no containers using this image in production
* Purple = Test_Threerunning in Monitor mode in production
* Dark Green = Test_Threerunning in Protect mode in production
* Light Blue = Test_Threerunning in Discover mode in production (unprotected)
...
=== Test_ThreeAccepting Vulnerabilities
...
=== Test_ThreeManaging Vulnerability Profiles
