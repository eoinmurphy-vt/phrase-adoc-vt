...
En sélectionnant Google Container Registry comme type de dépôt, ce tableau de bord est personnalisé pour accepter les données nécessaires à l'utilisation de votre GCR.

. Nom - C'est ici que vous donnez à cette entrée de dépôt un nom de votre choix. C'est simplement pour l'identifier dans l'interface de \{product-name} plus tard.
. Registre - C'est le premier endroit où vous voudrez vous assurer que les bonnes données sont collectées depuis votre instance GCR. Bien que l'exemple de https://gcr.io soit le plus courant, nous voulons nous assurer qu'il reflète fidèlement comment votre GCR a été configuré dans GCP. Cela pourrait être https://us.gcr.io par exemple. Nous allons le vérifier dans la section suivante.
...
Une fois que vous avez saisi les données pour la première étape de création d'un compte de service, vous devez appuyer sur le bouton "`+CRÉER+`" pour que l'étape 2 soit prête à accepter l'entrée.

image:gcr5.png[GCR]

Assurez-vous de sélectionner --> Accès de base pour le visionneur. Si vous avez un compte de service existant, assurez-vous que l'accès est configuré de cette manière. (Indice : Même les autorisations d'accès qui semblent plus puissantes ne semblent pas permettre un accès approprié. Ne sautez pas cette étape.
...
Si vous n'atterrissez pas immédiatement sur le panneau d'informations de votre nouveau compte de service, assurez-vous d'y aller dans la liste des comptes de service. Voir la figure 5 ci-dessous.
...
Comme vous l'avez déjà conclu, JSON est la solution ici. Sélectionner "`+CRÉER+`" entraînera un fichier que vous pourrez télécharger dans votre navigateur. Le contenu de ce fichier doit être collé dans le champ 3, Clé JSON dans \{product-name} ; voir la figure 1.

Avant de vous exciter trop, il y a une chose de plus à vérifier. Pour que le scanner dans \{product-name} utilise l'API pour scanner et protéger vos images, cette API doit être activée dans votre compte GCP. Vous pouvez soit l'activer via la ligne de commande via
...
Ou vous pouvez utiliser l'interface graphique GCP. Allez à "`+Bibliothèque d'API+`" et recherchez "`+API du registre d'artefacts+`" et assurez-vous qu'elle est activée pour votre projet. Voir la figure 7.
