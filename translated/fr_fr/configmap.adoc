= Test_TwoDeploy Utiliser ConfigMap
:revdate: 2025-01-10
:page-revdate: {revdate}
:page-opendocs-origin: /02.deploying/01.production/01.configmap/01.configmap.md
:page-opendocs-slug: /déploiement/production/configmap

== Test_TwoKubernetes ConfigMap

\{product-name} prend en charge la configuration automatisée en utilisant la fonctionnalité ConfigMap de Kubernetes. Cela permet le déploiement des conteneurs \{product-name} avec les configurations appropriées, les intégrations et d'autres paramètres de manière automatisée.

Le paramètre 'always_reload: true' peut être ajouté dans n'importe quel yaml ConfigMap pour forcer le rechargement de ce yaml chaque fois que le contrôleur démarre (version 4.3.2+). Sinon, le ConfigMap ne sera chargé qu'au démarrage initial ou après un redémarrage complet du cluster (voir la section sur le stockage persistant ci-dessous).

=== Test_TwoComplete Sample \{product-name} ConfigMap (initcfg.yaml)

Le dernier ConfigMap peut être trouvé https://raw.githubusercontent.com/neuvector/manifests/main/kubernetes/5.4.0/initcfg.yaml[ici].
...
[NOTE]
====
Si vous utilisez configmap dans un secret, voir la section ci-dessous pour les modifications de formatage.
====
...
data:
  fedinitcfg.yaml: |
    # ============ Test_Twothis section is used for primary cluster ============ Test_Two>>>
    ...
    # <<< ============ Test_Twothis section is used for primary cluster ============
    # ============ Test_Twothis section is used for remote cluster ============ Test_Two>>>
    # Optional. If specified, it overwrites the cluster name specified in the system configuration in the sysinitcfg.yaml file
    ...
    # Optional. Supported value: https
    Use_Proxy: ""
    # <<< ============ Test_Twothis section is used for remote cluster ============
...
=== Test_TwoFederated Exemples de ConfigMap (fedinitcfg.yaml)

NeuVector v5.4.0 prend en charge l'automatisation de la fédération via le ConfigMap. Ci-dessous se trouvent des exemples de configurations `+fedinitcfg.yaml+` qui peuvent être appliquées à vos clusters principaux et gérés en fonction de votre cas d'utilisation.
...
== Test_TwoProtéger les données sensibles à l'aide d'un secret
...
Notez la suppression du caractère pipe ci-dessous si vous utilisez des sections de configmap dans un secret, activé sur true, et décommentez la section à inclure dans le secret.
...
Notez que le secret est référencé dans les fichiers yaml de xref:kubernetes.adoc#_deploy_suse_security[déploiement standard Kubernetes et OpenShift] sous Volumes.

== Test_TwoConfigMaps et stockage persistant

Les ConfigMaps et la sauvegarde de xref:production.adoc#_backups_and_persistent_data[stockage persistant] ne sont lus que lorsqu'un nouveau cluster \{product-name} est déployé, ou lorsque le cluster échoue et est redémarré. Ils ne sont pas utilisés lors des mises à niveau progressives.

La sauvegarde de la configuration de stockage persistant est lue en premier, puis les ConfigMaps sont appliqués, de sorte que les paramètres de ConfigMap prennent le pas. Tous les paramètres de ConfigMap (par exemple, les mises à jour) seront également enregistrés dans le stockage persistant.
