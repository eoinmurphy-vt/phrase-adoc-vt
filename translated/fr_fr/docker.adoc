= Test_TwoDocker & Mirantis Kubernetes Engine
:revdate: 2025-01-10
:page-revdate: {revdate}
:page-opendocs-origin: /02.deploying/05.docker/05.docker.md
:page-opendocs-slug: /deploying/docker

== Test_TwoKubernetes Déploiement sur Mirantis Kubernetes Engine
...
== Test_TwoDéployer des Conteneurs \{product-name} en utilisant Docker Native ou UCP/Swarm
...
Voici les fichiers de configuration docker compose d'exemple. Notez qu'utiliser Docker natif ne prend pas en charge le déploiement de l'enforcer sur le même nœud que le contrôleur, nécessitant l'utilisation du conteneur Allinone si les fonctions de contrôleur et d'enforcer sont souhaitées sur un nœud.
...
=== Test_TwoDéployer Allinone pour Haute Disponibilité

Pour HA dans des environnements de production Docker natif ou EE, déployez le conteneur Allinone sur les trois premiers hôtes de production. Chaque Allinone doit pointer vers les adresses IP de tous les hôtes Allinone. Par exemple, trois conteneurs Allinone est le minimum pour HA, et le CLUSTER_JOIN_ADDR doit lister les trois adresses IP séparées par des virgules. Des Allinone supplémentaires peuvent être déployés en nombres impairs, par exemple 5, 7. Déployez l'Enforcer sur les hôtes restants dans le cluster, dans n'importe quel.

=== Test_TwoDéployer Allinone en utilisant docker-compose (mode privilégié)
...
La variable d'environnement la plus importante est le *CLUSTER_JOIN_ADDR*. C'est l'adresse IP à laquelle d'autres agents se connectent. Normalement, elle doit être définie sur l'adresse IP du nœud où le conteneur all-in-one fonctionne.

Les ports 18300 et 18301 sont les ports par défaut pour la communication de cluster. Ils doivent être identiques pour tous les contrôleurs et agents dans le cluster. Veuillez vous référer à la section _"Détails de Docker-compose"_ pour savoir comment changer les ports par défaut.
...
=== Test_TwoAjouter un conteneur d'agent en utilisant docker-compose (mode privilégié)

Ceci est un exemple de fichier docker-compose pour rejoindre un agent dans le cluster. Les déploiements greenfield et brownfield sont tous deux pris en charge.
...
La variable d'environnement la plus importante est *CLUSTER_JOIN_ADDR*. Pour les agents, remplacez `+<controller_node_ip>+` par l'adresse IP du nœud du contrôleur. Typiquement, *CLUSTER_JOIN_ADDR* dans le fichier docker-compose du contrôleur/all-in-one et le fichier docker-compose de l'agent ont la même valeur.

=== Test_TwoDéployer le conteneur Scanner \{product-name}
...
=== Test_TwoDeployment sans utiliser le mode privilégié
...
=== Test_TwoDeploy allinone (SANS mode privilégié) avec docker-compose
...
=== Test_TwoDeploy enforcer (SANS mode privilégié) avec docker-compose
...
=== Test_TwoDeploy allinone (mode privilégié) avec docker run
...
=== Test_TwoDeploy enforcer (mode privilégié) avec docker run
...
=== Test_TwoDeploy allinone (SANS mode privilégié) avec docker run
...
=== Test_TwoDeploy enforcer (SANS mode privilégié) avec docker run
...
== Test_TwoDéployer des composants \{product-name} séparés sur différents hôtes

Si vous prévoyez de dédier un hôte docker à un contrôleur et/ou un gestionnaire (sans exécuteur), ces conteneurs peuvent être déployés individuellement au lieu de l'Allinone. Notez que docker ne prend pas en charge le déploiement de l'exécuteur sur le même nœud que le contrôleur en tant que composants séparés, nécessitant l'utilisation du conteneur Allinone si les fonctions de contrôleur et d'exécuteur sont souhaitées sur un nœud.

Fichier compose du contrôleur (remplacez [controller IP] par l'IP du premier nœud contrôleur)
...
Le fichier compose pour l'exécuteur :
...
== Test_TwoSurveillance et redémarrage de \{product-name}

Puisque les conteneurs \{product-name} ne sont pas déployés en tant que service UCP/Swarm, ils ne sont pas automatiquement démarrés/redémarrés sur les nœuds. Vous devez configurer des alertes via votre système SIEM pour les événements SYSLOG de \{product-name} ou via DataCenter pour détecter si un conteneur \{product-name} ne fonctionne pas.

== Test_TwoDéploiement sans mode privilégié
...
== Test_TwoMises à jour Docker Native
