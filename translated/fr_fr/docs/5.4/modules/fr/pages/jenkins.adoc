...
La valeur du temps d'attente mettra fin à la phase de construction au moment saisi. La valeur par défaut de 0 signifie qu'aucun temps d'attente ne se produit.
...
Pour exécuter un scan de vulnérabilités en mode autonome, le plugin Jenkins doit tirer l'image du scanner sur l'hôte sur lequel l'agent s'exécute. Par conséquent, vous devez entrer l'URL du registre {product-name} Scanner, le dépôt d'images et les informations d'identification, si nécessaire, sur la page de configuration du plugin {product-name}.
...
Pour utiliser l'authentification basée sur des jetons pour le registre OpenShift, utilisez NONAME comme utilisateur et saisissez le jeton dans le champ du mot de passe.
