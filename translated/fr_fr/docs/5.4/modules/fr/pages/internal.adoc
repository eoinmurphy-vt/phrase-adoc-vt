...
[IMPORTANT]
====
Les versions de NeuVector 5.4.2 et supérieures doivent amener les utilisateurs à générer/remplacer les certificats internes avant d'utiliser NeuVector.
Après mars 2025, les versions de NeuVector antérieures à 5.4.2 doivent amener les utilisateurs à générer/remplacer les certificats internes avant d'utiliser NeuVector.
====

== Communication interne et certificats

{product-name} contient par défaut des certificats auto-signés pour le chiffrement pour le gestionnaire (accès console/UI), le contrôleur (API REST, interne), l'enforcer (interne) et la communication du scanner (interne).

Ces certificats peuvent être remplacés par les vôtres pour sécuriser davantage la communication. Pour remplacer les certificats utilisés pour l'accès externe à {product-name} (c'est-à-dire Navigateur vers le gestionnaire ou API REST vers le contrôleur), veuillez consulter xref:replacecert.adoc[cette section]. Voir ci-dessous pour remplacer les certificats utilisés dans la communication interne entre les conteneurs {product-name}.

[WARNING]
====
Il est recommandé de remplacer les certificats uniquement lors du premier déploiement de {product-name}. Le remplacement dans un cluster en cours d'exécution (même lors d'une mise à niveau progressive) peut entraîner un état instable où les pods {product-name} ne peuvent pas communiquer entre eux, en raison d'une discordance dans les certificats, et cela peut entraîner une PERTE DE DONNÉES.
====
...
Nouveau certificat::
+
Si votre certificat est sur le point d'expirer et que vous devez en générer un nouveau, veuillez suivre les étapes suivantes :
+
...
----
+
* Modifiez ensuite les fichiers de déploiement YAML du contrôleur, de l'enforcer et du scanner en ajoutant ce qui suit :
+
[source,yaml]
----
      containers:
...
----
+
Ensuite, continuez à {product-name} comme précédemment. Vous pouvez également accéder à l'interpréteur de commandes dans les Pods Controller/Enforcer/Scanner pour confirmer que les fichiers ca.crt, tls.key, tls.crt sont les personnalisés et que les communications {product-name} fonctionnent avec les nouveaux certificats.
+
Exemples de commandes de patch pour le contrôleur (modifiez l'espace de noms en cattle-neuvector-system si nécessaire, et adaptez-le pour une utilisation sur l'Enforcer, le Scanner)
+
[source,bash]
----
NAMESPACE=neuvector
...
+
Si vos fichiers de certificat ont été créés avant la version {product-name} 5.3, vous devez mettre à jour le certificat avec au moins un Subject Alternative Name ou SAN. Si vous avez encore les fichiers `+ca.key+` et `+ca.crt+` accessibles, exécutez les commandes comme suit :
+
[source,bash]
----
kubectl delete secret internal-cert -n neuvector 
...
+
Si vos fichiers de certificat ont été créés avant la version {product-name} 5.3, vous devez mettre à jour le certificat avec au moins un Subject Alternative Name ou SAN. Si vous n'avez plus les fichiers `+ca.key+` et `+ca.crt+`, suivez les étapes ci-dessous : 
+
...
+
Si vous n'avez pas remplacé le certificat interne auparavant et que vous souhaitez migrer vers un nouvel ensemble de certificats, suivez les étapes ci-dessous :
+
...
+
Si vous pouvez voir le secret mais ne trouvez pas ces secrets, envisagez d'activer `+internal.autoRotateCert+` dans le jeu d'outils du graphiste Helm. Cette option génère et fait pivoter automatiquement votre certificat interne.
+
