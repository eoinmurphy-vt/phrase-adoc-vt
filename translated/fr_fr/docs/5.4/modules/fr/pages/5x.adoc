...
* *NVSHAS-9969*: Erreurs fatales: Les opérations d'écriture de carte simultanées entraînent le redémarrage du composant Enforcer.
...
* *NVSHAS-10010*: La tempête TCP SYN bloque l'entrée, ce qui entraîne un blocage complet du trafic entrant.
...
* **NVSHAS-9812**: L'exportateur Prometheus de NeuVector a des métriques incorrectes.
...
* **NVSHAS-9447**: Les pods du contrôleur/scanner se bloquent - "Sortie système non prise en charge".
...
* **NVSHAS-9597**: L'interface utilisateur ne réagit pas aux erreurs lorsque le contrôleur 403 est renvoyé pour POST(v1/group).
...
* **NVSHAS-7853** : EOF du handshake TLS.
...
* **NVSHAS-9337**: Envoyez un message de journal lorsque l'attaque SYN-Flood est détectée.
...
* (Scanner) Détecter le paquet/module `+R+` sur Ubuntu et Red Hat Enterprise Linux.
...
* Afficher les URL cibles externes (FQDN) dans le tableau de bord (Egress), dans les rapports PDF et CSV ainsi que dans l'écran d'activité réseau et dans les listes d'événements de sécurité (violations).
...
* Das Löschen von verwundbaren Dateien (z.  jar) ne les retire pas de la liste des vulnérabilités.
...
==== Modifications mises en évidence qui peuvent nécessiter des changements pour des déploiements manuels (toutes les modifications sont déjà reflétées dans le dernier diagramme Helm pour 5.3.x).
...
* La suppression de l'exigence d'exécution du contrôleur en mode privilégié élimine la nécessité de monter le socket d'exécution et de monter /host/cgroup/.
...
* Blocage des connexions SSL, même si une règle réseau autorise le trafic dans certaines conditions initiales.
...
* Le jeton de compte de service par défaut de l'espace de noms, monté dans chaque pod, a trop de permissions.
...
* Activez les paramètres d'intégration du marché AWS (adaptateur de facturation) dans le diagramme Helm.
* Mettez à jour le ConfigMap pour prendre en charge de nouvelles fonctionnalités (plusieurs certificats ADFS, Null-Derivation, New_Service_Profile_Baseline, SYSLOG TLS, délai d'expiration de l'utilisateur)
...
* Le champ de détail du registre (en bas) n'est pas mis à jour.
...
Pour les utilisateurs de Helm, mettez à jour vers {product-name} le graphique Helm 2.0.0 ou supérieur. Si vous mettez à niveau un opérateur ou une installation Helm sur OpenShift, consultez la note suivante.
