...
* *NVSHAS-9988*: UI: Les pages de groupe n'affichent pas tous les groupes s'il y a plus de 2 000 groupes.
...
* **NVSHAS-9931**: Ajouter un avertissement si des versions incohérentes des produits NeuVector sont présentes dans un multi-cluster.
...
* **NVSHAS-9909**: Le message "Déconnecté - Aller à la page de chargement" s'affiche lorsque le lien de navigation NeuVector Rancher est utilisé.
* **NVSHAS-9898**: Dans un environnement fédéré, changer les critères d'un groupe créé par le client fed.* (par exemple, B. fed.core-systems) nicht.
...
* **NVSHAS-9783**: Après l'ajout de la politique réseau interdisant les paquets ICMP, NeuVector ne voit pas d'avertissement dans `+Security Events+`.
...
* **NVSHAS-9326**: NeuVector - Module de scanner Harbor extensible.
...
* **NVSHAS-9756**: L'Enforcer a atteint la CPU maximale et a plusieurs avertissements de surcharge mémoire.
...
[NOTE]
====
Les types par défaut du gestionnaire et du service d'adaptateur d'enregistrement ont été modifiés en ClusterIP.  Les utilisateurs peuvent toujours remplacer le paramètre en remplaçant `+manager.svc.type+` et `+cve.adapter.svc.type+` lorsque NodePort est préféré.
====
...
- Activer les drapeaux `+internal.autoGenerateCert+` et `+internal.autoRotateCert+` dans les graphiques Helm (ceux-ci sont activés par défaut à partir de la version 5.4.2). Alternativement, une méthode YAML est liée ci-dessous.
...
Veuillez noter que le scanner autonome ne sera pas affecté par ces modifications.
...
* **NVSHAS-9572**: [Vulnérabilités] Les données filtrées ont été conservées indépendamment de la mise à jour de l'utilisateur ou de la reconnexion sur la page.
* **NVSHAS-9597**: L'interface utilisateur ne réagit pas aux erreurs lorsque le contrôleur 403 renvoie pour POST(v1/group).
* **NVSHAS-8682**: Le service de webhooks CRD doit être déplacé du diagramme Helm crd vers le diagramme Helm de l'application.
...
* **NVSHAS-9285**: UI : Le bouton déroulant chevauche d'autres éléments.
...
* **NVSHAS-9091**: Il n'est pas possible de signaler tous les modules sous ol:9.1, photon:5.0, rhel:9.1 et amzn:2023 dans le dépôt, dans le registre et dans l'analyse autonome.
...
* **NVSHAS-4858** : Ne développez pas davantage le groupe de conteneurs dans le contrôleur pour améliorer les performances de déploiement de la stratégie et réduire l'utilisation du CPU et de la mémoire.
...
* **NVSHAS-9301** : NV, déployé sur Rancher Prime, ne peut pas afficher son goût Rancher.
...
* **NVSHAS-7687**: Ajoutez le paramètre de niveau de journalisation Syslog pour l'appliquer.
* **NVSHAS-9292**: Corrigez l'exposition Ingress Egress qui montre 0 vulnérabilités.
...
* Permet aux utilisateurs de bloquer l'utilisation de certaines classes de stockage à partir de la page `+Admission Controls+`.
...
* (Scanner) Ajout de la prise en charge du scan PHP Composer.
...
* Après la mise à niveau vers v5.3.1 à partir d'une version précédente {product-name}, les ressources NvClusterSecurityRule personnalisées existantes peuvent être supprimées par inadvertance. REMARQUE : La version 5.3.1 a été retirée du Docker Hub pour éviter le problème de mise à niveau.
...
HTTP Strict Transport Security max-age=15724800, 2) Ajoutez un en-tête CSP (par exemple, B. eine Richtlinie '\`+default-src\+`' festlegen), 3) Entfernen Sie die Offenlegung des Servernamens
...
* Ajouter un lien vers le groupe dans les risques de sécurité -> vulnérabilités -> ajouter des impacts contextuels pour changer facilement le mode de groupe
...
* Ajouter la détection automatique de l'exécution des conteneurs (Socket) pour l'exécution dans le diagramme Helm.
...
* Les évaluations de configuration par rapport au contrôle d'admission devraient afficher toutes les violations avec un seul scan.
* Ajoutez d'autres options pour les critères de rapport CVE dans les règles de réponse. Exemple 1 - "cve-high-with-fix:X" signifie : Si le nombre de (vulnérabilités critiques qui ont été corrigées) >= X, déclencher la règle de réponse. Exemple 2 - "cve-high-with-fix:X/Y" signifie : Si le nombre de (vulnérabilités critiques qui ont été signalées il y a Y jours et corrigées) >= X, déclencher la règle de réponse.
...
* Prenez en charge les annotations de Pod pour les politiques d'admission qui sont disponibles via des critères personnalisés.
...
* Prise en charge de la signature des images pour les contrôles d'admission. Les utilisateurs peuvent demander {product-name} pour vérifier que les images sont signées par certaines parties avant de pouvoir être déployées en environnement de production, grâce à une intégration avec Sigstore/Cosign. Voir les actifs -> vérificateur Sigstore, pour créer de nouveaux actifs de signature. Des règles peuvent ensuite être créées avec les critères de signature d'image et/ou de vérificateur d'image Sigstore.
* Activez chaque règle pour les contrôles d'admission afin qu'elle ait son propre mode de surveillance ou de protection. Une action de rejet en mode de surveillance alertera, et une action de rejet en mode de protection bloquera. Les actions autorisées ne sont pas affectées.
...
* Activez l'administrateur pour configurer le délai d'expiration de session par défaut de l'utilisateur dans les paramètres -> Utilisateur, clé API et rôles.
...
* Le mot de passe contenant un caractère spécial échoue lors d'une tentative d'authentification future.
...
* Insérer l'adresse IP pour la source/cible externe dans csv/pdf pour les violations d'abandon implicites.
...
* Diagramme Helm mis à jour pour permettre l'échange de certificats pour les communications internes
...
* Le verrouillage de la connexion après des tentatives de connexion échouées est désormais le paramètre par défaut. La valeur par défaut est de 5 tentatives et est configurable dans les paramètres -> Utilisateurs & Rôles-> Profil de mot de passe.
...
* Le renforcement des clusters Rancher et SELinux est pris en charge.

==== Corrections d'erreurs

* Processus d'agent intensif en CPU sur les systèmes k3s.
...
* L'opérateur OpenShift actuel prend en charge les routes pass-through pour les services API et de fédération. Des paramètres de valeur Helm supplémentaires sont ajoutés pour prendre en charge les types de fin de route Edge et de ré-encryption.

==== Corrections d'erreurs

* Le cluster AKS pourrait ajouter une clé inattendue dans le webhook de contrôle d'admission.
* L'enforcer ne sera pas opérationnel dans le cluster K8s 1.24 avec le runtime containerd 1.64. Séparément, l'enforcer échoue parfois à démarrer.
* Tout utilisateur avec le rôle d'administrateur (utilisateur local ou SSO) qui promeut un cluster en master-fed devrait être automatiquement promu au rôle de fedAdmin.
* Lorsque SSO utilise l'administrateur par défaut de Rancher dans {product-name} sur le cluster master, le rôle de connexion {product-name} est admin, pas fedAdmin.
...
* La correspondance des groupes LDAP n'est parfois pas affichée.
...
* Intégration SSO Rancher pour démarrer la {product-name} console via Rancher Manager. Cette fonction n'est disponible que si les conteneurs {product-name} sont déployés via Rancher. Ce déploiement concerne le dépôt miroir Rancher (z. B. rancher/mirrored-neuvector-controller:5.0.0) et est déployé dans l'espace de noms cattle-neuvector-system. REMARQUE : Nécessite une version mise à jour de Rancher 2.6.5 mai 2022 ou ultérieure, et actuellement seules les rôles Administrateur et Propriétaire de cluster sont pris en charge.
* Prend en charge le déploiement sur RKE2.
* Prend en charge la fédération de clusters (gestionnaire multi-clusters) via un proxy. Configurez le proxy dans les paramètres -> Configuration et activez le proxy lors de la configuration des connexions de fédération.
* Surveillez les rôles/liaisons de cluster RBAC requis et alertez dans les événements et l'interface utilisateur si quelque chose manque.
* Unterstützen Sie die Ressourcenbeschränkungsanforderungen in den Zulassungsprüfungsregeln.
* Prend en charge le format Microsoft Teams pour les webhooks.
...
* Permet à l'utilisateur de changer de mode lorsqu'il sélectionne tous les groupes dans le menu -> Groupes. Alertez lorsque le groupe de nœuds est également sélectionné.
...
Optionnel : Supprimez toutes les références à la {product-name} licence et au secret de registre dans les Helm-Charts, le YAML de déploiement, le ConfigMap, les scripts, etc., car ils ne sont plus nécessaires pour tirer les images ou pour utiliser {product-name}.

*Note sur SCC et mise à niveau via Operator/Helm*

Le SCC privilégié est ajouté au compte de service spécifié dans le YAML de déploiement par la version de l'opérateur 1.3.4 et supérieure dans les nouvelles déploiements. En cas de mise à niveau de l'opérateur {product-name} d'une version précédente vers 1.3.4 ou de Helm vers 2.0.0, veuillez supprimer le SCC privilégié avant la mise à niveau.
...
* CRD pour WAF, DLP et contrôles d'accès. REMARQUE : Zusätzliche Cluster-Rollenbindungen/Berechtigungen sind erforderlich. Voir les sections de déploiement Kubernetes et OpenShift. Import/Export de CRD et versionnage pour les contrôles d'accès pris en charge via CRD.
* Intégration SSO Rancher pour démarrer la {product-name} console via Rancher Manager. Cette fonction n'est disponible que si les conteneurs {product-name} sont déployés via Rancher. REMARQUE : Nécessite une version mise à jour de Rancher (date/version à déterminer).
* Prend en charge le déploiement sur RKE2.
...
* Le graphique Helm v1.8.9 est publié pour les déploiements 5.0.0. Si vous utilisez cela avec la version d'aperçu 5.0.0, les modifications suivantes doivent être apportées à values.yml :
 ** Mettez à jour le registre sur docker.io
 ** Mettez à jour les noms/étiquettes de l'image vers la version d'aperçu sur Docker Hub
