...
* *NVSHAS-9934*: Fonctionnalité suspectée de dérive zéro en panne.
...
* *NVSHAS-9945*: When the process name is too long, it is difficult to determine how to create the appropriate process profile rule.
* *NVSHAS-9946*: Display issue with the admission control alert for the credential type.
* *NVSHAS-9947*: [UI-ext] The compliance graph is missing the "Manual" status in Rancher NeuVector.
* *NVSHAS-9948*: Après la mise à niveau vers `+5.4.3+`, la configuration de NeuVector a été perdue.
* *NVSHAS-9949*: [Harbor][Incorrect username/password] It continues to scan images even with incorrect username/password input.
* *NVSHAS-9952*: Supprimer 'signature' du rapport d'utilisation car NV n'émet plus/vérifie la clé de licence.
* *NVSHAS-9953*: The Enforcer pods keep restarting.
...
* *NVSHAS-9960*: The scanners are not working.
* *NVSHAS-9969*: Fatal error: concurrent map writes cause the Enforcer component to restart.
...
* **NVSHAS-9729**: An incorrect number of vulnerabilities is observed when multiple statefulsets with the "same name" in the namespaces are deployed in a project.
* **NVSHAS-9810**: The NeuVector controller is unresponsive and the user interface is not accessible.

=== 5.4.2 January 2025

==== Nouvelles fonctionnalités

* **NVSHAS-9726**: The monitor now passes the proxy URL.
* **NVSHAS-9719**: Annonce la retraite des certificats intégrés.
* **NVSHAS-9715**: Helm Chart value support to set the nodeport on the controller and the manager.
* **NVSHAS-9710**: Include a sortable column `+feed_rating+` in the **Vulnerabilities** tab.
...
* **NVSHAS-9678**: Excessive error traces after linter changes.
* **NVSHAS-9670**: Manager: Double quotes issue in the plain text response and unnamed java library issue in sbt execution.
* **NVSHAS-9667**: Setting the environment variable `+CTRL_PATH_DEBUG+` to an error in the controller deployment does not work.
* **NVSHAS-9665**: Règle d'accès au fichier: La suppression des règles prédéfinies produit une erreur "setRowData".
* **NVSHAS-9664**: Policy group: La suppression du script personnalisé produit une erreur de type "setRowData".

[NOTE]
====
The default types for the manager and registry adapter service have been changed to ClusterIP.  Users can still override the parameter by replacing `+manager.svc.type+` and `+cve.adapter.svc.type+` if NodePort is preferred.
====

In NeuVector version 5.4.2, support is deprecated for deployments using the built-in internal certificate. Le certificat trouvé à `+/etc/neuvector/certs/internal+` dans les images de conteneur NeuVector 5.4.2 sera supprimé. To continue using NeuVector, users must:
...
* **NVSHAS-8676**: Improvement of NV Protect for benchmark scripts.
* **NVSHAS-9255**: Customize admission control search registers for image names without FQDN.
* **NVSHAS-9144**: ID added for vulnerability control file for easy identification.
* **NVSHAS-7687**: Support for logging level configuration (debug/error/info/warn) for executor and controller from command line interface.
* **NVSHAS-7518**: Change internal certificates for components {product-name}.
* **NVSHAS-9287**: Enable internal certificate rotation.
* **NVSHAS-8562**: Add internal certificate expiration notification.
* **NVSHAS-8486**: Support de l'interface réseau Multus.
* **NVSHAS-7447**: Rancher RBAC integration with {product-name}.
* **NVSHAS-7822**: Automation of federation without script API calls.
...
* **NVSHAS-9005**: TypeError in logs: Impossible de lire les propriétés de non défini (lecture de 'total_records').
* **NVSHAS-9085**: The PDF report of the asset view shows 0% vulnerability even with vulnerabilities present.
* **NVSHAS-9084**: The PDF report of the asset view shows NaN when the image list is empty.
* **NVSHAS-9128**: Événements de sécurité : The container cannot be displayed if there is no workload namespace value.
* **NVSHAS-9025**: Portée d'acceptation des vulnérabilités Neuvector pour les conteneurs.
* **NVSHAS-9155**: Incorrect column name and missing filename when parsing image log
* **NVSHAS-9122**: Neuvector main logs disconnect every time "Multiple Cluster" is used with Rancher SSO connection.
* **NVSHAS-9266**: Registry scan: The Layer Scan Report button should be hidden or disabled when there are no vulnerabilities.
* **NVSHAS-9219**: Allow users to enable server certificate validation for authentication servers.
* **NVSHAS-9246**: CSV/PDF export filtering is not working.
* **NVSHAS-8947**: Unable to import NV configuration when authenticating via Rancher SSO.
...
* **NVSHAS-9267**: Do not allow the UI on the main cluster 5.4 to switch to managed clusters before 5.4 due to REST API changes.
...
* **NVSHAS-8242**: gRPC call to test if the controller handles critical severity.
* **NVSHAS-8908**: Properly parse the X-Forwarded port considering the comma separator.
* **NVSHAS-9024**: Rôle risqué de contrôle d'admission perf.
* **NVSHAS-9091**: Unable to report all modules under ol:9.1, photon:5.0, rhel:9.1 and amzn:2023 source in the repository, registry, and standalone analysis.
...
* **NVSHAS-9108**: Port 18500 should not be open.
...
* **NVSHAS-7687**: Ajouter le paramètre de niveau de journal syslog pour l'enforcer.
...
* Allow when configuring a registry whether it should use the defined proxy for registry image scans.

==== Corrections de bogues

* Security risks > Vulnerabilities > The advanced filter does not filter 'CVE without fix'
* Unexpected container-to-container violation in host mode
* Accept OCI image format when switching to docker API 1.24
* The registry scan should not scan non-image artifacts / should not log an error
* Allow key pair image signature verification without root dependency on the internet or sigstore.
* Unauthorized security events by network rules in a specific node (related to the error messages "Container Task chan full")
...
* (Scanner) The registry scan should not scan non-image artifacts / should not log an error.
* (Scanner) Add support for scanning php composer.

==== {product-name} UI extension v. 1.0 for Rancher March 2024

* After installing {product-name}, xref:rancher.adoc#_suse_security_ui_extension_for_rancher[activation/installation] of the UI extension {product-name} for Rancher will display a dashboard for the cluster, including links to SSO to the full cluster {product-name}. REMARQUE : The extension may display as a third party, which will be fixed in a future version. Additionally, after installation, users of Rancher 2.7.x may see two icons {product-name} UI Ext in the list (bug). One icon will indicate Uninstall (meaning it is installed), and the other should say Install. This can remain as is, i.e., do not Install again if the extension is already installed.
...
==== Modifications mises en évidence qui peuvent nécessiter des changements pour les déploiements manuels (toutes les modifications sont déjà reflétées dans le dernier graphique Helm pour 5.3.x)
...
* Amélioration de l'utilisabilité : Utiliser le scanner pour analyser une seule image et imprimer le résultat (voir exemple ci-dessous).
...
* Inclure l'adresse IP pour la source/destination externe dans csv/pdf pour les violations de refus implicites
...
* Incident inattendu de Process.Profile.Violation dans le groupe NV.Protect sur lequel commande sur coreos.
...
* L'enforcer ne devient pas opérationnel sur le cluster k8s 1.24 avec le composant d'exécution containerd 1.64. Séparément, l'enforcer échoue parfois à démarrer.
...
Exemple ci-joint pour activer psp à partir du graphique Rancher et ci-dessous les commandes pour patcher le déploiement du Manager et du Scanner. L'ID utilisateur dans la commande de patch peut être n'importe quel numéro.
...
* Protection des processus et des fichiers sans dérive. C'est le nouveau mode par défaut pour les protections de processus et de fichiers. Zero-drift permet automatiquement uniquement les processus qui proviennent du processus parent qui se trouve dans l'image de conteneur d'origine, et n'autorise pas les mises à jour de fichiers ou l'installation de nouveaux fichiers. Lorsque vous êtes en mode Découverte ou Surveillance, le zéro-dérive alertera sur toute activité de processus ou de fichier suspecte. En mode Protection, il bloquera une telle activité. Zero drift does not require processes to be learned or added to an allow list. Disabling zero drift for a group will ensure that the listed process and file rules for the group take effect instead.
* Protection en mode politique divisée pour le réseau, le processus/fichier. There is now a global setting available in the Settings -> Configuration to separately set the network protection mode for applying network rules. Enabling this (disabled by default) ensures that all network rules are in the selected protection mode (Discovery, Monitoring, Protection), while the process/file rules remain in the protection mode for that group, as shown in the Policy -> Groups screen. In this way, network rules can be set to Protection (blocking), while the process/file policy can be set to Monitoring, or vice versa.
* Détection de règles WAF, règles DLP améliorées (en-tête, URL, paquet complet). Used for incoming connections to web application pods as well as for outgoing connections to API services to apply API security.
* CRD for WAF, DLP, and admission controls. NOTE: additional cluster role bindings/permissions are required. Voir les sections de déploiement Kubernetes et OpenShift. Import/export of CRD and versioning for supported admission controls via CRD.
* Rancher SSO integration to launch {product-name} console via Rancher Manager. Cette fonctionnalité n'est disponible que si les conteneurs {product-name} sont déployés via Rancher. This deployment pulls from the Rancher mirror repository (e.g., rancher/mirrored-neuvector-controller:5.0.0) and deploys into the cattle-neuvector-system namespace. REMARQUE : Requires an updated version of Rancher 2.6.5 May 2022 or later, and only administrator and cluster owner roles are supported at this time.
* Supports deployment on RKE2.
* Prise en charge de la fédération de clusters (gestionnaire multi-cluster) via un proxy. Configure the proxy in the -> Configuration settings, and enable the proxy when configuring federation connections.
* Monitor required cluster roles/rbac bindings and alert in events and the UI if any are missing.
...
==== Version Preview.3 released in March 2022

[CAUTION]
.important
====

To update previous preview deployments for the new WAF CRD, DLP, and admission control features, please update the CRD yaml and add new rbac/role bindings:
...
* CRD for WAF, DLP, and admission controls. NOTE: additional cluster role bindings/permissions are required. Voir les sections de déploiement Kubernetes et OpenShift. Import/export of CRD and versioning for supported admission controls via CRD.
* Rancher SSO integration to launch {product-name} console via Rancher Manager. Cette fonctionnalité n'est disponible que si les conteneurs {product-name} sont déployés via Rancher. REMARQUE : Nécessite une version mise à jour de Rancher (date/version à déterminer).
* Supports deployment on RKE2.
...
==== Version Preview.2 released in February 2022
...
* Add support for OWASP Top-10, similar WAF rules to detect network attacks in headers or body. Includes support for CRD signature definitions and application to appropriate groups.
* Removes Serverless scan features.
...
* Le graphique Helm v1.8.9 est publié pour les déploiements 5.0.0. If you are using this with the preview version of 5.0.0, the following changes must be made to values.yml:
