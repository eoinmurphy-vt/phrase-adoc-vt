...
Vous pouvez utiliser Kubernetes pour déployer des conteneurs de gestion, de contrôleur et d'exécuteur séparés et vous assurer que tous les nouveaux nœuds ont un exécuteur déployé. {product-name} nécessite et prend en charge les plugins réseau Kubernetes tels que flannel, weave ou calico.

Le fichier d'exemple déploiera un gestionnaire et 3 contrôleurs. Il déploiera un exécuteur sur chaque nœud en tant que DaemonSet. Par défaut, l'exemple suivant sera également déployé sur le nœud maître.

Voir la section inférieure pour spécifier des nœuds de gestionnaire ou de contrôleur dédiés à l'aide de balises de nœud. 

[NOTE]
====
Il n'est pas recommandé de déployer plus d'un gestionnaire derrière un équilibreur de charge (scalabilité), car cela peut entraîner des problèmes potentiels avec l'état de session. Si vous prévoyez d'utiliser une demande PersistentVolume pour stocker la sauvegarde des fichiers de configuration {product-name}, veuillez lire la section générale Backup/Données persistantes dans l'aperçu xref:production.adoc#_backups_and_persistent_data[Déploiement {product-name}].
====

Si votre déploiement prend en charge un équilibreur de charge intégré, changez le type NodePort en LoadBalancer pour la console dans le fichier yaml suivant.

{product-name} prend en charge le déploiement basé sur Helm avec un diagramme Helm à https://github.com/neuvector/neuvector-helm.

Il existe une section séparée pour les instructions OpenShift, et Docker EE sur Kubernetes a quelques étapes spécifiques décrites dans la section Docker.

=== {product-name} images sur Docker Hub

Les images se trouvent dans le registre Docker Hub {product-name}. Utilisez le tag de version approprié pour le gestionnaire, le contrôleur, l'exécuteur et laissez la version comme 'latest' pour le scanner et la mise à jour. Par exemple :
...
Si vous déployez avec le diagramme Helm actuel {product-name} (v1.8.9+), les modifications suivantes doivent être apportées à values.yml :

* Mettez à jour le dépôt sur docker.io
* Mettez à jour les noms/étiquettes des images vers la version actuelle sur Docker Hub, comme indiqué ci-dessus
* Laissez les imagePullSecrets vides

[NOTE]
====
Si vous déployez à partir du diagramme Rancher Manager 2.6.5+ {product-name}, les images seront automatiquement récupérées depuis le dépôt d'images miroir du registre Rancher et déployées dans l'espace de noms cattle-neuvector-system.
====
...
----
--
. (*Optionnel*) Créez l'approbation de sécurité des pods (PSA) ou la politique de sécurité des pods (PSP) {product-name}. Si vous avez activé l'approbation de sécurité des pods (également connue sous le nom de normes de sécurité des pods) dans Kubernetes 1.25+ ou les politiques de sécurité des pods (avant 1.25) dans votre cluster Kubernetes, ajoutez ce qui suit pour {product-name} (z. B. nv_psp.yaml). 
+
[NOTE]
====
* La PSP est obsolète dans Kubernetes 1.21 et sera complètement supprimée dans 1.25. 
* Les pods Manager et Scanner fonctionnent sans uid. Si votre PSP a une règle `+Run As User: Rule: MustRunAsNonRoot+`, ajoutez ce qui suit dans l'exemple YAML ci-dessous (avec la valeur appropriée pour `+###+`) :
====
+
--
[,yaml]
...
Pour la PSA dans Kubernetes 1.25+, marquez l'espace de noms {product-name} avec un profil privilégié pour le déploiement dans un cluster activé pour la PSA. 
[,shell]
----
...
[IMPORTANT]
====
Le déploiement par défaut {product-name} 5.2+ utilise des comptes de service avec des autorisations minimales au lieu des comptes par défaut. Voir ci-dessous si vous mettez à niveau à partir d'une version antérieure à 5.3. 
====
+
[WARNING]
====
...
Appliquez les autorisations de lecture via les commandes "Créer ClusterRole" suivantes : 
...
----
--
. (*Optionnel*) Créez le maître de fédération et/ou les services de gestion multi-clusters distants. Si vous souhaitez utiliser les fonctionnalités de gestion multi-clusters dans {product-name}, un cluster doit avoir fourni le service maître de fédération, et chaque cluster distant doit avoir le service de travailleur de fédération. Pour plus de flexibilité, vous pouvez choisir de fournir à la fois les services maîtres et les services de travailleurs dans chaque cluster, de sorte que chaque cluster puisse être un maître ou distant. Gestion de Cluster Fédéré 
+
--
[,yaml]
----
...
Créez ensuite le(s) service(s) correspondant(s) : 
...
----
--
. Créez les services principaux et les pods {product-name} avec les commandes de version prédéfinies ou modifiez l'exemple YAML suivant. La version prédéfinie appelle un LoadBalancer pour la console {product-name}. Si vous utilisez le fichier YAML d'exemple suivant, remplacez les noms d'image et les balises <version> pour les références d'image du gestionnaire, du contrôleur et de l'exécuteur dans le fichier YAML. Apportez également toutes les autres modifications nécessaires pour votre environnement de déploiement (comme LoadBalancer/NodePort/Ingress pour accéder au gestionnaire, etc.). Le YAML suivant doit être modifié pour les changements de certificats internes s'il est fourni par v5.4.2 ou supérieur. Référez-vous à ce <<_kubernetes_deployment_yaml_for_v5_4_2_onwards,YAML>>.
+
--
[,shell]
----
...
Ou, si vous modifiez l'un des fichiers YAML ci-dessus ou les exemples suivants : 
...
C'est tout ! Vous devriez être en mesure de vous connecter avec admin:admin, par exemple `+https://<public-ip>:8443+`, à la console {product-name}.
--

[NOTE]
====
Le service NodePort spécifié dans le fichier neuvector.yaml ouvre un port aléatoire sur tous les nœuds Kubernetes pour le port de la console de gestion web {product-name}. Alternativement, vous pouvez utiliser un LoadBalancer ou Ingress en utilisant une IP publique et le port par défaut 8443. Pour NodePort, assurez-vous que l'accès via les pare-feu pour ce port, si nécessaire, est ouvert. Si vous souhaitez voir quel port est ouvert sur le nœud hôte, exécutez les commandes suivantes :
...
[NOTE]
====
PKS est testé sur le terrain et nécessite l'activation de conteneurs privilégiés pour le plan/Tiles et l'ajustement du YAML hostPath comme suit pour All-in-One, Controller, Enforcer :
...
*Master Node Taints and Tolerances*

Toutes les informations de Taint doivent correspondre pour planifier les Enforcers sur les nœuds. Pour vérifier les informations de Taint sur un nœud (par exemple : B. Master) :
...
S'il y a des Taints supplémentaires comme ci-dessus, ajoutez-les à la section Tolerances YAML de l'exemple :
...
== Utilisation des labels de nœud pour les nœuds Manager et Controller

Pour contrôler sur quels nœuds le Manager et le Controller sont déployés, étiquetez chaque nœud. Remplacez nodename par le nom de nœud approprié ('\`+kubectl get nodes\+`'). Remarque : Par défaut, Kubernetes ne planifie pas de Pods sur le nœud Master.
...
Pour empêcher l'Enforcer d'être déployé sur un nœud Controller, s'il s'agit d'un nœud de gestion dédié (sans surveiller les conteneurs d'application), ajoutez une nodeAffinity à la section YAML de l'Enforcer. Par exemple :
...
Des outils d'orchestration comme Kubernetes, RedHat OpenShift et Rancher prennent en charge les mises à jour progressives avec des politiques configurables. Vous pouvez utiliser cette fonctionnalité pour mettre à jour les conteneurs {product-name}. L'essentiel sera de s'assurer qu'au moins un Controller (ou All-in-One) fonctionne, afin que les politiques, les journaux et les informations d'identification ne soient pas perdues. Assurez-vous qu'il y a au moins 120 secondes entre les mises à jour des conteneurs afin qu'un nouveau leader puisse être élu et que les données soient synchronisées entre les contrôleurs.

Les fichiers de déploiement YAML d'exemple fournis configurent déjà la stratégie de mise à jour continue. Lorsque vous mettez à jour via le chart Helm {product-name}, veuillez tirer le dernier chart pour configurer correctement les nouvelles fonctionnalités telles que le contrôle d'admission, et supprimer l'ancienne rôle de cluster et le lien de rôle de cluster pour {product-name}. Lorsque vous mettez à jour via Kubernetes, vous pouvez manuellement mettre à jour vers une nouvelle version avec les exemples de commandes suivants.

=== Exemple de mise à jour continue de Kubernetes

Pour les mises à niveau qui nécessitent simplement une mise à jour vers une nouvelle version d'image, vous pouvez utiliser cette approche simple.

Si votre déploiement ou DaemonSet est déjà en cours d'exécution, vous pouvez modifier le fichier YAML vers la nouvelle version et ensuite appliquer la mise à jour :
...
Pour le contrôleur en tant que déploiement (également à faire pour le gestionnaire)
...
Pour chaque conteneur en tant que DaemonSet :
...
Pour exposer l'API REST pour un accès depuis l'extérieur du cluster Kubernetes, voici un exemple de fichier YAML :
...
Les instructions suivantes peuvent être utilisées pour déployer {product-name} sans utiliser de conteneurs en mode privilégié. Le contrôleur fonctionne déjà en mode non privilégié, et le déploiement de l'enforcer doit être modifié, comme indiqué dans les extraits suivants.
...
L'exemple YAML suivant s'applique aux versions 5.4.2 et supérieures, où nous devons fournir les certificats internes sur les Pods Controller, Enforcer et Scanner, car nous ne supportons plus les certificats codés en dur. Créez le secret de certificat interne à partir du lien fourni avant de déployer : xref:internal.adoc[Remplacement des certificats internes].
...
[NOTE]
====
PKS est testé sur le terrain et nécessite l'activation de conteneurs privilégiés pour le plan/tuile et la modification du YAML hostPath comme suit pour Allinone, Enforcer:
