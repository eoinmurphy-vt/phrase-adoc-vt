...
== Important: The deployment on Amazon ECS is no longer supported

The reference section below is no longer maintained. However, it may provide some support to understand how to deploy the All-in-One on ECS.
...
--
. Créez la définition de tâche All-in-One. Créez une nouvelle définition de tâche pour le conteneur All-in-One. Vous pouvez utiliser l'interface ECS pour le créer manuellement ou insérer le fichier JSON d'exemple (voir ci-dessous pour des exemples). Référez-vous à la section "\`+1. Fournissez {product-name}\+`" de cette documentation pour savoir comment configurer l'All-in-One.
+
--
...
====
--
. Créez un nouveau service pour la tâche All-in-One. Définissez les "\`+Modèles de Placement\+`" sur "\`+Une Tâche Par Hôte\+`", afin qu'un seul All-in-One/Contrôleur puisse s'exécuter sur un hôte. Vous verrez également que la contrainte "`membreDe(attribute:allinone-node=~true)" est utilisée, ce qui exige que le nœud ait cet attribut.
+
--
image:3taskplacement.png[AllinonePlace]
--
. Vous pouvez maintenant déployer le service All-in-One. Définissez le "\`+Nombre de tâches\+`" sur le nombre souhaité d'All-in-One/Contrôleurs. Les {product-name} conteneurs All-in-One ou Contrôleurs seront maintenant lancés sur les nœuds sélectionnés. Une fois que l'All-in-One fonctionne, vous devriez être en mesure d'accéder à la {product-name} console via HTTPS sur le port 8443.
...
--
. Créez un nouveau service pour la tâche Enforcer. Réinitialisez le placement des tâches sur "\`+Une tâche par hôte\+`", afin qu'un seul Enforcer soit déployé sur chaque hôte. Notez également que la restriction supplémentaire montre que le déploiement sur un nœud tout-en-un est empêché.
+
--
image:5taskplacement.png[EnforcerPlacement]

Déployez ce service avec le nombre souhaité de nœuds d'application dans "\`+Nombre de tâches\+`". Dans un très court laps de temps, tous les Enforcers seront opérationnels. Depuis la console {product-name}, vous pouvez voir tous les nœuds détectés avec des Enforcers.
--
...
. Arrêtez et mettez à jour les Enforcers des services ECS. Tirez manuellement ou automatiquement les nouvelles images des Enforcers. Démarrez ou mettez ensuite à jour l'Enforcer sur tous les nœuds. Depuis la console {product-name}, vous pouvez voir que tous les Enforcers sont à jour.
