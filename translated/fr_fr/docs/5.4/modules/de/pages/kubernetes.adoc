...
Vous pouvez utiliser Kubernetes pour déployer des conteneurs de gestion, de contrôle et d'application distincts et vous assurer que tous les nouveaux nœuds ont un conteneur d'application déployé. {product-name} nécessite et prend en charge des plugins réseau Kubernetes tels que Flannel, Weave ou Calico.

Le fichier d'exemple déploiera un gestionnaire et 3 contrôleurs. Il déploiera un conteneur d'application en tant que Daemonset sur chaque nœud. Par défaut, l'exemple ci-dessous sera également déployé sur le nœud maître.

Voir la section ci-dessous pour spécifier des nœuds de gestion ou de contrôle dédiés à l'aide d'étiquettes de nœud. 

[NOTE]
====
Il n'est pas recommandé de déployer plus d'un gestionnaire derrière un équilibreur de charge (scalabilité), car cela peut entraîner des problèmes potentiels avec l'état de session. Si vous prévoyez d'utiliser une revendication PersistentVolume pour stocker la sauvegarde des fichiers de configuration {product-name}, veuillez lire la section générale Sauvegarde/Données persistantes dans l'aperçu xref:production.adoc#_backups_and_persistent_data[Déploiement {product-name}].
====

Si votre déploiement prend en charge un équilibreur de charge intégré, changez le type de NodePort à LoadBalancer pour la console dans le fichier yaml ci-dessous.

{product-name} prend en charge le déploiement basé sur Helm avec un graphique Helm sous https://github.com/neuvector/neuvector-helm.
...
Les images se trouvent dans le registre Docker Hub {product-name}. Utilisez le tag de version approprié pour le gestionnaire, le contrôleur, l'application et laissez la version pour le scanner et le mise à jour comme 'latest'. Par exemple :
...
Veuillez vous assurer que vous mettez à jour les références d'image dans les fichiers YAML appropriés.
...
[NOTE]
====
Lorsque vous déployez avec Rancher Manager 2.6.5+ {product-name} Chart, les images sont automatiquement répliquées depuis le registre Rancher, et le déploiement se fait dans l'espace de noms cattle-neuvector-system.
====
...
[NOTE]
====
* PSP est obsolète dans Kubernetes 1.21 et sera complètement supprimé dans 1.25. 
...
[IMPORTANT]
====
Le déploiement par défaut {product-name} 5.2+ utilise des comptes de service avec les moindres privilèges au lieu du compte par défaut. Voir ci-dessous si vous mettez à jour depuis une version antérieure à 5.3. 
====
+
[WARNING]
====
Si vous mettez à jour vers 5.3.0+, exécutez les commandes suivantes en fonction de votre version actuelle :
====
+
--
[tabs]
...
----
--
. (*Optionnel*) Créez le Master de Fédération et/ou les Services de Gestion Multi-Cluster à Distance. Si vous souhaitez utiliser les fonctionnalités de gestion multi-cluster dans {product-name}, un cluster doit avoir fourni le service Master de Fédération, et chaque cluster distant doit avoir le service Worker de Fédération. Pour plus de flexibilité, vous pouvez choisir de fournir à la fois des services Master et Worker dans chaque cluster, de sorte que chaque cluster puisse être un Master ou un Distant. Gestion de Cluster Fédéré 
+
--
[,yaml]
----
...
Créez ensuite les services correspondants : 
...
----
--
. Créez les services et Pods principaux {product-name} avec les commandes de version prédéfinies ou modifiez l'exemple YAML ci-dessous. La version prédéfinie appelle un LoadBalancer pour la console {product-name}. Si vous utilisez le fichier YAML d'exemple ci-dessous, remplacez les noms d'image et les tags <version> pour les références d'image du Manager, du Contrôleur et de l'Application dans le fichier YAML. Apportez également toutes les autres modifications nécessaires pour votre environnement de déploiement (comme LoadBalancer/NodePort/Ingress pour l'accès au Manager, etc.). Le YAML ci-dessous doit être modifié pour les changements de certificats internes s'il est fourni par v5.4.2 ou supérieur. Référez-vous à ce <<_kubernetes_deployment_yaml_for_v5_4_2_onwards,YAML>>.
+
--
[,shell]
----
...
C'est tout ! Vous devriez être en mesure de vous connecter à la console {product-name} et de vous connecter avec admin:admin, par exemple `+https://<public-ip>:8443+`.
--

[NOTE]
====
Le service nodeport spécifié dans le fichier neuvector.yaml ouvre un port aléatoire sur tous les nœuds Kubernetes pour le port de la console Web de gestion {product-name}. Alternativement, vous pouvez utiliser un LoadBalancer ou Ingress en utilisant une IP publique et le port par défaut 8443. Pour nodeport, assurez-vous que l'accès via les pare-feu pour ce port est ouvert, si nécessaire. Si vous souhaitez voir quel port est ouvert sur le nœud hôte, veuillez exécuter les commandes suivantes :
...
[NOTE]
====
PKS est testé sur le terrain et nécessite l'activation de conteneurs privilégiés pour le plan/tuile et la modification du yaml hostPath comme suit pour Allinone, Controller, Enforcer :
...
*Taints et tolérances des nœuds maîtres*

Toutes les informations de taint doivent correspondre pour planifier l'Enforcer sur les nœuds. Pour vérifier les informations de taint sur un nœud (par exemple. Maître) :
...
S'il y a des taints supplémentaires comme ci-dessus, ajoutez-les par exemple à la section tolérances yaml :
...
Pour contrôler sur quel nœud le Manager et le Controller sont déployés, étiquetez chaque nœud. Remplacez nodename par le nom de nœud approprié ('\`+kubectl get nodes\+`'). Remarque : Par défaut, Kubernetes ne planifie pas de pods sur le nœud maître.
...
Des outils d'orchestration comme Kubernetes, RedHat OpenShift et Rancher prennent en charge les mises à jour progressives avec des politiques configurables. Vous pouvez utiliser cette fonctionnalité pour mettre à jour les {product-name} conteneurs. Il est important qu'au moins un contrôleur (ou tout-en-un) soit en fonctionnement afin que les politiques, les protocoles et les données de connexion ne soient pas perdues. Assurez-vous qu'il y a au moins 120 secondes entre les mises à jour des conteneurs afin qu'un nouveau leader puisse être élu et que les données soient synchronisées entre les contrôleurs.

Les exemples de fichiers de déploiement YAML fournis configurent déjà la politique pour les mises à jour progressives. Lorsque vous mettez à jour via le chart Helm {product-name}, veuillez tirer le dernier chart pour configurer correctement de nouvelles fonctionnalités telles que le contrôle d'admission, et supprimer l'ancien rôle de cluster et le lien de rôle de cluster pour {product-name}. Lorsque vous mettez à jour via Kubernetes, vous pouvez manuellement mettre à jour vers une nouvelle version avec les exemples de commandes suivants.

=== Exemple de mise à jour progressive Kubernetes

Pour les mises à niveau qui doivent simplement être mises à jour vers une nouvelle version d'image, vous pouvez utiliser cette approche simple.
...
Pour les contrôleurs en tant que déploiement (à faire également pour le gestionnaire)
...
== Déployer l'API REST dans Kubernetes

Pour exposer l'API REST pour un accès en dehors du cluster Kubernetes, voici un exemple de fichier yaml :
...
Les instructions suivantes peuvent être utilisées pour déployer {product-name} sans utiliser de conteneurs en mode privilégié. Le contrôleur est déjà en mode non privilégié, et le déploiement de l'application doit être modifié, comme le montrent les extraits ci-dessous.
...
== YAML de déploiement Kubernetes pour v5.4.2 et supérieur

L'exemple de YAML suivant est pour les versions 5.4.2 et supérieures, où nous devons intégrer les certificats internes dans les pods Controller, Enforcer et Scanner, car nous ne supportons plus les certificats codés en dur. Créez le secret de certificat interne à partir du lien fourni avant de déployer : xref:internal.adoc[Remplacer les certificats internes].
...
[NOTE]
====
PKS est testé sur le terrain et nécessite l'activation des conteneurs privilégiés pour le plan/le tile et la modification du YAML hostPath comme suit pour All-in-One, Enforcer :
