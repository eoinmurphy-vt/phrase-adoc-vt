...
Les ConfigMaps et la sauvegarde du xref:production.adoc#_backups_and_persistent_data[stockage permanent] ne sont lus que lorsqu'un nouveau cluster {product-name} est déployé, ou lorsque le cluster tombe en panne et est redémarré. Ils ne sont pas utilisés lors des mises à niveau continues.
