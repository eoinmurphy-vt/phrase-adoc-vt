...
Les registres fédérés ne peuvent être configurés que par un administrateur fédéré sur le cluster principal dans Actifs -> Registres. Après avoir ajouté et scanné un dépôt fédéré, les résultats de numérisation seront synchronisés à tous les clusters gérés. Les règles de contrôle d'admission dans chaque cluster géré qui nécessitent un scan d'image (par exemple, les CVE, les règles basées sur la conformité) utiliseront automatiquement à la fois les résultats de scan fédérés ainsi que les résultats de scan de registre configurés localement.
