...
* Utilisez une demande de volume persistant pour stocker la {product-name} configuration au cas où tous les contrôleurs/nœuds tomberaient en panne dans le cluster.
...
Il peut être utile d'inspecter les journaux des outils d'orchestration pour voir toute l'activité de déploiement, y compris les horodatages de création de pod et l'état, les déploiements, les daemonsets et d'autres actions de gestion des conteneurs {product-name} effectuées par l'outil d'orchestration.
