...
Le conteneur de scanner la dernière base de données CVE et est régulièrement mis à jour (avec le tag 'latest') par {product-name}. Le mise à jour redéploie le scanner, forçant un tirage de la dernière image de scanner afin d'obtenir la dernière base de données CVE. Voir la section xref:updating.adoc[Mettre à jour la base de données CVE] pour plus de détails sur le mise à jour.
...
Les résultats de scan de tous les scanners sont affichés dans le menu Actifs -> Registries. Des fonctionnalités de surveillance supplémentaires des scanners seront ajoutées dans les futures versions.
