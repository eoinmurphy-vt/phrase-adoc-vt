...
[upperalpha]
. Voir le &quot;`+gcr.io+`&quot; sous Nom d'hôte ? C'est ce qui appartient à l'élément n°2, Registre dans l'interface {product-name}. (N'oubliez pas la partie \https://)
...
Avant de vous exciter trop, il y a une chose de plus à assurer. Pour que le scanner dans {product-name} utilise l'API pour scanner et protéger vos images, ladite API doit être activée dans votre compte GCP. Vous pouvez soit l'activer via la ligne de commande via
