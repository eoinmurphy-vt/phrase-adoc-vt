...
* **NVSHAS-9267**: Ne pas permettre à l'interface utilisateur sur le cluster principal 5.4 de passer aux clusters gérés avant 5.4 en raison des modifications de l'API REST.
...
* Ajouter le support pour la plateforme ARM64. Le téléchargement depuis des plateformes basées sur ARM tirera automatiquement les images ARM64 {product-name} appropriées.
...
* La détection automatique du runtime de conteneur (socket) supprime le besoin de spécifier le runtime de conteneur et le chemin du socket.
...
* Les évaluations de configuration contre le contrôle d'admission doivent montrer toutes les violations avec un seul scan.
...
. Mettre à jour les noms et chemins d'image pour tirer {product-name} des images depuis Docker hub (docker.io), par exemple.
...
En option, supprimer toute référence à la {product-name} licence et au secret de registre dans les charts Helm, le yaml de déploiement, le configmap, les scripts, etc., car ceux-ci ne sont plus nécessaires pour tirer les images ou pour commencer à utiliser {product-name}.
