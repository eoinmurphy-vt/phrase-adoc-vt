...
== Utilisation de la {product-name} interface de ligne de commande

La solution {product-name} prend en charge un ensemble limité de fonctions utilisant l'interface de ligne de commande. L'interface de ligne de commande est prise en charge par le gestionnaire, qui utilise à son tour une RestAPI pour émettre des commandes au contrôleur. Le contrôleur gère ensuite les enforceurs de manière appropriée. Un ensemble complet d'opérations est pris en charge via l'API REST, qui peut être exposée directement depuis le contrôleur. Vous pouvez accéder à l'interface de ligne de commande {product-name} en tapant la commande cli pour le gestionnaire ou Allinone, par exemple :
...
Bien que l'interface de ligne de commande soit disponible via le gestionnaire, nous recommandons d'utiliser directement l'xref:automation.adoc[API REST] dans le contrôleur pour les requêtes et l'automatisation.
...
Utilisez le même utilisateur/mot de passe que vous utilisez pour la console.
...
== Référence de ligne de commande & Commandes
...
=== Afficher/déboguer les commandes
...
[NOTE]
====
Les fichiers de l'analyseur sont stockés dans le répertoire /var/neuvector/pcap dans le conteneur Enforcer. Assurez-vous de mapper le volume à votre répertoire de machine invitée ou à votre répertoire système local pour pouvoir accéder aux fichiers. Par exemple, dans le fichier docker-compose, ajoutez &apos;`+- /var/neuvector:/var/neuvector+`&apos; dans les volumes.
====

Pour commencer la capture de paquets sur un pod, vous devrez connaître l'ID du conteneur à passer dans le champ ID_OR_NAME. Vous pouvez le faire avec `+show container -c <container_name>+`. puis démarrez l'analyseur avec `+request sniffer start <container_id>+`. Par exemple,
...
[IMPORTANT]
====
Si la durée n'est pas définie, vous devrez trouver l'ID de l'analyseur afin d'arrêter l'analyseur.  Pour ce faire, `+show sniffer -c <containerID>+`.  Suivez par `+request sniffer stop <sniffer_ID>+`.
====
