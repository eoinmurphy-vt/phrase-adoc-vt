...
. Clé JSON - Comme il est assez évident, ce sera une clé au format JSON. Et, comme vous le voyez probablement, nous allons le trouver via GCP.
...
Une fois que vous avez saisi les données pour la première étape de création d'un compte de service, vous devez appuyer sur le bouton &quot;`+CREATE+`&quot; pour que l'étape 2 soit prête à accepter l'entrée.

image:gcr5.png[GCR]

Assurez-vous de sélectionner --> Visionneur de base pour l'accès. Si vous avez un compte de service existant, assurez-vous que l'accès est configuré de cette manière. (Indice : Même les autorisations d'accès qui semblent plus puissantes ne semblent pas permettre un accès approprié. Ne sautez pas cette étape.
