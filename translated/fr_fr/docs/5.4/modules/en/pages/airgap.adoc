= Isolation de l'air {product-name}
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /02.deploying/09.airgap/09.airgap.md
:page-opendocs-slug: /déploiement/isolationdelair
...
* https://github.com/containers/skopeo[Skopeo] - Outil d'image/répertoire
...
== Obtenir des images et un graphique

Pour obtenir toutes les images, nous allons utiliser le graphique lui-même. En utilisant https://helm.sh/[Helm], ajoutons le dépôt et téléchargeons le graphique. Nous allons également utiliser https://github.com/containers/skopeo[skopeo] pour télécharger et télécharger.
...
Vous devriez maintenant voir un fichier comme `+core-2.4.0.tgz+`. La version peut varier, mais c'est correct. Ceci est le graphique téléchargé. Maintenant, nous avons besoin des images. Heureusement, nous pouvons utiliser le graphique pour le découvrir.
...
Il suffit maintenant de déplacer le 400M `+neuvector_airgap.zst+` vers votre réseau.
...
Charger les images dans un registre nécessitera une compréhension de votre réseau interne. Pour ce document, utilisons "registry.awesome.sauce" comme nom DNS. Charger les images est assez simple encore une fois avec `+skopeo+`. Veuillez vous assurer qu'il est installé sur la machine "intérieure". Vous aurez probablement besoin de vous authentifier avec `+skopeo login+` pour que cela fonctionne.
