...
----
+
* Modifiez ensuite les fichiers de déploiement YAML du contrôleur, de l'enforcer et du scanner en ajoutant :
+
[source,yaml]
----
      containers:
...
----
+
Ensuite, continuez à {product-name} comme précédemment. Vous pouvez également accéder à l'interpréteur de commandes dans les Pods Controller/Enforcer/Scanner pour confirmer que les fichiers ca.crt, tls.key, tls.crt sont personnalisés et que les communications {product-name} fonctionnent avec les nouveaux certificats.
+
...
+
* Suivez les étapes dans l'onglet `+New certificate+` pour utiliser un secret Kubernetes afin de gérer le certificat interne. Au lieu de générer un nouveau certificat, utilisez le certificat, `+old-ca.crt+`, `+old-tls.crt+` et `+old-tls.key+`, qui a été récupéré ci-dessous :
+
[source,shell]
----
