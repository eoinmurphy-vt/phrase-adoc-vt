...
Vous pouvez utiliser Kubernetes pour déployer des conteneurs de gestion, de contrôleur et d'exécuteur séparés et vous assurer que tous les nouveaux nœuds ont un exécuteur déployé. {product-name} nécessite et prend en charge des plugins réseau Kubernetes tels que flannel, weave ou calico.
...
Voir la section ci-dessous pour spécifier des nœuds de gestionnaire ou de contrôleur dédiés à l'aide de balises de nœud. 
...
[NOTE]
====
Si vous déployez à partir du Rancher Manager 2.6.5+ {product-name} Chart, les images seront automatiquement tirées du dépôt d'images miroir du registre Rancher et déployées dans l'espace de noms cattle-neuvector-system.
====
...
Pour PSA dans Kubernetes 1.25+, marquez l'espace de noms {product-name} avec un profil privilégié pour le déploiement dans un cluster activé pour PSA. 
[,shell]
----
...
C'est tout ! Vous devriez être en mesure de vous connecter à la console {product-name} avec admin:admin, par exemple `+https://<public-ip>:8443+`.
--

[NOTE]
====
Le service NodePort spécifié dans le fichier neuvector.yaml ouvre un port aléatoire sur tous les nœuds Kubernetes pour le port de la console Web de gestion {product-name}. Alternativement, vous pouvez utiliser un LoadBalancer ou Ingress en utilisant une IP publique et le port par défaut 8443. Pour NodePort, assurez-vous que l'accès via les pare-feu pour ce port est ouvert, si nécessaire. Si vous souhaitez voir quel port est ouvert sur le nœud hôte, exécutez les commandes suivantes :
...
*Master-nœud Taints et Tolerances*
...
S'il y a des Taints supplémentaires comme ci-dessus, ajoutez-les à la section YAML des Tolerances dans l'exemple :
...
Pour revenir à la mise à jour précédente :
...
Les instructions suivantes peuvent être utilisées pour déployer {product-name} sans utiliser de conteneurs en mode privilégié. Le contrôleur fonctionne déjà en mode non privilégié, et le déploiement de l'Enforcer doit être modifié, comme le montrent les extraits suivants.
