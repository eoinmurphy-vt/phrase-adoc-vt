= Test_TwoRapports et Notifications
:revdate: 2025-05-02
...
Les résultats de l'analyse des vulnérabilités et des benchmarks CIS pour les registres, conteneurs, nœuds et plateformes peuvent également être téléchargés sous forme de fichiers CSV à partir de leurs menus respectifs dans les sections de menu Actifs.
...
Tous les événements sont stockés en mémoire pour affichage dans les écrans Tableau de bord et Notifications. Il est prévu que les événements soient envoyés via SYSLOG, webhook ou d'autres moyens pour être stockés et gérés par un système SIEM. Il y a actuellement une limite de 4K sur chaque type d'événement ci-dessous :
...
=== Test_TwoSortie SYSLOG

==== Test_TwoViolation Réseau
...
=== Test_TwoIntégration avec Splunk
...
== Test_TwoNotifications et Journaux
...
=== Test_TwoÉvénements de sécurité

Les violations sont des connexions qui violent les règles de la liste blanche ou correspondent à une règle de liste noire. Les violations réseau sont capturées et les adresses IP sources peuvent être examinées plus en détail. Les événements de violation de la liste blanche apparaissent comme "La règle de refus implicite est violée" pour indiquer que la connexion réseau ne correspondait à aucune règle de liste blanche.
...
==== Test_TwoAjouter de nouvelles règles pour les événements de sécurité
...
image:security_events_review.png[RevueRègle]
...
* Fichier. Violation d'accès au fichier. Soit un fichier/répertoire sensible surveillé a été accédé (voir la liste de la surveillance par défaut) ou une règle de surveillance de fichier personnalisée a été déclenchée. Reportez-vous à la page xref:./filerules.adoc[Règles d'Accès aux Fichiers] pour plus d'informations.
...
Des alertes Webhook peuvent être envoyées en configurant le point de terminaison du web hook dans les Paramètres -> Configuration. Ensuite, créez la ou les règles de réponse appropriées dans le menu Politique -> Règles de réponse pour sélectionner le type d'événement et le webhook comme action.
