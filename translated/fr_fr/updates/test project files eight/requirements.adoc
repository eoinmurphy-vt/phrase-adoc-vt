...
Le scan d'image du registre est effectué par le scanner et géré par le contrôleur, et l'image est récupérée par le scanner et étendue en mémoire. +
La recommandation de mémoire minimale suppose que les images à scanner ne dépassent pas 0,5 Go. +
Lors du scan d'images de plus de 1 Go, la mémoire du scanner doit être calculée en prenant la plus grande taille d'image et en ajoutant 0,5 Go. +
Exemple - taille d'image la plus grande = Test_Eight1,3 Go, la mémoire du conteneur du scanner doit être de 1,8 Go.
...
== Test_EightPlateformes prises en charge
...
* CoreOS est pris en charge (novembre 2023) pour le scan CVE via le tableau de correspondance RHEL fourni par RedHat. Une fois qu'un flux officiel est publié par RedHat pour CoreOS, il sera pris en charge.
...
* \{product-name} est compatible avec la plupart des CNI commercialement pris en charge. Officiellement testés et pris en charge sont openshift ovs (sous-réseau/multitenant), calico, flannel, cilium, antrea et les clouds publics (gke, aks, iks, eks). Le support pour Multus a été ajouté dans v5.4.0.
...
[NOTE]
====
PKS est testé sur le terrain et nécessite l'activation des conteneurs privilégiés pour le plan/tile, et le changement du hostPath yaml comme suit pour Allinone, Controller, Enforcer :
...
== Test_EightPerformance et Scalabilité
...
Comme le montre le graphique ci-dessous, les tests de référence de débit de base ont montré un débit maximum de 1,3 Gbps PAR NŒUD sur une petite instance de cloud public avec 4 cœurs CPU. Par exemple, un cluster de 10 nœuds pourrait alors gérer un maximum de 13 Gbps de débit pour l'ensemble du cluster pour les services en mode Protéger.

image:throughput.png[Débit]

Ce débit devrait être projeté pour évoluer à mesure qu'un CPU dédié est attribué à l'Enforcer, ou que la vitesse du CPU change, et/ou que de la mémoire supplémentaire est allouée. Encore une fois, l'évolutivité dépendra du type de trafic réseau/application des charges de travail.
...
Le tableau ci-dessous montre la latence moyenne de 2 à 10 % mesurée à l'aide de l'outil de référence Redis. Le Benchmark Redis utilise des paquets assez petits, donc la latence avec des paquets plus grands devrait être attendue pour être plus faible.
...
=== Test_HuitAjout de contraintes de mise à l'échelle pour des environnements de charge de travail importants
