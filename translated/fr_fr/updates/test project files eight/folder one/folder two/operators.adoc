= Test_HuitDéployer en utilisant des opérateurs
:revdate: 2025-06-02
...
== Test_Huit\{nom-du-produit} Opérateurs
...
. Lorsque l'opérateur est installé et prêt à l'emploi, une instance de \{product-name} peut être installée. 
...
Mettez à niveau la version de \{product-name} en mettant à jour la version de l'Operator associée à la version souhaitée de \{product-name}.
======

.Déployer en utilisant l'Operator communautaire
[%collapsible]
======
*Déployer en utilisant l'Operator communautaire \{product-name} depuis l'Operator Hub*

*Notes techniques*

* Les images de conteneur \{product-name} sont récupérées depuis Docker Hub à partir du compte \{product-name}. 
* L'interface utilisateur du gestionnaire \{product-name} est généralement exposée via un itinéraire de passage OpenShift sur un domaine. Par exemple, sur IBM Cloud neuvector-route-webui-neuvector.(nom_du_cluster)-(hash_aleatoire)-0000.(région).containers.appdomain.cloud. Elle peut également être exposée en tant que service neuvector-service-webui via une adresse de port de nœud ou une IP publique. 
* OpenShift version 4.6+ 
...
----
--
. Installez l'Operator communautaire \{product-name} depuis l'Operator Hub 
** Dans l'interface utilisateur de la console OpenShift, naviguez vers OperatorHub 
** Recherchez l'Operator \{product-name} et sélectionnez l'annonce avec le badge communautaire 
** Cliquez sur Installer 
