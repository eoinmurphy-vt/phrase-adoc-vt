...
Si vous déployez avec le chart Helm actuel de \{product-name} (v1.8.9+), les changements suivants doivent être apportés à values.yml :
...
=== Test_HuitMeilleures Pratiques, Conseils, Q&R pour Déployer et Gérer \{product-name}
...
== Test_EightDéploiement en utilisant Helm ou des opérateurs
...
== Test_EightDéploiement en utilisant ConfigMap
...
== Test_EightDéploiement des contrôleurs

Nous recommandons que plusieurs contrôleurs soient exécutés pour une configuration de haute disponibilité (HA). Les contrôleurs utilisent le protocole RAFT basé sur le consensus pour élire un leader et si le leader tombe, élire un autre leader. À cause de cela, le nombre de contrôleurs actifs doit être impair, par exemple 3, 5, 7, etc.

== Test_EightController HA
...
== Test_EightSauvegardes et données persistantes
...
=== Test_HuitExemple de Volume Persistant
...
=== Test_HuitConfigMaps et stockage persistant

Les sauvegardes de ConfigMaps et de stockage persistant ne sont lues que lorsqu'un nouveau \{product-name} cluster est déployé, ou lorsque le cluster échoue et redémarre. Elles ne sont pas utilisées lors des mises à jour progressives.

La sauvegarde de la configuration de stockage persistant est lue en premier, puis les ConfigMaps sont appliqués, de sorte que les paramètres de ConfigMap prennent le pas. Tous les paramètres de ConfigMap (par exemple, mises à jour) seront également enregistrés dans le stockage persistant.

Pour plus d'informations, voir la section xref:configmap.adoc[ConfigMaps].

== Test_HuitMise à jour de la base de données de vulnérabilités CVE en production
...
== Test_HuitAccéder à la Console

Par défaut, la console est exposée en tant que service sur le port 8443, ou nodePort avec un port aléatoire sur chaque hôte. Veuillez consulter la première section Bases -> xref:rest-api.adoc[Connecter au Manager] pour des options pour désactiver HTTPS ou accéder à la console via un pare-feu d'entreprise qui n'autorise pas le port 8443 pour l'accès à la console.

== Test_HuitGestion des mises à jour d'hôte ou des nœuds d'auto-scaling avec un budget de perturbation de pod
...
== Test_HuitDéploiement Sans Mode Privilégié
...
== Test_HuitArchitecture Multi-site, Multi-Cluster
