...
* **NVSHAS-9783**: Après avoir ajouté la politique réseau pour refuser les paquets icmp, NeuVector ne voit aucune alerte dans `+Security Events+`.
...
Dans la version NeuVector 5.4.2, le support est interrompu pour les déploiements utilisant le certificat interne intégré. Le certificat trouvé à `+/etc/neuvector/certs/internal+` dans les images de conteneur NeuVector 5.4.2 sera supprimé. Pour continuer à utiliser NeuVector, les utilisateurs doivent :
...
* **NVSHAS-9092**: L'utilisateur avec espace de noms ne devrait pas voir les ressources globales.
...
* Après la mise à niveau vers v5.3.1 à partir d'une version précédente de {product-name}, les ressources personnalisées NvClusterSecurityRule préexistantes peuvent être supprimées par inadvertance. REMARQUE : La version 5.3.1 a été supprimée de docker hub afin de prévenir le problème de mise à niveau.
...
* CVE médium : CVE-2023-52426 dans expat, CVE-2024-20926, CVE-2024-20921, CVE-2024-20945 et CVE-2024-20919 dans openjdk11, CVE-2024-0727 et CVE-2023-6237 dans openssl
...
* Supporter l'API NVD 2.0 pour la base de données CVE de scan
...
* Utilisez des comptes de services moins privilégiés selon les besoins pour chaque composant {product-name}. Une variable {Q}&quot;`++leastPrivilege++`&quot;{/Q} est introduite. La valeur par défaut est fausse. REMARQUE : Utiliser le graphique helm actuel avec cette variable sur une version antérieure à 5.2.0 ne fonctionnera pas correctement.
...
* Diverses optimisations de performance et de scalabilité pour l'utilisation du CPU et de la mémoire du contrôleur et de l'exécuteur.
...
* Ajouter {Q}&quot;`++package++`&quot;{/Q} comme information à l'événement syslog pour une vulnérabilité détectée.
* Ajouter la variable d'environnement de l'exécuteur ENF_NETPOLICY_PULL_INTERVAL - Valeur en secondes (valeur recommandée 60) pour réduire le trafic réseau et la consommation de ressources résultante par l'exécuteur en raison des mises à jour/recalculs de politique. (Remarque : il s'agissait d'un ajout non documenté jusqu'en août 2023).
...
* Violations implicites inattendues pour le trafic egress istio avec une règle d'autorisation en place
...
* Ajoutez la possibilité de masquer les espaces de noms et groupes sélectionnés dans la vue d'activité réseau.
...
. Mettez à jour les noms et chemins d'image pour tirer {product-name} des images du hub Docker (docker.io), par ex.
