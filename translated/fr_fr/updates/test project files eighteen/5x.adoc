...
* **NVSHAS-9754**: [UI] Empêcher Rancher de désactiver l'authentification de l'utilisateur SSO lié à OpenShift ou au RBAC de Rancher.
...
* **NVSHAS-7687**: Support de la configuration du niveau de journalisation (déboguer/erreur/info/avertir) pour l'enforcer et le contrôleur depuis l'interface de ligne de commande.
...
* Nœud non géré avec l'exécuteur "zombie" en cours d'exécution
* Le filtre avancé montre les champs de remédiation et d'impact vides
* Corriger la gestion des chaînes pour éviter un redémarrage inattendu de l'exécuteur
* Violations inattendues liées aux groupes intégrés
* Appel RPC de débogage de l'exécuteur de bundle de support pour les données renvoie une erreur
...
* Supprimer le paramètre pour exécuter le contrôleur en mode privilégié dans le graphique Helm, et l'exigence pour le contrôleur/l'exécuteur de monter "/host/cgroup".
...
* Fournir la date de construction de l'image du conteneur dans les détails des Conteneurs ->
...
* Supporter le paramètre Proxy dans la vérification de la signature d'image.
...
* Lors de la mise à niveau depuis la version {product-name} 4.x, une appartenance incorrecte au groupe de pods entraîne une violation de politique inattendue
...
{product-name} n'a pas besoin de paramètres supplémentaires pour les clusters avec SELinux activé pour déployer et fonctionner. Testé le déploiement de {product-name} sur un cluster RKE2 durci basé sur RHEL 8.5 avec SELinux activé. {product-name} déployé avec succès si PSP est activé et déploiement du gestionnaire de patchs et du scanner. La prochaine version du graphique devrait corriger le problème ci-dessous.
...
. Mettre à jour les noms et chemins d'image pour tirer {product-name} des images du hub Docker (docker.io), par exemple.
...
En option, supprimer toute référence à la {product-name} licence et au secret de registre dans les charts Helm, yaml de déploiement, configmap, scripts, etc., car ceux-ci ne sont plus nécessaires pour tirer les images ou commencer à utiliser {product-name}.
