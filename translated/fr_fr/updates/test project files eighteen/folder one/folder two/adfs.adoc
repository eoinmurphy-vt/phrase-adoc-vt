= Projet de test dix-huit SAML (ADFS)
:revdate: 2025-05-08
...
. Dans la gestion AD FS, faites un clic droit sur {Q}&quot;`++Relying Party Trusts++`&quot;{/Q} et sélectionnez {Q}&quot;`++Add Relying Party Trust...++`&quot;{/Q}.
+
image:adfs1.png[adfsSetup]

. Sélectionnez le bouton {Q}&quot;`++Start++`&quot;{/Q} à partir de l'étape de bienvenue.
+
image:adfs2.png[adfsSetup]

. Sélectionnez {Q}&quot;`++Enter data about the relying party manually++`&quot;{/Q} et sélectionnez {Q}&quot;`++Next++`&quot;{/Q}.
+
image:adfs3.png[adfsSetup]

. Entrez un nom unique pour le champ Nom d'affichage et sélectionnez {Q}&quot;`++Next++`&quot;{/Q}.
+
image:adfs4.png[adfsSetup]

. Sélectionnez {Q}&quot;`++Next++`&quot;{/Q} pour ignorer le cryptage du jeton.
+
image:adfs5.png[adfsSetup]

. Cochez {Q}&quot;`++Enable support for the SAML 2.0 WebSSO protocol++`&quot;{/Q} et entrez l'URI de redirection SAML depuis {product-name} Paramètres>page de paramètres SAML dans le champ {Q}&quot;`++Relying party SAML 2.0 SSO service URL++`&quot;{/Q}.  Sélectionnez {Q}&quot;`++Next++`&quot;{/Q} pour continuer.
+
image:adfs6.png[adfsSetup]

. Entrez la même URI de redirection SAML dans le champ {Q}&quot;`++Relying party trust identifier++`&quot;{/Q} et cliquez sur {Q}&quot;`++Add++`&quot;{/Q}; puis sélectionnez {Q}&quot;`++Next++`&quot;{/Q} pour continuer.
+
image:adfs7.png[adfsSetup]

. Personnalisez le contrôle d'accès ; puis sélectionnez {Q}&quot;`++Next++`&quot;{/Q} pour continuer.
+
image:adfs8.png[adfsSetup]

. Sélectionnez {Q}&quot;`++Next++`&quot;{/Q} pour continuer.
+
image:adfs9.png[adfsSetup]

. Sélectionnez {Q}&quot;`++Close++`&quot;{/Q} pour terminer.
. Sélectionnez Modifier la politique d'émission de revendications...
+
image:adfs10-11.png[adfsSetup]

. Sélectionnez {Q}&quot;`++Add Rule...++`&quot;{/Q} et choisissez {Q}&quot;`++Send LDAP Attributes as Claims++`&quot;{/Q}; puis sélectionnez {Q}&quot;`++Next++`&quot;{/Q}.  Nommez la règle et choisissez Active Directory comme magasin d'attributs. Seule la revendication sortante du nom d'utilisateur est requise pour l'authentification si le rôle par défaut est défini ; sinon, les groupes sont nécessaires pour le mappage des rôles.  L'email est facultatif.
+
--
...
--
. Sélectionnez {Q}&quot;`++Add Rule...++`&quot;{/Q} et choisissez {Q}&quot;`++Transform an Incoming Claim++`&quot;{/Q}; puis sélectionnez {Q}&quot;`++Next++`&quot;{/Q}.  Nommez la règle et définissez le champ comme capturé dans la capture d'écran ci-dessous.  Le format d'identifiant de nom sortant doit être Identifiant transitoire.
+
...
* Voir les points de terminaison depuis la gestion AD FS > Service et utiliser l'URL de point de terminaison {Q}&quot;`++SAML 2.0/WS-Federation++`&quot;{/Q}.
* Exemple : `++https://<adfs-fqdn>/adfs/ls++`

. Émetteur du fournisseur d'identité
* Cliquez avec le bouton droit sur AD FS depuis la console de gestion AD FS et sélectionnez {Q}&quot;`++Edit Federation Service Properties...++`&quot;{/Q}; utilisez le {Q}&quot;`++Federation Service identifier++`&quot;{/Q}.
* Exemple : `++http://<adfs-fqdn>/adfs/services/trust++`

. Certificat X.509
* Depuis la gestion AD FS, sélectionnez Service > Certificat, cliquez avec le bouton droit sur le certificat de signature de jeton et choisissez {Q}&quot;`++View Certificate...++`&quot;{/Q}
* Sélectionnez l'onglet Détails et cliquez sur {Q}&quot;`++Copy to File++`&quot;{/Q}
...
. Rôle par défaut
* Recommandé d'être {Q}&quot;`++None++`&quot;{/Q} à moins que vous ne souhaitiez permettre à tout utilisateur authentifié un rôle par défaut.
...
. La signature de réponse ADFS SamlResponseSignature doit être soit MessageOnly soit MessageAndAssertion.  Utilisez la commande Get-AdfsRelyingPartyTrust pour vérifier ou mettre à jour.
+
...
Veuillez vérifier et confirmer que les hôtes ADFS et {product-name} sont synchronisés et que les retards potentiels ne dépassent pas 10 secondes. Vous pouvez utiliser des commandes Linux et Windows pour vérifier les dates, heures et l'activité du serveur NTP.
...
Si l'un de ces paramètres est incorrect, vous recevrez une `+Authentication Failed+` erreur lorsque vous essayez de vous connecter à {product-name} avec un utilisateur ADFS utilisant l'authentification SAML.
