= Projet de test dix-huit OpenID Connect Azure/Okta
:revdate: 2024-09-27
...
Localisez l'URI de redirection OpenID Connect en haut de la page des paramètres OpenID Connect {product-name}. Vous devrez copier cet URI dans les *URI de redirection de connexion* pour Okta et *URL de réponse* pour Microsoft Azure.
...
Dans le panneau central, cliquez sur {Q}&quot;`++Create App Integration++`&quot;{/Q} :

image:okta1.png[créer]

Un nouveau panneau apparaîtra pour sélectionner le {Q}&quot;`++Sign-in method++`&quot;{/Q} :

image:okta2.png[nouvelleapp]

Sélectionnez l'option {Q}&quot;`++OIDC -- OpenID Connect++`&quot;{/Q}.

Un panneau dérivé apparaîtra, pour la sélection de {Q}&quot;`++Application Type++`&quot;{/Q} :

image:okta3.png[typeapp]

Sélectionnez l'option {Q}&quot;`++Native Application++`&quot;{/Q}.
...
Allez à votre console {product-name} et naviguez vers {Q}&quot;`++Settings++`&quot;{/Q} -> {Q}&quot;`++OpenId Connect Settings++`&quot;{/Q}.  En haut de la page, à côté de l'étiquette {Q}&quot;`++OpenID Connect Redirect URI++`&quot;{/Q}, cliquez sur {Q}&quot;`++Copy to Clipboard++`&quot;{/Q}.
...
Sélectionnez {Q}&quot;`++Allow everyone in your organization to access++`&quot;{/Q} pour rendre cette intégration disponible pour tout le monde dans votre organisation.
...
Dans la section {Q}&quot;`++Client Credentials++`&quot;{/Q}, cliquez sur modifier et modifiez la section {Q}&quot;`++Client Authentication++`&quot;{/Q} de {Q}&quot;`++Use PKCE (for public clients)++`&quot;{/Q} à {Q}&quot;`++Use Client Authentication++`&quot;{/Q}, puis cliquez sur enregistrer. Cela générera automatiquement un nouveau secret dont nous aurons besoin dans les prochaines étapes de configuration {product-name} :

image:okta7.png[clientauth]

Naviguez vers l'onglet {Q}&quot;`++Sign On++`&quot;{/Q} et modifiez la section {Q}&quot;`++OpenID Connect ID Token++`&quot;{/Q} :
Changez l'émetteur de {Q}&quot;`++Dynamic (based on request domain)++`&quot;{/Q} à {Q}&quot;`++Okta URL++`&quot;{/Q} fixe :

image:okta8.png[token]

La console Okta peut fonctionner en deux modes, Mode Classique et Mode Développeur.
En mode classique, l'URL de l'émetteur se trouve dans l'onglet Se connecter de la page de l'application Okta. Pour que l'appartenance au groupe de l'utilisateur soit renvoyée dans la revendication, vous devez ajouter le champ "groups" dans la page de configuration OpenID Connect {product-name} :
...
Après que l'utilisateur soit authentifié, le rôle approprié peut être dérivé avec la configuration de mappage de rôle basé sur les groupes. Pour configurer le mappage de rôle basé sur les groupes,
