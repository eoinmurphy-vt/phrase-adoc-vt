...
--
. Vous pouvez maintenant déployer le service Allinone. Définissez le &quot;+`+Nombre de tâches+`+&quot; au nombre souhaité d'Allinone/Contrôleur. Maintenant, les {product-name} conteneurs Allinone ou Contrôleur commenceront à s'exécuter sur les nœuds sélectionnés. Après le démarrage de l'Allinone, vous devriez pouvoir vous connecter à la {product-name} console via HTTPS sur le port 8443.
