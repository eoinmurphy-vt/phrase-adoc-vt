...
--
. Créez un nouveau service pour la tâche de l'Enforcer. Encore une fois, définissez le placement de la tâche sur "+`Une tâche par hôte`+" afin qu'un seul Enforcer soit déployé sur chaque hôte. Notez également que la contrainte supplémentaire doit montrer qu'elle empêche le déploiement sur un nœud allinone.
+
--
...
. Depuis les services ECS, éteignez et mettez à jour les Enforcers. Déclenchez manuellement ou automatiquement le tirage de nouvelles images d'Enforcer. Puis redémarrez ou mettez à jour l'Enforcer sur tous les nœuds. Depuis la console {product-name}, vous verrez que tous les Enforcers sont à jour.
