...
--
. Vous pouvez maintenant déployer le service Allinone. Définissez le {Q}&quot;+`+Nombre de tâches+`+&quot;{/Q} au nombre souhaité d'Allinone/Contrôleur. Maintenant, les {product-name} conteneurs Allinone ou Contrôleur commenceront à s'exécuter sur les nœuds sélectionnés. Après que l'Allinone commence à s'exécuter, vous devriez pouvoir vous connecter à la console {product-name} via HTTPS sur le port 8443.
...
--
. Créez un nouveau service pour la tâche de l'Enforcer. Encore une fois, définissez le placement de la tâche sur {Q}&quot;+`+Une tâche par hôte+`+&quot;{/Q} afin qu'un seul Enforcer soit déployé sur chaque hôte. Notez également que la contrainte supplémentaire doit montrer qu'elle empêche le déploiement sur un nœud allinone.
+
--
image:5taskplacement.png[EnforcerPlacement]

Déployez ce service avec le nombre souhaité de nœuds d'enforcer dans {Q}&quot;+`+Nombre de tâches+`+&quot;{/Q}. Très bientôt, tous les enforcers seront opérationnels. Depuis la console {product-name}, vous pourrez voir tous les nœuds détectés avec des enforcers.
--
