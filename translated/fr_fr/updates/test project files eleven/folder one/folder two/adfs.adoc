...
== Test_TwoConfigurer ADFS et l'intégration {product-name}

Cette section décrit d'abord les étapes de configuration dans ADFS, puis dans la console {product-name}.

=== Test_TwoADFS Setup

. Dans la gestion AD FS, faites un clic droit sur "`+Confiances des parties de confiance+`" et sélectionnez "`+Ajouter une confiance de partie+`".
+
...
. La signature de la réponse ADFS SamlResponseSignature doit être soit MessageOnly soit MessageAndAssertion.  Utilisez la commande Get-AdfsRelyingPartyTrust pour vérifier ou mettre à jour cela.
+
