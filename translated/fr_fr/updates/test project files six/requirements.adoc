...
Exemple - taille d'image la plus grande = Test_Six1,3 Go, la mémoire du conteneur du scanner doit être de 1,8 Go.
...
== Test_SixPlateformes prises en charge
...
== Test_SixPerformance et Scalabilité
...
Ce débit serait projeté pour augmenter à mesure qu'un CPU dédié est assigné à l'Enforcer, ou que la vitesse du CPU change, et/ou que de la mémoire supplémentaire est allouée. Encore une fois, l'évolutivité dépendra du type de trafic réseau/application des charges de travail.
...
=== Test_SixAjout de contraintes d'échelle pour des environnements de charge de travail importants
...
Comme solution de contournement pour les environnements de charge de travail importants, vous devez créer un fichier tel que `+example-fs-max.conf+` à l'emplacement `+/etc/sysctl.d/+` et ajouter des contraintes d'échelle avec la configuration suivante :
