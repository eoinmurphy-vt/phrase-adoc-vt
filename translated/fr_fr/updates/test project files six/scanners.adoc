= Test_SixScanners parallèles et autonomes
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /06.scanning/06.scanners/06.scanners.md
:page-opendocs-slug:  /scanning/scanners

== Test_SixAugmenter l'évolutivité des scanners avec plusieurs scanners
...
=== Test_SixMise à l'échelle automatique des pods de scanner
...
** Lorsque le contrôleur principal voit en continu que "compte de tâches" est 0 pendant > 180 secondes, il réduit d'un pod de scanner si minScannerPods n'est pas encore atteint.
* Stratégie immédiate :
...
=== Test_SixOperations et Débogage
...
== Test_SixStandalone Scanner pour le scan local
...
* SCANNER_REGISTRY= Test_Sixurl du registre (optionnel au lieu du scan local)
* SCANNER_REPOSITORY= Test_Sixdépôt à scanner
* SCANNER_TAG= Test_Sixétiquette de version
* SCANNER_REGISTRY_USERNAME= Test_Sixutilisateur (optionnel au lieu du scan local)
* SCANNER_REGISTRY_PASSWORD= Test_Sixmot de passe (optionnel au lieu du scan local)
* SCANNER_SCAN_LAYERS= Test_Sixvrai ou faux (pour retourner les résultats de scan en couches)
...
=== Test_SixHost Analyse en mode autonome
...
=== Test_SixManual Déploiement de plusieurs scanners sur Kubernetes
