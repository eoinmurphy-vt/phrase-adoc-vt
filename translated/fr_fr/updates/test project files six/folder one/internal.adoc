= Test_SixRemplacement des certificats internes
:revdate: 2025-01-27
...
== Test_SixCommunication interne et certificats

\{product-name} inclut des certificats auto-signés par défaut pour le chiffrement pour le Manager (accès console/UI), le Controller (API REST, interne), l'Enforcer (interne) et les communications Scanner (interne).

Ces certificats peuvent être remplacés par les vôtres pour renforcer davantage la communication. Pour remplacer les certificats utilisés par l'accès externe à \{product-name} (c'est-à-dire, navigateur vers le Manager, ou API REST vers le Controller), veuillez consulter xref:replacecert.adoc[cette section]. Voir ci-dessous pour remplacer les certificats utilisés dans la communication interne entre les conteneurs \{product-name}.
...
=== Test_SixRemplacement des certificats utilisés dans les communications internes de \{product-name}
...
----
+
* Puis modifiez les yamls de déploiement du Controller, de l'Enforcer et du Scanner, en ajoutant :
+
[source,yaml]
----
      containers:
...
=== Test_SixMise à jour/Déploiement avec Helm
