...
=== Installation du plug-in Test_SixJenkins
...
=== Test_SixLocal Configuration pour scanner un hôte Docker distant

==== Test_SixPrérequis pour un scan local sur un hôte Docker distant
...
=== Test_SixÉtapes supplémentaires

Ajoutez vos propres étapes de scan d'image avant et après, par exemple dans l'exemple ci-dessous de la vue des étapes du pipeline.

image:jenkins7a.png[stages]

Vous êtes maintenant prêt à démarrer vos builds Jenkins et à déclencher le Scanner de Vulnérabilités \{product-name} pour signaler toute vulnérabilité !

=== Test_SixConfiguration du pipeline pour construire des scans parallèles à grande échelle
...
== Test_SixExemple de route OpenShift et de token de registre
...
== Test_SixCas d'utilisation spécial pour Jenkins dans le même cluster Kubernetes
