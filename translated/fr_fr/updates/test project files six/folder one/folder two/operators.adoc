= Test_SixDéployer en utilisant des opérateurs
:revdate: 2025-06-02
...
== Test_Six\{nom-du-produit} Opérateurs
...
----
--
. Préparez les valeurs de configuration YAML pour l'installation de \{product-name} en commençant par le snippet YAML suivant. Assurez-vous de spécifier la version souhaitée de \{product-name} dans la valeur 'tag'. Vérifiez la référence des valeurs dans le graphique Helm de \{product-name} pour obtenir les options de configuration disponibles. Il existe d'autres valeurs Helm possibles qui peuvent être configurées dans le YAML, telles que la configuration du cluster pour permettre la gestion multi-cluster en exposant les services Master (Master fédéré) ou remote (Worker fédéré).
+
--
[,yaml]
----
...
* Examinez et vérifiez les valeurs du graphique Helm de \{product-name}
