...
* Helm chart sur github pour le déploiement automatisé sur Kubernetes
...
[IMPORTANT]
====
Si vous effectuez des appels d'API REST avec un nom d'utilisateur/mot de passe, veuillez vous assurer de faire un appel DELETE contre /v1/auth une fois terminé. Il y a un maximum de 32 sessions concurrentes pour chaque utilisateur. Si cela est dépassé, un échec d'authentification se produira.
====
...
=== Test_TwoExpose API REST dans Kubernetes
...
[NOTE]
====
Si vous utilisez le type LoadBalancer, définissez le _controllerIP_ dans les exemples ci-dessous sur l'IP externe ou l'URL pour le loadbalancer.
====

=== Test_TwoAuthentication pour l'API REST

L'API REST prend en charge deux types d'authentification : nom d'utilisateur/mot de passe et jeton. Les deux peuvent être configurés dans Paramètres -> Utilisateurs, Clés API & Rôles, et être associés à des rôles par défaut ou personnalisés pour limiter les privilèges d'accès. Les exemples ci-dessous montrent une authentification basée sur le nom d'utilisateur/mot de passe où un jeton est créé en premier, puis utilisé dans les appels d'API REST suivants. Si vous utilisez un jeton, il peut être utilisé directement dans chaque appel d'API REST. Remarque : les connexions basées sur le nom d'utilisateur ont un nombre limité de sessions concurrentes, il est donc important de supprimer le jeton du nom d'utilisateur comme indiqué ci-dessous une fois terminé. L'authentification basée sur un jeton n'a pas de limite, mais expire selon la durée sélectionnée lors de sa création.
...
=== Test_TwoTrigger Analyse de Vulnérabilité à partir d'un script
...
[NOTE]
====
Dans un déploiement avec plusieurs contrôleurs, les demandes doivent être envoyées à une seule IP de contrôleur afin que plusieurs demandes pour le statut des analyses d'images de longue durée aillent au contrôleur effectuant l'analyse.
====
...
=== Test_TwoCréer des règles de stratégie automatiquement
...
=== Test_TwoExport/Import fichier de configuration
...
[IMPORTANT]
====
Ces exemples sont fournis à titre d'exemple uniquement et ne sont pas officiellement supportés à moins qu'un accord de support d'entreprise spécifique n'ait été mis en place.
====
...
=== Test_TwoSetting ou Changer le mot de passe de l'utilisateur
...
=== Test_TwoDémarrer la capture de paquets sur un Container

Lorsqu'un container présente un comportement suspect, commencez une capture de paquets.
...
=== Test_TwoVérifier et accepter le contrat de licence utilisateur final (nouveaux déploiements)
...
Vérifiez à nouveau le CLUF.

=== Test_TwoConfigurer la numérisation du registre
...
=== Test_TwoActiver la capture de paquets sur tous les pods dans un espace de noms
...
=== Test_TwoActiver Désactiver la mise en quarantaine des conteneurs
...
=== Test_TwoActiver le mode de débogage pour le support {product-name}
...
=== Test_TwoSignaler si une vulnérabilité se trouve dans les couches de l'image de base
...
=== Test_TwoObtenez la version et la date de la base de données CVE
...
=== Test_TwoGérer la fédération pour les clusters principaux et distants (travailleurs)
