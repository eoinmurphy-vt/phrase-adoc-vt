= Test_DixRemplacement des certificats internes
:revdate: 2025-01-27
...
== Test_DeuxCommunication interne et certificats
...
=== Test_DeuxRemplacement des certificats utilisés dans les communications internes de {product-name}
...
+
Si vos fichiers de certificat ont été créés avant la version {product-name} 5.3, vous devez mettre à jour le certificat avec au moins un Nom Alternatif de Sujet, ou SAN pour faire court. Si vous avez toujours les fichiers `+ca.key+` et `+ca.crt+` accessibles, exécutez les commandes comme suit :
+
[source,bash]
----
kubectl delete secret internal-cert -n neuvector 
