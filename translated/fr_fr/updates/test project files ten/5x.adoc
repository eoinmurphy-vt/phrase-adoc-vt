...
* **NVSHAS-9447**: Les pods Contrôleur/Scanner se plantent - "Sortie système non prise en charge".
...
* **NVSHAS-8562**: Ajouter une notification d'expiration de certificat interne.
...
* **NVSHAS-9024**: Rôle risqué de contrôle d'admission.
...
* Correction d'un problème où l'URL du fournisseur OpenID échouait à cause de l'expression régulière.
...
* Correction d'un bug lié à `+regex+` lors de l'utilisation d'une virgule (`+,+`) dans un multi-entrée `+Admission Control user criteria+`.
 ...
 ** Le xref:replacecert.adoc[remplacement manuel du certificat] est toujours pris en charge et recommandé pour les versions précédentes ou les déploiements basés sur yaml. Voir le {product-name} conseil de sécurité GitHub https://github.com/neuvector/neuvector/security/advisories/GHSA-622h-h2p8-743x[ici] pour une description.
...
* Ajoutez des variables d'environnement Enforcer pour désactiver la recherche de secrets et exécuter des benchmarks CIS
...
* Prise en charge de l'analyse de vulnérabilité de openSUSE Leap OS (dans l'image du scanner).
