...
Ces certificats peuvent être remplacés par les vôtres pour renforcer davantage la communication. Pour remplacer les certificats utilisés par l'accès externe à \{product-name} (c'est-à-dire, du navigateur au Manager, ou de l'API REST au Controller), veuillez consulter xref:replacecert.adoc[cette section]. Voir ci-dessous pour remplacer les certificats utilisés dans la communication interne entre les conteneurs \{product-name}.
...
+
Si vos fichiers de certificat ont été créés avant la version 5.3 de \{product-name}, vous devez mettre à jour le certificat avec au moins un Nom Alternatif du Sujet, ou SAN pour faire court. Si vous avez toujours les fichiers `+ca.key+` et `+ca.crt+` accessibles, exécutez les commandes comme suit :
+
[source,bash]
----
kubectl delete secret internal-cert -n neuvector 
...
+
Si vos fichiers de certificat ont été créés avant la version 5.3 de \{product-name}, vous devez mettre à jour le certificat avec au moins un Nom Alternatif du Sujet, ou SAN pour faire court. Si vous n'avez plus les fichiers `+ca.key+` et `+ca.crt+`, suivez les étapes ci-dessous : 
+
