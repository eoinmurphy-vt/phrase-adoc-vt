...
Le plug-in \{product-name} Vulnerability Scanner Jenkins peut analyser les images après que votre image soit construite dans Jenkins. Le code source du plug-in et la dernière documentation peuvent être trouvés https://github.com/jenkinsci/neuvector-vulnerability-scanner-plugin[ici] sur la page GitHub de \{product-name}.
...
=== Test_TwoInstallation du plug-in Jenkins
...
. *Scan d'image de registre*. Si vous utilisez le plugin pour scanner des images de registre (après les avoir poussées vers des registres, mais dans le cadre du processus de construction Jenkins), le Scanner \{product-name} peut être installé sur n'importe quel nœud du réseau avec connectivité entre le registre, le Scanner \{product-name} et Jenkins.

=== Test_TwoGlobal Configuration dans Jenkins

Après avoir installé le plugin, trouvez la section '`+Scanner de vulnérabilité \{product-name}+`' sur la page de configuration globale (Jenkins '`+Configurer le système+`'). Entrez les valeurs pour l'IP du Controller \{product-name}, le port, le nom d'utilisateur et le mot de passe. Vous pouvez cliquer sur le bouton '`+Tester la connexion+`' pour valider les valeurs. Cela affichera '`+Succès de la connexion+`' ou un message d'erreur.
...
Cliquez sur le '`+Ajouter un registre+`' pour entrer les valeurs pour le registre que vous utiliserez dans votre projet. Si vous ne scannez que des images locales, vous n'avez pas besoin d'ajouter un registre ici.
...
==== Test_TwoStandalone Scanner

Exécuter un scan Jenkins en mode autonome est un moyen léger de scanner les vulnérabilités des images dans le pipeline. Le scanner est invoqué dynamiquement et aucune installation de configuration du contrôleur n'est requise. C'est particulièrement utile lors du scan d'une image avant qu'elle ne soit poussée vers un registre. Il n'y a également aucune limite au nombre de tâches de scan pouvant s'exécuter en même temps.

Pour exécuter un scan de vulnérabilités en mode autonome, le plugin Jenkins doit tirer l'image du scanner vers l'hôte où l'agent s'exécute, donc vous devez entrer l'URL du registre du Scanner \{product-name}, le dépôt d'images et les informations d'identification si nécessaire, dans la page de configuration du plugin \{product-name}.
...
=== Test_TwoConfiguration locale pour scanner un hôte Docker distant

==== Test_TwoPrérequis pour un scan local sur un hôte Docker distant
...
=== Configuration de Test_TwoProject

Dans votre projet, choisissez le plugin '\{product-name} Scanner de vulnérabilités' dans le menu déroulant dans 'Ajouter une étape de construction.' Cochez la case "Scanner avec le scanner autonome" si vous souhaitez effectuer le scan en mode scanner autonome. Par défaut, il utilise le mode "Contrôleur et Scanner" pour effectuer le scan.
...
Après la fin de la construction, un rapport \{product-name} sera généré. Il affichera les détails du scan et les erreurs, le cas échéant.
...
=== Test_TwoJenkins Pipeline

Pour le projet de pipeline Jenkins, vous pouvez écrire votre propre script de pipeline directement, ou cliquer sur le '`syntaxe de pipeline`' pour générer le script si vous êtes nouveau dans la tâche de style pipeline.

image:jenkins5a.png[pipeline]

Sélectionnez le Scanner de vulnérabilités \{product-name} dans le menu déroulant, configurez-le et générez le script.
...
=== Test_TroisÉtapes Supplémentaires
...
Vous êtes maintenant prêt à commencer vos builds Jenkins et à déclencher le Scanner de Vulnérabilités \{product-name} pour signaler toute vulnérabilité !

=== Test_TroisConfiguration du Pipeline pour Construire des Scans Parallèles à Grande Échelle

Disponible avec NeuVector v5.4.3 et ultérieur, le plug-in Jenkins NeuVector Vulnerability Scanner v2.5 et ultérieur prend en charge le scan parallèle de jusqu'à 2000 scans concurrents lors de l'utilisation du mode clé API. Pour les versions antérieures de NeuVector, le nombre maximum de scans concurrents est limité à 32 avec l'utilisation du mode Token. Cliquez pour développer et voir les exemples ci-dessous pour des configurations de pipeline échantillons.

.Utilisation de la Configuration d'Échantillon en Mode Token (plug-in v2.4 et inférieur, ou v2.5 et ultérieur) 
[%collapsible]
====
[,groovy]
----
pipeline {
    agent any
    environment {
        REPO_NAME = Test_Two'your repo'
        REGISTRY_SELECTION = Test_Two'your registry'
        CONTROLLER = Test_Two'your controller'
        MAX_CONCURRENT_SCANS = Test_Two32
    }
    stages {
        stage('Parallel Vulnerability Scanning') {
            steps {
                script {
                    ...
                    TAGS_LIST_PART1 = Test_Two["your tags"...]
                    TAGS_LIST_PART2 = Test_Two["your tags"...]
                    TAGS_LIST_PART3 = Test_Two["your tags"...]
                    TAGS_LIST_PART4 = Test_Two["your tags"...]
                    TAGS_LIST_PART5 = Test_Two["your tags"...]...
                    def allTags = Test_TwoTAGS_LIST_PART1 + TAGS_LIST_PART2 + TAGS_LIST_PART3 + TAGS_LIST_PART4 + TAGS_LIST_PART5
                    def batches = Test_TwoallTags.collate(MAX_CONCURRENT_SCANS.toInteger()) // Ensure MAX_CONCURRENT_SCANS is an integer
                    def batchCounter = Test_Two1                    for (batch in batches) {
                        stage("Batch ${batchCounter}") {
                            def scans = Test_Two[:]
                            batch.each { tag ->
                                def currentTag = Test_Twotag
                                scans["Scan ${currentTag}"] = Test_Two{
...
== Test_TroisExemple de Route OpenShift et de Token de Registre
...
== Test_TroisCas d'Utilisation Spécial pour Jenkins dans le Même Cluster Kubernetes
