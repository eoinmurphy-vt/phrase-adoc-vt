= Test_CinqRemplacement des certificats internes
:revdate: 2025-01-27
...
== Test_CinqCommunication interne et certificats

\{product-name} inclut des certificats auto-signés par défaut pour le chiffrement pour le Manager (accès console/UI), le Contrôleur (API REST, interne), l'Enforcer (interne) et les communications du Scanner (interne).

Ces certificats peuvent être remplacés par les vôtres pour renforcer davantage la communication. Pour remplacer les certificats utilisés par l'accès externe à \{product-name} (c'est-à-dire, du navigateur au Manager, ou de l'API REST au Contrôleur), veuillez consulter xref:replacecert.adoc[cette section]. Voir ci-dessous pour remplacer les certificats utilisés dans la communication interne entre les conteneurs \{product-name}.
...
=== Test_CinqRemplacement des certificats utilisés dans les communications internes de \{product-name}
...
=== Test_CinqMise à jour/Déploiement avec Helm
