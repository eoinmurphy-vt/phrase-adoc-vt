...
== Test_CinqPlanification des Déploiements
...
=== Test_CinqMeilleures Pratiques, Conseils, Q&R pour Déployer et Gérer \{product-name}
...
== Test_FiveDéploiement en utilisant Helm ou des opérateurs
...
== Test_FiveDéploiement en utilisant ConfigMap
...
== Test_FiveDéploiement des contrôleurs
...
== Test_FiveSauvegardes et données persistantes
...
=== Test_CinqExemple de volume persistant
...
=== Test_CinqConfigMaps et stockage persistant
...
== Test_CinqMise à jour de la base de données des vulnérabilités CVE en production
...
== Test_CinqGestion des mises à jour d'hôte ou des nœuds d'auto-scaling avec un budget de perturbation de Pod
...
== Test_CinqDéploiement Sans Mode Privilégié
...
== Test_CinqArchitecture Multi-site, Multi-Cluster

Pour les entreprises ayant plusieurs emplacements et où un cluster \{nom-du-produit} séparé peut être déployé pour chaque emplacement, l'architecture de référence proposée est la suivante. Chaque cluster a son propre ensemble de contrôleurs et est géré séparément.

image:multisite.png[Multi-Site]

Voir une description plus détaillée dans ce fichier >
xref:attachment$multisite.pdf[\{nom-du-produit} Architecture Multi-Site]
