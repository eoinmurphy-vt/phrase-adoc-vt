...
=== Installation du plug-in Test_CinqJenkins
...
=== Test_CinqConfiguration globale dans Jenkins
...
=== Test_CinqConfiguration pour scanner un hôte Docker distant

==== Test_CinqPrérequis pour un scan local sur un hôte Docker distant
...
=== Test_FiveProject Configuration
...
=== Test_CinqÉtapes supplémentaires
...
=== Test_CinqConfiguration du pipeline pour des scans parallèles à grande échelle
...
== Test_CinqExemple de route OpenShift et de token de registre
...
== Test_CinqCas d'utilisation spécial pour Jenkins dans le même cluster Kubernetes
