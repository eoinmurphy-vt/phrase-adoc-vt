= Test projet quatorze SAML (ADFS)
:revdate: 2025-05-08
...
Cette section décrit les étapes de configuration dans ADFS d'abord, puis dans la console {product-name}.

=== Test_TwoADFS Setup

. Dans la gestion AD FS, faites un clic droit sur "`+Confiances de partie de confiance+`" et sélectionnez "`+Ajouter une confiance de partie de confiance...+`".
+
...
. Entrez la même URI de redirection SAML dans le champ "`+Identifiant de confiance de partie de confiance+`" et cliquez sur "`+Ajouter+`"; puis sélectionnez "`+Suivant+`" pour continuer.
+
