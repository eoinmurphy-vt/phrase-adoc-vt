= Test_TroisOpenID Connect Azure/Okta
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /08.integration/07.openid/07.openid.md
:page-opendocs-slug:  /intégration/openid

== Test_TroisIntégration avec OpenID Connect (OIDC) pour Azure et Okta
...
Par défaut, \{product-name} recherche "groupes" dans la revendication pour identifier l'appartenance au groupe de l'utilisateur. Si un autre nom de réclamation est utilisé, vous pouvez personnaliser le nom de la réclamation dans la page des paramètres OpenID Connect de \{product-name}.
...
Un nouveau panneau apparaîtra pour sélectionner le "`+Méthode de connexion+`" :
...
Pour la section URI de redirection de connexion :
...
Dans la section "`+Informations d'identification du client+`", cliquez sur modifier et modifiez la section "`+Authentification du client+`" de "`+Utiliser PKCE (pour les clients publics)+`" à "`+Utiliser l'authentification du client+`", puis cliquez sur enregistrer. Cela générera automatiquement un nouveau secret dont nous aurons besoin dans les prochaines étapes de configuration de \{product-name} :

image:okta7.png[clientauth]

Accédez à l'onglet "`+Connexion+`" et modifiez la section "`+Jeton d'identité OpenID Connect+`" :
Changez l'émetteur de 	"`+Dynamique (basé sur le domaine de la demande)+`" à l'URL fixe "`+Okta URL+`" :

image:okta8.png[token]

La console Okta peut fonctionner en deux modes, Mode Classique et Mode Développeur.
En mode classique, l'URL de l'émetteur se trouve dans l'onglet Connexion de la page de l'application Okta. Pour que l'appartenance du groupe de l'utilisateur soit renvoyée dans la revendication, vous devez ajouter le champ "groups" dans la page de configuration OpenID Connect de \{product-name} :
...
=== Test_ThreeMappage des groupes aux rôles et espaces de noms
