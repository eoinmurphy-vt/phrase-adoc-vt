...
Exemple - taille d'image la plus grande = Test_Two1.3Go, la mémoire du conteneur du scanner doit être de 1,8 Go.
...
* Pour la sauvegarde de configuration/HA, un PVC RWX de 1Gi ou plus. Voir xref:production.adoc#_backups_and_persistent_data[Section des sauvegardes et des données persistantes] pour plus de détails.
...
Remarque sur AWS Bottlerocket : Doit changer le chemin du socket containerd spécifique à Bottleneck. Veuillez consulter la section de déploiement Kubernetes pour plus de détails.
...
[NOTE]
====
PKS est testé sur le terrain et nécessite d'activer les conteneurs privilégiés pour le plan/tuile, et de modifier le yaml hostPath comme suit pour Allinone, Controller, Enforcer :
...
La latence est une autre métrique de performance qui dépend du type de connexions réseau. Tout comme le débit, la latence n'est pas affectée en mode Surveiller, seulement pour les services en mode Protéger (pare-feu en ligne). Les petits paquets ou les services simples/rapides généreront une latence plus élevée par \{product-name} en pourcentage, tandis que les paquets plus volumineux ou les services nécessitant un traitement complexe montreront un pourcentage de latence ajoutée par l'enforcer \{product-name} plus faible.

Le tableau ci-dessous montre la latence moyenne de 2 à 10 % mesurée à l'aide de l'outil de référence Redis. Le Benchmark Redis utilise des paquets assez petits, donc la latence avec des paquets plus volumineux serait attendue pour être plus faible.
...
=== Test_TwoAjout de contraintes de mise à l'échelle pour des environnements de charge de travail importants
