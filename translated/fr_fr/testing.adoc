= Test_TwoÉvaluation et Test \{product-name}
:revdate: 2025-05-08
:page-revdate: {revdate}
:page-opendocs-origin: /09.testing/01.testing/01.testing.md
:page-opendocs-slug:  /test/test

== Test_TwoSample Applications

Après avoir déployé les composants \{product-name}, vous pouvez les évaluer en utilisant les applications de test d'exemple que nous fournissons. Ceci est situé dans le dépôt `'nvbeta`' sur docker hub.
...
== Test_TwoPlan de Test Kubernetes
...
Après avoir déployé \{product-name}, vous pouvez exécuter du trafic de test à travers les applications de démonstration pour générer les règles de liste blanche, puis déplacer tous les services en mode Monitor ou Protect pour voir les violations et les attaques.

=== Test_TwoGénération de Violations Réseau sur Kubernetes
...
==== Test_TwoGénérer une Menace/Attaque
...
=== Test_TwoTests de Protection des Processus et des Fichiers
...
=== Test_TwoAnalyse de Registre et Contrôle d'Admission
...
==== Test_TwoDéployer une Autre Application
...
Après avoir déployé les composants \{product-name} et l'application d'exemple, vous pourrez découvrir, surveiller et protéger les conteneurs en cours d'exécution. Le plan de test ci-dessous fournit des suggestions pour générer des violations de comportement d'application autorisé en temps réel et scanner les conteneurs pour des vulnérabilités.
