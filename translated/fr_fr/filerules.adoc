...
=== Test_TroisProtections de Fichiers de Base

Si une installation de paquet est détectée, un rescannage automatique du conteneur ou de l'hôte sera déclenché pour détecter d'éventuelles vulnérabilités, SI le scan automatique a été activé dans Risques de Sécurité -> Vulnérabilités.
...
=== Test_TroisApplications autorisées basées sur l'apprentissage comportemental en mode Découverte
...
=== Test_TroisCréation de règles de surveillance de fichiers/répertoires personnalisées
...
=== Ordre de priorité de la règle d'accès au fichier Test_ThreeFile

Un conteneur peut hériter de la règle d'accès aux fichiers de plusieurs groupes personnalisés et de la règle d'accès aux fichiers créée par l'utilisateur sur le groupe appris automatiquement.
...
* Règle d'accès au fichier créée par l'utilisateur autre que les règles d'accès aux fichiers prédéfinies
...
Affichage de la règle d'accès qui protège le fichier /etc/passwd, qui est l'un des fichiers couverts par la règle d'accès prédéfinie afin de modifier l'action d'accès au fichier, pour la modification ainsi que la lecture. Cette règle personnalisée change l'action par défaut de la règle d'accès au fichier prédéfinie. L'application Nano peut avoir un accès 'lecture et modification' à ces fichiers. Il faut également ajouter l'application Nano (processus) en tant que règle 'autoriser' dans la règle de profil de processus pour que ce service exécute l'application Nano à l'intérieur du service (si elle n'était pas déjà sur la liste blanche), sinon le processus sera bloqué par \{product-name}.

image:example3.png[RèglesFichiers]

Affichage que l'application python a été apprise en accédant au fichier sous le répertoire /var/opt lorsque le mode service du node-pod était en Découverte. Cela ne se produit que lorsque la règle est définie sur Bloquer et que le service est en mode Découverte.
...
Affichage d'un événement de sécurité d'exemple dans Notifications -> Événements de sécurité, alerté comme refus d'accès au fichier lorsque la modification du fichier /etc/hostname par l'application python a été refusée en raison d'une règle d'accès au fichier personnalisée avec action de blocage.

image:securityevent.png[RèglesFichiers]

=== Test_TroisAutres Réponses
...
== Test_ThreeSplit Mode File Protections
