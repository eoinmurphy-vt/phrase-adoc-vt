...
=== Tester la suppression automatique des groupes inutilisés
...
=== Tester la protection des hôtes - le groupe 'Nœuds'
...
Pour activer la protection des hôtes avec des règles de profil de processus, sélectionnez le groupe 'nœuds' et examinez les processus appris sur le nœud. Personnalisez si nécessaire en ajoutant, supprimant ou modifiant des règles de processus. Puis changez le mode en Surveiller ou Protéger.

[NOTE]
====
Les violations de règles de connexion réseau affichées dans les Règles Réseau pour les Nœuds ne sont jamais bloquées, même en mode Protéger. Seules les violations de processus sont bloquées en mode Protéger sur les nœuds.
====

=== Tester des Groupes Personnalisés

Les groupes peuvent être ajoutés manuellement en saisissant les critères pour le groupe. Remarque : Les groupes créés sur mesure n'ont pas de mode de Protection. C'est parce qu'ils peuvent contenir des conteneurs de différents groupes sous-jacents, chacun pouvant être dans un mode différent, ce qui entraîne une confusion sur le comportement.
...
+
____
Sélectionnez des conteneurs par les nœuds sur lesquels ils s'exécutent. Exemples : nœud=ip-12-34-56-78.us-west-2
____

* *Conteneurs individuels*
+
____
Sélectionnez des conteneurs par leurs noms d'instance. Exemples : conteneur=nodejs_1, conteneur@nodejs
____

* *Services*
+
____
Sélectionnez des conteneurs par leurs services. Si un conteneur est déployé par Docker Compose, sa valeur de tag de service sera "nom_du_projet:nom_du_service" ; si un conteneur est déployé par le service en mode Docker swarm, sa valeur de tag de service sera le nom du service swarm.
____
...
+
____
Créez un groupe par nom DNS ou plages d'adresses IP. Exemples : address=www.google.com, address=10.1.0.1, address=10.1.0.0/24, address=10.1.0.1-10.1.0.25. Le nom DNS peut être n'importe quel nom résolvable. Les critères d'adresse n'acceptent pas l'opérateur de test !=. Voir ci-dessous pour les groupes d'adresses d'hôtes virtuels 'vh' spéciaux.
____
...
==== Tester la politique réseau basée sur l'hôte virtuel ('vh')
...
== Tester des exemples de groupes personnalisés

=== Tester des critères généraux
...
* Pour sélectionner tous les conteneurs NON dans l'espace de noms 'apache1' et 'apache2' (appuyez sur entrée après chaque entrée)
+
____
namespace!=apache1 namespace!=apache2
____
