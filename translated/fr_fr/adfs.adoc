...
== Tester la configuration de l'intégration ADFS et \{product-name}
...
. Dans la gestion AD FS, faites un clic droit sur "`+Relying Party Trusts+`" et sélectionnez "`+Ajouter un Relying Party Trust...+`".
+
...
. Sélectionnez "`+Entrer les données sur le relying party manuellement+`" et sélectionnez "`+Suivant+`".
+
...
. Cochez "`+Activer le support pour le protocole SAML 2.0 WebSSO+`" et entrez l'URI de redirection SAML depuis la page Paramètres>\{product-name}>Paramètre SAML dans le champ "`+URL de service SSO SAML 2.0 du relying party+`".  Sélectionnez "`+Suivant+`" pour continuer.
+
image:adfs6.png[adfsSetup]

. Entrez la même URI de redirection SAML dans le champ "`+Identifiant de confiance du relying party+`" et cliquez sur "`+Ajouter+`"; puis sélectionnez "`+Suivant+`" pour continuer.
+
...
. Sélectionnez Modifier la politique d'émission des revendications...
+
...
=== Tester la configuration \{product-name}

. Identifier l'URL de connexion unique du fournisseur
* Voir les points de terminaison depuis la gestion AD FS > Service et utiliser l'URL de point de terminaison "`+SAML 2.0/WS-Fédération+`".
...
=== Tester la cartographie des groupes aux rôles et espaces de noms
...
. Les caractères SAML doivent être sensibles à la casse dans l'interface utilisateur de \{product-name}
+
...
[CAUTION]
====
Ces paramètres doivent être validés sur votre serveur ADFS et dans l'interface utilisateur de \{product-name}.
====
