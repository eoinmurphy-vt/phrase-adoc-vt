...
* **NVSHAS-8583**: Festlegen von granularen Richtlinienmodi für Regelsets, separaten Netzwerkrichtlinienmodus und Profilmodus auf Gruppenebene.
...
* **NVSHAS-9480**: Die NV UI, die im Rancher-Downstream-Cluster bereitgestellt wird, gibt nach dem Abmelden von Rancher einen HTTP/403-Fehler aus.
...
* **NVSHAS-9403**: Filemonitor-UI: Erlauben Sie dem Benutzer, eine vordefinierte Regel für die Dateimonitorüberwachung zu löschen.
...
* **NVSHAS-7687**: Unterstützung für die Konfiguration des Protokollniveaus (debug/error/info/warn) für den Durchsetzer und den Controller über die CLI.
...
* **NVSHAS-9266**: Registrierungs-Scan: Der Scanbericht-Button nach Schicht sollte ausgeblendet oder deaktiviert sein, wenn es keine Schwachstelle gibt.
...
* **NVSHAS-9080**: Fed-Leserbenutzer kann auf einige REST-APIs nicht zugreifen.
...
* Die Versionierung des Prometheus-Exporter-Containers wurde überprüft und von der `+controller+` Versionierung disssoziiert.
...
* (Scanner) OCI-Image-Format akzeptieren, wenn auf Docker-API 1.24 umgeschaltet wird.
...
* Vereinheitlichen Sie die Anmeldung der Browsersitzung. Damit teilen sich alle Tabs im Browser dieselbe Anmeldesitzung, das Öffnen eines neuen Tabs aus einer bestehenden Sitzung erfordert keine Anmeldeinformationen, und wenn sich ein Tab abmeldet, werden alle Tabs abgemeldet.
...
* Unterstützen Sie Deep Linking in URLs zur Seite mit Bild- und/oder Containeranfälligkeiten
...
* Unterstützung der AWS Marketplace PAYG-Abrechnung für {product-name} monatliche Support-Abonnements. Benutzer können sich über den SUSE-Support für {product-name} anmelden, die monatlich auf ihr AWS-Konto basierend auf der durchschnittlichen Knotenanzahl des Vormonats abgerechnet werden. Details https://open-docs.neuvector.com/deploying/awsmarketplace[hier].
...
{product-name} benötigt keine zusätzlichen Einstellungen für SELinux-aktivierte Cluster, um bereitgestellt und zu funktionieren. Das Bereitstellen von {product-name} auf einem SELinux-aktivierten RKE2-härteten Cluster, der auf RHEL 8.5 basiert, wurde getestet. {product-name} wurde erfolgreich bereitgestellt, wenn PSP aktiviert ist und das Patch-Manager- und Scanner-Bereitstellung. Die nächste Chart-Version sollte das untenstehende Problem beheben.
