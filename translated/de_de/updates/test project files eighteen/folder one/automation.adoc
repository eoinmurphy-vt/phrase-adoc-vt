...
Stellen Sie sicher, dass Sie die IP-Adresse des Controllers im Skript eingeben und den Namen des Container-Images in das gewünschte Bild ändern. Aktualisieren Sie auch die Felder für Benutzername/Passwort.
...
=== Test_ZweiExport/Import-Konfigurationsdatei
...
=== Test_ZweiEinstellung oder Ändern des Benutzerpassworts
...
=== Test_ZweiStarten der Paketaufzeichnung auf einem Container
...
=== Test_ZweiÜberprüfen und Akzeptieren der EULA (neue Bereitstellungen)

Holen Sie sich das Authentifizierungstoken wie oben beschrieben. Ersetzen Sie auch die IP-Adresse des Controllers durch Ihre, wie angemessen.
...
=== Test_TwoPacket Capture für alle Pods in einem Namespace aktivieren
...
=== Test_TwoContainer-Quarantäne deaktivieren
...
=== Test_TwoDebug-Modus für {product-name} Support aktivieren

Zugriffstoken mit Ihrer IP, Benutzer, Passwort festlegen:
...
=== Test_TwoBerichten, ob eine Schwachstelle in den Basisschichten des Images vorhanden ist
