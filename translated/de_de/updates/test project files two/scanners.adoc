= Test_ZweiParallel & Standalone Scanner
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /06.scanning/06.scanners/06.scanners.md
:page-opendocs-slug:  /scanning/scanner

== Test_ZweiErhöhung der Scanner-Skalierbarkeit mit mehreren Scannern
...
Standardmäßig stellt \{product-name} 2 Scanner-Pods als Teil der Beispielbereitstellungen im Abschnitt Bereitstellung von \{product-name} bereit. Diese Replicasets können bei Bedarf hoch- oder heruntergefahren werden.
...
=== Test_ZweiAuto-Skalierung von Scanner-Pods
...
=== Test_ZweiOperationen und Debugging
...
== Test_TwoStandalone Scanner für lokale Scans
...
* SCANNER_REGISTRY= Test_Twourl des Registrys (optional anstelle des lokalen Scans)
* SCANNER_REPOSITORY= Test_Tworepository zum Scannen
* SCANNER_TAG= Test_Twoversionstag
* SCANNER_REGISTRY_USERNAME= Test_Twouser (optional anstelle des lokalen Scans)
* SCANNER_REGISTRY_PASSWORD= Test_Twopasswort (optional anstelle des lokalen Scans)
* SCANNER_SCAN_LAYERS= Test_Twowahr oder falsch (um geschichtete Scannergebnisse zurückzugeben)
...
=== Test_TwoHost Scannen im Standalone-Modus
...
=== Test_TwoManual Bereitstellung mehrerer Scanner auf Kubernetes
...
Verwenden Sie die Datei unten, um mehrere Scanner bereitzustellen. Bearbeiten Sie die Replikate, um die Anzahl der parallel laufenden Scanner zu erhöhen oder zu verringern.
