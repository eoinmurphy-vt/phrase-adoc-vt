= Testprojekt vierzehn SAML (ADFS)
:revdate: 2025-05-08
...
. Wählen Sie "`+Daten über die vertrauende Partei manuell eingeben+`" und wählen Sie "`+Weiter+`".
+
...
. Überprüfen Sie "`+Unterstützung für das SAML 2.0 WebSSO-Protokoll aktivieren+`" und geben Sie die SAML-Redirect-URI von {product-name} Einstellungen>SAML-Einstellungsseite in das Feld "`+SSO-Dienst-URL für die vertrauende Partei SAML 2.0+`" ein.  Wählen Sie "`+Weiter+`", um fortzufahren.
+
...
* Klicken Sie mit der rechten Maustaste auf AD FS in der AD FS-Verwaltungskonsole und wählen Sie "`+Federation Service-Eigenschaften bearbeiten...+`"; verwenden Sie die "`+Federation Service-Identifikator+`".
