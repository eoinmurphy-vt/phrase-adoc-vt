...
== Test_TwoEinrichten von ADFS und {product-name} Integration

Dieser Abschnitt beschreibt zuerst die Einrichtungsschritte in ADFS und dann in der {product-name} Konsole.

=== Test_TwoADFS Setup

. Klicken Sie mit der rechten Maustaste auf "`+Vertrauensstellung für Parteien+`" in der AD FS-Verwaltung und wählen Sie "`+Vertrauensstellung für Parteien hinzufügen...+`".
+
...
. Wählen Sie "`+Geben Sie die Daten zur vertrauenden Partei manuell ein+`" aus und wählen Sie "`+Weiter+`".
+
...
. Überprüfen Sie "`+Unterstützung für das SAML 2.0 WebSSO-Protokoll aktivieren+`" und geben Sie die SAML-Redirect-URI von {product-name} Einstellungen>SAML-Einstellungsseite in das Feld "`+SSO-Dienst-URL der vertrauenden Partei SAML 2.0+`" ein.  Wählen Sie "`+Weiter+`", um fortzufahren.
+
