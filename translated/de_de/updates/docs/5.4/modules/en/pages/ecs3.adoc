...
--
. Jetzt können Sie den Allinone-Dienst bereitstellen. Setzen Sie die &quot;`+Number of tasks+`+&quot; auf die gewünschte Anzahl von Allinone/Controllern. Jetzt werden die {product-name} Allinone- oder Controller-Container auf den ausgewählten Knoten ausgeführt. Nachdem der Allinone läuft, sollten Sie in der Lage sein, über HTTPS auf die {product-name} Konsole über Port 8443 zuzugreifen.
...
. In ECS-Diensten setzen und löschen Sie den alten Allinone/Controller-Dienst. Ziehen Sie die aktualisierten {product-name} Bilder manuell oder lösen Sie AWS ECS aus, um neue Versionen der Allinone/Controller-Container von Dockerhub oder Ihrem privaten Repository abzurufen.
