...
====
--
. Erstellen Sie einen neuen Dienst für die Allinone-Aufgabe. Setzen Sie die &quot;+`+Platzierungsvorlagen+`+&quot; auf &quot;+`+Eine Aufgabe pro Host+`+&quot;, damit nur ein Allinone/Controller auf einem Host ausgeführt werden kann. Sie werden auch sehen, dass die Einschränkung verwendet wird "+`memberOf(attribute:allinone-node=~true), was erfordert, dass der Knoten dieses Attribut hat.
+
--
image:3taskplacement.png[AllinonePlace]
--
. Jetzt können Sie den Allinone-Dienst bereitstellen. Setzen Sie die &quot;`+Number of tasks+`+&quot; auf die gewünschte Anzahl an Allinone/Controller. Jetzt werden die {product-name} Allinone- oder Controller-Container auf den ausgewählten Knoten ausgeführt. Nachdem der Allinone gestartet wurde, sollten Sie in der Lage sein, über HTTPS auf die {product-name} Konsole auf Port 8443 zuzugreifen.
...
. In ECS-Diensten setzen und löschen Sie den alten Allinone/Controller-Dienst. Ziehen Sie die aktualisierten {product-name} Bilder manuell oder lösen Sie AWS ECS aus, um neue Versionen der Allinone/Controller-Container von Dockerhub oder Ihrem privaten Repository zu ziehen.
...
. Von ECS-Diensten aus, fahren Sie die Durchsetzer herunter und aktualisieren Sie sie. Manuell oder automatisch das Abrufen neuer Durchsetzer-Bilder auslösen. Starten oder aktualisieren Sie dann den Durchsetzer auf allen Knoten neu. Aus der {product-name} Konsole sehen Sie, dass alle Durchsetzer auf dem neuesten Stand sind.
