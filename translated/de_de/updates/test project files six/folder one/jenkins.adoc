...
=== Test_SixJenkins Plugin-Installation
...
Um einen Sicherheits-Scan im Standalone-Modus durchzuführen, muss das Jenkins-Plugin das Scanner-Image auf den Host ziehen, auf dem der Agent läuft. Daher müssen Sie die \{product-name} Scanner-Registry-URL, das Image-Repository und die Anmeldeinformationen, falls erforderlich, auf der Konfigurationsseite des \{product-name} Plugins eingeben.

Das Scan-Ergebnis kann auch an den Controller übermittelt und in der Zulassungssteuerungsfunktion verwendet werden. In diesem Fall benötigen Sie ein Controller-Setup und müssen angeben, wie Sie sich auf der Konfigurationsseite des \{product-name} Plugins mit dem Controller verbinden.

=== Test_SixLocal-Konfiguration zum Scannen eines Remote-Docker-Hosts

==== Test_SixVoraussetzungen für den lokalen Scan auf einem Remote-Docker-Host
...
Wählen Sie Local oder einen Registrierungsnamen, der der Spitzname ist, den Sie in der globalen Konfiguration eingegeben haben. Geben Sie den Repository- und Bild-Tag-Namen ein, die gescannt werden sollen. Sie können die Standardumgebungsvariablen von Jenkins für das Repository oder den Tag wählen, z.B. $JOB_NAME, $BUILD_TAG, $BUILD_NUMBER. Geben Sie die Werte für die Anzahl der hohen oder mittleren und für jeden Namen der vorhandenen Schwachstellen ein, um den Build fehlschlagen zu lassen.
...
=== Test_SixEinrichten der Pipeline für großangelegte parallele Scans
...
pipeline {
    agent any
    environment {
        REPO_NAME = Test_Six'your repo'
        REGISTRY_SELECTION = Test_Six'your registry'
        CONTROLLER = Test_Six'your controller'
        MAX_CONCURRENT_SCANS = Test_Six32
    }
    stages {
        stage('Parallel Vulnerability Scanning') {
            steps {
                script {
                    ...
                    TAGS_LIST_PART1 = Test_Six["your tags"...]
                    TAGS_LIST_PART2 = Test_Six["your tags"...]
                    TAGS_LIST_PART3 = Test_Six["your tags"...]
                    TAGS_LIST_PART4 = Test_Six["your tags"...]
                    TAGS_LIST_PART5 = Test_Six["your tags"...]...
                    def allTags = Test_SixTAGS_LIST_PART1 + TAGS_LIST_PART2 + TAGS_LIST_PART3 + TAGS_LIST_PART4 + TAGS_LIST_PART5
                    def batches = Test_SixallTags.collate(MAX_CONCURRENT_SCANS.toInteger()) // Ensure MAX_CONCURRENT_SCANS is an integer
                    def batchCounter = Test_Six1                    for (batch in batches) {
                        stage("Batch ${batchCounter}") {
                            def scans = Test_Six[:]
                            batch.each { tag ->
                                def currentTag = Test_Sixtag
                                scans["Scan ${currentTag}"] = Test_Six{
...
== Test_SixOpenShift-Routen- und Registry-Token-Beispiel
...
== Test_SixBesonderer Anwendungsfall für Jenkins im selben Kubernetes-Cluster
