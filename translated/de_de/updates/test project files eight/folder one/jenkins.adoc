...
=== Test_EightJenkins Plugin-Installation
...
Um einen Schwachstellenscan im Standalone-Modus auszuführen, muss das Jenkins-Plugin das Scanner-Image auf den Host ziehen, auf dem der Agent läuft. Daher müssen Sie die \{product-name} Scanner-Registry-URL, das Image-Repository und die Anmeldeinformationen, falls erforderlich, auf der Konfigurationsseite des \{product-name} Plugins eingeben.

Das Scan-Ergebnis kann auch an den Controller übermittelt und in der Funktion zur Zulassungskontrolle verwendet werden. In diesem Fall benötigen Sie ein Controller-Setup und müssen angeben, wie Sie auf den Controller auf der Konfigurationsseite des \{product-name} Plugins zugreifen.

=== Test_EightLocal Konfiguration zum Scannen eines Remote-Docker-Hosts

==== Test_EightVoraussetzungen für den lokalen Scan auf einem Remote-Docker-Host
...
=== Test_EightProject-Konfiguration
...
Sie sind jetzt bereit, Ihre Jenkins-Bauten zu starten und den \{product-name} Vulnerability Scanner auszulösen, um alle Schwachstellen zu melden!

=== Test_EightEinrichten der Pipeline für große parallele Scans
...
pipeline {
    agent any
    environment {
        REPO_NAME = Test_Eight'your repo'
        REGISTRY_SELECTION = Test_Eight'your registry'
        CONTROLLER = Test_Eight'your controller'
        MAX_CONCURRENT_SCANS = Test_Eight32
    }
    stages {
        stage('Parallel Vulnerability Scanning') {
            steps {
                script {
                    ...
                    TAGS_LIST_PART1 = Test_Eight["your tags"...]
                    TAGS_LIST_PART2 = Test_Eight["your tags"...]
                    TAGS_LIST_PART3 = Test_Eight["your tags"...]
                    TAGS_LIST_PART4 = Test_Eight["your tags"...]
                    TAGS_LIST_PART5 = Test_Eight["your tags"...]...
                    def allTags = Test_EightTAGS_LIST_PART1 + TAGS_LIST_PART2 + TAGS_LIST_PART3 + TAGS_LIST_PART4 + TAGS_LIST_PART5
                    def batches = Test_EightallTags.collate(MAX_CONCURRENT_SCANS.toInteger()) // Ensure MAX_CONCURRENT_SCANS is an integer
                    def batchCounter = Test_Eight1                    for (batch in batches) {
                        stage("Batch ${batchCounter}") {
                            def scans = Test_Eight[:]
                            batch.each { tag ->
                                def currentTag = Test_Eighttag
                                scans["Scan ${currentTag}"] = Test_Eight{
...
== Test_EightOpenShift-Routen- und Registry-Token-Beispiel

Um das Plugin mit einer OpenShift-Route für den Zugriff auf den Controller zu konfigurieren, fügen Sie die Route in das Feld Controller-IP ein.
