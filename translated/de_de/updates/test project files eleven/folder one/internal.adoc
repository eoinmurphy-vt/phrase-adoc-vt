= Test_ElevenInterne Zertifikate ersetzen
:revdate: 2025-01-27
...
[IMPORTANT]
====
NeuVector Versionen 5.4.2 und höher müssen Benutzer dazu bringen, interne Zertifikate zu generieren/ersetzen, bevor sie NeuVector verwenden.
Nach März 2025 müssen NeuVector-Versionen vor 5.4.2 Benutzer dazu bringen, interne Zertifikate zu generieren/ersetzen, bevor sie NeuVector verwenden.
====

== Test_TwoInterne Kommunikation und Zertifikate
...
=== Test_TwoZertifikate ersetzen, die in internen Kommunikationen von {product-name} verwendet werden
...
----
+
* Bearbeiten Sie dann die Controller-, Enforcer- und Scanner-Bereitstellungs-YAMLs und fügen Sie Folgendes hinzu:
+
[source,yaml]
----
      containers:
...
+
Beispiel-Patch-Befehle für den Controller (Namespace bei Bedarf in cattle-neuvector-system ändern und für die Verwendung bei Enforcer, Scanner anpassen)
+
[source,bash]
----
NAMESPACE=neuvector
...
----
+
* Aktualisieren Sie das Kubernetes-Secret mit dem zusammengeführten `+ca.crt+`
+
[source,bash]
----
kubectl delete secret internal-cert -n neuvector 
...
----
+
* Stellen Sie sicher, dass die Konsole zugänglich ist und alle Controller online sind.* Aktualisieren Sie das Kubernetes-Secret mit dem neuen `+tls.key+` 
+
[source,bash]
----
kubectl delete secret internal-cert -n neuvector 
...
----
+
* Stellen Sie sicher, dass die Konsole zugänglich ist und alle Controller online sind.* Aktualisieren Sie das Kubernetes-Secret mit dem neuen `+ca.crt+` 
+
[source,bash]
----
kubectl delete secret internal-cert -n neuvector
...
=== Test_TwoAktualisieren/Bereitstellen mit Helm
