= Test_FiveJenkins Details
:revdate: 2025-05-05
:page-revdate: {revdate}
:page-opendocs-origin: /06.scanning/03.build/01.jenkins/01.jenkins.md
:page-opendocs-slug:  /scanning/build/jenkins

== Test_FiveDetaillierte Konfiguration für das Jenkins-Plugin

Container bieten eine einfache und effiziente Möglichkeit, Anwendungen bereitzustellen. Aber Container-Images können Open-Source-Code enthalten, über den Sie keine vollständige Kontrolle haben. Viele Schwachstellen in Open-Source-Projekten wurden gemeldet, und Sie können entscheiden, ob Sie diese Bibliotheken mit Schwachstellen verwenden möchten oder nicht, nachdem Sie die Images gescannt und die Schwachstelleninformationen überprüft haben.

Der \{product-name} Vulnerability Scanner Jenkins-Plugin kann die Images scannen, nachdem Ihr Image in Jenkins erstellt wurde. Der Quellcode des Plugins und die neuesten Dokumentationen finden Sie https://github.com/jenkinsci/neuvector-vulnerability-scanner-plugin[hier] auf der \{product-name} GitHub-Seite.
...
Für den "Controller & Scanner"-Modus müssen Sie den \{product-name} Controller und Scanner im Netzwerk bereitstellen. Um das lokale Image (das Image auf der Jenkins-Maschine) zu scannen, muss der "Controller & Scanner" auf demselben Knoten installiert sein, auf dem das Image vorhanden ist.
...
=== Test_FiveJenkins Plugin-Installation
...
Es gibt zwei Szenarien für das Scannen von Bildern: lokales Scannen und Scannen von Registries.
...
Klicken Sie auf '`+Registry hinzufügen+`', um Werte für die Registry einzugeben, die Sie in Ihrem Projekt verwenden werden. Wenn Sie nur lokale Images scannen, müssen Sie hier keine Registry hinzufügen.

Szenario 1: Beispiel für globale Konfiguration zum Scannen lokaler Images

image:jenkins1a.png[global-image]

Szenario 2: Beispiel für globale Konfiguration zum Scannen von Registry-Images

Für die globale Registry-Konfiguration folgen Sie den Anweisungen oben für lokal und fügen Sie dann die Registry-Details wie unten hinzu.

image:registry_console.png[global-registry]

==== Test_FiveStandalone Scanner

Das Ausführen des Jenkins-Scans im Standalone-Modus ist eine leichte Möglichkeit, Bildanfälligkeiten in der Pipeline zu scannen. Der Scanner wird dynamisch aufgerufen und es ist keine Installation des Controller-Setups erforderlich. Dies ist besonders nützlich, wenn Sie ein Image scannen, bevor es in eine Registry hochgeladen wird. Es gibt auch keine Begrenzung, wie viele Scan-Aufgaben gleichzeitig ausgeführt werden können.

Um einen Sicherheits-Scan im Standalone-Modus auszuführen, muss das Jenkins-Plugin das Scanner-Image auf den Host ziehen, auf dem der Agent läuft. Daher müssen Sie die \{product-name} Scanner-Registry-URL, das Image-Repository und die Anmeldeinformationen, falls erforderlich, auf der Konfigurationsseite des \{product-name} Plugins eingeben.

Das Scan-Ergebnis kann auch an den Controller übermittelt und in der Funktion zur Zulassungskontrolle verwendet werden. In diesem Fall benötigen Sie ein Controller-Setup und müssen angeben, wie Sie sich auf der Konfigurationsseite des \{product-name} Plugins mit dem Controller verbinden.

=== Test_FiveLocal Konfiguration zum Scannen eines Remote-Docker-Hosts

==== Test_FiveVoraussetzungen für den lokalen Scan auf einem Remote-Docker-Host

Um \{product-name} zu aktivieren, um ein Image zu scannen, das sich nicht auf demselben Host wie der Controller/allinone befindet:
...
=== Test_FiveProject-Konfiguration

Wählen Sie in Ihrem Projekt das Plugin '\{product-name} Vulnerability Scanner' aus dem Dropdown-Menü in "Build-Schritt hinzufügen" aus. Aktivieren Sie das Kontrollkästchen "Mit Standalone-Scanner scannen", wenn Sie den Scan im Standalone-Scanner-Modus durchführen möchten. Standardmäßig verwendet es den Modus "Controller & Scanner", um den Scan durchzuführen.
...
Wählen Sie den \{product-name} Vulnerability Scanner aus dem Dropdown-Menü aus, konfigurieren Sie ihn und generieren Sie das Skript.

image:jenkins6a.png[script]

Kopieren Sie das Skript in Ihr Jenkins-Aufgabenskript.

Szenario 1: Ein einfaches lokales Pipeline-Skriptbeispiel (zum Einfügen in Ihr Pipeline-Skript):
...
Szenario 2: Ein einfaches Registry-Pipeline-Skriptbeispiel (zum Einfügen in Ihr Pipeline-Skript):
...
=== Test_FiveAdditional Stages

Fügen Sie Ihre eigenen Vor- und Nachbild-Scan-Stufen hinzu, zum Beispiel im untenstehenden Beispiel der Pipeline-Stufenansicht.
...
=== Test_FünfEinrichten der Pipeline für großangelegte parallele Scans

Verfügbar mit NeuVector v5.4.3 und später unterstützt das NeuVector Schwachstellenscanner Jenkins-Plugin v2.5 und später das parallele Scannen von bis zu 2000 gleichzeitigen Scans im API-Schlüsselmodus. Für frühere Versionen von NeuVector sind die maximalen gleichzeitigen Scans auf 32 im Token-Modus beschränkt. Klicken Sie hier, um die Beispiele unten zu erweitern und die Beispiel-Pipeline-Konfigurationen anzuzeigen.
...
pipeline {
    agent any
    environment {
        REPO_NAME = Test_Five'your repo'
        REGISTRY_SELECTION = Test_Five'your registry'
        CONTROLLER = Test_Five'your controller'
        MAX_CONCURRENT_SCANS = Test_Five32
    }
    stages {
        stage('Parallel Vulnerability Scanning') {
            steps {
                script {
                    ...
                    TAGS_LIST_PART1 = Test_Five["your tags"...]
                    TAGS_LIST_PART2 = Test_Five["your tags"...]
                    TAGS_LIST_PART3 = Test_Five["your tags"...]
                    TAGS_LIST_PART4 = Test_Five["your tags"...]
                    TAGS_LIST_PART5 = Test_Five["your tags"...]...
                    def allTags = Test_FiveTAGS_LIST_PART1 + TAGS_LIST_PART2 + TAGS_LIST_PART3 + TAGS_LIST_PART4 + TAGS_LIST_PART5
                    def batches = Test_FiveallTags.collate(MAX_CONCURRENT_SCANS.toInteger()) // Ensure MAX_CONCURRENT_SCANS is an integer
                    def batchCounter = Test_Five1                    for (batch in batches) {
                        stage("Batch ${batchCounter}") {
                            def scans = Test_Five[:]
                            batch.each { tag ->
                                def currentTag = Test_Fivetag
                                scans["Scan ${currentTag}"] = Test_Five{
...
== Test_FünfOpenShift-Routen- und Registry-Token-Beispiel
...
== Test_FünfBesonderer Anwendungsfall für Jenkins im selben Kubernetes-Cluster

Um Scans in der Build-Phase durchzuführen, bei denen die Jenkins-Software im selben Kubernetes-Cluster wie der Scanner ausgeführt wird, stellen Sie sicher, dass der Scanner und Jenkins auf demselben Knoten ausgeführt werden. Der Knoten muss so gekennzeichnet sein, dass die Jenkins- und Scanner-Container auf demselben Knoten ausgeführt werden, da der Scanner Zugriff auf das lokale docker.sock des Knotens benötigt, um auf das Bild zuzugreifen.
