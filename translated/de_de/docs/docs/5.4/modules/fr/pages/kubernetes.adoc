...
== Mit Kubernetes bereitstellen

Sie können Kubernetes verwenden, um separate Manager-, Controller- und Executor-Container bereitzustellen und sicherzustellen, dass alle neuen Knoten einen bereitgestellten Executor haben. {product-name} erfordert und unterstützt Kubernetes-Netzwerk-Plugins wie flannel, weave oder calico.

Die Beispieldatei wird einen Manager und 3 Controller bereitstellen. Es wird einen Executor auf jedem Knoten als Daemonset bereitstellen. Standardmäßig wird das folgende Beispiel auch auf dem Master-Knoten bereitgestellt.

Siehe den unteren Abschnitt, um dedizierte Manager- oder Controller-Knoten mithilfe von Knotentags anzugeben. 

[NOTE]
====
Es wird nicht empfohlen, mehr als einen Manager hinter einem Lastenausgleich bereitzustellen (skalieren), da dies potenzielle Probleme mit dem Sitzungsstatus verursachen kann. Wenn Sie planen, eine PersistentVolume-Anforderung zur Speicherung der Sicherung von Konfigurationsdateien {product-name} zu verwenden, lesen Sie bitte den allgemeinen Abschnitt Backup/Persistent Data im Überblick xref:production.adoc#_backups_and_persistent_data[Bereitstellung {product-name}].
====

Wenn Ihre Bereitstellung einen integrierten Lastenausgleich unterstützt, ändern Sie den Typ NodePort in LoadBalancer für die Konsole in der folgenden yaml-Datei.

{product-name} unterstützt die Helm-basierte Bereitstellung mit einem Helm-Diagramm bei https://github.com/neuvector/neuvector-helm.

Es gibt einen separaten Abschnitt für OpenShift-Anweisungen, und Docker EE auf Kubernetes hat einige spezielle Schritte, die im Abschnitt Docker beschrieben sind.

=== {product-name} Bilder auf Docker Hub

Die Bilder befinden sich im Docker Hub-Register {product-name}. Verwenden Sie das entsprechende Versions-Tag für den Manager, den Controller, den Executor und lassen Sie die Version als 'latest' für den Scanner und das Update. Zum Beispiel:
...
Wenn Sie mit dem aktuellen Helm-Chart {product-name} (v1.8.9+) bereitstellen, müssen die folgenden Änderungen an values.yml vorgenommen werden:

* Aktualisieren Sie das Repository auf docker.io
* Aktualisieren Sie die Namen/Tags der Bilder auf die aktuelle Version auf Docker Hub, wie oben angegeben
* Laissez les imagePullSecrets vides

[NOTE]
====
Wenn Sie aus dem Rancher Manager 2.6.5+ {product-name} Chart bereitstellen, werden die Bilder automatisch aus dem Mirror-Image-Repository des Rancher-Registrys gezogen und im Namespace cattle-neuvector-system bereitgestellt.
====


== Déployer {product-name}

. Erstellen Sie den Namespace {product-name} und die erforderlichen Dienstkonten: 
+
--
[,shell]
----
...
----
--
. (*Optional*) Erstellen Sie die Pod-Sicherheitszulassung (PSA) oder die Pod-Sicherheitsrichtlinie (PSP) {product-name}. Wenn Sie die Pod-Sicherheitszulassung (auch bekannt als Pod-Sicherheitsstandards) in Kubernetes 1.25+ oder die Pod-Sicherheitsrichtlinien (vor 1.25) in Ihrem Kubernetes-Cluster aktiviert haben, fügen Sie Folgendes für {product-name} hinzu (z. B. nv_psp.yaml). 
+
[NOTE]
====
* La PSP est obsolète dans Kubernetes 1.21 et sera complètement supprimée dans 1.25. 
* Die Pods Manager und Scanner laufen ohne uid. Wenn Ihre PSP eine Regel `+Run As User: Rule: MustRunAsNonRoot+` hat, fügen Sie Folgendes im folgenden YAML-Beispiel hinzu (mit dem entsprechenden Wert für `+###+`):
====
+
--
[,yaml]
...
Für PSA in Kubernetes 1.25+ kennzeichnen Sie den Namespace {product-name} mit einem privilegierten Profil für die Bereitstellung in einem für PSA aktivierten Cluster. 
[,shell]
----
kubectl label namespace neuvector "pod-security.kubernetes.io/enforce=privileged" 
----
--
. Erstellen Sie die benutzerdefinierten Ressourcen (CRD) für die Sicherheitsregeln {product-name}. Für Kubernetes 1.19+: 
+
--
[,shell]
----
...
----
--
. Fügen Sie die Leseberechtigung hinzu, um auf die Kubernetes-API zuzugreifen. 
+
[IMPORTANT]
====
Die Standardbereitstellung {product-name} 5.2+ verwendet Dienstkonten mit minimalen Berechtigungen anstelle der Standardkonten. Voir ci-dessous si vous mettez à niveau à partir d'une version antérieure à 5.3. 
====
+
[WARNING]
====
Wenn Sie auf 5.3.0+ aktualisieren, führen Sie die folgenden Befehle je nach Ihrer aktuellen Version aus:
====
+
--
[tabs]
...
Wenden Sie die Leseberechtigungen über die folgenden "ClusterRole erstellen"-Befehle an: 
...
----
--
. (*Optional*) Erstellen Sie den Föderationsmaster und/oder die Remote-Multi-Cluster-Managementdienste. Wenn Sie die Multi-Cluster-Managementfunktionen in {product-name} verwenden möchten, muss ein Cluster den Föderationsmaster-Dienst bereitgestellt haben, und jeder Remote-Cluster muss den Föderationsarbeiter-Dienst haben. Für mehr Flexibilität können Sie wählen, sowohl die Master- als auch die Arbeiterdienste in jedem Cluster bereitzustellen, sodass jeder Cluster ein Master oder Remote sein kann. Gestion de Cluster Fédéré 
+
--
[,yaml]
----
...
----
--
. Erstellen Sie die Hauptdienste und Pods {product-name} mit den vordefinierten Versionsbefehlen oder ändern Sie das folgende Beispiel-YAML. Die vordefinierte Version ruft einen LoadBalancer für die Konsole {product-name} auf. Wenn Sie die folgende Beispiel-YAML-Datei verwenden, ersetzen Sie die Bildnamen und Tags <version> für die Bildreferenzen des Managers, des Controllers und des Executors in der YAML-Datei. Nehmen Sie auch alle anderen erforderlichen Änderungen für Ihre Bereitstellungsumgebung vor (wie LoadBalancer/NodePort/Ingress für den Zugriff auf den Manager usw.). Die folgende YAML muss für interne Zertifikatsänderungen geändert werden, wenn sie ab v5.4.2 oder höher bereitgestellt wird. Référez-vous à ce <<_kubernetes_deployment_yaml_for_v5_4_2_onwards,YAML>>.
+
--
[,shell]
----
...
Oder, wenn Sie eine der oben genannten YAML-Dateien oder die folgenden Beispiele ändern: 
...
Das ist alles! Sie sollten in der Lage sein, sich mit admin:admin, z.B. `+https://<public-ip>:8443+`, an der Konsole {product-name} anzumelden.
--

[NOTE]
====
Der im neuvector.yaml-Datei angegebene NodePort-Dienst öffnet einen zufälligen Port auf allen Kubernetes-Knoten für den Port der Verwaltungs-Webkonsole {product-name}. Alternativ können Sie einen LoadBalancer oder Ingress verwenden, indem Sie eine öffentliche IP und den Standardport 8443 nutzen. Für NodePort stellen Sie sicher, dass der Zugriff über die Firewalls für diesen Port geöffnet ist, falls erforderlich. Wenn Sie sehen möchten, welcher Port auf den Host-Knoten geöffnet ist, führen Sie die folgenden Befehle aus:
...
[NOTE]
====
PKS wird im Feld getestet und erfordert die Aktivierung von privilegierten Containern für den Plan/Tiles und die Anpassung des hostPath-YAML wie folgt für All-in-One, Controller, Enforcer:
...
Alle Taint-Informationen müssen übereinstimmen, um die Enforcer auf den Knoten zu planen. Um die Taint-Informationen auf einem Knoten (z. B. Master) zu überprüfen:
...
Wenn es zusätzliche Taints wie oben gibt, fügen Sie diese zur YAML-Sektion der Toleranzen im Beispiel hinzu:
...
Pour contrôler sur quels nœuds le Manager et le Controller sont déployés, étiquetez chaque nœud. Ersetzen Sie nodename durch den entsprechenden Knotennamen ('\`+kubectl get nodes\+`'). Hinweis: Standardmäßig plant Kubernetes keine Pods auf dem Master-Knoten.
...
Fügen Sie dann einen nodeSelector zur YAML-Datei für die Bereitstellungsabschnitte des Managers und des Controllers hinzu. Zum Beispiel:
...
Um zu verhindern, dass der Enforcer auf einem Controller-Knoten bereitgestellt wird, wenn es sich um einen dedizierten Management-Knoten handelt (ohne Anwendungscontainer zu überwachen), fügen Sie eine nodeAffinity zur YAML-Sektion des Enforcers hinzu. Zum Beispiel:
...
Orchestrierungstools wie Kubernetes, RedHat OpenShift und Rancher unterstützen progressive Updates mit konfigurierbaren Richtlinien. Sie können diese Funktion nutzen, um die Container {product-name} zu aktualisieren. Das Wichtigste wird sein, sicherzustellen, dass mindestens ein Controller (oder All-in-One) läuft, damit die Richtlinien, Protokolle und Anmeldedaten nicht verloren gehen. Stellen Sie sicher, dass zwischen den Container-Updates mindestens 120 Sekunden liegen, damit ein neuer Leader gewählt werden kann und die Daten zwischen den Controllern synchronisiert werden.

Die bereitgestellten Beispiel-YAML-Bereitstellungsdateien konfigurieren bereits die kontinuierliche Aktualisierungsstrategie. Wenn Sie über das Helm-Chart {product-name} aktualisieren, ziehen Sie bitte das neueste Chart, um die neuen Funktionen wie Admission Control korrekt zu konfigurieren, und entfernen Sie die alte Clusterrolle und den Clusterrollen-Link für {product-name}. Wenn Sie über Kubernetes aktualisieren, können Sie manuell auf eine neue Version mit den folgenden Beispielbefehlen aktualisieren.

=== Beispiel für eine kontinuierliche Kubernetes-Aktualisierung

Für Upgrades, die einfach nur ein Update auf eine neue Bildversion erfordern, können Sie diesen einfachen Ansatz verwenden.

Wenn Ihre Bereitstellung oder Daemonset bereits läuft, können Sie die YAML-Datei auf die neue Version ändern und dann das Update anwenden:
...
Für den Controller als Bereitstellung (auch für den Manager zu tun)
...
Um den Status der kontinuierlichen Aktualisierung zu überprüfen:
...
Um zur vorherigen Aktualisierung zurückzukehren:
...
== Die REST-API in Kubernetes exponieren

Um die REST-API für den Zugriff von außerhalb des Kubernetes-Clusters zu exponieren, hier eine Beispiel-YAML-Datei:
...
Bitte konsultieren Sie den Abschnitt Automatisierung für weitere Informationen zur REST-API.

== Déploiement Kubernetes en mode non privilégié

Die folgenden Anweisungen können verwendet werden, um {product-name} ohne Verwendung von Containern im privilegierten Modus bereitzustellen. Der Controller läuft bereits im nicht privilegierten Modus, und die Bereitstellung des Enforcers muss geändert werden, wie in den folgenden Auszügen gezeigt.
...
== YAML de déploiement Kubernetes pour v5.4.2 et plus

Das folgende YAML-Beispiel gilt für die Versionen 5.4.2 und höher, in denen wir die internen Zertifikate auf den Pods Controller, Enforcer und Scanner bereitstellen müssen, da wir keine fest codierten Zertifikate mehr unterstützen. Erstellen Sie das interne Zertifikat-Secret aus dem angegebenen Link, bevor Sie bereitstellen: xref:internal.adoc[Ersetzen der internen Zertifikate].
...
L'exemple suivant est une référence de déploiement complète (Kubernetes 1.19+).
...
[NOTE]
====
PKS wird im Feld getestet und erfordert die Aktivierung von privilegierten Containern für den Plan/Kachel und die Änderung des hostPath-YAML wie folgt für Allinone, Enforcer:
