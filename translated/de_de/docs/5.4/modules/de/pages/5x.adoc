...
* *NVSHAS-9981*:    Sicherheitsereignis wird ausgelöst, wenn eine neue "Prozessprofilregel" in einer Gruppe hinzugefügt oder geändert wird.
...
* *NVSHAS-9789*: Entfernen Sie unnötige Managerprotokolle bei der Konfiguration des Remote-Registrierungsdienstes.
...
* *NVSHAS-9930*: CVE-2018-20796 für `+glibc/libc-bin : 2.36-9+deb12u10+` – Falschmeldung.
...
* *NVSHAS-9969*: Fatales Fehler: gleichzeitige Kartenänderungen führen zum Neustart der Enforcer-Komponente.
...
* **NVSHAS-9821**: Der Prozessname wird nicht von der Befehlszeile für die Prozessgewinnregelwarnung übereingestimmt.
...
* **NVSHAS-9783**: Nachdem die Netzwerkrichtlinie hinzugefügt wurde, um ICMP-Pakete zu verweigern, sieht der Benutzer keine Warnung in "Sicherheitsereignisse".
...
* **NVSHAS-7555**: Fügen Sie die Option "Automatische Aktualisierung" unter Sicherheitsereignisse hinzu.
...
* **NVSHAS-9670**: Manager: Problem mit doppelten Anführungszeichen in der Antwort und unbenanntem Java-Bibliotheksproblem im sbt-Lauf.
* **NVSHAS-9667**: Das Setzen der `+CTRL_PATH_DEBUG+` Umgebungsvariable auf Fehler in der Controller-Bereitstellung funktioniert nicht.
* **NVSHAS-9665**: Dateizugriffsregel: Das Löschen vordefinierter Regeln erzeugt einen "setRowData"-Fehler.
* **NVSHAS-9664**: Richtlinientruppe: Das Löschen des benutzerdefinierten Skripts erzeugt einen "setRowData" TypeError.
...
* **NVSHAS-9440**: Unterstützung separater Netzwerkmodus und Prozess- und Dateimodus in CRD.
...
* **NVSHAS-9212**: Zeigen Sie eine Warnmeldung in GET(/v1/eula) an, wenn die Rolle (Binding) des neuvector-binding-secret falsch ist.
...
* **NVSHAS-9085**: Der PDF-Bericht zur Vermögensansicht zeigt 0% Verwundbarkeit, selbst bei vorhandenen Verwundbarkeiten.
* **NVSHAS-9084**: Der PDF-Bericht zur Vermögensansicht zeigt NaN, wenn die Bildliste leer ist.
* **NVSHAS-9128**: Sicherheitsereignisse: Container können nicht angezeigt werden, wenn kein Wert für den Namensraum der Arbeitslast vorhanden ist.
...
* **NVSHAS-9443**: Upgrade/Installation über ArgoCD schlägt fehl, da das Objekt leases.coordination.k8s.io nicht erstellt werden kann.
...
* Ein Problem wurde behoben, bei dem die Regex-URL des OpenID-Ausstellers fehlschlug.
...
* Fügen Sie Schweregrad-, Punktzahl- und Feed-Bewertungsfilter zu Assets > Registry > Ansicht der Bildschwachstellen hinzu.
...
* Sicherheitsereignisse, die durch Netzwerkregeln in einem bestimmten Knoten nicht erlaubt werden (bezogen auf "Container Task chan full" Fehlermeldungen)
...
* (Scanner) Akzeptieren Sie das OCI-Image-Format beim Wechsel zu Docker API 1.24.
* (Scanner) Das Registry-Scan sollte keine Nicht-Image-Artefakte scannen / keinen Fehler protokollieren.
...
* Unterstützung von Deep Linking in URLs zur Seite für Bild- und/oder Containeranfälligkeiten.
...
* Füllen Sie das Nachrichtenfeld für /v1/log/activity logs aus.
...
* Entfernen der Einstellung für den Betrieb des Controllers im privilegierten Modus im Helm-Diagramm und der Anforderung, dass Controller/Durchsetzer "/host/cgroup" einbinden müssen.
...
* Behebt CVE-2023-6129 in openssl sowie CVE-2023-46219 und CVE-2023-46218 in curl.
...
* Erstellen Sie keinen Standard-Admin mit dem Standardpasswort in der initialen {product-name} Bereitstellung für das AWS-Abrechnungsangebot (CSP-Adapter), sodass der Benutzer ein Geheimnis verwenden muss, um den Admin-Benutzernamen und das Passwort zu erstellen.
...
* Unterstützung für AWS Marketplace PAYG-Abrechnung für {product-name} monatliche Support-Abonnements. Benutzer können sich für {product-name} durch den SUSE-Support anmelden, der monatlich auf ihr AWS-Konto basierend auf dem durchschnittlichen Knotenverbrauch des Vormonats abgerechnet wird. Details https://open-docs.neuvector.com/deploying/awsmarketplace[hier].
* Unterstützung für die Signierung von Bildern für Zulassungssteuerungen. Benutzer können {product-name} verlangen, um zu überprüfen, dass Bilder von bestimmten Parteien signiert sind, bevor sie in die Produktionsumgebung bereitgestellt werden können, durch eine Integration mit Sigstore/Cosign. Siehe Assets -> Sigstore-Überprüfer zur Erstellung neuer Signatur-Assets. Regeln können dann mit den Kriterien Bildsignierung und/oder Bild-Sigstore-Überprüfer erstellt werden.
...
* Fügen Sie Unterstützung für Kriterien zur Zulassungssteuerung wie Ressourcenlimits hinzu. Ein neues Kriterium wird für Ressourcenlimits hinzugefügt, und zusätzliche Kriterien werden über die Einstellungen für benutzerdefinierte Kriterien unterstützt.
* Unterstützen Sie das Aufrufen des {product-name} Scanners von Harbor-Registrierungen über die https://github.com/goharbor/pluggable-scanner-spec[pluggable scanner] Schnittstelle. Dies erfordert die Konfiguration der Verbindung zum Controller (exponierte API). Der Harbor-Adapter ruft den Controller-Endpunkt auf, um einen Scan auszulösen, der automatisch beim Pushen scannen kann. Interrogationsdienste können für regelmäßige Scans verwendet werden. Scanergebnisse von Federation Primary Controllern werden an entfernte Cluster propagiert.  HINWEIS: Es gibt ein Problem mit dem HTTPS-basierten Adapter-Endpunktfehler: Bitte ignorieren Sie den Testverbindungsfehler, es funktioniert, auch wenn ein Fehler angezeigt wird (Zertifikatsvalidierung überspringen).
...
* Wiederholende Protokolle im Durchsetzungs-/Steuerungssystem reduzieren.
...
* Trotz der Definition der Netzwerkregeln werden Verstöße in Sicherheitsereignissen protokolliert (falsche Positivmeldungen), wenn der Container aufgrund eines Speichermangels (OOM) gestoppt wurde.
...
* Aktivierung der automatischen Überprüfung und Löschung von nicht verwendeten Gruppen im Alter zu configMap hinzufügen.
...
* SUSE Linux (SLES) 15.4 CVE stimmt nicht im Scanner überein. Mit diesem Fix wird, wenn die Schweregrad in dem Feed bereitgestellt wird, die Schwachstelle zur Datenbank hinzugefügt, auch wenn der NVD-Eintrag fehlt. Es ist möglich, dass der Bericht Schwachstellen ohne CVE-Werte enthält.
...
* Deutlicher zwischen Feature-Diagramm und Partner-Diagrammen in der Rancher-Benutzeroberfläche unterscheiden. + Verbesserung der Ingress-Annotation für Nginx im Rancher-Helm-Diagramm. Hinzufügen / Aktualisieren
...
* Der Enforcer wird im k8s 1.24-Cluster mit der Containerd-Laufzeit 1.64 nicht betriebsbereit. Außerdem startet der Enforcer manchmal nicht.
...
* Wenn SSO den Standard-Admin von Rancher in {product-name} im Master-Cluster verwendet, ist die {product-name}-Anmeldrolle admin, nicht fedAdmin.
...
{product-name} benötigt keine zusätzlichen Einstellungen für SELinux-aktivierte Cluster, um bereitgestellt und betrieben zu werden. Das Bereitstellen von {product-name} auf einem RHEL 8.5-basierten SELinux-aktivierten RKE2-härteten Cluster wurde getestet. {product-name} wurde erfolgreich bereitgestellt, wenn PSP aktiviert ist und das Patch-Manager- und Scanner-Bereitstellung. Die nächste Chart-Version sollte das untenstehende Problem beheben.
...
* Automatisierte Förderung von Gruppenmodi. Fördert den Schutzmodus einer Gruppe basierend auf verstrichener Zeit und Kriterien. Gilt nicht für CRD-erstellte Gruppen. Dieses Feature ermöglicht es einer neuen Anwendung, für einen bestimmten Zeitraum in Discover zu laufen, das Verhalten zu lernen und {product-name} Regeln für die Erlaubenliste für Netzwerk und Prozess zu erstellen, um dann automatisch in den Überwachungsmodus und dann in den Schutzmodus zu wechseln. Kriterium von Discover zu Monitor: Verstrichene Zeit zum Lernen aller Netzwerk- und Prozessaktivitäten von mindestens einem aktiven Pod in der Gruppe. Kriterium von Monitor zu Protect: Es gibt keine Sicherheitsereignisse (Netzwerk, Prozess usw.) für den festgelegten Zeitraum für die Gruppe.
* Unterstützung für Rancher 2.6.5 Apps und Marketplace-Chart. Wird im Namespace cattle-neuvector-system bereitgestellt und aktiviert SSO von Rancher zu {product-name}. Hinweis: Frühere Bereitstellungen von Rancher (z. B. Partnerkatalog-Charts, Version 1.9.x und früher) müssen vollständig entfernt werden, um auf das neue Chart zu aktualisieren.
...
* Geteilte Richtlinienmodus-Schutz für Netzwerk, Prozess/Datei. Es gibt jetzt eine globale Einstellung in den Einstellungen -> Konfiguration, um den Netzwerk-Schutzmodus separat für die Durchsetzung von Netzwerkregeln festzulegen. Das Aktivieren dieser Funktion (Standard ist deaktiviert) bewirkt, dass alle Netzwerkregeln im ausgewählten Schutzmodus (Entdecken, Überwachen, Schützen) sind, während die Prozess-/Dateiregeln im Schutzmodus für diese Gruppe bleiben, wie im Richtlinien -> Gruppenbildschirm angezeigt. Auf diese Weise können Netzwerkregeln auf Schützen (blockieren) gesetzt werden, während die Prozess-/Dateirichtlinie auf Überwachen gesetzt werden kann, oder umgekehrt.
...
* Falten Sie die Compliance-Prüfartikel mit demselben Namen zusammen und machen Sie sie erweiterbar.
...
. Erstellen Sie eine neue Clusterrolle und Clusterrolebinding für Admission, DLP und WAF
...
. Aktualisieren Sie die Bildnamen und -pfade zum Abrufen von {product-name} Bildern von Docker Hub (docker.io), z.B.
...
* Unterstützung für OWASP Top-10, WAF-ähnliche Regeln zur Erkennung von Netzwerkangriffen in Headern oder im Body hinzufügen. Beinhaltet Unterstützung für CRD-Definitionen von Signaturen und Anwendung auf geeignete Gruppen.
* Entfernt serverloses Scannen.
