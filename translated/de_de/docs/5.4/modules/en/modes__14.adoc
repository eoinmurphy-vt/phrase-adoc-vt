...
Wenn neue Dienste von {product-name} entdeckt werden, zum Beispiel wenn ein zuvor unbekannter Container zu laufen beginnt, kann er auf einen Standardmodus eingestellt werden. Im Discover-Modus beginnt {product-name}, sein Verhalten zu lernen und Regeln zu erstellen. Im Monitor-Modus wird ein Verstoß generiert, wenn Verbindungen zum neuen Dienst erkannt werden. Im Protect-Modus werden alle Verbindungen zum neuen Dienst blockiert, es sei denn, die Regeln wurden zuvor erstellt.
