...
{product-name} auf einem beliebigen öffentlichen Cloud K8s-Dienst wie AWS EKS, Azure AKS, IBM Cloud K8s, Google Cloud, Alibaba Cloud oder Oracle Cloud bereitstellen.
...
Die GKE Auto Pilot Unterstützung ist mit NeuVector v5.4.3 und später verfügbar. Bitte folgen Sie den folgenden Schritten, um NeuVector im Auto Pilot-Cluster bereitzustellen.
...
Überprüfen Sie nach Ausführen des Befehls `+kubectl apply -f <YAML file>+`, ob der `+AllowlistSynchronizer+` bereit ist.
...
Um mehr über die `+allowlistSynchronizer+` zu erfahren, sehen Sie sich bitte die https://cloud.google.com/kubernetes-engine/docs/how-to/run-autopilot-partner-workloads[GKE-Dokumentation] an.
