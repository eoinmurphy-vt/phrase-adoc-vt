...
Die {product-name} Open-Source-Bilder sind auf Docker Hub unter `+/neuvector/{image name}+` gehostet.
...
{product-name} unterstützt die Helm-basierte Bereitstellung mit einem Helm-Diagramm unter https://github.com/neuvector/neuvector-helm.
...
Bevor Sie xref:docker.adoc[bereitstellen {product-name}] mit docker run oder compose, müssen Sie die CLUSTER_JOIN_ADDR auf die entsprechende IP-Adresse setzen. Finden Sie die Knoten-IP-Adresse, den Knotennamen (wenn Sie einen Namensserver verwenden) oder die Knotenvariable (wenn Sie Orchestrierungstools verwenden) für das All-in-One (Controller), um die &quot;`+node IP+`&quot; in den Docker-Compose-Dateien sowohl für All-in-One als auch für den Durchsetzer zu verwenden.  Zum Beispiel:
...
Die folgende Tabelle listet die Kommunikationen von jedem {product-name} Container auf. Der All-in-One-Container kombiniert die Manager-, Controller- und Enforcer-Container und benötigt daher die für diese Container aufgeführten Ports.
