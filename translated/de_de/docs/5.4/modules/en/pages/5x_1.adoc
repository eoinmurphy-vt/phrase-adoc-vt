...
* **NVSHAS-8723**: Archivieren Sie die Cloud-Abrechnungsdaten.
...
* **NVSHAS-9092**: Benutzer mit Namensraum sollten keine globalen Ressourcen sehen.
* **NVSHAS-9116**: Der Arbeitscluster kann sich trennen, wenn die Verbindung abbricht.
...
* **NVSHAS-9189**: Der Scan bleibt bei der Planung hängen, nachdem der Controller heruntergefahren und neu gestartet wurde.
...
* Unterstützung von Deep Linking in URLs zur Seite für Bild- und/oder Container-Schwachstellen
...
* Fügen Sie das Feld "Zuletzt geändert" hinzu, um nach der Druckausgabe von Schwachstellenberichten zu filtern, sowie einen erweiterten Filter in der Schwachstellenansicht
...
* SUSE Linux (SLES) 15.4 CVE stimmt nicht mit dem Scanner überein. Mit diesem Fix wird, wenn die Schwere in dem Feed angegeben ist, die Schwachstelle in die Datenbank aufgenommen, auch wenn der NVD-Eintrag fehlt. Es ist möglich, dass der Bericht Schwachstellen ohne CVE-Wertungen enthält.
...
Für Helm-Benutzer, aktualisieren Sie auf {product-name} Helm-Chart 2.0.0 oder später. Wenn Sie einen Operator oder eine Helm-Installation auf OpenShift aktualisieren, siehe die Anmerkung unten.
