...
Um den Scanner zu aktualisieren, um die neuesten CVE-Datenbankaktualisierungen von {product-name} zu erhalten, erstellen Sie einen Cron-Job, um den Scanner zu stoppen und neu zu starten, um die neuesten abzurufen. Siehe xref:docker.adoc#_docker_native_updates[dieser Abschnitt] für Details.
