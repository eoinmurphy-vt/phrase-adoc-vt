...
{product-name} unterstützt die Helm-basierte Bereitstellung mit einem https://github.com/neuvector/neuvector-helm[Helm-Chart] bei https://github.com/neuvector/neuvector-helm.. Der {product-name} Operator kann ebenfalls verwendet werden und basiert auf dem Helm-Chart. Um die neuesten {product-name} Container-Versionen mit einem Operator bereitzustellen, verwenden Sie bitte entweder den von Red Hat zertifizierten Operator aus dem Operator Hub oder den Community-Operator, wie im xref:operators.adoc[Operator-Bereich] beschrieben.
...
Wenn Sie Ihren eigenen Namespace erstellt haben, anstatt &quot;`+neuvector+`&quot; zu verwenden, ersetzen Sie alle Vorkommen von &quot;`+namespace: neuvector+`&quot; und anderen Namespace-Referenzen durch Ihren Namespace in den untenstehenden Beispiel-YAMLs.
...
Um zu verhindern, dass der Vollstrecker auf einem Controller-Knoten bereitgestellt wird, wenn es sich um einen dedizierten Verwaltungs-Knoten handelt (ohne Anwendungscontainer, die überwacht werden sollen), fügen Sie eine nodeAffinity zum YAML-Bereich des Vollstreckers hinzu. Zum Beispiel:
...
Um zu taggen/pushen, wenn die neueste Version 2.0.1 ist, genauso wie in Schritt 3 oben auf dieser Seite:
