...
[IMPORTANT]
====
Alle Antwortregeln werden ausgewertet, um zu bestimmen, ob sie der Bedingung/dem Kriterium entsprechen. Wenn es mehrere Regelübereinstimmungen gibt, werden die jeweiligen Aktionen ausgeführt. Dies unterscheidet sich vom Verhalten der Netzwerkregeln, die von oben nach unten ausgewertet werden und nur die erste übereinstimmende Regel ausgeführt wird.
====
...
Es gibt 6 Standard-Antwortregeln, die mit {product-name} enthalten sind und auf den Status &apos;`+disabled,+`&apos; gesetzt sind, eine für jede Kategorie. Benutzer können entweder eine Standardregel ändern, um ihren Anforderungen zu entsprechen, oder neue erstellen. Stellen Sie sicher, dass alle Regeln aktiviert sind, die angewendet werden sollen.
...
* Status aktivieren
* Die Protokollebene oder Prozessnamen können als andere Überechnungskriterien verwendet werden
...
* Fügen Sie den Namen des Ereignisprotokolls hinzu, der als Überechnungskriterium einbezogen werden soll
* Wählen Sie die anzuwendenden Aktionen - Quarantäne, Webhook oder Protokoll unterdrücken
* Status aktivieren
* Die Protokollebene kann als weiteres Überechnungskriterium verwendet werden

=== Beispielereignisse, die für eine Antwortregel ausgewählt werden können

image:resp5.png[ereignisse]

=== Beispielkriterien für Admission-Control-Ereignisse

image:admission.png[ereignisse]

=== Erstellen einer Antwortregel für die Kategorie cve-Bericht (Protokollebene und Berichtsname als Überechnungskriterium)

* Klicken Sie auf "oben einfügen", um die Regel oben einzufügen
* Wählen Sie einen Dienstgruppennamen, wenn die Regel auf eine bestimmte Dienstgruppe angewendet werden muss
* Wählen Sie die Kategorie CVE-Bericht
* Fügen Sie die Protokollebene als Überechnungskriterium oder cve-Berichtstyp hinzu
...
=== Popup-Option zur Aufhebung der Quarantäne, wenn die entsprechende Antwortregel gelöscht wird
