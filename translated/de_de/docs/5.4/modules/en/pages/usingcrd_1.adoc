...
Die Bedeutung des Ressourcentyps NvSecurityRule mit einem Geltungsbereich auf Namensraumebene liegt in der Durchsetzung des konfigurierten Bereichs der Zielgruppe, der mit dem konfigurierten Namensraum in der Sicherheitsrichtlinie der CRD von NeuVector übereinstimmen muss. Dies bietet eine Durchsetzung, um unerwünschte Richtlinien zur Erstellung von übergreifenden Namensräumen zu verhindern, die eine Zielgruppenrichtlinie betreffen.
...
Als nächstes können wir den Benutzerkontext auf user2 mit einem breiteren Scope-Privileg, Cluster-Level-Scope, wechseln.  Dieser user2 hat eine Clusterrolebinding, die nicht an einen Namespace gebunden ist, sondern einen Cluster-Level-Scope hat und mit der Ressource NvClusterSecurityRules assoziiert ist.
...
{product-name} unterstützt den Export bestimmter {product-name} Gruppentypen aus einem Kubernetes-Cluster in eine yaml-Datei und den Import in einen anderen Kubernetes-Cluster durch die Nutzung nativer kubectl-Befehle.
...
* Exportierte Gruppen ohne die definierten Kriterien als domain=xx (Namespace) werden mit dem Ressourcentyp NvClusterSecurityRule und einem Namespace als Standard exportiert.  Beispiele für exportierte Gruppen ohne einen Namespace sind extern, Container usw.
