...
Nach Abschluss des Scans sehen Sie die Ergebnisse darunter. Klicken Sie auf das Repository/Tag, um Schwachstellen zu sehen, und klicken Sie auf die Schwachstelle, um weitere Informationen zu erhalten. Sie können auch den Bericht in einer CSV-Datei herunterladen oder die Ergebnisse in den Ereignisprotokollen anzeigen.
...
Hinzufügen des JFrog Artifactory-Registrierungs (Docker-Zugriffsmethode -- Repository-Pfad)
JFrog-Managementseite admin->HTTP-Einstellung zeigt die Docker-Zugriffsmethode - Repository-Pfad
...
Fügen Sie das subdomainbasierte JFrog-Registry hinzu, um Bilder von der docker-local Subdomain zu scannen

image:jfrog_sub_local.png[local]

Fügen Sie das subdomainbasierte JFrog-Registry hinzu, um alle Tags von allen Subdomains zu scannen
...
Anzeigen des Azure-Portals Benutzernamen und Passwort für den Zugriff auf die Container-Registry
...
* Das Zugriffstoken ist nicht mehr verfügbar, sobald der Benutzer von der Seite mit dem generierten Token navigiert ist.  Daher wird dringend empfohlen, eine Kopie des Zugriffstokens zu erstellen, bevor Sie die folgende Seite verlassen oder schließen:
...
* Geben Sie die Registry-URL mit Port an
* Geben Sie Benutzernamen und Passwort an, falls vom Repository erforderlich
* Geben Sie die externe GitLab-URL und das private Token aus dem letzten Abschnitt an
