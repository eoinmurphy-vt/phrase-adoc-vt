...
Wählen Sie in Ihrem Projekt das Plugin '{product-name} Schwachstellenscanner' aus dem Dropdown-Menü in 'Build-Schritt hinzufügen'. Aktivieren Sie das Kontrollkästchen "Mit Standalone-Scanner scannen", wenn Sie den Scan im Standalone-Scanner-Modus durchführen möchten. Standardmäßig wird der "Controller & Scanner"-Modus verwendet, um den Scan durchzuführen.
