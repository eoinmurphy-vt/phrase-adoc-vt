...
* Definieren Sie die Sicherheitsrichtlinie während der Anwendungsentwicklung, um sie in die Produktion zu pushen.
* Lernen Sie das Verhalten mit {product-name} und exportieren Sie die CRD zur Überprüfung, bevor Sie sie in die Produktion pushen.
...
CRDs bringen viele Vorteile mit sich, einschließlich:

* Definieren / Deklarieren Sie die Sicherheitsrichtlinie als Code.
* Versionieren und verfolgen Sie die Sicherheitsrichtlinien genauso wie Anwendungsbereitstellungsmanifeste.
...
Beginnend mit v5.4.3 verwendet NeuVector das Attribut `+name_referral+` (boolean) als Einstellung im Gruppenselektor (`+target.selector+`, `+ingress.items[].selector+`, `+egress.items[].selector+`) im Schema der CRD NvSecurityRule/NvClusterSecurityRule. Sie können diese Einstellung in der Benutzeroberfläche aktivieren, indem Sie "Name Referral verwenden" im Dialogfeld Gruppenexport aktivieren. 

Wenn das Attribut `+name_referral+` auf `+true+` gesetzt ist, werden die Felder `+criteria/comment+` des Gruppenselektors in NvSecurityRule/NvClusterSecurityRule von NeuVector ignoriert. Das bedeutet, dass NeuVector versuchen wird, die `+criteria/comment+` der Gruppe durch Verweisung zu bestimmen. Dies führt zu "Gruppenverweisung" in den CRDs NvSecurityRule/NvClusterSecurityRule, um bei Änderungen beim Bearbeiten von Gruppen zu helfen. Wenn nicht gesetzt, müssen Benutzer Gruppen an jedem definierten Ort in den jeweiligen YAML-Dateien aktualisieren, wenn Änderungen erforderlich sind.
...
Die Bedeutung des Ressourcentyps NvSecurityRule mit einem Geltungsbereich auf Namensraumebene liegt in der Durchsetzung des konfigurierten Bereichs der Zielgruppe, der mit dem konfigurierten Namensraum in der Sicherheitsrichtlinie der CRD von NeuVector übereinstimmen muss. Dies bietet eine Durchsetzung, um unerwünschte Richtlinienerstellungen über Namensräume hinweg zu verhindern, die eine Zielgruppenrichtlinie betreffen.

Für die benutzerdefinierte Ressourcen-Definition NvClusterSecurityRule hat dies einen Geltungsbereich auf Cluster-Ebene und erzwingt daher keine Namensraumgrenze für ein definiertes Ziel. Der Benutzerkontext, der zum Importieren der CRD-YAML-Datei verwendet wird, muss jedoch über die erforderlichen Berechtigungen verfügen, um auf den gleichen Namensraum zuzugreifen oder sich darin zu befinden, wie der in der CRD-YAML-Datei konfigurierte, oder der Import wird abgelehnt.
...
[NOTE]
====
Zusätzlich zu den ausgewählten Gruppe(n) werden auch alle 'verknüpften' Gruppen exportiert. Eine verknüpfte Gruppe ist jede andere Gruppe, mit der eine ausgewählte Gruppe gemäß einer Netzwerkregel verbunden ist oder von der sie verbunden wird.
====
...
Die Konfiguration des Richtlinienmodus und die Gruppendefinition werden innerhalb der CRD-Konfigurations-YAML-Datei unterstützt. Mit dem konfigurierten Richtlinienmodus in der YAML-Konfigurationsdatei wird beim Importieren einer solchen Datei die Zielgruppe auf diesen Wert für den CRD-Import gesetzt.

[IMPORTANT]
====
Der importierte Zielrichtlinienmodus darf nicht über die {product-name} Konsole (Richtlinie -> Gruppen) geändert werden. Zum Beispiel, sobald der Modus auf Überwachung gesetzt ist, kann er nur durch eine CRD-Änderung geändert werden, nicht über die Konsole.
====
...
* Ein konfiguriertes Schlüssel: Domain muss mit dem Dienstdomänen-Suffix des konfigurierten Dienstes übereinstimmen
...
* Entspricht den Kriterien: {caret}[a-zA-Z0-9]+[.:a-zA-Z0-9_-]*$
...
* Wenn der zu importierende Gruppenname bereits im Zielsystem existiert, müssen die Kriterien zwischen der importierten CRD und der im Zielsystem übereinstimmen.  Wenn es Unterschiede gibt, wird der CRD-Import abgelehnt.
...
* Ist der Datenverkehr, der vom Ziel abfließt.
...
** App: Liste von Apps
** Verhalten: block_access / monitor_change  #Dies blockiert den Zugriff auf den definierten Filter unten.  Wenn monitor_change gewählt wird, wird ein Sicherheitsereignis aus der {product-name}'s Webkonsole Benachrichtigungen > Sicherheitsereignisse Seite generiert.
...
=== Rolebinding & Clusterolebinding mit 2 Benutzern in verschiedenen Namespaces zu einem Clusterrole (NvSecurityRules & NvClusterSecurityRules-Ressourcen)
...
Die wichtigste Erkenntnis hier ist, dass die Verwendung von Rolebinding einen Namespace-Level-Scope hat, während die Verwendung von Clusterrolebinding einen Cluster-Level-Scope hat.  User1 wird Rolebind (Namespace-Level-Scope) und User2 wird Clusterrolebind (Cluster-Level-Scope).  Dies ist besonders wichtig während der Durchsetzung von RBAC basierend auf dem Scope-Level, das den Zugriff der erstellten Benutzer einschränkt.
...
----
--
. Der Wechsel des Benutzerkontexts zu user1 (gehört zum ns1 Namespace) hat ein Rolebind zur Ressource NvSecurityRules, die Namespace-gebunden an den Namespace ns1 ist.  Daher sollte das Importieren der Test-YAML-Datei (kubectl create --f nvsecurity.yaml) erlaubt sein, da diese YAML-Datei die Ressource NvSecurityRules und den Namespace enthält, an den dieser Benutzer gebunden ist.

Wenn jedoch ein Versuch unternommen wird, die Test-YAML-Datei (nvclustersecurity.yaml) zu importieren, wird dies abgelehnt, da die Import-CRD-YAML-Datei mit der Ressource NvClusterSecurityRules definiert ist, die einen Cluster-Scope hat, aber user1 mit einem Namespace-Scope gebunden wurde.  Der Namespace-Scope hat ein niedrigeres Privileg als der Cluster-Scope.  Daher wird Kubernetes RBAC eine solche Anfrage ablehnen.
...
Als nächstes können wir den Benutzerkontext auf user2 mit einem breiteren Scope-Privileg, dem Cluster-Level-Scope, wechseln.  Dieser user2 hat eine Clusterrolebinding, die nicht an einen Namespace gebunden ist, sondern einen Cluster-Level-Scope hat und mit der Ressource NvClusterSecurityRules assoziiert ist.
...
Benutzerdefinierte, vom Benutzer erstellte Gruppen dürfen nicht das Präfix 'nv.' haben. Nur entdeckte/erlernte Gruppen mit den Domain- und Dienstobjekten sollten das Präfix haben. Zum Beispiel:
...
* Erstens ermöglicht dies, dass dieselben Regeln auf mehreren Kubernetes-Umgebungen angewendet werden, was die Synchronisierung zwischen Clustern ermöglicht.
* Zweitens ermöglicht dies die präventive Bereitstellung von Regeln, bevor die Anwendungen online kommen, was einen proaktiven und effektiven Workflow zur Bereitstellung von Sicherheitsregeln bietet.
* Drittens ermöglicht dies, dass der Policymodus von einem Evaluierungsmodus (wie Discover oder Monitor) zu einem wechselt, der die endgültige Staging-Umgebung schützt.
