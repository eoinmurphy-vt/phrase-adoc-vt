...
{product-name} enthält standardmäßige selbstsignierte Zertifikate zur Verschlüsselung für den Manager (Konsolen-/UI-Zugriff), Controller (REST-API, intern), Enforcer (intern) und Scanner (intern) Kommunikationen.
...
=== Ersetzen von Zertifikaten, die in der internen Kommunikation von {product-name} verwendet werden
...
+
* Befolgen Sie die Schritte im `+New certificate+` Tab, um ein Kubernetes-Geheimnis zu verwenden, um das interne Zertifikat zu verwalten. Verwenden Sie anstelle der Generierung eines neuen Zertifikats das Zertifikat, `+old-ca.crt+`, `+old-tls.crt+` und `+old-tls.key+`, das Sie unten abgerufen haben:
+
[source,shell]
----
