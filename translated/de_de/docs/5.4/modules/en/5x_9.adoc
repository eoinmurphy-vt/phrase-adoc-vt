...
* **NVSHAS-9759**: Füge Datendetails im Ingress-Exposure-Bericht hinzu.
...
* **NVSHAS-9811**: [Manager] Zugriff auf die GUI nicht möglich, wenn ein benutzerdefiniertes Zertifikat (CA) mit Root-CA und Zwischen-CA auf dem Geheimnis verwendet wird.
...
* **NVSHAS-9729**: Falsche Anzahl von Schwachstellen wird beobachtet, wenn mehrere StatefulSets mit dem "gleichen Namen" in Namespaces in einem Projekt bereitgestellt werden.
...
* Erlauben Sie Benutzern, Ressourcen für `+updater-cron-job+` festzulegen, wenn sie {product-name} mit dem Helm-Diagramm installieren.
...
* Container kann nicht erfolgreich zur Arbeitslast hinzugefügt werden (häufige Vorkommen). Resultierend aus einem Deadlock durch Kanalnachrichten.
...
* Unterstützung von Deep Linking in URLs zur Seite mit Bild- und/oder Containeranfälligkeiten
...
* Trotz der Definition der Netzwerkregeln werden Verstöße in Sicherheitsereignissen protokolliert (falsche Positives), wenn der Container aufgrund eines Speichermangels (OOM) gestoppt wurde.
