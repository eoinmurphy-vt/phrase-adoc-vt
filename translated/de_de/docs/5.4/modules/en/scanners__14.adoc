...
Experimentieren Sie mit unterschiedlichen Zahlen von Scannern auf Registrierungen mit einer großen Anzahl von Bildern, um das Verhalten der Scanabschlusszeit in Ihrer Umgebung zu beobachten. 2-5 Scanner als die Replikateinstellung sollte für die meisten Fälle ausreichend sein.
...
Große Bilder benötigen mehr Zeit zum Abrufen und müssen erweitert werden, um sie zu scannen, was mehr Speicher verbraucht. Stellen Sie sicher, dass jedem Scanner genügend Speicher zugewiesen ist, um mehr als das größte erwartete Bild (mindestens 10 % mehr) zu verarbeiten.
...
* SCANNER_SCAN_LAYERS= wahr oder falsch (um geschichtete Scannergebnisse zurückzugeben)
