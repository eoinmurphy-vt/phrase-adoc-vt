...
* *NVSHAS-9789*: Entfernen Sie unnötige Manager-Protokolle zur Konfiguration des Remote-Registrierungsdienstes.
...
* *NVSHAS-9933*: Registry-Adapter-Funktion (Harbor) zeigt Fehler in der Ziel-Registry während des Scannens an.
...
* *NVSHAS-10003*: Seite neu laden funktioniert nicht auf der eigenständigen NV-Seite, während die Rancher UI geöffnet ist.
...
* **NVSHAS-9670**: Manager: Problem mit doppelten Anführungszeichen in der einfachen Textantwort und unbenanntem Java-Bibliotheksproblem im sbt-Ausführen.
...
* **NVSHAS-9212**: Anzeige der Alarmnachricht in GET(/v1/eula), wenn die Rolle (Binding) des neuvector-binding-secret falsch ist.
...
* **NVSHAS-9122**: Neuvector meldet sich jederzeit ab, wenn "Multiple Cluster" mit Rancher SSO-Login verwendet wird.
...
* Im Entdeckungsmodus lernen, Egress zu externen FQDN-Adressgruppen automatisch. Eine neue benutzerdefinierte Gruppe für externe FQDN wird erstellt, es sei denn, die externe Verbindung entspricht einer bestehenden Regel.
...
* Verbessern Sie das Beschneiden von Namensräumen mit einer speziellen Funktion.
...
* Aktivieren Sie, dass jede Zulassungsregel ihren eigenen Modus Monitor oder Protect hat. Eine Ablehnungsaktion im Überwachungsmodus wird alarmieren, und eine Ablehnungsaktion im Schutzmodus wird blockieren. Erlaubte Aktionen sind nicht betroffen.
* Fügen Sie einen neuen Regex-Operator in Richtlinie > Zulassungssteuerung > Regel hinzufügen für Benutzer und Benutzergruppen hinzu, um Regex zu unterstützen. Unterstützen Sie Operatoren "entspricht ANY regex in" und "entspricht NONE regex in".
...
* Fügen Sie eine Überprüfung der imagePullPolicy in den Kriterien der Zulassungssteuerungsregeln hinzu.
...
* Der Enforcer wird in einem k8s 1.24-Cluster mit dem 1.64 Containerd-Runtime nicht betriebsbereit. Außerdem startet der Enforcer manchmal nicht.
