...
{product-name} erstellt automatisch eine Gruppe namens 'Knoten', die jeden Knoten (Host) im Cluster darstellt. {product-name} bietet automatisierte grundlegende Überwachung von Hosts auf verdächtige Prozesse (wie Portscans, Reverse Shells usw.) und Privilegieneskalationen. Darüber hinaus wird {product-name} das Prozessverhalten jedes Knotens im Entdeckungsmodus lernen, um diese Prozesse auf die Whitelist zu setzen, ähnlich wie es bei Containerprozessen gemacht wird.  Die Liste der lokalen (gelernten) Prozessregeln ist eine Kombination aller Prozesse von allen Knoten im Cluster im Entdeckungsmodus.
...
[NOTE]
====
Netzwerkverbindungsverstöße gegen die Regeln, die in den Netzwerkregeln für Knoten angezeigt werden, werden niemals blockiert, selbst im Schutzmodus. Nur Prozessverstöße werden im Schutzmodus auf Knoten blockiert.
====
