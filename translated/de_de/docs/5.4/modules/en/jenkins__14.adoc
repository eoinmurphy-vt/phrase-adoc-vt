...
Klicken Sie auf die &apos;`+Add Registry+`&apos;, um Werte für das Registry einzugeben, die Sie in Ihrem Projekt verwenden werden. Wenn Sie nur lokale Bilder scannen, müssen Sie hier kein Registry hinzufügen.
...
Das Ausführen des Jenkins-Scans im Standalone-Modus ist eine leichte Möglichkeit, Bildanfälligkeiten in der Pipeline zu scannen. Der Scanner wird dynamisch aufgerufen und es ist keine Installation des Controller-Setups erforderlich. Dies ist besonders nützlich, wenn Sie ein Bild scannen, bevor es in ein Registry hochgeladen wird. Es gibt auch keine Begrenzung, wie viele Scan-Aufgaben gleichzeitig ausgeführt werden können.

Um einen Schwachstellenscan im Standalone-Modus auszuführen, muss das Jenkins-Plugin das Scanner-Bild auf den Host ziehen, auf dem der Agent läuft. Daher müssen Sie die {product-name} Scanner-Registry-URL, das Bild-Repository und die Anmeldeinformationen, falls erforderlich, auf der {product-name} Plugin-Konfigurationsseite eingeben.

Das Scan-Ergebnis kann auch an den Controller übermittelt und in der Funktion zur Zulassungskontrolle verwendet werden. In diesem Fall benötigen Sie ein Controller-Setup und müssen angeben, wie Sie sich auf der {product-name} Plugin-Konfigurationsseite mit dem Controller verbinden.
...
Nachdem der Build abgeschlossen ist, wird ein {product-name} Bericht erstellt. Er zeigt die Scan-Details und Fehler an, falls vorhanden.
...
Für das Jenkins-Pipeline-Projekt können Sie Ihr eigenes Pipeline-Skript direkt schreiben oder auf &apos;`+pipeline syntax+`&apos; klicken, um das Skript zu generieren, wenn Sie neu im Pipeline-Stil-Task sind.
...
Kopieren Sie das Skript in Ihr Jenkins-Task-Skript.
