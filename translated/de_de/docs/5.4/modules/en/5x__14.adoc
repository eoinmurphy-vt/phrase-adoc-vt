...
* **NVSHAS-9912**: Aktualisiere Helm über einige K8s RBAC-Berechtigungen, die vom Dienstkonto des Controllers benötigt werden.
...
* **NVSHAS-9821**: Der Prozessname wird nicht von der Befehlszeile für die Prozess Profit Regelwarnung übereinstimmt.
...
* **NVSHAS-9227**: Der Registry-Scan bleibt mitten im Scanprozess hängen und wechselt in den "Idle"-Zustand.
...
* **NVSHAS-9447**: Controller/Scanner-Pods stürzen ab - "Nicht unterstützter Systemausstieg".
...
* Container kann nicht erfolgreich zur arbeitslast hinzugefügt werden (häufige vorkommen). Resultierend aus einem toten Punkt durch Kanalnachrichten.
...
* Fügen Sie einen Link zur Gruppe in Sicherheitsrisiken -> Verwundbarkeiten -> Auswirkungen Popup hinzu, um den Gruppenmodus einfach zu bearbeiten
...
* Interner Serverfehler in Sicherheitsrisiken -> Verwundbarkeiten mit einer hohen Anzahl von CVEs
* SIGSEGV: Segmentierungsverletzung im Controller
* Das Löschen verwundbarer Dateien (z. B. jar) entfernt sie nicht aus der Liste der Verwundbarkeiten
...
* Entfernen der Einstellung für den Betrieb des Controllers im privilegierten Modus im Helm-Diagramm und der Anforderung, dass Controller/Durchsetzer "/host/cgroup" einhängen müssen.
...
* Scan auf einem Knoten schlägt aufgrund eines festgefahrenen docker cp / grpc-Problems fehl.
...
* Fügen Sie weitere Optionen für die CVE-Berichtskriterien in den Reaktionsregeln hinzu. Beispiel 1 - "cve-high-with-fix:X" bedeutet: Wenn die Anzahl der (hochgradigen Schwachstellen, die behoben wurden) >= X, lösen Sie die Reaktionsregel aus. Beispiel 2 - "cve-high-with-fix:X/Y" bedeutet: Wenn die Anzahl der (hochgradigen Schwachstellen, die vor Y Tagen gemeldet wurden und behoben wurden) >= X, lösen Sie die Reaktionsregel aus.
...
* Erweiterter Filter in Sicherheitsereignissen fehlt die "Fehler"-Ebene.
...
* Compliance-Containerliste zum Ausschließen von beendeten Containern.
...
* SUSE Linux (SLES) 15.4 CVE stimmt nicht mit dem Scanner überein. Mit diesem Fix wird, wenn die Schwere in dem Feed angegeben ist, die Schwachstelle zur Datenbank hinzugefügt, auch wenn der NVD-Eintrag fehlt. Es ist möglich, dass der Bericht Schwachstellen ohne CVE-Werte enthält.
...
* CRD für WAF, DLP und Zulassungssteuerungen. HINWEIS: Zusätzliche Clusterrollenbindungen/Berechtigungen erforderlich. Siehe Kubernetes- und OpenShift-Bereitstellungsabschnitte. CRD-Import/Export und Versionierung für Zulassungssteuerungen, die über CRD unterstützt werden.
...
* Funktional vollständig, einschließlich automatisierter Förderung von Gruppenmodi. Fördert den Schutzmodus einer Gruppe basierend auf verstrichener Zeit und Kriterien. Gilt nicht für Gruppen, die durch CRD erstellt wurden. Dieses Feature ermöglicht es einer neuen Anwendung, für einen bestimmten Zeitraum in Discover zu laufen, das Verhalten zu lernen und {product-name} Erlauben-Listenregeln für Netzwerk und Prozess zu erstellen, bevor sie automatisch in den Überwachungs- und dann in den Schutzmodus wechselt. Kriterium von Discover zu Monitor: Verstrichene Zeit zum Lernen aller Netzwerk- und Prozessaktivitäten von mindestens einem aktiven Pod in der Gruppe. Kriterium von Monitor zu Protect: Es gibt keine Sicherheitsereignisse (Netzwerk, Prozess usw.) für den festgelegten Zeitraum für die Gruppe.
* Unterstützung für Rancher 2.6.5 Apps und Marketplace-Chart. Wird im Namespace cattle-neuvector-system bereitgestellt und aktiviert SSO von Rancher zu {product-name}. Hinweis: Frühere Bereitstellungen von Rancher (z. B. Partnerkatalog-Charts, Version 1.9.x und früher) müssen vollständig entfernt werden, um auf das neue Chart zu aktualisieren.
