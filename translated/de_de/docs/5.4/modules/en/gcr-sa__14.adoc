...
Beginnen Sie in {product-name}, wo man zunächst ein neues Repository einrichtet:
...
. Filter - Seien Sie sich bewusst, dass Sie wahrscheinlich alle Filter hier durch den tatsächlichen Namen des Repos ersetzen müssen. Nochmals, das ist in der GCR-Benutzeroberfläche.
...
Bevor Sie zu aufgeregt werden, gibt es noch eine Sache, die Sie sicherstellen müssen. Damit der Scanner in {product-name} die API verwenden kann, um Ihre Bilder zu scannen und zu schützen, muss die genannte API in Ihrem GCP-Konto aktiviert sein. Sie können es entweder über die Befehlszeile aktivieren über
