= Test_Drei benutzerdefinierte Compliance-Prüfungen
:revdate: 2024-09-25
:page-revdate: {revdate}
:page-opendocs-origin: /05.policy/11.customcompliance/11.customcompliance.md
:page-opendocs-slug:  /policy/customcompliance

== Test_Drei benutzerdefinierte Skripte für Compliance-Prüfungen erstellen
...
[NOTE]
====
Die Möglichkeit, benutzerdefinierte Skripte zu erstellen, ist standardmäßig deaktiviert, um Missbrauch zu verhindern. Dies kann aktiviert werden, indem die Umgebungsvariable CUSTOM_CHECK_CONTROL xref:details.adoc#_environment_variables[Umgebungsvariable] im Controller und Enforcer gesetzt wird. Werte sind "deaktivieren" (Standard, nicht erlaubt), "streng" (nur Admin-Rolle) oder "locker" (Admin-, Compliance- und Runtime-Policy-Rollen).
====
...
* Ein benutzerdefiniertes Skript wird durch die Berechtigung der Laufzeitpolitik mit namespaced RBAC gesteuert; Benutzer sollten die Kubernetes-Benutzerrollen ordnungsgemäß einrichten.
...
=== Test_ThreeSample-Skript zur Überprüfung, ob der Container ein Root-Konto ohne Passwort hat.
...
==== Test_ThreeSample-Skript zur Überprüfung der schmutzigen Cow-Datei im Container.
...
== Test_ThreeErstellen eines benutzerdefinierten Überprüfungsskripts
...
== Test_DreiErstellen einer Antwortregel für den Compliance-Bericht

Antwortregeln können in der Richtlinie -> Antwortregeln erstellt werden, die auf den Ergebnissen von benutzerdefinierten Compliance-Überprüfungen basieren. Die Ergebnisse sind Teil der Kategorie Compliance, und es können Antworten für alle Ereignisse eines bestimmten Levels erstellt werden.
...
* Geben Sie das Level ein und wählen Sie das Level:Warnung aus der automatischen Auswahl aus.
