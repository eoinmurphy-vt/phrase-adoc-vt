...
* Für Konfigurationssicherung/HA, ein RWX PVC von 1Gi oder mehr. Siehe xref:production.adoc#_backups_and_persistent_data[Sicherungen und Abschnitt für persistente Daten] für weitere Details.
...
* CoreOS wird unterstützt (November 2023) für CVE-Scans über die von RedHat bereitgestellte RHEL-Mapping-Tabelle. Sobald ein offizieller Feed von RedHat für CoreOS veröffentlicht wird, wird er unterstützt.
...
AWS Bottlerocket Hinweis: Pfad des Containerd-Sockets, der spezifisch für Bottleneck ist, muss geändert werden. Bitte siehe den Kubernetes-Bereitstellungsabschnitt für Details.
...
Die folgende Tabelle zeigt die durchschnittliche Latenz von 2-10%, die mit dem Redis-Benchmark-Tool gemessen wurde. Der Redis-Benchmark verwendet ziemlich kleine Pakete, sodass die Latenz mit größeren Paketen voraussichtlich niedriger sein wird.
