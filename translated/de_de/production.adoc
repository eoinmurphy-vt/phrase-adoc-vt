...
Der Controller verwaltet den Cluster von Durchsetzern und kann auf demselben Knoten wie ein Durchsetzer oder auf einem separaten Verwaltungs-Knoten bereitgestellt werden. Der Manager sollte auf dem Knoten bereitgestellt werden, auf dem der Controller läuft, und wird Zugriff auf die Konsole des Controllers bieten. Andere erforderliche \{product-name} Container wie der Manager, Scanner und Aktualisierer werden im Best Practices-Leitfaden, der unten referenziert wird, detaillierter beschrieben.
...
=== Testen Sie Best Practices, Tipps, Fragen und Antworten für die Bereitstellung und Verwaltung von \{product-name}
...
== Testen Sie die Bereitstellung mit Helm oder Operatoren
...
== Testen Sie die Bereitstellung mit ConfigMap
...
== Testen Sie die Bereitstellung der Controller
...
== Testen Sie Backups und persistente Daten
...
\{product-name} unterstützt persistente Daten für die \{product-name}-Richtlinie und -Konfiguration. Dies konfiguriert ein Echtzeit-Backup, um ein Volume von /var/neuvector/ aus dem Controller-Pod zu mounten. Der Hauptanwendungsfall ist, wenn das persistente Volume gemountet ist, werden die Konfiguration und die Richtlinie zur Laufzeit im persistenten Volume gespeichert. Im Falle eines totalen Ausfalls des Clusters wird die Konfiguration automatisch wiederhergestellt, wenn das neue Cluster erstellt wird. Konfiguration und Richtlinie können auch manuell vom /var/neuvector/ Volume wiederhergestellt oder entfernt werden.
...
=== Testbeispiel für persistentes Volume
...
=== Testen von ConfigMaps und persistentem Speicher
...
== Testen der Aktualisierung der CVE-Sicherheitsdatenbank in der Produktion
...
== Testen des Zugriffs auf die Konsole
...
== Testen von Host-Updates oder Auto-Scaling-Knoten mit einem Pod-Störungshaushalt
...
== Testen Sie die Bereitstellung ohne privilegierten Modus
...
== Testen Sie die Multi-Site-, Multi-Cluster-Architektur
